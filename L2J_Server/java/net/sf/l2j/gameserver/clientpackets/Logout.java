	...
	void runImpl()
	{
		...
		if (AttackStanceTaskManager.getInstance().getAttackStanceTask(player))
		{
			...
			player.sendPacket(new SystemMessage(
					SystemMessage.CANT_LOGOUT_WHILE_FIGHTING));
		...
		if (player.atEvent)
		{
			player.sendPacket(SystemMessage.sendString(
					"A superior power doesn't allow you to leave the event"));
			return;
		}
		...
		if (player.isFestivalParticipant())
		{
			...
			{
				player.sendMessage(
						"You cannot log out while you are a participant in a festival.");
				return;
			}
		...
		// normally the server would send serveral "delete object" before
		// "leaveWorld"
