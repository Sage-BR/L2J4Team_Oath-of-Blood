...
public class GeoEngine extends GeoData
{
	...
	private static Map<Short, MappedByteBuffer> Geodata = new FastMap<>();

	private static Map<Short, IntBuffer> Geodata_index = new FastMap<>();
	...
	{
		return NgetType((x - L2World.MAP_MIN_X) >> 4,
	...
	{
		return NgetHeight((x - L2World.MAP_MIN_X) >> 4,
				(y - L2World.MAP_MIN_Y) >> 4, z);
	}
	...
	{
		return NgetSpawnHeight((x - L2World.MAP_MIN_X) >> 4,
				(y - L2World.MAP_MIN_Y) >> 4, zmin, zmax, spawnid);
	}
	...
	public String geoPosition(int x, int y)
	{
		...
		return "bx: " + getBlock(gx) + " by: " + getBlock(gy) + " cx: "
				+ getCell(gx) + " cy: " + getCell(gy) + "  region offset: "
	...
	public boolean canSeeTarget(L2Object cha, L2Object target)
	{
		if (cha.getZ() >= target.getZ())
			return canSeeTarget(cha.getX(), cha.getY(), cha.getZ(),
					target.getX(), target.getY(), target.getZ());
		else
			return canSeeTarget(target.getX(), target.getY(), target.getZ(),
					cha.getX(), cha.getY(), cha.getZ());
	}

	/**
	 * @see net.sf.l2j.gameserver.GeoData#canSeeTargetDebug(net.sf.l2j.gameserver.model.actor.instance.L2PcInstance,
	 *      net.sf.l2j.gameserver.model.L2Object)
	 */
	@Override
	public boolean canSeeTargetDebug(L2PcInstance gm, L2Object target)
	{
		if (gm.getZ() >= target.getZ())
			return canSeeDebug(gm, (gm.getX() - L2World.MAP_MIN_X) >> 4,
					(gm.getY() - L2World.MAP_MIN_Y) >> 4, gm.getZ(),
					(target.getX() - L2World.MAP_MIN_X) >> 4,
					(target.getY() - L2World.MAP_MIN_Y) >> 4, target.getZ());
		else
			return canSeeDebug(gm, (target.getX() - L2World.MAP_MIN_X) >> 4,
					(target.getY() - L2World.MAP_MIN_Y) >> 4, target.getZ(),
	...
	{
		return NgetNSWE((x - L2World.MAP_MIN_X) >> 4,
				(y - L2World.MAP_MIN_Y) >> 4, z);
	}

	/**
	 * @see net.sf.l2j.gameserver.GeoData#moveCheck(int, int, int, int, int,
	 *      int)
	 */
	@Override
	public Location moveCheck(int x, int y, int z, int tx, int ty, int tz)
	{
		...
		return MoveCheck(destiny, (x - L2World.MAP_MIN_X) >> 4,
				(y - L2World.MAP_MIN_Y) >> 4, z, (tx - L2World.MAP_MIN_X) >> 4,
				(ty - L2World.MAP_MIN_Y) >> 4, tz);
	}

	/**
	 * @see net.sf.l2j.gameserver.GeoData#addGeoDataBug(net.sf.l2j.gameserver.model.actor.instance.L2PcInstance,
	 *      java.lang.String)
	 */
	@Override
	...
	{
		return canSee((x - L2World.MAP_MIN_X) >> 4,
				(y - L2World.MAP_MIN_Y) >> 4, z, (tx - L2World.MAP_MIN_X) >> 4,
				(ty - L2World.MAP_MIN_Y) >> 4, tz);
	}

	private static boolean canSee(int x, int y, double z, int tx, int ty,
			int tz)
	{
		int dx = (tx - x);
		int dy = (ty - y);
	...
	private static boolean canSeeDebug(L2PcInstance gm, int x, int y, double z,
			int tx, int ty, int tz)
	{
		...
		gm.sendMessage("Los: from X: " + x + "Y: " + y + "--->> X: " + tx
				+ " Y: " + ty);
	...
	private static Location MoveCheck(Location destiny, int x, int y, double z,
			int tx, int ty, int tz)
	{
		int dx = (tx - x);
		int dy = (ty - y);
			...
			for (int i = 0; i <= dx; i++)
			{
				...
				if (d > 0)
				{
					...
					if (!NcanMoveNext(x, y, (int) z, next_x, next_y, tz))
						return new Location((x << 4) + L2World.MAP_MIN_X,
								(y << 4) + L2World.MAP_MIN_Y, (int) z);
					next_y += inc_y;
					if (!NcanMoveNext(x, y, (int) z, next_x, next_y, tz))
						return new Location((x << 4) + L2World.MAP_MIN_X,
								(y << 4) + L2World.MAP_MIN_Y, (int) z);
	...
	private static void NinitGeodata()
	{
		...
		try
		{
			...
			_geo_bugs_out = new BufferedOutputStream(
					new FileOutputStream(geo_bugs, true));
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public static boolean LoadGeodataFile(byte rx, byte ry)
	{
		...
		short regionoffset = (short) ((rx << 5) + ry);
		_log.info("Geo Engine: - Loading: " + fname + " -> region offset: "
				+ regionoffset + "X: " + rx + " Y: " + ry);
		...
		try
		{
			...
			_log.info("Geo Engine: - Max Layers: " + flor + " Size: " + size
					+ " Loaded: " + index);
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	private static short NgetType(int x, int y)
	{
		...
		int index = 0;
		// Geodata without index - it is just empty so index can be calculated
		// on the fly
		if (Geodata_index.get(region) == null)
			index = ((blockX << 8) + blockY) * 3;
		// Get Index for current block of current geodata region
		else
			index = Geodata_index.get(region).get((blockX << 8) + blockY);
		// Buffer that Contains current Region GeoData
		ByteBuffer geo = Geodata.get(region);
		if (geo == null)
		{
			if (Config.DEBUG)
				_log.warning("Geo Region - Region Offset: " + region
						+ " dosnt exist!!");
			return 0;
		}
	...
	private static short NgetHeight(int geox, int geoy, int z)
	{
		...
		} else // multilevel
		{
			...
			{
				_log.warning("Geo Engine: - invalid layers count: " + layers
						+ " at: " + geox + " " + geoy);
				return (short) z;
			}
			...
			int spawnid)
	{
		...
		} else// multilevel
		{
			...
			while (layers > 0)
			{
				...
				height = (short) (height & 0x0fff0);
				height = (short) (height >> 1); // height / 2
				if ((zmin - temph) * (zmin - temph) > (zmin - height)
						* (zmin - height))
					temph = height;
				layers--;
				index += 2;
			...
			{
				// Just log error - we trust GeoData and spawn NPC on nearlest
				// GeoData Z
				if (Config.DEBUG)
					_log.warning(
							"SpawnHeight Error - Couldnt find correct layer to spawn NPC - GeoData or Spawnlist Bug!: zmin: "
									+ zmin + " zmax: " + zmax + " value: "
									+ temph + " SpawnId: " + spawnid + " at: "
									+ geox + " : " + geoy);
				return temph;
			}
		}
		...
		{
			// Just log error - we trust GeoData and spawn NPC on Z given by
			// GeoData
			if (Config.DEBUG)
				_log.warning(
						"SpawnHeight Error - Spawnlist z value is wrong or GeoData error: zmin: "
								+ zmin + " zmax: " + zmax + " value: " + temph
								+ " SpawnId: " + spawnid + " at: " + geox
								+ " : " + geoy);
		}
		return temph;
	}
	 ...
	 */
	private static boolean NcanMoveNext(int x, int y, int z, int tx, int ty,
			int tz)
	{
		short region = getRegionOffset(x, y);
		int blockX = getBlock(x);
		...
		} else// multilevel
		{
			...
			while (layers > 0)
			{
				...
				if ((tz - tempz2) * (tz - tempz2) > (tz - height)
						* (tz - height))
					tempz2 = height;
				layers--;
				index += 2;
