...
public class RequestGetItemFromPet extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestGetItemFromPet.class.getName());
	...
	void runImpl()
	{
		...
		if (_amount < 0)
		{
			Util.handleIllegalPlayerAction(player,
					"[RequestGetItemFromPet] count < 0! ban! oid: " + _objectId
		...
		if (pet.transferItem("Transfer", _objectId, _amount,
				player.getInventory(), player, pet) == null)
			_log.warning("Invalid Item transfer request: " + pet.getName()
					+ "(pet) --> " + player.getName());
	}
