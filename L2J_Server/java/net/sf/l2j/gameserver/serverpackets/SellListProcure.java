...
public class SellListProcure extends ServerBasePacket
{
	...
	// Logger.getLogger(SellListProcure.class.getName());
	...
	public SellListProcure(L2PcInstance player, int castleId)
	{
		...
		_char = player;
		_castle = castleId;
		_procureList = CastleManager.getInstance().getCastle(_castle)
				.getCropProcure();
	}
	...
	final void runImpl()
	{
		...
		{
			L2ItemInstance item = _char.getInventory()
					.getItemByItemId(c.getId());
	...
	final void writeImpl()
	{
		...
		for (L2ItemInstance item : _sellList.keySet())
		{
			...
			writeH(0); // unknown
			writeD(0); // price, u shouldnt get any adena for crops, only raw
						// materials
		}
	}
