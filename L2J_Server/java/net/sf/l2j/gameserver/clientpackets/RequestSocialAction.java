...
public class RequestSocialAction extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestSocialAction.class.getName());

	// format cd
	private final int _actionId;

	/**
	 * packet type id 0x1b format: cd
	 *
	 * @param decrypt
	 */
	...
	void runImpl()
	{
		...
		if (_actionId < 2 || _actionId > 13)
		{
			Util.handleIllegalPlayerAction(activeChar,
					"Warning!! Character " + activeChar.getName()
							+ " of account " + activeChar.getAccountName()
		...
		if (activeChar.getPrivateStoreType() == 0
				...
				&& !activeChar.isAllSkillsDisabled() && activeChar.getAI()
						.getIntention() == CtrlIntention.AI_INTENTION_IDLE)
		{
			if (Config.DEBUG)
				_log.fine("Social Action:" + _actionId);

			SocialAction atk = new SocialAction(activeChar.getObjectId(),
					_actionId);
