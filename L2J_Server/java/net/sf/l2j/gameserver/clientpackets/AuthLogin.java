...
public class AuthLogin extends ClientBasePacket
{
	...
	void runImpl()
	{
		SessionKey key = new SessionKey(_loginKey1, _loginKey2, _playKey1,
				_playKey2);
		if (Config.DEBUG)
		{
		...
		// This packet could be send again (by stupid cheaters) - so GS should
		// wait till LS will confirm again
		client.setAuthed(false);
		client.setLoginName(_loginName);
		LoginServerThread.getInstance().addGameServerLogin(_loginName,
				getConnection());

		LoginServerThread.getInstance()
				.addWaitingClientAndSendRequest(_loginName, client, key);
	}
