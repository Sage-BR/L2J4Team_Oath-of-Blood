...
public class PacketHandler
{
	...
	private static LinkedList<CustomPacketHandlerInterface> customhandlers = new LinkedList<>();
	...
	public static ClientBasePacket handlePacket(ByteBuffer data,
			ClientThread client)
	{
		...
		{
			_log.warning("Player with IP: " + client.getConnection().getIP()
					+ " tried to hack account : " + client.getLoginName());
...
//				// Ride
//				break;
			case 0x6b: // send when talking to trainer npc, to show list of
						// available skills
...
//				break;
			...
			case 0xab:
				...
				break;
			case 0xac: // we still need this packet to handle BACK button of
						// craft dialog
			...
			case 0xd0: // Request auto ss and (b)sps.
				...
				default:
					...
					_log.warning(
							"Unknown Packet: 0xd0:" + Integer.toHexString(id2));
			...
			/*
			 * case 0xee: msg = new RequestChangePartyLeader(data, _client);
			 * break;
			 */
			...
			default:
			{
				...
				_log.warning("Unknown Packet 0x" + Integer.toHexString(id)
						+ " from " + client.getActiveChar());
