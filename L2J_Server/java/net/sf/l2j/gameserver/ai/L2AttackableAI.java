...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2PcInstance)
		{
			...
			// move mode
			if ((((L2PcInstance) target).isGM() && ((L2PcInstance) target)
					.getAccessLevel() <= Config.GM_DONT_TAKE_AGGRO)
					|| (!(me instanceof L2RaidBossInstance)
	...
	private void thinkActive()
	{
				...
				&& ((L2MinionInstance) _actor).getLeader() != null)
		{
			...
			if (_actor.getPlanDistanceSq(
					((L2MinionInstance) _actor).getLeader()) > offset * offset)
			{
				int x1, y1, z1;
		...
		else if (npc.getSpawn() != null && Rnd.nextInt(RANDOM_WALK_RATE) == 0)
		{
			...
			if (npc.getSpawn().getLocx() == 0 && npc.getSpawn().getLocy() == 0)
			{
				...
				if (Territory.getInstance()
						.getProcMax(npc.getSpawn().getLocation()) > 0)
					return;

				// Calculate a destination point in the spawn area
				int p[] = Territory.getInstance()
						.getRandomPoint(npc.getSpawn().getLocation());
				x1 = p[0];
				y1 = p[1];
				z1 = p[2];
	...
	private void thinkAttack()
	{
		...
		} else
		{
			...
			if (((L2NpcInstance) _actor).getFactionId() != null)
			{
						...
						.values())
				{
					if (obj instanceof L2NpcInstance)
					{
						...
						if (npc == null || getAttackTarget() == null
			...
			if (weapon != null && weapon.getItemType() == L2WeaponType.BOW)
			{
				...
				double distance2 = _actor.getPlanDistanceSq(
						getAttackTarget().getX(), getAttackTarget().getY());
				if (distance2 <= 10000)
				{
					int chance = 5;
			...
			if (dist2 > range * range)
			{
				...
				if (!_actor.isMuted() && (!Config.ALT_GAME_MOB_ATTACK_AI
			...
			else
			{
				...
				if (!_actor.isMuted() /* && _rnd.nextInt(100) <= 5 */)
				{
					...
					{
						if (/*
							 * sk.getCastRange() >= dist && sk.getCastRange() <=
							 * 70 &&
							 */!sk.isPassive()
								&& _actor.getCurrentMp() >= _actor.getStat()
										.getMpConsume(sk)
								&& !_actor.isSkillDisabled(sk.getId())
								&& (Rnd.nextInt(100) <= 8
										|| (_actor instanceof L2PenaltyMonsterInstance
												&& Rnd.nextInt(100) <= 20)))
						{
							L2Object OldTarget = _actor.getTarget();
							if (sk.getSkillType() == L2Skill.SkillType.BUFF
							...
							if (!useSkillSelf && !GeoData.getInstance()
									.canSeeTarget(_actor, _actor.getTarget()))
								return;
							clientStopMoving(null);
							_accessor.doCast(sk);
							_actor.setTarget(OldTarget);
