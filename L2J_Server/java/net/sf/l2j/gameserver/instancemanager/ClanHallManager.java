...
public class ClanHallManager
{
	protected static Logger _log = Logger
			.getLogger(ClanHallManager.class.getName());
	...
	private final void load()
	{
		...
		try
		{
			...
			statement = con
					.prepareStatement("SELECT id from clanhall order by id");
			...
			System.out.println(
					"Loaded: " + getClanHalls().size() + " clan halls");
		} catch (Exception e)
		{
			System.out.println(
					"Exception: ClanHallManager.load(): " + e.getMessage());
			e.printStackTrace();
		}
	...
	{
		return getClanHall(activeObject.getPosition().getX(),
	...
	public final int getClanHallIndex(int clanHallId)
	{
		...
		for (int i = 0; i < getClanHalls().size(); i++)
		{
			...
			if (clanHall != null && clanHall.getId() == clanHallId)
				return i;
		}
		return -1;
	}
	...
	{
		return getClanHallIndex(activeObject.getPosition().getX(),
				activeObject.getPosition().getY());
	}
	...
	public final int getClanHallIndex(int x, int y, int offset)
	{
		...
		for (int i = 0; i < getClanHalls().size(); i++)
		{
			...
			if (zone != null)
			{
				...
				if (Config.DEBUG)
					_log.warning("ch" + i + ":(" + x + "," + y + ") distance "
							+ Util.calculateDistance(x, y, 0, coord[0],
									coord[1]));
				if (clanHall != null
						&& Util.calculateDistance(x, y, 0, x1, y1) < offset)
				{
					id = i;
	...
	public final int getClanHallIndex(String name)
	{
		...
		for (int i = 0; i < getClanHalls().size(); i++)
		{
			clanHall = getClanHalls().get(i);
			if (clanHall != null
					&& clanHall.getName().equalsIgnoreCase(name.trim()))
				return i;
		}
		return -1;
	}
