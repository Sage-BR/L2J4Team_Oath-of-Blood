...
public class AdminMobGroup implements IAdminCommandHandler
{
	private static String[] _adminCommands =
		{ "admin_mobmenu", "admin_mobgroup_list", "admin_mobgroup_create",
	...
	private void attack(String command, L2PcInstance activeChar,
			L2Character target)
	{
		int groupId;
	...
	private void follow(String command, L2PcInstance activeChar,
			L2Character target)
	{
		int groupId;
	...
	private void removeGroup(String command, L2PcInstance activeChar)
	{
		...
		if (MobGroupTable.getInstance().removeGroup(groupId))
			activeChar.sendMessage(
					"Mob group " + groupId + " unspawned and removed.");
	}
	...
	private void noMove(String command, L2PcInstance activeChar)
	{
		...
		else if (enabled.equalsIgnoreCase("off")
	...
	private void doAnimation(L2PcInstance activeChar)
	{
		Broadcast.toSelfAndKnownPlayersInRadius(activeChar,
				new MagicSkillUser(activeChar, 1008, 1, 4000, 0),
				2250000/* 1500 */);
	...
	private void showGroupList(L2PcInstance activeChar)
	{
		...
		for (MobGroup mobGroup : mobGroupList)
			activeChar.sendMessage(
					mobGroup.getGroupId() + ": " + mobGroup.getActiveMobCount()
							+ " alive out of " + mobGroup.getMaxMobCount()
							+ " of NPC ID " + mobGroup.getTemplate().npcId
							+ " (" + mobGroup.getStatus() + ")");

		activeChar
				.sendPacket(new SystemMessage(SystemMessage.FRIEND_LIST_FOOT));
	}
}