...
public class TradeRequest extends ClientBasePacket
{
	...
	void runImpl()
	{
		...
		{
			player.sendMessage(
					"You or your target cant request trade in Olympiad mode");
			return;
		}

		// Alt game - Karma punishment
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TRADE
				&& (player.getKarma() > 0 || partner.getKarma() > 0))
		{
		...
		SystemMessage sm = new SystemMessage(
				SystemMessage.REQUEST_S1_FOR_TRADE);
		sm.addString(partner.getName());
		player.sendPacket(sm);
	}
