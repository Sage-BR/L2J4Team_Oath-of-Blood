...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_edit_npc", "admin_save_npc", "admin_show_droplist",
				"admin_edit_drop", "admin_add_drop", "admin_del_drop",
				"admin_showShop", "admin_showShopList", "admin_addShopItem",
				"admin_delShopItem", "admin_box_access", "admin_editShopItem",
				"admin_close_window" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		} else if (!Config.ALT_PRIVILEGES_ADMIN
			...
			try
			{
				...
				} else if (st.countTokens() == 6)
				{
					try
					{
						...
						updateDropData(activeChar, npcId, itemId, min, max,
								category, chance);
					} catch (Exception e)
					{
						_log.fine(
								"admin_edit_drop parements error: " + command);
					}
				} else
				{
					SystemMessage sm = new SystemMessage(SystemMessage.S1_S2);
					sm.addString("Command error:");
					sm.addString(
							"//edit_drop <npc_id> <item_id> <category> [<min> <max> <chance>]");
					activeChar.sendPacket(sm);
				}
			} catch (StringIndexOutOfBoundsException e)
			{
		...
		} else if (command.startsWith("admin_add_drop "))
		{
			...
			try
			{
				...
				if (st.countTokens() == 1)
				{
					...
					if (npcId > 0)
					{
						L2NpcTemplate npcData = NpcTable.getInstance()
								.getTemplate(npcId);
				...
				} else if (st.countTokens() == 6)
				{
					try
					{
						...
						addDropData(activeChar, npcId, itemId, min, max,
								category, chance);
					} catch (Exception e)
					{
				...
				} else
				{
					...
					sm.addString("Command error:");
					sm.addString(
							"//add_drop <npc_id> [<item_id> <category> <min> <max> <chance>]");
					activeChar.sendPacket(sm);
				}
			} catch (StringIndexOutOfBoundsException e)
			{
	...
	private void editShopItem(L2PcInstance admin, String[] args)
	{
		...
		L2TradeList tradeList = TradeController.getInstance()
				.getBuyList(tradeListID);
		...
		if (args.length > 3)
		{
			...
			int order = findOrderTradeList(itemID,
					tradeList.getPriceForItemId(itemID), tradeListID);

			tradeList.replaceItem(itemID, Integer.parseInt(args[3]));
			updateTradeList(itemID, price, tradeListID, order);

			admin.sendMessage("Updated price for " + item.getName()
					+ " in Trade List " + tradeListID);
	...
	private void delShopItem(L2PcInstance admin, String[] args)
	{
		...
		replyMSG.append("<br>Item to Delete: "
				+ ItemTable.getInstance().getTemplate(itemID).getName());
		replyMSG.append("<table>");
		replyMSG.append(
				"<tr><td width=100>Property</td><td width=100>Value</td></tr>");
		replyMSG.append("<tr><td><br></td><td></td></tr>");
		replyMSG.append("<tr><td>Price</td><td>"
				+ tradeList.getPriceForItemId(itemID) + "</td></tr>");
		replyMSG.append("</table>");
		replyMSG.append("<center><br><br><br>");
		replyMSG.append(
	...
	private void addShopItem(L2PcInstance admin, String[] args)
	{
		...
		if (args.length > 3)
		{
			...
			L2ItemInstance newItem = ItemTable.getInstance()
					.createDummyItem(itemID);
			newItem.setPriceToSell(price);
			tradeList.addItem(newItem);
			storeTradeList(itemID, price, tradeListID, order);

			admin.sendMessage("Added " + newItem.getItem().getName()
					+ " to Trade List " + tradeList.getListId());
			showShopList(admin, tradeListID, 1);
			return;
		}
	...
	private TextBuilder itemListHtml(L2TradeList tradeList, int page)
	{
		...
		replyMSG.append(
				"<html><title>Merchant Shop List Page: " + page + "</title>");
		replyMSG.append("<body>");
		...
		int end = Math.min(((page - 1) * PAGE_LIMIT) + (PAGE_LIMIT - 1),
				tradeList.getItems().size() - 1);
		// System.out.println(end);
		for (L2ItemInstance item : tradeList.getItems(start, end + 1))
		{
			replyMSG.append("<tr><td><a action=\"bypass -h admin_editShopItem "
					+ tradeList.getListId() + " " + item.getItemId() + "\">"
		...
		if (page > 1)
		{
			replyMSG.append("<td><button value=\"Page" + (page - 1)
			...
			if (tradeList != null)
				replyMSG.append(
						"<tr><td><a action=\"bypass -h admin_showShopList "
								+ tradeList.getListId() + " 1\">Trade List "
								+ tradeList.getListId() + "</a></td></tr>");
		}
	...
	private void storeTradeList(int itemID, int price, int tradeListID,
			int order)
	{
		...
		try
		{
			...
			PreparedStatement stmt = con.prepareStatement(
					"INSERT INTO merchant_buylists values (" + itemID + ","
	...
	private void updateTradeList(int itemID, int price, int tradeListID,
			int order)
	{
		java.sql.Connection con = null;
		try
		{
			...
			PreparedStatement stmt = con
					.prepareStatement("UPDATE merchant_buylists SET `price`='"
							+ price + "' WHERE `shop_id`='" + tradeListID
							+ "' AND `order`='" + order + "'");
			stmt.execute();
			stmt.close();
		} catch (SQLException esql)
	...
	private void deleteTradeList(int tradeListID, int order)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement stmt = con.prepareStatement(
					"DELETE FROM merchant_buylists WHERE `shop_id`='"
							+ tradeListID + "' AND `order`='" + order + "'");
			stmt.execute();
			stmt.close();
		} catch (SQLException esql)
		{
	...
	private int findOrderTradeList(int itemID, int price, int tradeListID)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement stmt = con.prepareStatement(
					"SELECT * FROM merchant_buylists WHERE `shop_id`='"
							+ tradeListID + "' AND `item_id` ='" + itemID
							+ "' AND `price` = '" + price + "'");
	...
	private List<L2TradeList> getTradeLists(int merchantID)
	{
		...
		String content = HtmCache.getInstance()
				.getHtm("data/html/merchant/" + merchantID + ".htm");

		if (content == null)
		{
			content = HtmCache.getInstance()
					.getHtm("data/html/merchant/7001.htm");
		...
		for (String line : lines)
		{
			...
			if (pos >= 0)
			{
				int tradeListID = Integer
						.decode((line.substring(pos + target.length() + 1))
								.split("\"")[0]);
				// System.out.println(tradeListID);
				tradeLists.add(
						TradeController.getInstance().getBuyList(tradeListID));
			}
		}
	...
	private void Show_Npc_Property(L2PcInstance adminPlayer, L2NpcTemplate npc)
	{
		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		String content = HtmCache.getInstance()
				.getHtm("data/html/admin/editnpc.htm");
		...
		if (content != null)
		{
			...
			adminReply.replace("%serverSideName%",
					npc.serverSideName ? "1" : "0");
			adminReply.replace("%title%", npc.title);
			adminReply.replace("%serverSideTitle%",
					npc.serverSideTitle ? "1" : "0");
			adminReply.replace("%collisionRadius%",
					String.valueOf(npc.collisionRadius));
			adminReply.replace("%collisionHeight%",
					String.valueOf(npc.collisionHeight));
			...
			adminReply.replace("%attackRange%",
					String.valueOf(npc.baseAtkRange));
			...
			adminReply.replace("%walkSpd%",
					String.valueOf(npc.baseRunSpd * 0.7));
			adminReply.replace("%runSpd%", String.valueOf(npc.baseRunSpd));
			adminReply.replace("%factionId%",
					npc.factionId == null ? "" : npc.factionId);
			adminReply.replace("%factionRange%",
					String.valueOf(npc.factionRange));
			adminReply.replace("%isUndead%", npc.isUndead ? "1" : "0");
			adminReply.replace("%absorbLevel%",
					String.valueOf(npc.absorb_level));
		} else
			adminReply.setHtml(
					"<html><head><body>File not found: data/html/admin/editnpc.htm</body></html>");
	...
	private void save_npc_property(L2PcInstance adminPlayer, String command)
	{
		...
		try
		{
			...
			if (statToSet.equals("templateId"))
				...
				newNpcData.set("serverSideTitle",
						Integer.valueOf(value) == 1 ? 1 : 0);
			...
			else if (statToSet.equals("sex"))
			{
				...
				newNpcData.set("sex", intValue == 0 ? "male"
						: intValue == 1 ? "female" : "etc");
			} else if (statToSet.equals("type"))
			{
				Class.forName("net.sf.l2j.gameserver.model.actor.instance."
						+ value + "Instance");
			...
			else if (statToSet.equals("absorbLevel"))
			{
				...
				newNpcData.set("absorb_level",
						intVal < 0 ? 0 : intVal > 12 ? 0 : intVal);
			}
		} catch (Exception e)
		{
		...
		Show_Npc_Property(adminPlayer,
				NpcTable.getInstance().getTemplate(npcId));
	}
	...
	private void showNpcDropList(L2PcInstance admin, int npcId)
	{
		...
		TextBuilder replyMSG = new TextBuilder(
				"<html><title>NPC: " + npcData.name + "(" + npcData.npcId
						+ ") 's drop manage</title>");
		replyMSG.append("<body>");
		replyMSG.append(
		...
		for (L2DropCategory cat : npcData.getDropData())
			for (L2DropData drop : cat.getAllDrops())
			{
				replyMSG.append("<tr><td><a action=\"bypass -h admin_edit_drop "
						+ npcData.npcId + " " + drop.getItemId() + " "
						+ cat.getCategoryType() + "\">" + npcData.npcId + " "
						+ drop.getItemId() + " " + cat.getCategoryType()
						+ "</a></td>" + "<td>"
						+ ItemTable.getInstance().getTemplate(drop.getItemId())
								.getName()
						+ "[" + drop.getItemId() + "]" + "</td><td>"
						+ (drop.isQuestDrop() ? "Q"
								: (cat.isSweep() ? "S" : "D"))
						+ "</td><td>" + "<a action=\"bypass -h admin_del_drop "
						+ npcData.npcId + " " + drop.getItemId() + " "
			...
			int category)
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT mobId, itemId, min, max, category, chance FROM droplist WHERE mobId="
							+ npcId + " AND itemId=" + itemId + " AND category="
							+ category);
			...
			TextBuilder replyMSG = new TextBuilder(
					"<html><title>the detail of dropdata: (" + npcId + " "
			...
			if (dropData.next())
			{
				...
				replyMSG.append("<tr><td>Appertain of NPC</td><td>"
						+ NpcTable.getInstance()
								.getTemplate(dropData.getInt("mobId")).name
						+ "</td></tr>");
				replyMSG.append("<tr><td>ItemName</td><td>"
						+ ItemTable.getInstance()
								.getTemplate(dropData.getInt("itemId"))
								.getName()
				...
				replyMSG.append("<center>");
				replyMSG.append(
						"<button value=\"Save Modify\" action=\"bypass -h admin_edit_drop "
								+ npcId + " " + itemId + " " + category
	...
	private void showAddDropData(L2PcInstance admin, L2NpcTemplate npcData)
	{
		...
		TextBuilder replyMSG = new TextBuilder("<html><title>Add dropdata to "
				+ npcData.name + "(" + npcData.npcId + ")</title>");
		replyMSG.append("<body>");
		replyMSG.append("<table>");
		replyMSG.append(
	...
	private void updateDropData(L2PcInstance admin, int npcId, int itemId,
			int min, int max, int category, int chance)
	{
		java.sql.Connection con = null;
		...
		try
		{
			...
			PreparedStatement statement2 = con.prepareStatement(
					"SELECT mobId FROM droplist WHERE mobId=? AND itemId=? AND category=?");
	...
	private void addDropData(L2PcInstance admin, int npcId, int itemId, int min,
			int max, int category, int chance)
	{
		java.sql.Connection con = null;
			...
			int category)
	{
		...
		try
		{
			...
			{
				PreparedStatement statement2 = con.prepareStatement(
						"DELETE FROM droplist WHERE mobId=? AND itemId=? AND category=?");
				statement2.setInt(1, npcId);
				statement2.setInt(2, itemId);
				statement2.setInt(3, category);
				statement2.execute();
				...
				NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
				TextBuilder replyMSG = new TextBuilder(
						"<html><title>Delete drop data(" + npcId + ", " + itemId
