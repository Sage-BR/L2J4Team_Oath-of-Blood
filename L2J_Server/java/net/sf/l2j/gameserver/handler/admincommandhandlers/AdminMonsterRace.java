...
public class AdminMonsterRace implements IAdminCommandHandler
{
	...
	{
		/*
		 * -1 0 to initial the race 0 15322 to start race 13765 -1 in middle of
		 * race -1 0 to end the race
		 *
		 * 8003 to 8027
		 */
		...
		if (state == -1)
		{
			...
			race.newRace();
			race.newSpeeds();
			MonRaceInfo spk = new MonRaceInfo(codes[state][0], codes[state][1],
		...
		} else if (state == 0)
		{
			...
			ThreadPoolManager.getInstance()
					.scheduleGeneral(new RunRace(codes, activeChar), 5000);
		}
	...
	class RunRace implements Runnable
	{
		...
		public void run()
		{
			...
			/*
			 * int[] speed = new int[8]; for (int i=0; i<8; i++) { for (int j=0;
			 * j<20; j++) { //System.out.println("Adding "+speeds1[i][j]
			 * +" and "+ speeds2[i][j]); speed[i] += (speeds1[i][j]*1);// +
			 * (speeds2[i][j]*1); }
			...
			activeChar.broadcastPacket(spk);
			ThreadPoolManager.getInstance()
					.scheduleGeneral(new RunEnd(activeChar), 30000);
		}
	}
