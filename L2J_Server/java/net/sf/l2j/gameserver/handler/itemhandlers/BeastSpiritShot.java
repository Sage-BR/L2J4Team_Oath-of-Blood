	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		if (playable instanceof L2Summon)
		{
			...
			activeOwner.sendPacket(
					new SystemMessage(SystemMessage.PET_CANNOT_USE_ITEM));
			return;
		...
		if (activePet instanceof L2PetInstance)
		{
			...
			if (weaponInst == null)
			{
				activeOwner.sendPacket(new SystemMessage(
						SystemMessage.CANNOT_USE_SPIRITSHOTS));
				return;
			}
			...
			if (isBlessed)
				weaponInst.setChargedSpiritshot(
						L2ItemInstance.CHARGED_BLESSED_SPIRITSHOT);
			else
				weaponInst.setChargedSpiritshot(
						L2ItemInstance.CHARGED_SPIRITSHOT);
		} else
		{
			...
			if (isBlessed)
				activePet.setChargedSpiritShot(
						L2ItemInstance.CHARGED_BLESSED_SPIRITSHOT);
			else
				activePet.setChargedSpiritShot(
						L2ItemInstance.CHARGED_SPIRITSHOT);
		...
		if (!activeOwner.destroyItem("Consume", item.getObjectId(),
				shotConsumption, null, false))
		{
			...
			activeOwner.sendPacket(
					new SystemMessage(SystemMessage.NOT_ENOUGH_SPIRITSHOTS));
			return;
		}
		...
		Broadcast
				.toSelfAndKnownPlayersInRadius(activeOwner,
						new MagicSkillUser(activePet, activePet,
								isBlessed ? 2009 : 2008, 1, 0, 0),
						360000/* 600 */);
	}
