...
public class L2Clan
{
	...
	private List<L2Skill> _skills = new FastList<>();
	...
	public void setNewLeader(L2ClanMember member)
	{
		...
		SystemMessage sm = new SystemMessage(
				SystemMessage.CLAN_LEADER_PRIVILEGES_HAVE_BEEN_TRANSFERRED_TO_S1);
	...
	{
		L2ClanMember member = new L2ClanMember(this, player.getName(),
				player.getLevel(), player.getClassId().getId(),
				player.getObjectId(), player.getPledgeType(),
				player.getPowerGrade(), player.getTitle());
		// store in memory
	...
	public void removeClanMember(String name, long clanJoinExpiryTime)
	{
		...
		if (exMember == null)
		{
			_log.warning("Member " + name
					+ " not found in clan while trying to remove");
			return;
		}
		...
		if (exMember.isOnline())
		{
			...
			if (player.isClanLeader())
			{
				...
				player.setClanCreateExpiryTime(System.currentTimeMillis()
						+ Config.ALT_CLAN_CREATE_DAYS * 86400000); // 24*60*60*1000
																	// =
																	// 86400000
			}
			player.setClan(null);
		...
		} else
		{
			removeMemberInDatabase(exMember, clanJoinExpiryTime,
					getLeaderName().equalsIgnoreCase(name)
							? System.currentTimeMillis()
									+ Config.ALT_CLAN_CREATE_DAYS * 86400000
							: 0);
		}
	}
	...
	public void setLevel(int level)
	{
		...
		if (_Forum == null)
		{
			if (_level >= 2)
			{
				_Forum = ForumsBBSManager.getInstance()
						.getForumByName("ClanRoot").GetChildByName(_name);
				if (_Forum == null)
				{
					_Forum = ForumsBBSManager.getInstance().CreateNewForum(
							_name,
							ForumsBBSManager.getInstance()
									.getForumByName("ClanRoot"),
							Forum.CLAN, Forum.CLANMEMBERONLY, getClanId());
				}
			}
		}
	}
	...
	public void updateClanInDB()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"UPDATE clan_data SET leader_id=?,ally_id=?,ally_name=?,reputation_score=?,ally_penalty_expiry_time=?,ally_penalty_type=?,char_penalty_expiry_time=?,dissolving_expiry_time=? WHERE clan_id=?");
	...
	public void store()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"INSERT INTO clan_data (clan_id,clan_name,clan_level,hasCastle,hasHideout,ally_id,ally_name,leader_id,crest_id,crest_large_id,ally_crest_id) values (?,?,?,?,?,?,?,?,?,?,?)");
	...
	private void removeMemberInDatabase(L2ClanMember member,
			long clanJoinExpiryTime, long clanCreateExpiryTime)
	{
		java.sql.Connection con = null;
		try
		{
			...
			statement = con.prepareStatement(
					"UPDATE characters SET apprentice=0 WHERE apprentice=?");
			statement.setInt(1, member.getObjectId());
			statement.execute();
			statement.close();

			statement = con.prepareStatement(
					"UPDATE characters SET sponsor=0 WHERE sponsor=?");
			statement.setInt(1, member.getObjectId());
			statement.execute();
			statement.close();
		} catch (Exception e)
	...
	private void UpdateWarsInDB()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"UPDATE clan_wars SET wantspeace1=? WHERE clan1=?");
	...
	private void restorewars()
	{
		...
		try
		{
			...
			PreparedStatement statement;
			statement = con.prepareStatement(
					"SELECT clan1, clan2, wantspeace1, wantspeace2 FROM clan_wars");
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{
				if (rset.getInt("clan1") == this._clanId)
					this.setEnemyClan(rset.getInt("clan2"));
//               if(rset.getInt("clan2") == this._clanId) this.setEnemyClan(rset.getInt("clan1"));
	...
	private void restore()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT clan_name,clan_level,hasCastle,hasHideout,ally_id,ally_name,leader_id,crest_id,crest_large_id,ally_crest_id,reputation_score,auction_bid_at,ally_penalty_expiry_time,ally_penalty_type,char_penalty_expiry_time,dissolving_expiry_time FROM clan_data where clan_id=?");
			statement.setInt(1, getClanId());
			...
			if (clanData.next())
			{
				...
				setAllyPenaltyExpiryTime(
						clanData.getLong("ally_penalty_expiry_time"),
						clanData.getInt("ally_penalty_type"));
				if (getAllyPenaltyExpiryTime() < System.currentTimeMillis())
				{
					setAllyPenaltyExpiryTime(0, 0);
				}
				setCharPenaltyExpiryTime(
						clanData.getLong("char_penalty_expiry_time"));
				if (getCharPenaltyExpiryTime() + Config.ALT_CLAN_JOIN_DAYS
						* 86400000 < System.currentTimeMillis()) // 24*60*60*1000
																	// =
																	// 86400000
				{
					setCharPenaltyExpiryTime(0);
				}
				setDissolvingExpiryTime(
						clanData.getLong("dissolving_expiry_time"));
				...
				PreparedStatement statement2 = con.prepareStatement(
						"SELECT char_name,level,classid,obj_Id,title,power_grade,subpledge,apprentice,sponsor FROM characters WHERE clanid=?");
				...
				while (clanMembers.next())
				{
					...
					if (member.getObjectId() == leaderId)
						...
						addClanMember(member);
					member.initApprenticeAndSponsor(
							clanMembers.getInt("apprentice"),
							clanMembers.getInt("sponsor"));
				}
				clanMembers.close();
				statement2.close();
			}
			...
			if (getName() != null)
				_log.config("Restored clan data for \"" + getName()
						+ "\" from database.");
			restorewars();
			restoreSubPledges();
			restoreRankPrivs();
	...
	private void restoreSkills()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT skill_id,skill_level FROM clan_skills WHERE clan_id=?");
			statement.setInt(1, getClanId());
	...
	public L2Skill addNewSkill(L2Skill newSkill)
	{
		...
		if (newSkill != null)
		{
			...
			try
			{
				...
				{
					statement = con.prepareStatement(
							"UPDATE clan_skills SET skill_level=? WHERE skill_id=? AND clan_id=?");
				...
				{
					statement = con.prepareStatement(
							"INSERT INTO clan_skills (clan_id,skill_id,skill_level,skill_name) VALUES (?,?,?,?)");
					statement.setInt(1, getClanId());
			...
			for (L2ClanMember temp : _members.values())
			{
				if (temp.isOnline() && temp.getPlayerInstance() != null)
				{
					if (newSkill.getMinPledgeClass() <= temp.getPlayerInstance()
							.getPledgeClass())
					{
						temp.getPlayerInstance().addSkill(newSkill, false); // Skill
																			// is
																			// not
																			// saved
																			// to
																			// player
																			// DB
						temp.getPlayerInstance().sendPacket(
								new PledgeSkillListAdd(newSkill.getId(),
										newSkill.getLevel()));
					}
				}
			}
	...
	public void addSkillEffects()
	{
		for (L2Skill skill : _Skills.values())
		{
			for (L2ClanMember temp : _members.values())
			{
				if (temp.isOnline() && temp.getPlayerInstance() != null)
				{
					if (skill.getMinPledgeClass() <= temp.getPlayerInstance()
							.getPledgeClass())
						temp.getPlayerInstance().addSkill(skill, false); // Skill
																			// is
																			// not
																			// saved
																			// to
	...
	public void addSkillEffects(L2PcInstance cm)
	{
		...
		for (L2Skill skill : _Skills.values())
		{
			// TODO add skills according to members class( in ex. don't add Clan
			// Agillity skill's effect to lower class then Baron)
	...
	public void broadcastToOtherOnlineMembers(ServerBasePacket packet,
			L2PcInstance player)
	{
		for (L2ClanMember member : _members.values())
		{
	...
	public class RankPrivs
	{
		...
		private int _party;// TODO find out what this stuff means and implement
							// it
	...
	private void restoreSubPledges()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT sub_pledge_id,name,leader_name FROM clan_subpledges WHERE clan_id=?");
			statement.setInt(1, getClanId());
			ResultSet rset = statement.executeQuery();
	...
	public final SubPledge[] getAllSubPledges()
	{
		...
		return _SubPledges.values()
				.toArray(new SubPledge[_SubPledges.values().size()]);
	}

	public SubPledge createSubPledge(int pledgeType, String leaderName,
			String subPledgeName)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"INSERT INTO clan_subpledges (clan_id,sub_pledge_id,name,leader_name) values (?,?,?,?)");
			statement.setInt(1, getClanId());
			...
			if (Config.DEBUG)
				_log.fine("New sub_clan saved in db: " + getClanId() + "; "
						+ pledgeType);
		} catch (Exception e)
		{
	...
	public void updateSubPledgeInDB(int pledgeType)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"UPDATE clan_subpledges SET leader_name=? WHERE clan_id=? AND sub_pledge_id=?");
	...
	private void restoreRankPrivs()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT privs,rank,party FROM clan_privs WHERE clan_id=?");
			statement.setInt(1, getClanId());
	...
	public void setRankPrivs(int rank, int privs)
	{
				...
				if (cm.isOnline())
					if (cm.getPowerGrade() == rank)
						if (cm.getPlayerInstance() != null)
						{
							...
							cm.getPlayerInstance().sendPacket(
									new UserInfo(cm.getPlayerInstance()));
						}
			}
			broadcastClanStatus();
		} else
		{
			...
			try
			{
				...
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement(
						"INSERT INTO clan_privs (clan_id,rank,party,privs) VALUES (?,?,?,?)");
				statement.setInt(1, getClanId());
				statement.setInt(2, rank);
				statement.setInt(3, 0);
				statement.setInt(4, privs);
			...
			{
				_log.warning(
						"Could not create new rank and store clan privs for rank: "
								+ e);
			} finally
			{
				try
	...
	public void setAuctionBiddedAt(int id, boolean storeInDb)
	{
		...
		if (storeInDb)
		{
			...
			try
			{
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement(
						"UPDATE clan_data SET auction_bid_at=? WHERE clan_id=?");
	 ...
	 * @return
	 */
	public boolean CheckClanJoinCondition(L2PcInstance activeChar,
			L2PcInstance target, int pledgeType)
	{
		if (activeChar == null)
		{
			return false;
		...
		if ((activeChar.getClanPrivileges()
				& L2Clan.CP_CL_JOIN_CLAN) != L2Clan.CP_CL_JOIN_CLAN)
		{
		...
		if (activeChar.getObjectId() == target.getObjectId())
		{
			activeChar.sendPacket(
					new SystemMessage(SystemMessage.CANNOT_INVITE_YOURSELF));
			return false;
		}
		...
		if ((target.getClanId() != 0) || (target.getClanId() != 0))
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_WORKING_WITH_ANOTHER_CLAN);
			sm.addString(target.getName());
			activeChar.sendPacket(sm);
			sm = null;
			return false;
		...
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_MUST_WAIT_BEFORE_JOINING_ANOTHER_CLAN);
			sm.addString(target.getName());
			activeChar.sendPacket(sm);
			sm = null;
			return false;
		}
		if ((target.getLevel() > 40 || target.getClassId().level() >= 2)
				&& pledgeType == -1)
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_DOESNOT_MEET_REQUIREMENTS_TO_JOIN_ACADEMY);
			sm.addString(target.getName());
			activeChar.sendPacket(sm);
			sm = null;
			activeChar.sendPacket(
					new SystemMessage(SystemMessage.ACADEMY_REQUIREMENTS));
			return false;
		}
		if (getSubPledgeMembersCount(pledgeType) >= getMaxNrOfMembers(
				pledgeType))
		{
			if (pledgeType == 0)
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.S1_CLAN_IS_FULL);
			...
			{
				activeChar.sendPacket(
						new SystemMessage(SystemMessage.SUBCLAN_IS_FULL));
			}
			return false;
		}
		return true;
	 ...
	 * @return
	 */
	public boolean CheckAllyJoinCondition(L2PcInstance activeChar,
			L2PcInstance target)
	{
		if (activeChar == null)
		{
		...
		if (leaderClan.getAllyPenaltyExpiryTime() > System.currentTimeMillis())
		{
			...
			{
				activeChar.sendPacket(new SystemMessage(
						SystemMessage.CANT_INVITE_CLAN_WITHIN_1_DAY));
				return false;
			}
		}
		if (target == null)
		...
		{
			activeChar.sendPacket(
					new SystemMessage(SystemMessage.TARGET_MUST_BE_IN_CLAN));
			return false;
		}
		if (!target.isClanLeader())
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_IS_NOT_A_CLAN_LEADER);
			sm.addString(target.getName());
			activeChar.sendPacket(sm);
			sm = null;
			return false;
		...
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_CLAN_ALREADY_MEMBER_OF_S2_ALLIANCE);
		...
		if (targetClan.getAllyPenaltyExpiryTime() > System.currentTimeMillis())
		{
			...
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.S1_CANT_ENTER_ALLIANCE_WITHIN_1_DAY);
			...
			{
				activeChar.sendPacket(new SystemMessage(
						SystemMessage.CANT_ENTER_ALLIANCE_WITHIN_1_DAY));
				return false;
			}
		}
		...
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.OPPOSING_CLAN_IS_PARTICIPATING_IN_SIEGE));
			return false;
		}
		if (leaderClan.isAtWarWith(targetClan.getClanId()))
		{
			activeChar.sendPacket(
					new SystemMessage(SystemMessage.MAY_NOT_ALLY_CLAN_BATTLE));
			return false;
		}
		...
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.YOU_HAVE_EXCEEDED_THE_LIMIT));
			return false;
		}
	...
	public void createAlly(L2PcInstance player, String allyName)
	{
		...
		if (Config.DEBUG)
			_log.fine(player.getObjectId() + "(" + player.getName()
					+ ") requested ally creation from ");

		if (!player.isClanLeader())
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.ONLY_CLAN_LEADER_CREATE_ALLIANCE));
			return;
		}
		if (getAllyId() != 0)
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.ALREADY_JOINED_ALLIANCE));
			return;
		}
		if (getLevel() < 5)
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.TO_CREATE_AN_ALLY_YOU_CLAN_MUST_BE_LEVEL_5_OR_HIGHER));
			return;
		}
		if (getAllyPenaltyExpiryTime() > System.currentTimeMillis())
		{
			...
			{
				player.sendPacket(new SystemMessage(
						SystemMessage.CANT_CREATE_ALLIANCE_10_DAYS_DISOLUTION));
				return;
			}
		}
		...
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.YOU_MAY_NOT_CREATE_ALLY_WHILE_DISSOLVING));
			return;
		}
		if (!Util.isAlphaNumeric(allyName))
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.INCORRECT_ALLIANCE_NAME));
			return;
		}
		if (allyName.length() > 16 || allyName.length() < 2)
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.INCORRECT_ALLIANCE_NAME_LENGTH));
			return;
		}
		if (ClanTable.getInstance().isAllyExists(allyName))
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.ALLIANCE_ALREADY_EXISTS));
			return;
		}
	...
	public void dissolveAlly(L2PcInstance player)
	{
		...
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_DISSOLVE_ALLY_WHILE_IN_SIEGE));
			return;
		}

		broadcastToOnlineAllyMembers(
				new SystemMessage(SystemMessage.ALLIANCE_DISOLVED));

		long currentTime = System.currentTimeMillis();
		for (L2Clan clan : ClanTable.getInstance().getClans())
		{
			if (clan.getAllyId() == getAllyId()
					&& clan.getClanId() != getClanId())
			{
				clan.setAllyId(0);
		...
		setAllyPenaltyExpiryTime(
				currentTime
						+ Config.ALT_CREATE_ALLY_DAYS_WHEN_DISSOLVED * 86400000,
	...
	public void levelUpClan(L2PcInstance player)
	{
		...
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_RISE_LEVEL_WHILE_DISSOLUTION_IN_PROGRESS));
			return;
		}
		...
		case 0:
		{
			...
			{
				if (player.reduceAdena("ClanLvl", 650000, player.getTarget(),
						true))
				{
		...
		case 1:
		{
			...
			{
				if (player.reduceAdena("ClanLvl", 2500000, player.getTarget(),
						true))
				{
		...
		{
			// upgrade to 3
			if (player.getSp() >= 500000
					&& player.getInventory().getItemByItemId(1419) != null)
			{
				// itemid 1419 == proof of blood
				if (player.destroyItemByItemId("ClanLvl", 1419, 1,
						player.getTarget(), false))
				{
		...
		case 4:
		{
					...
					&& player.getInventory().getItemByItemId(3870) != null)
			{
				...
				if (player.destroyItemByItemId("ClanLvl", 3870, 1,
						player.getTarget(), false))
				{
		...
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.FAILED_TO_INCREASE_CLAN_LEVEL);
			player.sendPacket(sm);
			return;
		}
	...
	public void changeLevel(int level)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"UPDATE clan_data SET clan_level = ? WHERE clan_id = ?");
		...
		if (getLeader().isOnline())
		{
			...
			if (4 < level)
			{
				leader.sendPacket(new SystemMessage(
						SystemMessage.CLAN_CAN_ACCUMULATE_CLAN_REPUTATION_POINTS));
			}
		}

		// notify all the members about it
		broadcastToOnlineMembers(
				new SystemMessage(SystemMessage.CLAN_LEVEL_INCREASED));
