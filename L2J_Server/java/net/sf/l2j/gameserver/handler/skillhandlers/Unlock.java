...
public class Unlock implements ISkillHandler
{
			...
			L2Object[] targets)
	{
		...
		{
			boolean success = Formulas.getInstance()
					.calculateUnlockChance(skill);
			if (target instanceof L2DoorInstance)
			{
				...
				if (!door.isUnlockable())
				{
					activeChar.sendPacket(new SystemMessage(
							SystemMessage.UNABLE_TO_UNLOCK_DOOR));
				...
				{
					activeChar.sendPacket(new SystemMessage(
							SystemMessage.FAILED_TO_UNLOCK_DOOR));
				}
			} else if (target instanceof L2ChestInstance)
			{
				...
				} else
				{
					...
					{
						activeChar.sendPacket(SystemMessage
								.sendString("Too hard to open for you.."));
						activeChar.sendPacket(new ActionFailed());
						chest.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED,
								activeChar);
						return;
					}
					...
					} else
					{
						activeChar.sendPacket(
								SystemMessage.sendString("Unlock failed!"));
