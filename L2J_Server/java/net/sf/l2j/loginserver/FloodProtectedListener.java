...
public abstract class FloodProtectedListener extends Thread
{
	private Logger _log = Logger
			.getLogger(FloodProtectedListener.class.getName());
	...
	public FloodProtectedListener(String listenIp, int port)
	{
		...
		try
		{
			...
			{
				_serverSocket = new ServerSocket(_port, 50,
						InetAddress.getByName(_listenIp));
			}
		} catch (IOException e)
		{
			try
	...
	public void run()
	{
		...
		while (true)
		{
			try
			{
				...
				{
					ForeignConnection fConnection = _floodProtection
							.get(connection.getInetAddress().getHostAddress());
					if (fConnection != null)
					{
						...
						if ((fConnection.connectionNumber > Config.FAST_CONNECTION_LIMIT
								...
								|| (System.currentTimeMillis()
										- fConnection.lastConnection) < Config.FAST_CONNECTION_TIME
								|| fConnection.connectionNumber > Config.MAX_CONNECTION_PER_IP)
						{
							fConnection.lastConnection = System
									.currentTimeMillis();
							connection.close();
							fConnection.connectionNumber -= 1;
							if (!fConnection.isFlooding)
								_log.warning("Potential Flood from "
													...
													// passed the check
						{
							...
							_log.info(connection.getInetAddress()
									.getHostAddress()
					...
					{
						fConnection = new ForeignConnection(
								System.currentTimeMillis());
						_floodProtection.put(
