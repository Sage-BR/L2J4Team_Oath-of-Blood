...
public class ValidatePosition extends ClientBasePacket
{
	private static Logger _log = Logger
			.getLogger(ValidatePosition.class.getName());
...
//                int heading = dist > 0 ? (int)(Math.atan2(-dys/dist, -dxs/dist) * 10430.378350470452724949566316381) + 32768 : 0;
				System.out.println("Client X:" + _x + ", Y:" + _y + ", Z:" + _z
						+ ", H:" + _heading/* + "(" + heading + ")" */
						+ ", Dist:"
						+ activeChar.getLastClientDistance(_x, _y, _z));
				System.out.println("Server X:" + realX + ", Y:" + realY + ", Z:"
						+ realZ + ", H:" + activeChar.getHeading() + ", Dist:"
						+ activeChar.getLastServerDistance(realX, realY,
								realZ));
			}

			if (diffSq > 0)
			{
				if ((Config.COORD_SYNCHRONIZE & 1) == 1 && (!activeChar
						.isMoving() // character is not moving, take coordinates
									// from client
				...
				{
					if (Config.DEVELOPER)
						System.out.println(activeChar.getName()
								+ ": Synchronizing position Client --> Server"
								+ (activeChar.isMoving() ? " (collision)"
										: " (stay sync)"));
					activeChar.setXYZ(_x, _y, _z);
					activeChar.setHeading(_heading);
				} else if ((Config.COORD_SYNCHRONIZE & 2) == 2
						&& diffSq > 10000) // more than can be considered to be
											// result of latency
				{
					if (Config.DEVELOPER)
						System.out.println(activeChar.getName()
								+ ": Synchronizing position Server --> Client");
			...
			activeChar.setLastServerPosition(activeChar.getX(),
		...
		} else if (Config.COORD_SYNCHRONIZE == -1)
		{
			...
			if (Point3D.distanceSquared(
					activeChar.getPosition().getWorldPosition(),
					new Point3D(_x, _y, _z)) < 250000)
			...
			/*
			 * double dx = (_x - realX); double dy = (_y - realY); double dist =
			 * Math.sqrt(dx*dx + dy*dy); if ((dist < 500)&&(dist > 2)) //check
			 * it wasnt teleportation, and char isn't there yet
			 * activeChar.sendPacket(new CharMoveToLocation(activeChar));
			 */

			if (Config.DEBUG)
			{
				_log.fine("client pos: " + _x + " " + _y + " " + _z + " head "
						+ _heading);
				_log.fine("server pos: " + realX + " " + realY + " " + realZ
						+ " head " + realHeading);
			}

			if (Config.ACTIVATE_POSITION_RECORDER && !activeChar.isFlying()
					&& Universe.getInstance()
							.shouldLog(activeChar.getObjectId()))
			...
			if (Config.DEVELOPER)
			{
				...
				{
					if (Config.DEBUG)
						_log.fine("client/server dist diff "
								+ (int) Math.sqrt(diff2));
					if (activeChar.isInBoat())
					{
						sendPacket(new ValidateLocationInVehicle(activeChar));
			...
			ThreadPoolManager.getInstance().executeTask(
					new KnownListAsynchronousUpdateTask(activeChar));
...
//          }
		...
		if (activeChar.getParty() != null)
			activeChar.getParty().broadcastToPartyMembers(activeChar,
					new PartyMemberPosition(activeChar));
