...
final class DocumentSkill extends DocumentBase
{
	...
	private List<L2Skill> skillsInFile = new FastList<>();
	...
	protected Number getTableValue(String name)
	{
		...
		{
			_log.log(Level.SEVERE, "error in table: " + name + " of skill Id "
					+ currentSkill.id, e);
			return 0;
		}
	}
	...
	protected Number getTableValue(String name, int idx)
	{
		...
		} catch (RuntimeException e)
		{
			_log.log(Level.SEVERE,
					"wrong level count in skill Id " + currentSkill.id, e);
			return 0;
		}
	}
	...
	protected void parseSkill(Node n)
	{
		...
		if (attrs.getNamedItem("enchantLevels1") != null)
			enchantLevels1 = Integer.parseInt(
					attrs.getNamedItem("enchantLevels1").getNodeValue());
		if (attrs.getNamedItem("enchantLevels2") != null)
			enchantLevels2 = Integer.parseInt(
					attrs.getNamedItem("enchantLevels2").getNodeValue());
		...
		if (currentSkill.sets.length != lastLvl)
			throw new RuntimeException(
					"Skill id=" + skillId + " number of levels missmatch, "
							+ lastLvl + " levels expected");
		...
		for (int i = 0; i < enchantLevels1; i++)
		{
			...
			// currentSkill.enchsets1[i] =
			// currentSkill.sets[currentSkill.sets.length-1];
			...
			{
				if ("set".equalsIgnoreCase(n.getNodeName()))
					parseBeanSet(n, currentSkill.enchsets1[i],
		...
		for (int i = 0; i < enchantLevels2; i++)
		{
			...
			{
				if ("set".equalsIgnoreCase(n.getNodeName()))
					parseBeanSet(n, currentSkill.enchsets2[i],
							currentSkill.sets.length);
			}
		...
		if (currentSkill.enchsets2.length != enchantLevels2)
			throw new RuntimeException(
					"Skill id=" + skillId + " number of levels missmatch, "
							+ enchantLevels2 + " levels expected");
			...
			for (n = first; n != null; n = n.getNextSibling())
			{
				if ("cond".equalsIgnoreCase(n.getNodeName()))
				{
					Condition condition = parseCondition(n.getFirstChild(),
							currentSkill.currentSkills.get(i));
		...
		{
			// [Nemesiss] Enchtanted skill will default take effects from maxLvL
			// of norm skill
		...
		for (int i = lastLvl + enchantLevels1; i < lastLvl + enchantLevels1
				+ enchantLevels2; i++)
		{
			// [Nemesiss] Enchtanted skill will default take effects from maxLvL
			// of norm skill
	...
	{
		int count = 0;
		currentSkill.currentSkills = new FastList<>(
				currentSkill.sets.length + currentSkill.enchsets1.length
		...
		for (int i = 0; i < currentSkill.sets.length; i++)
		{
			try
			{
				currentSkill.currentSkills.add(i,
						currentSkill.sets[i]
								.getEnum("skillType", SkillType.class)
								.makeSkill(currentSkill.sets[i]));
				count++;
			} catch (Exception e)
			{
				_log.log(Level.SEVERE,
						"Skill id=" + currentSkill.sets[i]
								.getEnum("skillType", SkillType.class)
								.makeSkill(currentSkill.sets[i]).getDisplayId()
								+ "level"
								+ currentSkill.sets[i]
										.getEnum("skillType", SkillType.class)
										.makeSkill(currentSkill.sets[i])
		...
		for (int i = 0; i < currentSkill.enchsets1.length; i++)
		{
			try
			{
				currentSkill.currentSkills.add(_count + i,
						currentSkill.enchsets1[i]
								.getEnum("skillType", SkillType.class)
								.makeSkill(currentSkill.enchsets1[i]));
				count++;
			} catch (Exception e)
			{
				_log.log(Level.SEVERE, "Skill id=" + currentSkill.enchsets1[i]
						.getEnum("skillType", SkillType.class)
						.makeSkill(currentSkill.enchsets1[i]).getDisplayId()
						+ " level="
						+ currentSkill.enchsets1[i]
								.getEnum("skillType", SkillType.class)
								.makeSkill(currentSkill.enchsets1[i])
		...
		for (int i = 0; i < currentSkill.enchsets2.length; i++)
		{
			try
			{
				currentSkill.currentSkills.add(_count + i,
						currentSkill.enchsets2[i]
								.getEnum("skillType", SkillType.class)
								.makeSkill(currentSkill.enchsets2[i]));
				count++;
			} catch (Exception e)
			{
				_log.log(Level.SEVERE, "Skill id=" + currentSkill.enchsets2[i]
						.getEnum("skillType", SkillType.class)
						.makeSkill(currentSkill.enchsets2[i]).getDisplayId()
						+ " level="
						+ currentSkill.enchsets2[i]
								.getEnum("skillType", SkillType.class)
								.makeSkill(currentSkill.enchsets2[i])
