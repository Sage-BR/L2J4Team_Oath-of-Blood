...
public class NewCrypt
{
	...
	public boolean checksum(byte[] raw)
	{
		...
		long ecx = -1; // avoids ecs beeing == chksum if an error occured in the
						// try
		int i = 0;
		try
		{
		...
		{
			// Looks like this will only happen on incoming packets as outgoing
			// ones are padded
			// and the error can only happen in last raw[i] =, raw [i+1] = ...
			// and it doesnt really matters for incomming packets
		}
