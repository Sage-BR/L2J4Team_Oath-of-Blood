...
public class Potions implements IItemHandler
{
	protected static final Logger _log = Logger
			.getLogger(Potions.class.getName());
			...
			L2ItemInstance item)
	{
		...
		case 8627:
			...
			if ((itemId == 8622 && activeChar.getExpertiseIndex() == 0)
				...
				res = usePotion(activeChar, 2287,
						(activeChar.getExpertiseIndex() + 1));
			else
			{
	...
	public boolean usePotion(L2PcInstance activeChar, int magicId, int level)
	{
		if (activeChar.isCastingNow() && magicId > 2277 && magicId < 2285)
		{
			...
			ThreadPoolManager.getInstance().scheduleAi(
					new HerbTask(activeChar, magicId, level), _herbstask);
		} else
		{
