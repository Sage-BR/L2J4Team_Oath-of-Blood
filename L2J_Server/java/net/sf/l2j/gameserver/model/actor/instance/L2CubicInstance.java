...
public class L2CubicInstance
{
	protected static Logger _log = Logger
			.getLogger(L2CubicInstance.class.getName());
	...
	public L2CubicInstance(L2PcInstance owner, int id, int level)
	{
		...
		case LIFE_CUBIC:
			_skills.add(4051);
			_disappearTask = ThreadPoolManager.getInstance()
					.scheduleGeneral(new Disappear(), 3600000); // disappear in
																// 60 mins
			doAction(_owner);
			break;
		case VIPER_CUBIC:
		...
		if (_disappearTask == null)
			_disappearTask = ThreadPoolManager.getInstance()
					.scheduleGeneral(new Disappear(), 1200000); // disappear in
																// 20 mins
	}
	...
	public void doAction(L2Character target)
	{
		...
		case STORM_CUBIC:
			_actionTask = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new Action(12), 0, 10000);
			break;
		case VAMPIRIC_CUBIC:
			_actionTask = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new Action(8), 0, 15000);
			break;
		case VIPER_CUBIC:
			_actionTask = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new Action(30), 0, 20000);
			break;
		case POLTERGEIST_CUBIC:
			_actionTask = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new Action(30), 0, 8000);
			break;
		case BINDING_CUBIC:
		case AQUA_CUBIC:
		case SPARK_CUBIC:
		...
		case LIFE_CUBIC:
			_actionTask = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new Heal(50), 0, 30000);
			break;
		}
	}
	...
	private class Action implements Runnable
	{
		...
		public void run()
		{
			if (_owner.isDead() || _target.isDead()
					|| _owner.getTarget() != _target)
			{
				stopAction();
				if (_owner.isDead())
				{
				...
				try
				{
					if (Rnd.get(1, 100) < _chance)
					{
						L2Skill skill = SkillTable.getInstance().getInfo(
								_skills.get(Rnd.get(_skills.size())), _level);
						if (skill != null)
						{
							L2Character[] targets =
								{ _target };
							...
							if ((x * x) + (y * y) + (z * z) <= (range * range))
							{
								...
								MagicSkillUser msu = new MagicSkillUser(_owner,
										_target, skill.getId(), _level, 0, 0);
					...
					if (skill != null)
					{
						...
						if (_owner.isInParty())
						{
							...
							if (party != null)
							{
								...
								List<L2PcInstance> partyList = party
										.getPartyMembers();
								...
								// cubics is known/coded
								int range = 400; // skill.getCastRange();
								for (L2PcInstance element : partyList)
								{
									partyMember = element;
									if (!partyMember.isDead())
									{
										// if party member not dead, check if he
										// is in castrange of heal cubic
										x = (caster.getX()
												- partyMember.getX());
										y = (caster.getY()
												- partyMember.getY());
										...
										// the lowest HP
										if (partyMember
												.getCurrentHp() < partyMember
														.getMaxHp())
										{
											if (percentleft > (partyMember
													.getCurrentHp()
													/ partyMember.getMaxHp()))
											{
												percentleft = (partyMember
														.getCurrentHp()
														/ partyMember
																.getMaxHp());
												target = partyMember;
