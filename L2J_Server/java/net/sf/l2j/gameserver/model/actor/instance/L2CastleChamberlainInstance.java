...
public class L2CastleChamberlainInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (condition == Cond_Owner)
		{
			...
			{
				getCastle().banishForeigner(player); // Move non-clan members
														// off castle area
				return;
			} else if (actualCommand.equalsIgnoreCase("list_siege_clans"))
			{
				getCastle().getSiege().listRegisterClan(player); // List current
																	// register
																	// clan
				return;
			...
			} else if (actualCommand.equalsIgnoreCase("manage_vault"))
			{
				...
				else
				{
					...
					msg.append(
							"<tr><td><a action=\"bypass -h npc_%objectId%_manage_vault deposit\">Deposit Item</a></td></tr>");
					msg.append(
							"<tr><td><a action=\"bypass -h npc_%objectId%_manage_vault withdraw\">Withdraw Item</a></td></tr>");
					msg.append("</table>");
			...
			} else if (actualCommand.equalsIgnoreCase("manor")) // manor control
			{
				...
				case 1: // view/edit manor stats
					...
					break;
				/*
				 * case 2: // set reward type int cropId =
				 * Integer.parseInt(st.nextToken()); int reward =
				 * Integer.parseInt(st.nextToken());
				 * L2World.getInstance().getCastle(_castle).setRewardType(cropId
				 * ,reward); break; case 3: // edit reward int crop =
				 * Integer.parseInt(st.nextToken()); int currentReward =
				 * Integer.parseInt(st.nextToken()); /msg = new
				 * NpcHtmlMessage(1);
				 * msg.setHtml(pageEditManor(crop,currentReward));
				 * player.sendPacket(msg); player.sendPacket(new
				 * ActionFailed()); break;
				 */
				default:
					// _log.info("Invalid bypass for manor control: "+command+"
					// by "+player.getName()+", hack?");
				}
				return;
																		...
																		// control
			{
				if (val != "")
				{
					...
					{
						getCastle().openCloseDoor(player,
								Integer.parseInt(st.nextToken()), open);
					}
				}

				NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile("data/html/chamberlain/" + getTemplate().npcId
						+ "-d.htm");
																	...
																	// control
			{
				...
				msg.append("Current tax rate: " + getCastle().getTaxPercent()
						+ "%<br>");
				msg.append("<table>");
				msg.append("<tr>");
	...
	private String pageEditManor(int crop, int current)
	{
		...
		/*
		 * 2be uncommented with manor system commit String cropName =
		 * ItemTable.getInstance().getTemplate(crop).getName(); String
		 * reward1Name =
		 * ItemTable.getInstance().getTemplate(L2Manor.getInstance().
		 * getRewardItem(crop,1)).getName(); String reward2Name =
		 * ItemTable.getInstance().getTemplate(L2Manor.getInstance().
		 * getRewardItem(crop,2)).getName(); int reward1Amount =
		 * L2Manor.getInstance().getRewardAmount(crop,1); int reward2Amount =
		 * L2Manor.getInstance().getRewardAmount(crop,2);
		 *
		 * text += "<tr><td><font color=\"LEVEL\">"
		 * +cropName+"</font></td><td><a action=\"bypass -h npc_"+getObjectId()
		 * +"_manor_2 "+crop+" 1\">"+reward1Name+"/"+
		 * reward1Amount+"</a></td><td><a action=\"bypass -h npc_"+getObjectId()
		 * +"_manor_2 "+crop+" 2\">"+reward2Name+"/"+reward2Amount+
		 * "</a></td></tr>";
		 */
	...
	private void showManorProcure(L2PcInstance player)
	{
		...
		{
			r += "<tr><td>" + ItemTable.getInstance().getTemplate(crop.getId())
					.getName() + "</td>";
	...
	private void showVaultWindowWithdraw(L2PcInstance player)
	{
		...
		player.sendPacket(new WareHouseWithdrawalList(player,
				WareHouseWithdrawalList.Clan)); // Or Castle ??
	}
