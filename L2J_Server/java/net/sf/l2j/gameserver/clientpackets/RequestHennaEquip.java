...
public class RequestHennaEquip extends ClientBasePacket
{
	...
	void runImpl()
	{
		...
		try
		{
			_count = activeChar.getInventory()
					.getItemByItemId(temp.getItemIdDye()).getCount();
		} catch (Exception e)
		{
		}

		if ((_count >= temp.getAmountDyeRequire())
				&& (activeChar.getAdena() >= temp.getPrice())
				&& activeChar.addHenna(temp))
		{
			...
			sm = null;
			activeChar
					.sendPacket(new SystemMessage(SystemMessage.SYMBOL_ADDED));

			// HennaInfo hi = new HennaInfo(temp,activeChar);
			// activeChar.sendPacket(hi);

			activeChar.getInventory().reduceAdena("Henna", temp.getPrice(),
					activeChar, activeChar.getLastFolkNPC());
			L2ItemInstance dyeToUpdate = activeChar.getInventory()
					.destroyItemByItemId("Henna", temp.getItemIdDye(),
							temp.getAmountDyeRequire(), activeChar,
		...
		} else
		{
			activeChar.sendPacket(
					new SystemMessage(SystemMessage.CANT_DRAW_SYMBOL));
		}
	}
