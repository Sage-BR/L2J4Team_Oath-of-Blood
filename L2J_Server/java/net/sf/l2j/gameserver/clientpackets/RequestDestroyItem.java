...
public class RequestDestroyItem extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestDestroyItem.class.getName());
	 ...
	 * sample
	 *
	 * 59 0b 00 00 40 // object id 01 00 00 00 // count ??
	 *
	 *
	 * format: cdd
	 *
	...
	void runImpl()
	{
		...
		if (_count <= 0)
		{
			Util.handleIllegalPlayerAction(activeChar,
					"[RequestDestroyItem] count <= 0! ban! oid: " + _objectId
		...
		{
			Util.handleIllegalPlayerAction(activeChar,
					"[RequestDestroyItem] count > 1 but item is not stackable! oid: "
		...
		if (L2PetDataTable.isPetItem(itemId))
		{
			...
			try
			{
				if (activeChar.getPet() != null
						&& activeChar.getPet().getControlItemId() == _objectId)
				{
