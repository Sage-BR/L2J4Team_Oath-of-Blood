...
public class CharStatus
{
	...
	private List<Double> _HpStatusWatch = new FastList<>();
	...
	// Method - Public
	/**
	 * Add the decimal value of a percent (current hp/max hp) when a status
	 * update should kick in
	 */
	 ...
	 * all L2PcInstance to inform of HP/MP updates. Players who must be informed
	 * are players that target this L2Character. When a RegenTask is in progress
	 ...
	 * <BR>
	 * <li>L2Attackable : Update the attacker AggroInfo of the L2Attackable
	 * _aggroList</li><BR>
	 * <BR>
	 *
	 * @param i
	...
	public void reduceHp(double value, L2Character attacker, boolean awake)
	{
		...
		if (getActiveChar() instanceof L2PcInstance)
		{
			if (getActiveChar().isDead() && !getActiveChar().isFakeDeath())
				return; // Disabled == null check so skills like Body to Mind
						// work again untill another solution is found
		} else
		{
		...
		{
			// If we're dealing with an L2Attackable Instance and the attacker
			// hit it with an over-hit enabled skill, set the over-hit values.
			// Anything else, clear the over-hit flag
			if (getActiveChar() instanceof L2Attackable)
			{
				if (((L2Attackable) getActiveChar()).isOverhit())
					((L2Attackable) getActiveChar()).setOverhitValues(attacker,
							value);
				else
		...
		{
			// If we're dealing with an L2Attackable Instance and the attacker's
			// hit didn't kill the mob, clear the over-hit flag
			if (getActiveChar() instanceof L2Attackable)
			{
				((L2Attackable) getActiveChar()).overhitEnabled(false);
			}
		...
		if (getActiveChar().isDead())
		{
			...
			synchronized (getActiveChar())
			{
				if (getActiveChar().isKilledAlready())
					return;
	...
	public synchronized final void startHpMpRegeneration()
	{
		if (_RegTask == null && !getActiveChar().isDead())
		{
			...
			int period = Formulas.getInstance()
					.getRegeneratePeriod(getActiveChar());
	...
	public final void setCurrentHp(double newHp, boolean broadcastPacket)
	{
		synchronized (getActiveChar())
		{
			...
			{
				maxHp = getCurrentHp() / maxHp; // Reused maxHp var as
												// percentOfMaxHp so that we
		...
		public void run()
		{
			try
			{
				...
				if (getCurrentCp() < charstat.getMaxCp())
					setCurrentCp(getCurrentCp() + Formulas.getInstance()
							.calcCpRegen(getActiveChar()), false);

				// Modify the current HP of the L2Character and broadcast
				// Server->Client packet StatusUpdate
				if (getCurrentHp() < charstat.getMaxHp())
					setCurrentHp(getCurrentHp() + Formulas.getInstance()
							.calcHpRegen(getActiveChar()), false);

				// Modify the current MP of the L2Character and broadcast
				// Server->Client packet StatusUpdate
				if (getCurrentMp() < charstat.getMaxMp())
					setCurrentMp(getCurrentMp() + Formulas.getInstance()
							.calcMpRegen(getActiveChar()), false);

				if (!getActiveChar().isInActiveRegion())
				{
					// no broadcast necessary for characters that are in
					// inactive regions.
					// stop regeneration for characters who are filled up and in
					// an inactive region.
					if ((getCurrentCp() == charstat.getMaxCp())
					...
					getActiveChar().broadcastStatusUpdate(); // send the
																// StatusUpdate
																// packet
			} catch (Throwable e)
			{
