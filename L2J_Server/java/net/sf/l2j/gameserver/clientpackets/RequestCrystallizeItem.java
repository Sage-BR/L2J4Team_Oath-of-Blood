...
public class RequestCrystallizeItem extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestCrystallizeItem.class.getName());
	...
	void runImpl()
	{
		...
		if (!itemToRemove.getItem().isCrystallizable()
				...
				|| (itemToRemove.getItem()
						.getCrystalType() == L2Item.CRYSTAL_NONE))
		{
			_log.warning(
					"" + activeChar.getObjectId() + " tried to crystallize "
		...
		if ((itemToRemove.getItem().getCrystalType() == L2Item.CRYSTAL_C
				&& skillLevel <= 1)
				|| (itemToRemove.getItem().getCrystalType() == L2Item.CRYSTAL_B
						&& skillLevel <= 2))
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.CRYSTALLIZE_LEVEL_TOO_LOW);
		...
		if ((itemToRemove.getItem().getCrystalType() == L2Item.CRYSTAL_A
				&& skillLevel <= 3)
				|| (itemToRemove.getItem().getCrystalType() == L2Item.CRYSTAL_S
						&& skillLevel <= 4))
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.CRYSTALLIZE_LEVEL_TOO_LOW);
