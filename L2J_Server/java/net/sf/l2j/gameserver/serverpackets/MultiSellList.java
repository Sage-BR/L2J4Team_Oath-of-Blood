...
public class MultiSellList extends ServerBasePacket
{
	...
	public MultiSellList(int listId, L2NpcInstance merchant,
			boolean inventoryOnly, L2PcInstance player)
	{
		_listId = listId;
	 ...
	 * affects the amount of adena and ancient adena in ingredients.
	 *
	...
	void runImpl()
	{
		...
		MultiSellListContainer listTemplate = L2Multisell.getInstance()
				.getList(_listId);
		...
		if (_inventoryOnly)
		{
			...
			if (listTemplate.getMaintainEnchantment())
				items = _player.getInventory()
						.getUniqueItemsByEnchantLevel(false, false);
			else
			...
			for (L2ItemInstance item : items)
			{
				...
				// the multisell list.
				if (!item.isWear() && ((item.getItem() instanceof L2Armor)
						|| (item.getItem() instanceof L2Weapon)))
				{
					enchantLevel = (listTemplate.getMaintainEnchantment()
					...
					for (MultiSellEntry ent : listTemplate.getEntries())
					{
						...
						// needed.
						if (doInclude)
							_list.addEntry(prepareEntry(ent,
									listTemplate.getApplyTaxes(),
		...
		else // this is a list-all type
		{
			...
			for (MultiSellEntry ent : listTemplate.getEntries())
				_list.addEntry(prepareEntry(ent, listTemplate.getApplyTaxes(),
						false, 0));
		}
	}
	...
	private MultiSellEntry prepareEntry(MultiSellEntry templateEntry,
			boolean applyTaxes, boolean maintainEnchantment, int enchantLevel)
	{
		...
		for (MultiSellIngredient ing : templateEntry.getIngredients())
		{
			...
			if (applyTaxes
					&& ((ing.getItemId() == 57) || (ing.getItemId() == 5575)))
			{
				double taxRate = 0.0;
				...
				if (ing.getItemId() == 57)
					newIngredient.setItemCount((int) Math
							.round(ing.getItemCount() * (1 + taxRate)));
				else // ancient adena
				{
					...
					newIngredient = L2Multisell
							.getInstance().new MultiSellIngredient(57,
									(int) Math.round(taxableCount * taxRate));
				}
			}
			...
			else if (maintainEnchantment)
			{
				L2Item tempItem = ItemTable.getInstance()
						.createDummyItem(ing.getItemId()).getItem();
		...
		if (_list != null)
		{
			for (MultiSellEntry ent : _list.getEntries())
			{
				...
				for (MultiSellIngredient i : ent.getProducts())
				{
					writeH(i.getItemId());
					writeD(0);
					writeH(ItemTable.getInstance().getTemplate(i.getItemId())
							.getType2());
				...
				{
					int typeE = ItemTable.getInstance()
							.getTemplate(i.getItemId()).getType2();
