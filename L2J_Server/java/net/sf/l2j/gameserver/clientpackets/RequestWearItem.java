...
public class RequestWearItem extends ClientBasePacket
{
	...
	protected static Logger _log = Logger
			.getLogger(RequestWearItem.class.getName());
	...
	/**
	 * Decrypt the RequestWearItem Client->Server Packet and Create _items table
	 * containing all ItemID to Wear.<BR>
	 * <BR>
	 *
	 */
	...
	void runImpl()
	{
		...
		L2MerchantInstance merchant = (target != null
				&& target instanceof L2MerchantInstance)
						? (L2MerchantInstance) target
						: null;
		...
		if ((totalPrice < 0) || !player.reduceAdena("Wear", (int) totalPrice,
		...
		for (int i = 0; i < _count; i++)
		{
			...
			// Inventory and set properties lastchanged to ADDED and _wear to
			// True
			// If player already own this item : Return its L2ItemInstance (will
			// not be destroy because property _wear set to False)
			L2ItemInstance item = player.getInventory().addWearItem("Wear",
					itemId, player, merchant);
		...
		if (_removeWearItemsTask == null)
			_removeWearItemsTask = ThreadPoolManager.getInstance()
					.scheduleGeneral(new RemoveWearItemsTask(),
