...
public class FaenorScriptEngine extends ScriptEngine
{
	...
	private void loadPackages()
	{
		...
		/*
		 * for (ScriptDocument script : scripts) { _log.sss("Script: "+script);
		 * } _log.sss("Sorting"); orderScripts(); for (ScriptDocument script :
		 * scripts) { _log.sss("Script: "+script); }
		 */
	}
	...
	public void parsePackages()
	{
		...
		try
		{
			context.eval("beanshell", "core", 0, 0,
					"double log1p(double d) { return Math.log1p(d); }");
			context.eval("beanshell", "core", 0, 0,
	...
	public void parseScript(ScriptDocument script, BSFManager context)
	{
		...
		{
			_log.warning(
					"ERROR: No parser registered for Script: " + parserClass);
			e.printStackTrace();
		}
