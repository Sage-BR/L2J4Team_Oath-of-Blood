...
public final class Connection
{
	...
	/**
	 * This is a protection timestamp, for last time of writing into the network
	 * buffer, if it's full. The connection will be closed if we cannot write
	...
	public Connection(ClientThread client, Socket socket, byte[] cryptKey)
	{
		...
		_receivedMsgQueue = new ConcurrentLinkedQueue<>();
		_sendMsgQueue = new BasePacketQueue();
		_inCrypt = new Crypt();
		_outCrypt = new Crypt();
		_cryptkey = cryptKey; // this is defined here but it is not used before
								// key packet was send
	}
	...
	/**
	 * Put a message received by NIO's thread Notifies all threads, that wait()
	 * on this Connection
	 */
	public synchronized void addReceivedMsg(ByteBuffer buf)
	{
		if (Config.ASSERT)
			assert Thread.currentThread() == SelectorThread.getInstance();
		...
		try
		{
			...
			} else if (_client.getLoginName() != null)
			{
				LoginServerThread.getInstance()
						.sendLogout(_client.getLoginName());
			}
		} catch (Throwable t)
		{
