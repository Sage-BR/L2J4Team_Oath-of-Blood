...
public class ZoneManager
{
	protected static Logger _log = Logger
			.getLogger(ZoneManager.class.getName());
	...
	public int addZoneCoord(String zoneType, String zoneName, int x1, int y1,
			int x2, int y2, int z)
	{
		return getZoneType(zoneType).getZone(zoneName).addCoord(x1, y1, x2, y2,
				z);
	}
	...
	{
		return ZoneManager.getInstance().getZoneType(zoneType).checkIfInZone(x,
				y);
	}
	...
	{
		return (checkIfInZone(
				ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Peace), x, y) || // object
																				// is
																				// inside
																				// peace
																				// zone
				...
				Config.ZONE_TOWN == 0 && // Town is set as peace zone and
						TownManager.getInstance().checkIfInZone(x, y) // object
																		// is
																		// inside
																		// Town
																		// zone
	...
	{
		return (checkIfInZone(
				ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.NoLanding), x, y)// object
																				// is
																				// trying
																				// to
																				// dismount
	...
	{
		return (checkIfInZone(
				ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Fishing), x, y)// check
																				// if
																				// the
																				// fishing
																				// float
	...
	{
		return (SiegeManager.getInstance().checkIfInZone(x, y) || // object is
																	// inside a
																	// siege
																	// zone or
				checkIfInZone(ZoneType.getZoneTypeName(
						ZoneType.ZoneTypeEnum.OlympiadStadia), x, y)
				|| // object is inside an olympaid stadia
				checkIfInZone(
						ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Arena),
						x, y)
				|| // object is inside pvp zone or
				( // (
				...
				|| (Config.JAIL_IS_PVP && checkIfInZone(
						ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Jail), x,
						y)) // object is inside jail
		);
	}
	...
	private void load()
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("Select * from zone order by id");
			...
			while (rs.next())
			{
				zone = getZone(rs.getString(columnNameType).trim(),
						rs.getString(columnNameName).trim());
				zone.setId(rs.getInt(columnNameId));
				zone.setTaxById(rs.getInt(columnNameTaxBy));
				zone.addCoord(rs.getInt(columnNameX1), rs.getInt(columnNameY1),
						rs.getInt(columnNameX2), rs.getInt(columnNameY2),
						rs.getInt(columnNameZ));
			}
			statement.close();
			...
			for (ZoneType zt : getZoneTypes())
				_log.info(msgZoneLoaded + zt.getZones().size() + msgSpace
						+ zt.getTypeName());
		} catch (Exception e)
		{
			e.getMessage();
	...
	public final ZoneType getZoneType(String typeName)
	{
		for (ZoneType zt : getZoneTypes())
		{
			if (zt.getTypeName().equalsIgnoreCase(typeName.trim()))
				return zt;
		}
