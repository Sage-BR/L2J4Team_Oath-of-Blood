...
public class RequestAquireSkillInfo extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestAquireSkillInfo.class.getName());
	...
	void runImpl()
	{
		...
		if (_skillType == 0)
		{
			if (!trainer.getTemplate()
					.canTeach(activeChar.getSkillLearningClassId()))
				return; // cheater

			L2SkillLearn[] skills = SkillTreeTable.getInstance()
					.getAvailableSkills(activeChar,
							activeChar.getSkillLearningClassId());
			...
			int requiredSp = SkillTreeTable.getInstance()
					.getSkillCost(activeChar, skill);
			AquireSkillInfo asi = new AquireSkillInfo(skill.getId(),
					skill.getLevel(), requiredSp, 0);
		...
		} else if (_skillType == 2)
		{
			...
			L2PledgeSkillLearn[] skills = SkillTreeTable.getInstance()
					.getAvailablePledgeSkills(activeChar);
