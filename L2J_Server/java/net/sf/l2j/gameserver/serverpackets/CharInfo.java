 ...
 * dddddSddddddddddddddddddddddddddddffffdddSdddddccccccch (h) c (dchd) ddc dcc
 * c cddd d
 *
...
public class CharInfo extends ServerBasePacket
{
	private static final Logger _log = Logger
			.getLogger(CharInfo.class.getName());
	...
	final void writeImpl()
	{
		...
		{
			L2NpcTemplate template = NpcTable.getInstance()
			...
			if (template != null)
			{
				...
				writeD(_inv.getPaperdollItemId(Inventory.PAPERDOLL_RHAND)); // right
																			// hand
																			// weapon
				writeD(0);
				writeD(_inv.getPaperdollItemId(Inventory.PAPERDOLL_LHAND)); // left
																			// hand
																			// weapon
				...
				writeC(_cha.getAppearance().getInvisible() ? 1 : 0); // invisible
																		// ??
																		// 0=false
																		// 1=true
																		// 2=summoned
			...
			{
				_log.warning("Character " + _cha.getName() + " ("
						+ _cha.getObjectId() + ") morphed in a Npc ("
						+ _cha.getPoly().getPolyId() + ") w/o template.");
			}
		} else
		{
			writeC(0x03);
			...
			if (_cha2 != null && _cha.getClan() != null
					&& _cha2.getClan() != null
					&& _cha.getClan().isAtWarWith(_cha2.getClanId()))
				writeS("*" + _cha.getName());
			else
				writeS(_cha.getName());
			...
			writeC((_cha.isHero() || (_cha.isGM() && Config.GM_HERO_AURA)) ? 1
					: 0); // Hero Aura

			writeC(_cha.isFishing() ? 1 : 0); // 0x01: Fishing Mode (Cant be
												// undone by setting back to 0)
			...
			// c5
			// writeC(_cha.isRunning() ? 0x01 : 0x00); //changes the Speed
			// display on Status Window
