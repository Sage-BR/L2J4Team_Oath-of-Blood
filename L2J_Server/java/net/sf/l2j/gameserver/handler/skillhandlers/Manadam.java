...
public class Manadam implements ISkillHandler
{
	...
	L2Character activeChar, L2Skill skill, L2Object[] targets)
	{
		...
		for (L2Object target2 : targets)
		{
			...
			boolean acted = Formulas.getInstance().calcMagicAffected(activeChar,
					target, skill);
			if (!acted)
			{
				activeChar.sendPacket(
						new SystemMessage(SystemMessage.MISSED_TARGET));
			} else
			{
				double damage = Formulas.getInstance().calcManaDam(activeChar,
						target, skill, ss, bss);

				double mp = (damage > target.getCurrentMp()
						? target.getCurrentMp()
						: damage);
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.S2_MP_HAS_BEEN_DRAINED_BY_S1);
				...
				if (activeChar instanceof L2PcInstance)
				{
					SystemMessage sm2 = new SystemMessage(
							SystemMessage.YOUR_OPPONENTS_MP_WAS_REDUCED_BY_S1);
