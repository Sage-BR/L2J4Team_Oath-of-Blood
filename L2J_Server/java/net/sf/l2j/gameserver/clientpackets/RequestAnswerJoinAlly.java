...
public class RequestAnswerJoinAlly extends ClientBasePacket
{
	...
	// Logger.getLogger(RequestAnswerJoinAlly.class.getName());
	...
	void runImpl()
	{
		...
		if (_response == 0)
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.YOU_DID_NOT_RESPOND_TO_ALLY_INVITATION));
			requestor.sendPacket(new SystemMessage(
					SystemMessage.NO_RESPONSE_TO_ALLY_INVITATION));
		} else
		{
			if (!(requestor.getRequest()
					.getRequestPacket() instanceof RequestJoinAlly))
			{
				return; // hax
			}
			...
			if (clan.CheckAllyJoinCondition(requestor, activeChar))
			{
				...
				requestor.sendPacket(new SystemMessage(
						SystemMessage.YOU_HAVE_SUCCEEDED_INVITING_FRIEND));

				activeChar.sendPacket(
						new SystemMessage(SystemMessage.YOU_ACCEPTED_ALLIANCE));
