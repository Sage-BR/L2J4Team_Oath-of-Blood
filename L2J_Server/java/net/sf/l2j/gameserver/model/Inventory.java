...
public abstract class Inventory extends ItemContainer
{
	...
	/*
	 * final class FormalWearListener implements PaperdollListener { public void
	 * notifyUnequiped(int slot, L2ItemInstance item) { if (!(getOwner() != null
	 * && getOwner() instanceof L2PcInstance)) return;
	 *
	 * L2PcInstance owner = (L2PcInstance)getOwner();
	 *
	 * if (item.getItemId() == 6408) owner.setIsWearingFormalWear(false); }
	 * public void notifyEquiped(int slot, L2ItemInstance item) { if
	 * (!(getOwner() != null && getOwner() instanceof L2PcInstance)) return;
	 *
	 * L2PcInstance owner = (L2PcInstance)getOwner();
	 *
	 * // If player equip Formal Wear unequip weapons and abort cast/attack if
	 * (item.getItemId() == 6408) { owner.setIsWearingFormalWear(true); if
	 * (owner.isCastingNow()) owner.abortCast(); if (owner.isAttackingNow())
	 * owner.abortAttack(); setPaperdollItem(PAPERDOLL_LHAND, null);
	 * setPaperdollItem(PAPERDOLL_RHAND, null);
	 * setPaperdollItem(PAPERDOLL_LRHAND, null); } else { if
	 * (!owner.isWearingFormalWear()) return;
	 *
	 * // Don't let weapons be equipped if player is wearing Formal Wear if
	 * (slot == PAPERDOLL_LHAND || slot == PAPERDOLL_RHAND || slot ==
			...
			L2PcInstance actor, L2Object reference)
	{
		...
		if (item.getCount() > count)
		{
			...
			item = ItemTable.getInstance().createItem(process, item.getItemId(),
					count, actor, reference);
...
//    	if (item.isEquipped()) unEquipItemInSlotAndRecord(item.getEquipSlot());
	...
	private void unEquipItemInBodySlot(int slot)
	{
		...
		case L2Item.SLOT_DHAIR:
			...
			setPaperdollItem(PAPERDOLL_FACE, null);// this should be the same as
													// in DHAIR
		...
		case L2Item.SLOT_LR_HAND:
			setPaperdollItem(PAPERDOLL_LHAND, null);
			setPaperdollItem(PAPERDOLL_RHAND, null);// this should be the same
													// as in LRHAND
	...
	public synchronized void equipItem(L2ItemInstance item)
	{
		if ((getOwner() instanceof L2PcInstance)
				&& ((L2PcInstance) getOwner()).getPrivateStoreType() != 0)
			return;
		...
		case L2Item.SLOT_L_HAND:
		{
			if (!(item.getItem() instanceof L2EtcItem)
					|| item.getItem().getItemType() != L2EtcItemType.ARROW)
			{
		...
		case L2Item.SLOT_LEGS:
		{
			...
			if (chest != null
					&& chest.getItem().getBodyPart() == L2Item.SLOT_FULL_ARMOR)
			{
				setPaperdollItem(PAPERDOLL_CHEST, null);
			}
