...
public class CastleManager
{
	...
	{
		return (getCastle(obj) != null);
	}

	/** Return true if object is inside zone */
	public final boolean checkIfInZone(int x, int y)
	{
		return (getCastle(x, y) != null);
	}
	...
	public final int findNearestCastleIndex(L2Object activeObject)
	{
		...
		if (index < 0)
		{
			...
			for (int i = 0; i < getCastles().size(); i++)
			{
				...
				if (castle == null)
					continue;
				distance = castle.getZone().findDistanceToZone(activeObject,
						false);
	...
	private final void load()
	{
		...
		try
		{
			...
			statement = con
					.prepareStatement("Select id from castle order by id");
	...
	{
		return getCastle(activeObject.getX(), activeObject.getY());
	}
	...
	public final Castle getCastleByTown(L2Object activeObject)
	{
		return getCastleByTown(activeObject.getPosition().getX(),
	...
	{
		return getCastleIndex(activeObject.getPosition().getX(),
				activeObject.getPosition().getY());
	}
	...
	public final int getCastleIndex(String name)
	{
		...
		for (int i = 0; i < getCastles().size(); i++)
		{
			castle = getCastles().get(i);
			if (castle != null
					&& castle.getName().equalsIgnoreCase(name.trim()))
				return i;
		}
		return -1;
	}
	...
	public final int getCastleIndexByTown(int townId)
	{
		...
		{
			castle = getCastles().get(i);
			if (castle != null && castle.getZoneTown(townId) != null)
				return i;
		}
		return -1;
	}

	public final int getCastleIndexByTown(L2Object activeObject)
	{
		return getCastleIndexByTown(activeObject.getX(), activeObject.getY());
	}
	...
	public final int getCastleIndexByTown(int x, int y)
	{
		...
		{
			castle = getCastles().get(i);
			if (castle != null && castle.checkIfInZoneTowns(x, y))
				return i;
		}
		return -1;
	}
	...
	public final int getCastleIndexByTown(String name)
	{
		...
		{
			castle = getCastles().get(i);
			if (castle != null && castle.getZoneTown(name) != null)
				return i;
		}
		return -1;
	}
