...
{
	private static Logger _logOlymp = Logger
			.getLogger(L2OlympiadManagerInstance.class.getName());
		...
		} else if (command.startsWith("OlympiadNoble"))
		{
			...
			case 2:
				...
				replyMSG.append("The number of people on the waiting list for "
						...
						+ "<td align=\"right\">" + classed + "</td>" + "</tr>"
						+ "<tr>" + "<td align=\"left\">Not class-defined</td>"
						+ "<td align=\"right\">" + nonClassed + "</td>"
						+ "</tr>" + "</table><br>"
						+ "<img src=\"L2UI.SquareWhite\" width=270 height=1> <img src=\"L2UI.SquareBlank\" width=1 height=3>"
						+ "<button value=\"Back\" action=\"bypass -h npc_"
						+ getObjectId() + "_OlympiadDesc 2a\" "
				...
				break;
			case 3:
				int points = Olympiad.getInstance()
						.getNoblePoints(player.getObjectId());
				if (points >= 0)
				{
					reply = new NpcHtmlMessage(getObjectId());
					replyMSG = new TextBuilder("<html><body>");
			...
			case 5:
				...
				break;
			case 6:
				int passes = Olympiad.getInstance()
						.getNoblessePasses(player.getObjectId());
				if (passes > 0)
				{
					L2ItemInstance item = player.getInventory().addItem(
							"Olympiad", _gatePass, passes, player, this);
				...
				{
					player.sendMessage(
							"Not enough points, or not currently in Valdation Period");
			...
			default:
				_logOlymp.warning(
						"Olympiad System: Couldnt send packet for request "
								+ val);
				break;
		...
		} else if (command.startsWith("Olympiad"))
		{
			...
			case 1:
				...
				else
				{
					...
					{
						replyMSG.append("<br><a action=\"bypass -h npc_"
								+ getObjectId() + "_Olympiad 3_" + i + "\">"
								+ matches[i] + "</a>");
					}
				}
				replyMSG.append("</body></html>");
			...
			case 2:
				...
				if (classId >= 88)
				{
					...
					List<String> names = Olympiad.getInstance()
