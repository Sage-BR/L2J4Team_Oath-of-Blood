...
public class AdminGeodata implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_geo_z", "admin_geo_type", "admin_geo_nswe", "admin_geo_los",
				"admin_geo_position", "admin_geo_bug", "admin_geo_load",
				"admin_geo_unload" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		} else if (command.equals("admin_geo_nswe"))
		{
			...
			activeChar.sendMessage(
					"GeoEngine: Geo_NSWE -> " + nswe + "->" + result);
		} else if (command.equals("admin_geo_los"))
		{
			if (activeChar.getTarget() != null)
			{
				if (GeoData.getInstance().canSeeTargetDebug(activeChar,
		...
		} else if (command.equals("admin_geo_position"))
		{
			...
			activeChar.sendMessage(".... world coords: x: " + activeChar.getX()
					+ " y: " + activeChar.getY() + " z: " + activeChar.getZ());
		...
		} else if (command.startsWith("admin_geo_load"))
		{
			...
			else
			{
				try
				{
					...
					if (result)
						activeChar.sendMessage("GeoEngine: File for region ["
								+ rx + "," + ry + "] loaded succesfuly");
					else
						activeChar.sendMessage("GeoEngine: File for region ["
								+ rx + "," + ry + "] couldn't be loaded");
				} catch (Exception e)
				{
					activeChar.sendMessage(
							"You have to write numbers of regions <regionX> <regionY>");
				}
			}
		} else if (command.startsWith("admin_geo_unload"))
		{
			...
			else
			{
				try
				{
					...
					activeChar.sendMessage("GeoEngine: File for region [" + rx
							+ "," + ry + "] unloaded.");
				} catch (Exception e)
				{
					activeChar.sendMessage(
		...
		} else if (command.startsWith("admin_geo_bug"))
		{
			...
			{
				activeChar
						.sendMessage("Usage: //admin_geo_bug you coments here");
			}
		}
		return true;
	}
