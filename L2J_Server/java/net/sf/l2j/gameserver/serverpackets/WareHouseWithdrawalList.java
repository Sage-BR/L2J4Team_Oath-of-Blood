...
public class WareHouseWithdrawalList extends ServerBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(WareHouseWithdrawalList.class.getName());
	...
	final void runImpl()
	{
		...
		if (_player.getActiveWarehouse() == null)
		{
			...
			_log.warning("error while sending withdraw request to: "
					+ _player.getName());
			return;
		} else
		...
		if (Config.DEBUG)
			for (L2ItemInstance item : _items)
				_log.fine("item:" + item.getItem().getName() + " type1:"
						+ item.getItem().getType1() + " type2:"
