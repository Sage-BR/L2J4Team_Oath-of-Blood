...
public class ItemTable
{
	...
	static
	{
		...
		_weaponTypes.put("bigsword", L2WeaponType.BIGSWORD); // Two-Handed
																// Swords
		...
		_slots.put("rfinger,lfinger",
				L2Item.SLOT_R_FINGER | L2Item.SLOT_L_FINGER);
	...
	/**
	 * Table of SQL request in order to obtain items from tables [etcitem],
	 * [armor], [weapon]
	 */
	...
	public ItemTable()
	{
		...
		for (L2Weapon weapon : SkillsEngine.getInstance()
				.loadWeapons(weaponData))
		{
	...
	private Item readWeapon(ResultSet rset) throws SQLException
	{
		...
		item.set.set("crystal_type",
				_crystalTypes.get(rset.getString("crystal_type")));
		item.set.set("crystallizable", Boolean
				.valueOf(rset.getString("crystallizable")).booleanValue());
		...
		item.set.set("destroyable",
				Boolean.valueOf(rset.getString("destroyable")));
	 ...
	 *
	 * @param rset
	 *            : ResultSet designating a record of the [armor] table of
	 *            database
	 * @return Item : object created from the database record
	 * @throws SQLException
	 */
	private Item readArmor(ResultSet rset) throws SQLException
	{
		...
		item.set.set("crystallizable",
				Boolean.valueOf(rset.getString("crystallizable")));
		item.set.set("crystal_count", rset.getInt("crystal_count"));
		item.set.set("sellable", Boolean.valueOf(rset.getString("sellable")));
		item.set.set("dropable", Boolean.valueOf(rset.getString("dropable")));
		item.set.set("destroyable",
				Boolean.valueOf(rset.getString("destroyable")));
		item.set.set("tradeable", Boolean.valueOf(rset.getString("tradeable")));
		if (bodypart == L2Item.SLOT_NECK || bodypart == L2Item.SLOT_HAIR
				|| bodypart == L2Item.SLOT_FACE || bodypart == L2Item.SLOT_DHAIR
	 ...
	 *
	 * @param rset
	 *            : ResultSet designating a record of the [etcitem] table of
	 *            database
	 * @return Item : object created from the database record
	 * @throws SQLException
	 */
	...
	/*
	 * private void fillEtcItemsTable() { for (Item itemInfo :
	 * itemData.values()) { L2EtcItem item =
	 * SkillsEngine.getInstance().loadEtcItem(itemInfo.id, itemInfo.type,
	 * itemInfo.name, itemInfo.set); if (item == null) { item = new
	 * L2EtcItem((L2EtcItemType)itemInfo.type, itemInfo.set); }
	 *
	 * _etcItems.put(item.getItemId(), item); } }
	 *
	 * private void fillArmorsTable() { List<L2Armor> armorList =
	 * SkillsEngine.getInstance().loadArmors(armorData);
	 *
	 * /*for (Item itemInfo : armorData.values()) {
	 *
	 * L2Armor armor = SkillsEngine.getInstance().loadArmor(itemInfo.id,
	 * itemInfo.type, itemInfo.name, itemInfo.set); if (armor == null) armor =
	 * new L2Armor((L2ArmorType)itemInfo.type, itemInfo.set);
	 *
	 * _armors.put(armor.getItemId(), armor); }* }
	 *
	 * private void FillWeaponsTable() {
	 *
	 * for (Item itemInfo : weaponData.values()) { L2Weapon weapon =
	 * SkillsEngine.getInstance().loadWeapon(itemInfo.id, itemInfo.type,
	 * itemInfo.name, itemInfo.set); if (weapon == null) weapon = new
	 * L2Weapon((L2WeaponType)itemInfo.type, itemInfo.set);
	 *
	 * _weapons.put(weapon.getItemId(), weapon); } }
	 */
	 ...
	 */
	public L2ItemInstance createItem(String process, int itemId, int count,
			L2PcInstance actor, L2Object reference)
	{
		...
		if (process.equalsIgnoreCase("loot") && !Config.AUTO_LOOT)
		{
			...
			ScheduledFuture itemLootShedule = ThreadPoolManager.getInstance()
					.scheduleGeneral(new resetOwner(item), 15000);
			item.setItemLootShedule(itemLootShedule);
		}

		if (Config.DEBUG)
			_log.fine("ItemTable: Item created  oid:" + item.getObjectId()
					+ " itemid:" + itemId);
	 ...
	 *            item or previous item in transformation
	 */
	public void destroyItem(String process, L2ItemInstance item,
			L2PcInstance actor, L2Object reference)
	{
		synchronized (item)
		{
			...
			if (L2PetDataTable.isPetItem(item.getItemId()))
			{
				...
				{
					_log.log(Level.WARNING, "could not delete pet objectid:",
							e);
				} finally
				{
					try
