...
public class Lottery
{
	...
	public void increasePrize(int count)
	{
		...
		{
			_log.warning(
					"Lottery: Could not increase current lottery prize: " + e);
		} finally
		{
			try
			{
	...
	private class startLottery implements Runnable
	{
		...
		public void run()
		{
			...
			if (Config.DEBUG)
				_log.info("Lottery: Starting ticket sell for lottery #"
						+ getId() + ".");
			_isSellingTickets = true;
			_isStarted = true;
	...
	private class stopSellingTickets implements Runnable
	{
		...
		{
			if (Config.DEBUG)
				_log.info("Lottery: Stopping ticket sell for lottery #"
						+ getId() + ".");
			_isSellingTickets = false;
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			if (Config.DEBUG)
				_log.info("Lottery: The lucky numbers are " + luckynums[0]
						+ ", " + luckynums[1] + ", " + luckynums[2] + ", "
						+ luckynums[3] + ", " + luckynums[4] + ".");
			...
			if (Config.DEBUG)
				_log.info("Lottery: Encoded lucky numbers are " + enchant + ", "
						+ type2);
			...
			if (count1 > 0)
				prize1 = (int) ((getPrize() - prize4)
						* Config.ALT_LOTTERY_5_NUMBER_RATE / count1);

			if (count2 > 0)
				prize2 = (int) ((getPrize() - prize4)
						* Config.ALT_LOTTERY_4_NUMBER_RATE / count2);

			if (count3 > 0)
				prize3 = (int) ((getPrize() - prize4)
						* Config.ALT_LOTTERY_3_NUMBER_RATE / count3);

			if (Config.DEBUG)
			{
				_log.info("Lottery: " + count1
						+ " players with all FIVE numbers each win " + prize1
						+ ".");
				_log.info("Lottery: " + count2
						+ " players with FOUR numbers each win " + prize2
						+ ".");
				_log.info("Lottery: " + count3
						+ " players with THREE numbers each win " + prize3
						+ ".");
				_log.info("Lottery: " + count4
						+ " players with ONE or TWO numbers each win " + prize4
						+ ".");
			}
			...
			{
				_log.warning(
						"Lottery: Could not store finished lottery data: " + e);
			} finally
			{
				try
				{
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new startLottery(),
					MINUTE);
			_number++;
	...
	{
		return checkTicket(item.getCustomType1(), item.getEnchantLevel(),
				item.getCustomType2());
	}
	...
	public int[] checkTicket(int id, int enchant, int type2)
	{
		...
		try
		{
			...
			if (rset.next())
			{
				...
				if (Config.DEBUG)
					_log.warning("count: " + count + ", id: " + id
							+ ", enchant: " + enchant + ", type2: " + type2);
			}

			statement.close();
		} catch (SQLException e)
		{
			_log.warning("Lottery: Could not check lottery ticket #" + id + ": "
					+ e);
		} finally
		{
			try
