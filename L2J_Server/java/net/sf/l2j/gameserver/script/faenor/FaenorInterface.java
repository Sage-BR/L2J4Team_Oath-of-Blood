...
public class FaenorInterface implements EngineInterface
{
	 ...
	 */
	@Override
	public void addQuestDrop(int npcID, int itemID, int min, int max,
			int chance, String questID, String[] states)
	{
	 ...
	 * @see net.sf.l2j.gameserver.script.EngineInterface#addQuestDrop(int)
	 */
	public void addDrop(int npcID, int itemID, int min, int max, boolean sweep,
			int chance) throws NullPointerException
	{
		L2NpcTemplate npc = _npcTable.getTemplate(npcID);
		if (npc == null)
		{
	...
	/**
	 * Adds a new drop to an NPC. If the drop is sweep, it adds it to the NPC's
	 * Sweep category If the drop is non-sweep, it creates a new category for
	 * this drop.
	 *
	 * @param npc
	 * @param drop
	...
	/**
	 * Adds a new drop to an NPC, in the specified category. If the category
	...
	@Override
	public void addEventDrop(int[] items, int[] count, double chance,
			DateRange range)
	{
		EventDroplist.getInstance().addGlobalDrop(items, count,
				(int) (chance * L2DropData.MAX_CHANCE), range);
	}
	...
	public void addPetData(BSFManager context, int petID, int levelStart,
			int levelEnd, Map<String, String> stats) throws BSFException
	{
		...
		for (int level = levelStart; level <= levelEnd; level++)
		{
			...
			{
				value = ((Number) Expression.eval(context, "beanshell",
						stats.get(stat))).intValue();
