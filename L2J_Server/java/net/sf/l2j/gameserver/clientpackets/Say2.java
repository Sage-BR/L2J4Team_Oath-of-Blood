...
public class Say2 extends ClientBasePacket
{
	...
	public final static String[] chatNames =
		{ "ALL  ", "SHOUT", "TELL ", "PARTY", "CLAN ", "GM   ",
				"PETITION_PLAYER", "PETITION_GM", "TRADE", "ALLIANCE",
				"ANNOUNCEMENT", // 10
				"WILLCRASHCLIENT:)", "FAKEALL?", "FAKEALL?", "FAKEALL?",
				"PARTYROOM_ALL", "PARTYROOM_COMMANDER", "HERO_VOICE" };
	...
	void runImpl()
	{
		if (Config.DEBUG)
			_log.info(
					"Say2: Msg Type = '" + _type + "' Text = '" + _text + "'.");
		...
		if (activeChar.isChatBanned())
		{
			if (_type == ALL || _type == SHOUT || _type == TRADE)
			{
				activeChar.sendMessage(
						"You may not chat while a chat ban is in effect.");
				return;
			}
		}
		...
		if (activeChar.isInJail() && Config.JAIL_DISABLE_CHAT)
		{
			...
			{
				activeChar.sendMessage(
						"You can not chat with the outside of the jail.");
				return;
			}
		}
		...
		if (Config.LOG_CHAT)
		{
			...
			if (_type == TELL)
				record.setParameters(new Object[]
					{ chatNames[_type], "[" + activeChar.getName() + " to "
							+ _target + "]" });
			else
				record.setParameters(new Object[]
					{ chatNames[_type], "[" + activeChar.getName() + "]" });

			_logChat.log(record);
		}

		CreatureSay cs = new CreatureSay(activeChar.getObjectId(), _type,
				activeChar.getName(), _text);
			...
			if (receiver != null && !BlockList.isBlocked(receiver, activeChar))
			{
				if (!receiver.getMessageRefusal())
				{
					...
					activeChar.sendPacket(
							new CreatureSay(activeChar.getObjectId(), _type,
									"->" + receiver.getName(), _text));
				} else
				{
		...
		case SHOUT:
			if (Config.DEFAULT_GLOBAL_CHAT.equalsIgnoreCase("on")
					|| (Config.DEFAULT_GLOBAL_CHAT.equalsIgnoreCase("gm")
							&& activeChar.isGM()))
			{
				int region = MapRegionTable.getInstance()
						...
						.getAllPlayers())
				{
					if (region == MapRegionTable.getInstance()
							.getMapRegion(player.getX(), player.getY()))
		...
		case ALL:
			...
			if (_text.startsWith("."))
			{
				...
				if (st.hasMoreTokens())
				{
					...
					IVoicedCommandHandler vch = VoicedCommandHandler
							.getInstance().getVoicedCommandHandler(command);

					if (vch != null)
						vch.useVoicedCommand(command, activeChar, target);
					else
						_log.warning("No handler registered for bypass '"
								+ command + "'");
				}
			}
			break;
		...
		case PETITION_GM:
			if (!PetitionManager.getInstance()
					.isPlayerInConsultation(activeChar))
			{
		...
		case PARTYROOM_COMMANDER:
			// PartyCommandManager.getInstance().sendChannelMessage(activeChar,
			// _text);
			break;
		case HERO_VOICE:
