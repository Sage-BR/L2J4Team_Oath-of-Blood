	...
	void runImpl()
	{
				...
				|| item.getItemId() >= 6611 && item.getItemId() <= 6621)
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.INAPPROPRIATE_ENCHANT_CONDITION));
			return;
		}
		if (item.isWear())
		{
			Util.handleIllegalPlayerAction(activeChar,
					"Player " + activeChar.getName()
							+ " tried to enchant a weared Item",
		...
		// SystemMessage(SystemMessage.ENCHANT_SCROLL_CANCELLED);
		...
		if (item.getEnchantLevel() < Config.ENCHANT_SAFE_MAX || (item.getItem()
				.getBodyPart() == L2Item.SLOT_FULL_ARMOR
		...
		} else
		{
			...
			if (!blessedScroll)
			{
				...
				int count = item.getCrystalCount()
						- (item.getItem().getCrystalCount() + 1) / 2;
				if (count < 1)
					count = 1;
				...
				L2ItemInstance crystals = activeChar.getInventory().addItem(
						"Enchant", crystalId, count, activeChar, destroyItem);
