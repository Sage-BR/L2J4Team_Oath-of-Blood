...
public class AdminFightCalculator implements IAdminCommandHandler
{
	...
	// Logger.getLogger(AdminFightCalculator.class.getName());
	private static String[] _adminCommands =
		{ "admin_fight_calculator", "admin_fight_calculator_show",
				"admin_fcs", };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		try
		{
			...
			if (command.startsWith("admin_fight_calculator_show"))
				handleShow(
						command.substring(
								"admin_fight_calculator_show".length()),
						activeChar);
				...
				handleStart(
						command.substring("admin_fight_calculator".length()),
						activeChar);
	...
	private void handleStart(String params, L2PcInstance activeChar)
	{
		...
		if (npc1 != null && npc2 != null)
		{
			...
			replyMSG.append("<tr><td>level " + lvl1 + "</td><td>level " + lvl2
					+ "</td></tr>");
			replyMSG.append("<tr><td>id " + npc1.npcId + "</td><td>id "
					+ npc2.npcId + "</td></tr>");
			replyMSG.append("<tr><td>" + npc1.name + "</td><td>" + npc2.name
					+ "</td></tr>");
			replyMSG.append("</table>");
			replyMSG.append("<center><br><br><br>");
			replyMSG.append(
					"<button value=\"OK\" action=\"bypass -h admin_fight_calculator_show "
							+ npc1.npcId + " " + npc2.npcId
		...
		} else
		{
			...
			replyMSG.append("<center><br><br><br>");
			replyMSG.append(
					"<button value=\"OK\" action=\"bypass -h admin_fight_calculator lvl1 $lvl1 lvl2 $lvl2\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
			replyMSG.append("</center>");
			replyMSG.append("</body></html>");
		}
	...
	private void handleShow(String params, L2PcInstance activeChar)
	{
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			if (!_miss1)
			{
				...
				double _dmg1 = f.calcPhysDam(npc1, npc2, null, _shld1, _crit1,
						false, false);
				dmg1 += _dmg1;
				npc1.abortAttack();
			}
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			if (!_miss2)
			{
				...
				double _dmg2 = f.calcPhysDam(npc2, npc1, null, _shld2, _crit2,
						false, false);
				dmg2 += _dmg2;
				npc2.abortAttack();
			}
		...
		int hp1 = (int) (f.calcHpRegen(npc1) * 100000
				/ f.getRegeneratePeriod(npc1));

		double maxHp2 = npc2.getMaxHp();
		int hp2 = (int) (f.calcHpRegen(npc2) * 100000
				/ f.getRegeneratePeriod(npc2));
		...
		{
			replyMSG.append(
					"<tr><td width=140>Parameter</td><td width=70>me</td><td width=70>target</td></tr>");
		} else
		{
			replyMSG.append("<tr><td width=140>Parameter</td><td width=70>"
					+ ((L2NpcTemplate) npc1.getTemplate()).name
					+ "</td><td width=70>"
					+ ((L2NpcTemplate) npc2.getTemplate()).name + "</td></tr>");
		}
		replyMSG.append("<tr><td>miss</td><td>" + miss1 + "%</td><td>" + miss2
				+ "%</td></tr>");
		replyMSG.append("<tr><td>shld</td><td>" + shld2 + "%</td><td>" + shld1
				+ "%</td></tr>");
		replyMSG.append("<tr><td>crit</td><td>" + crit1 + "%</td><td>" + crit2
				+ "%</td></tr>");
		replyMSG.append("<tr><td>pAtk / pDef</td><td>" + ((int) patk1) + " / "
				+ ((int) pdef1) + "</td><td>" + ((int) patk2) + " / "
				+ ((int) pdef2) + "</td></tr>");
		replyMSG.append("<tr><td>made hits</td><td>" + sAtk1 + "</td><td>"
				+ sAtk2 + "</td></tr>");
		replyMSG.append("<tr><td>dmg per hit</td><td>" + ((int) dmg1)
				+ "</td><td>" + ((int) dmg2) + "</td></tr>");
		replyMSG.append("<tr><td>got dmg</td><td>" + tdmg2 + "</td><td>" + tdmg1
				+ "</td></tr>");
		replyMSG.append("<tr><td>got regen</td><td>" + hp1 + "</td><td>" + hp2
				+ "</td></tr>");
		replyMSG.append("<tr><td>had HP</td><td>" + (int) maxHp1 + "</td><td>"
				+ (int) maxHp2 + "</td></tr>");
		replyMSG.append("<tr><td>die</td>");
		if (tdmg2 - hp1 > 1)
			replyMSG.append("<td>" + (int) (100 * maxHp1 / (tdmg2 - hp1))
					+ " sec</td>");
		else
			replyMSG.append("<td>never</td>");
		if (tdmg1 - hp2 > 1)
			replyMSG.append("<td>" + (int) (100 * maxHp2 / (tdmg1 - hp2))
					+ " sec</td>");
		else
			replyMSG.append("<td>never</td>");
		replyMSG.append("</tr>");
		...
		{
			replyMSG.append(
					"<button value=\"Retry\" action=\"bypass -h admin_fight_calculator_show\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
		} else
		{
			replyMSG.append(
					"<button value=\"Retry\" action=\"bypass -h admin_fight_calculator_show "
							+ ((L2NpcTemplate) npc1.getTemplate()).npcId + " "
							+ ((L2NpcTemplate) npc2.getTemplate()).npcId
							+ "\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
		}
		replyMSG.append("</center>");
		replyMSG.append("</body></html>");
