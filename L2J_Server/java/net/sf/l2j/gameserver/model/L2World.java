...
public final class L2World
{
	...
	/**
	 * HashMap(String Player name, L2PcInstance) containing all the players in
	 * game
	 */
	...
	public void storeObject(L2Object object)
	{
		...
		{
			if (Config.DEBUG)
				_log.warning("[L2World] objectId " + object.getObjectId()
						+ " already exist in OID map!");
			return;
		}
	 ...
	 * <B><U> Example of use </U> :</B><BR>
	 * <BR>
	 * <li>Delete item from inventory, tranfer Item from inventory to
	 * warehouse</li>
	 ...
	 * <B><U> Example of use </U> :</B><BR>
	 * <BR>
	 * <li>Client packets : Action, AttackRequest, RequestJoinParty,
	 * RequestJoinPledge...</li><BR>
	 *
	 * @param oID
	 ...
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * L2Object (including L2PcInstance) are identified in
	 * <B>_visibleObjects</B> of his current L2WorldRegion and in
	 * <B>_knownObjects</B> of other surrounding L2Characters <BR>
	 * L2PcInstance are identified in <B>_allPlayers</B> of L2World, in
	 * <B>_allPlayers</B> of his current L2WorldRegion and in
	 ...
	 * <li>Add object in _knownObjects and _knownPlayer* of all surrounding
	 ...
	 * synchronisation)</B></FONT><BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T ADD the
	 * object to _allObjects and _allPlayers* of L2World (need
	 * synchronisation)</B></FONT><BR>
	 * <BR>
	 *
	 * <B><U> Example of use </U> :</B><BR>
	 ...
	 *
	 */
	public void addVisibleObject(L2Object object, @SuppressWarnings("unused")
	L2WorldRegion newRegion, L2Character dropper)
	{
		// If selected L2Object is a L2PcIntance, add it in
		// L2ObjectHashSet(L2PcInstance) _allPlayers of L2World
		// XXX TODO: this code should be obsoleted by protection in putObject
		// func...
		if (object instanceof L2PcInstance)
		{
			...
			if (tmp != null)
			{
				_log.warning("Duplicate character!? Closing both characters ("
						+ player.getName() + ")");
		...
		{
			// Add the object in L2ObjectHashSet(L2Object) _knownObjects of the
			// visible L2Character according to conditions :
			// - L2Character is visible
			// - object is not already known
			// - object is in the watch distance
			// If L2Object is a L2PcInstance, add L2Object in
			// L2ObjectHashSet(L2PcInstance) _knownPlayer of the visible
			// L2Character
			element.getKnownList().addKnownObject(object, dropper);

			// Add the visible L2Object in L2ObjectHashSet(L2Object)
			// _knownObjects of the object according to conditions
			// If visible L2Object is a L2PcInstance, add visible L2Object in
			// L2ObjectHashSet(L2PcInstance) _knownPlayer of the object
	 ...
	 * <li>Remove the L2Object object from _visibleObjects and _allPlayers* of
	 * L2WorldRegion</li>
	 ...
	 * _knownObjects and all L2PcInstance from its _knownPlayer</li><BR>
	 * <BR>
	 *
	...
	public void removeVisibleObject(L2Object object, L2WorldRegion oldRegion)
	{
		...
		if (oldRegion != null)
		{
			// Remove the object from the L2ObjectHashSet(L2Object)
			// _visibleObjects of L2WorldRegion
			// If object is a L2PcInstance, remove it from the
			// L2ObjectHashSet(L2PcInstance) _allPlayers of this L2WorldRegion
			...
			for (L2WorldRegion reg : oldRegion.getSurroundingRegions())
			{
				...
				while (iter.hasNext())
				{
					...
					// Remove the L2Object from the L2ObjectHashSet(L2Object)
					// _knownObjects of the surrounding L2WorldRegion
					// L2Characters
					// If object is a L2PcInstance, remove the L2Object from the
					// L2ObjectHashSet(L2PcInstance) _knownPlayer of the
					// surrounding L2WorldRegion L2Characters
					// If object is targeted by one of the surrounding
					// L2WorldRegion L2Characters, cancel ATTACK and cast
					if (obj != null && obj.getKnownList() != null)
						obj.getKnownList().removeKnownObject(object);

					// Remove surrounding L2WorldRegion L2Characters from the
					// L2ObjectHashSet(L2Object) _KnownObjects of object
					// If surrounding L2WorldRegion L2Characters is a
					// L2PcInstance, remove it from the
					// L2ObjectHashSet(L2PcInstance) _knownPlayer of object
					// TODO Delete this line if all the stuff is done by the
					// next line object.removeAllKnownObjects()
			...
			// Remove all L2Object from L2ObjectHashSet(L2Object) containing all
			// L2Object detected by the L2Character
			// Remove all L2PcInstance from L2ObjectHashSet(L2PcInstance)
			// containing all player ingame detected by the L2Character
			object.getKnownList().removeAllKnownObjects();

			// If selected L2Object is a L2PcIntance, remove it from
			// L2ObjectHashSet(L2PcInstance) _allPlayers of L2World
			if (object instanceof L2PcInstance)
			{
				removeFromAllPlayers((L2PcInstance) object);

				// If selected L2Object is a GM L2PcInstance, remove it from
				// Set(L2PcInstance) _gmList of GmListTable
	 ...
	 * surrounding L2WorldRegion.<BR>
	 * <BR>
	 *
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * All visible object are identified in <B>_visibleObjects</B> of their
	 * current L2WorldRegion <BR>
	 * All surrounding L2WorldRegion are identified in
	 * <B>_surroundingRegions</B> of the selected L2WorldRegion in order to scan
	 * a large area around a L2Object<BR>
	 * <BR>
	 *
	 * <B><U> Example of use </U> :</B><BR>
	 * <BR>
	...
	public Collection<L2Object> getVisibleObjects(L2Object object)
	{
		...
		for (L2WorldRegion _region : _regions)
		{
			// Create an Iterator to go through the visible L2Object of the
			// L2WorldRegion
			Iterator<L2Object> _objects = _region.iterateVisibleObjects();
	 ...
	 * (radius) centered on the object.<BR>
	 * <BR>
	 *
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	...
	public L2Object[] getVisibleObjects(L2Object object, int radius)
	{
		...
		// Create an FastList in order to contain all visible L2Object
		List<L2Object> result = new FastList<>();

		// Create an FastList containing all regions around the current region
		List<L2WorldRegion> _regions = object.getWorldRegion()
		...
		for (int i = 0; i < _regions.size(); i++)
		{
			...
			// Create an Iterator to go through the visible L2Object of the
			// L2WorldRegion
			Iterator<L2Object> _objects = _regions.get(i)
	...
	public Collection<L2PlayableInstance> getVisiblePlayable(L2Object object)
	{
		...
		for (L2WorldRegion _region : _regions)
		{
			// Create an Iterator to go through the visible L2Object of the
			// L2WorldRegion
			Iterator<L2PlayableInstance> _playables = _region
					.iterateAllPlayers();
			...
			while (_playables.hasNext())
			{
				...
				if ((_object == null) || _object.equals(object)
						|| !_object.isVisible()) // GM invisible is different
													// than this...
					continue; // skip dying objects
	...
	{
		return _worldRegions[(point.getX() >> SHIFT_BY)
				+ OFFSET_X][(point.getY() >> SHIFT_BY) + OFFSET_Y];
	}

	public L2WorldRegion getRegion(int x, int y)
	{
		return _worldRegions[(x >> SHIFT_BY) + OFFSET_X][(y >> SHIFT_BY)
				+ OFFSET_Y];
	}
	...
	private void initRegions()
	{
			...
			for (int y = 0; y < REGIONS_Y; y++)
			{
				for (int a = -1; a <= 1; a++)
				{
					for (int b = -1; b <= 1; b++)
					{
						...
						{
							_worldRegions[x + a][y + b]
									.addSurroundingRegion(_worldRegions[x][y]);
						}
					}
				}
			}
		...
		_log.config("L2World: (" + REGIONS_X + " by " + REGIONS_Y
