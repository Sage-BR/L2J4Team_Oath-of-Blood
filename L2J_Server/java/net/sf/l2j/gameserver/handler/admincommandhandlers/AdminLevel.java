...
public class AdminLevel implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		String target = (targetChar != null ? targetChar.getName()
				: "no-target");
		...
		if (actualCommand.equalsIgnoreCase("admin_add_level"))
		{
			try
			{
				if (targetChar instanceof L2PlayableInstance)
					((L2PlayableInstance) targetChar).getStat()
							.addLevel(Byte.parseByte(val));
		...
		} else if (actualCommand.equalsIgnoreCase("admin_set_level"))
		{
			try
			{
				...
				{
					activeChar.sendPacket(new SystemMessage(144)); // incorrect
																	// target!
					return false;
				}
				...
				{
					activeChar
							.sendMessage("You must specify level between 1 and "
									+ Experience.MAX_LEVEL + ".");
					return false;
				}
			} catch (NumberFormatException e)
			{
				activeChar.sendMessage("You must specify level between 1 and "
						+ Experience.MAX_LEVEL + ".");
				return false;
			}
		}
