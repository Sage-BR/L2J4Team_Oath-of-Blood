...
//      Local Constants
		...
		_log.config("Stand Alone LoginServer Listening on port "
				+ Config.PORT_LOGIN);
		server.start();
		...
		} else
		{
		  ...
		  System.out.println(" 4TeamBR - Oath of Blood ");
	}
	...
	private LoginServer() throws IOException
	{
		...
		_log.config(
				"Hostname for external connections is: " + _externalHostname);
		_log.config(
				"Hostname for internal connections is: " + _internalHostname);
		...
		_gslistener.start();
		/*
		 * ArrayList<String> bannedIpList = new ArrayList<String>(); Properties
		 * bannedIpFile = new Properties(); try {
		 * bannedIpFile.loadFromXML(getClass().getResourceAsStream(Config.
		 * BANNED_IP_XML)); } catch (InvalidPropertiesFormatException e) {
		...
		finally
		{
		    ...
		    {
		        // Removed the empty catch block
		    }
		}
	}
	/**
	 * This returns a unique LoginServer instance (singleton) This doesnt start
	 * the Login in the case of the creation of a new instance like it used to
	 * do.
	 *
	 ...
	 *
	 * @see
	 * net.sf.l2j.loginserver.FloodProtectedListener#addClient(java.net.Socket)
	 */
	@Override
