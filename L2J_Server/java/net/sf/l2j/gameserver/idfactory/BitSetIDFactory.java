...
public class BitSetIDFactory extends IdFactory
{
	private static Logger _log = Logger
			.getLogger(BitSetIDFactory.class.getName());
	...
	protected BitSetIDFactory()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(
				new BitSetCapacityCheck(), 30000, 30000);
		initialize();
	...
	public synchronized void initialize()
	{
		try
		{
			...
			for (int usedObjectId : extractUsedObjectIDTable())
			{
				...
				if (objectID < 0)
				{
					_log.warning("Object ID " + usedObjectId
	...
	public synchronized void releaseId(int objectID)
	{
		if ((objectID - FIRST_OID) > -1)
		{
			...
			_log.warning("BitSet ID Factory: release objectID " + objectID
					+ " failed (< " + FIRST_OID + ")");
	}
	...
	protected synchronized void increaseBitSetCapacity()
	{
		BitSet newBitSet = new BitSet(
				PrimeFinder.nextPrime(usedIdCount() * 11 / 10));
