...
public class L2Fishing implements Runnable
{
	...
	public void run()
	{
		if (_FishCurHP >= _FishMaxHP * 2)
		{
			// The fish got away
			_Fisher.sendPacket(
					new SystemMessage(SystemMessage.BAIT_STOLEN_BY_FISH));
			DoDie(false);
		} else if (_time <= 0)
		{
			// Time is up, so that fish got away
			_Fisher.sendPacket(
					new SystemMessage(SystemMessage.FISH_SPIT_THE_HOOK));
			DoDie(false);
		} else
			AiTask();
	}
			...
			boolean isUpperGrade)
	{
		...
		ExFishingStartCombat efsc = new ExFishingStartCombat(_Fisher, _time,
				_FishMaxHP, _mode, _lureType, _deceptiveMode);
		...
		if (_fishAItask == null)
		{
			_fishAItask = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(this, 1000, 1000);
		}
	...
	public void ChangeHp(int hp, int pen)
	{
		...
		ExFishingHpRegen efhr = new ExFishingHpRegen(_Fisher, _time, _FishCurHP,
				_mode, _gooduse, _anim, pen, _deceptiveMode);
	...
	public void DoDie(boolean win)
	{
		...
		if (win)
		{
			...
			{
				_Fisher.sendPacket(
						new SystemMessage(SystemMessage.YOU_CAUGHT_SOMETHING));
	...
	protected void AiTask()
	{
		...
		{
			thinking = false;
			ExFishingHpRegen efhr = new ExFishingHpRegen(_Fisher, _time,
					_FishCurHP, _mode, 0, _anim, 0, _deceptiveMode);
			if (_anim != 0)
				_Fisher.broadcastPacket(efhr);
			else
	...
	public void UseRealing(int dmg, int pen)
	{
		...
		if (Rnd.get(100) > 90)
		{
			_Fisher.sendPacket(new SystemMessage(
					SystemMessage.FISH_RESISTED_ATTEMPT_TO_BRING_IT_IN));
			_gooduse = 0;
			ChangeHp(0, pen);
			return;
		}
		...
		if (_mode == 1)
		{
			if (_deceptiveMode == 0)
			{
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.REELING_SUCCESFUL_S1_DAMAGE);
				sm.addNumber(dmg);
				_Fisher.sendPacket(sm);
				if (pen == 50)
				{
					sm = new SystemMessage(
							SystemMessage.REELING_SUCCESSFUL_PENALTY_S1);
					sm.addNumber(pen);
					_Fisher.sendPacket(sm);
				}
				_gooduse = 1;
			...
			} else
			{
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.FISH_RESISTED_REELING_S1_HP_REGAINED);
				sm.addNumber(dmg);
				_Fisher.sendPacket(sm);
				_gooduse = 2;
				ChangeHp(-dmg, pen);
	...
	public void UsePomping(int dmg, int pen)
	{
		...
		if (_mode == 0)
		{
			if (_deceptiveMode == 0)
			{
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.PUMPING_SUCCESFUL_S1_DAMAGE);
				sm.addNumber(dmg);
				_Fisher.sendPacket(sm);
				if (pen == 50)
				{
					sm = new SystemMessage(
							SystemMessage.PUMPING_SUCCESSFUL_PENALTY_S1);
					sm.addNumber(pen);
					_Fisher.sendPacket(sm);
				}
				_gooduse = 1;
			...
			} else
			{
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.FISH_RESISTED_PUMPING_S1_HP_REGAINED);
				sm.addNumber(dmg);
				_Fisher.sendPacket(sm);
				_gooduse = 2;
				ChangeHp(-dmg, pen);
	...
	private void PenaltyMonster()
	{
		...
		_Fisher.sendPacket(new SystemMessage(
				SystemMessage.YOU_CAUGHT_SOMETHING_SMELLY_THROW_IT_BACK));
		switch (lvl)
		{
		case 0:
		case 1:
		...
		if (temp != null)
		{
			try
			{
				...
				spawn.stopRespawn();
				((L2PenaltyMonsterInstance) spawn.doSpawn())
						.SetPlayerToKill(_Fisher);
			} catch (Exception e)
			{
				// Nothing
			}
