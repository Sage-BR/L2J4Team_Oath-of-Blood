...
public class L2SummonInstance extends L2Summon
{
	protected static Logger log = Logger
			.getLogger(L2SummonInstance.class.getName());

	private float _expPenalty = 0; // exp decrease multiplier (i.e. 0.3 (= 30%)
									// for shadow)
	...
	public L2SummonInstance(int objectId, L2NpcTemplate template,
			L2PcInstance owner, L2Skill skill)
	{
		...
		if (skill != null)
		{
			...
			if (skill.getItemConsumeTime() > 0)
				_itemConsumeSteps = (SUMMON_LIFETIME_INTERVAL
						/ skill.getItemConsumeTime()) - 1;
		}

		// When no item consume is defined task only need to check when summon
		// life time has ended.
		// Otherwise have to destroy items from owner's inventory in order to
		// let summon live.
		int delay = SUMMON_LIFETIME_INTERVAL / (_itemConsumeSteps + 1);

		if (Config.DEBUG && (_itemConsumeCount != 0))
			_log.warning("L2SummonInstance: Item Consume ID: " + _itemConsumeId
					+ ", Count: " + _itemConsumeCount + ", Rate: "
					+ _itemConsumeSteps + " times.");
		if (Config.DEBUG)
			_log.warning("L2SummonInstance: Task Delay " + (delay / 1000)
					+ " seconds.");
	...
	public void reduceCurrentHp(int damage, L2Character attacker)
	{
		...
		SystemMessage sm = new SystemMessage(
				SystemMessage.SUMMON_RECEIVED_DAMAGE_OF_S2_BY_S1);
	...
	{
		if (Config.DEBUG)
			_log.warning("L2SummonInstance: " + getTemplate().name + " ("
					+ getOwner().getName() + ") has been killed.");
	 ...
	 * { if (crit) { getOwner().sendPacket(new
	 * SystemMessage(SystemMessage.SUMMON_CRITICAL_HIT)); }
	 *
	 * if (miss) { getOwner().sendPacket(new
	 * SystemMessage(SystemMessage.MISSED_TARGET)); } else { SystemMessage sm =
	 * new SystemMessage(SystemMessage.SUMMON_GAVE_DAMAGE_OF_S1);
	 * sm.addNumber(damage); getOwner().sendPacket(sm); } }
	 */
	...
	static class SummonLifetime implements Runnable
	{
		...
		public void run()
		{
			...
			if (_summon.getItemConsumeStepsElapsed() >= _summon
					.getItemConsumeSteps())
			{
			...
			// check if owner has enought itemConsume, if requested
					...
					&& _summon.getItemConsumeId() != 0 && !_summon.isDead()
					&& !_summon.destroyItemByItemId("Consume",
							_summon.getItemConsumeId(),
	...
	{
		return getOwner().destroyItem(process, objectId, count, reference,
				sendMessage);
	}
			...
			L2Object reference, boolean sendMessage)
	{
		if (Config.DEBUG)
			_log.warning("L2SummonInstance: " + getTemplate().name + " ("
					+ getOwner().getName() + ") consume.");

		return getOwner().destroyItemByItemId(process, itemId, count, reference,
				sendMessage);
	}
}
