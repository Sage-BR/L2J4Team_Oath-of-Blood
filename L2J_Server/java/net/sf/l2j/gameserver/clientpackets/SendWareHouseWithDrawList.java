...
public class SendWareHouseWithDrawList extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(SendWareHouseWithDrawList.class.getName());
	...
	void runImpl()
	{
		...
		for (int i = 0; i < _count; i++)
		{
			...
			if (oldItem == null || oldItem.getCount() < count)
				player.sendMessage("Can't withdraw requested item"
						+ (count > 1 ? "s" : ""));
			L2ItemInstance newItem = warehouse.transferItem("Warehouse",
					objectId, count, player.getInventory(), player,
					player.getLastFolkNPC());
			if (newItem == null)
			{
				_log.warning("Error withdrawing a warehouse object for char "
						+ player.getName());
				continue;
			}
