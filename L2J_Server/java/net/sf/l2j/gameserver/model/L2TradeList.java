...
public class L2TradeList
{
	...
	public void tradeItems(L2PcInstance player, L2PcInstance reciever)
	{
		...
		// boolean isValid;
		// LinkedList<L2ItemInstance> itemsToAdd = new
		// LinkedList<L2ItemInstance>();
		// LinkedList<L2ItemInstance> itemsToRemove = new
		// LinkedList<L2ItemInstance>();
		...
		for (L2ItemInstance _item : _items)
		{
			...
			if (playerItem == null)
				continue;
			newitem = itemTable.createItem("L2TradeList",
					playerItem.getItemId(), playerItem.getCount(), player);
			newitem.setEnchantLevel(temp.getEnchantLevel());

			// DIRTY FIX: Fix for trading pet collar not updating pet with new
			// collar object id
			changePetItemObjectId(playerItem.getObjectId(),
					newitem.getObjectId());
			...
			if (reciever.isGM() || player.isGM())
			{
				...
				GMAudit.auditGMAction(gm.getName(), "trade", target.getName(),
						newitem.getItem().getName() + " - "
								+ newitem.getItemId());
			}
			playerItem = playersInv.destroyItem("!L2TradeList!",
					playerItem.getObjectId(), temp.getCount(), null, null);
			recieverItem = recieverInv.addItem("!L2TradeList!", newitem, null,
					null);

			if (playerItem == null)
			{
				_log.warning(
						"L2TradeList: PlayersInv.destroyItem returned NULL!");
				continue;
			}
	...
	public synchronized void buySellItems(L2PcInstance buyer,
			List<TradeItem> buyerslist, L2PcInstance seller,
			List<TradeItem> sellerslist)
	{
		...
		List<SystemMessage> sysmsgs = new FastList<>();
		...
		for (TradeItem buyerItem : buyerslist)
		{
			for (x = 0; x < sellerslist.size(); x++)// find in sellerslist
			{
				...
				{
					sellerItem = sellerInv
							.getItemByItemId(buyerItem.getItemId());
					break;
				}
			}
			...
			if (sellerItem != null)
			{
				...
				if (buyerItem.getCount() > Integer.MAX_VALUE
						/ buyerItem.getOwnersPrice())
				{
					_log.warning(
							"Integer Overflow on Cost. Possible Exploit attempt between "
									+ buyer.getName() + " and "
									+ seller.getName() + ".");
					return;
				}
				...
				sellerItem = sellerInv.destroyItem("", sellerItem.getObjectId(),
						amount, null, null);
//		        buyer.reduceAdena(cost);
//		        seller.addAdena(cost);
				newitem = itemTable.createItem("L2TradeList",
						sellerItem.getItemId(), amount, buyer, seller);
				...
				} else
				{
					...
					{
						buyerItem.setCount(
								buyerItem.getCount() - temp2.getCount());
					}
				}
