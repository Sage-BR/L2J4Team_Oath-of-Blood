...
public class L2AdventurerInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		if (command.startsWith("npcfind_byid"))
		{
			try
			{
				...
				switch (RaidBossSpawnManager.getInstance()
						.getRaidBossStatusId(bossId))
				{
				case ALIVE:
				case DEAD:
					L2Spawn spawn = RaidBossSpawnManager.getInstance()
							.getSpawns().get(bossId);
					player.sendPacket(new RadarControl(0, 1, spawn.getLocx(),
							spawn.getLocy(), spawn.getLocz()));
					break;
				case UNDEFINED:
					player.sendMessage(
							"This Boss isn't in game - notify L2J Datapack Dev Team");
					break;
				}
		...
		} else if (command.startsWith("raidInfo"))
		{
			...
			if (bossLevel != 0)
			{
				filename = "data/html/adventurer_guildsman/raid_info/level"
						+ bossLevel + ".htm";
			}
	...
	private void showChatWindow(L2PcInstance player, int bossLevel,
			String filename)
	{
