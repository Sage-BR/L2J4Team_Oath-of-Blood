...
public class LoginController
{
	...
	/**
	 * this map contains the connections of the players that are in the
	 * loginserver
	 */
	 ...
	 * {@link net.sf.l2j.loginserver.serverpacket.LoginOk LoginOk} packet and
	 * the other is sent in {@link net.sf.l2j.loginserver.serverpacket.PlayOk
	 * PlayOk}
	 * </p>
	 *
	 * @author -Wooden-
	...
	public static class SessionKey
	{
		...
		{
			return "PlayOk: " + playOkID1 + " " + playOkID2 + " LoginOk:"
					+ loginOkID1 + " " + loginOkID2;
		}
		 ...
		 * </p>
		 * <p>
		 * Only checks the PlayOk part of the session key if server doesnt show
		 * the licence when player logs in.
		 * </p>
		 *
		 * @param key
		 */
		...
		{
			// when server doesnt show licence it deosnt send the LoginOk
			// packet, client doesnt have this part of the key then.
	...
	private LoginController()
	{
		...
		_logins = new FastMap<>();
		_accountsInLoginServer = new FastMap<>();
		...
		_rnd = new Random();
		if (Config.DEBUG)
			_log.info("LoginContoller : RSA keygen initated");
	...
	public SessionKey assignSessionKeyToLogin(String account, Socket _csocket)
	{
		...
		key = new SessionKey(_rnd.nextInt(), _rnd.nextInt(), _rnd.nextInt(),
				_rnd.nextInt());
	...
	{
		List<GameServerThread> gslist = GameServerListener.getInstance()
				.getGameServerThreads();
		synchronized (gslist)
		{
	...
	{
		return ((getOnlinePlayerCount(ServerID) < _maxAllowedOnlinePlayers)
				|| (access >= 50));
	}
	 ...
	 * </p>
	 * <p>
	 * One of them the re-newed asynchronously using a {@link UpdateKeyPairTask}
	 * if necessary.
	 * </p>
	 *
	...
	{
		// ensure that the task will update the keypair only after a keypair is
		// returned.
		synchronized (_keyPairs)
		{
			if ((System.currentTimeMillis() - _lastKeyPairUpdate) > 1200000) // update
																				// a
																				// key
																				// every
																				// 20
																				// minutes
			{
				...
				UpdateKeyPairTask task = new UpdateKeyPairTask(
						_keyPairToUpdate.getAndIncrement());
				task.start();
				_lastKeyPairUpdate = System.currentTimeMillis();
			}
			...
			throws HackingException
	{
		...
		if (failedConnects != null
				&& failedConnects > Config.LOGIN_TRY_BEFORE_BAN)
		{
		...
		try
		{
			...
			// int busy =
			// L2DatabaseFactory.getInstance().getBusyConnectionCount();
			// int idle =
			// L2DatabaseFactory.getInstance().getIdleConnectionCount();
					...
					if ((user.length() >= 2) && (user.length() <= 14))
					{
						...
						if (LoginServer.statusServer != null)
							LoginServer.statusServer.SendMessageToTelnets(
									"Account created for player " + user);

						return true;

					}
					_log.warning(
							"Invalid username creation/use attempt: " + user);
					return false;
				}
				_log.warning("[" + address.getHostAddress()
						+ "]: account missing for user " + user);
				return false;
			}
		...
		if (!ok)
		{
			Log.add("'" + user + "' " + address.getHostAddress(),
					"logins_ip_fails");
			...
			{
				_hackProtection.put(address.getHostAddress(),
						new Integer(failedCount));
	...
	private class UpdateKeyPairTask extends Thread
	{
		...
		public void run()
		{
			_keyPairs[_keyPairId] = new ScrambledKeyPair(
					_keyGen.generateKeyPair());
	...
	public static class ScrambledKeyPair
	{
		...
		{
			this.pair = pPair;
			scrambledModulus = scrambleModulus(
					((RSAPublicKey) this.pair.getPublic()).getModulus());
		}
		...
		private byte[] scrambleModulus(BigInteger modulus)
		{
			...
			{
				scrambledMod[i] = (byte) (scrambledMod[i]
						^ scrambledMod[0x40 + i]);
			}
			...
			for (int i = 0; i < 0x40; i++)
			{
				scrambledMod[0x40 + i] = (byte) (scrambledMod[0x40 + i]
						^ scrambledMod[i]);
			}
