...
public class Zone
{
	...
	{
		return checkIfInZone(obj.getPosition().getX(),
	...
	{
		return findDistanceToZone(obj.getPosition().getX(),
				obj.getPosition().getY(), obj.getPosition().getZ(),
				includeZAxis);
	}

	public final double findDistanceToZone(int x, int y, int z,
			boolean includeZAxis)
	{
		...
		{
			distance = Util.calculateDistance(x, y, z,
					Math.abs(coord[0] + coord[2]),
					Math.abs(coord[1] + coord[3]), coord[4], includeZAxis);
	...
	public final int[] getCoord(int x, int y)
	{
		for (int[] coord : getCoords())
		{
			if (coord[2] == 0 && coord[3] == 0)
				continue;
			if (x >= Math.min(coord[0], coord[2])
					&& x <= Math.max(coord[0], coord[2])
					&& y >= Math.min(coord[1], coord[3])
					&& y <= Math.max(coord[1], coord[3]))
				return coord;
		}
		return null;
	}
