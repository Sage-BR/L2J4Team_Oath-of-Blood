	...
	public void useSkill(L2Character caster, L2Object[] targets)
	{
		...
		for (L2Object target2 : targets)
		{
			...
			if (target.isAlikeDead()
					&& getTargetType() != SkillTargetType.TARGET_CORPSE_MOB)
				continue;
			...
			/*
			 * for (int j=0;j<effects.length;j++){ if
			 * (effects[j].getEffectType()==L2Effect.EffectType.SEED){
			 * EffectSeed e = (EffectSeed)effects[j]; if (e.getInUse() ||
			 * e.getSkill().getId()==this.getId()){ e.rescheduleEffect(); } } }
			 */
		}
	}
}
