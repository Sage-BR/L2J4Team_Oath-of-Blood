...
public class RequestFriendInvite extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestFriendInvite.class.getName());
	...
	void runImpl()
	{
		...
		_name = Util.capitalizeFirst(_name); // FIXME: is it right to capitalize
												// a nickname?
		...
		if (friend == null)
		{
			...
			sm = new SystemMessage(
					SystemMessage.THE_USER_YOU_REQUESTED_IS_NOT_IN_GAME);
		...
		} else if (friend == activeChar)
		{
			...
			sm = new SystemMessage(
					SystemMessage.YOU_CANNOT_ADD_YOURSELF_TO_OWN_FRIEND_LIST);
			activeChar.sendPacket(sm);
			sm = null;
			return;
		}
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT char_id FROM character_friends WHERE char_id=? AND friend_id=?");
			...
			if (rset.next())
			{
				...
				sm = new SystemMessage(
						SystemMessage.S1_ALREADY_IN_FRIENDS_LIST);
				sm.addString(_name);
			} else
			{
				if (!friend.isProcessingRequest())
				{
					...
					sm = new SystemMessage(
							SystemMessage.S1_REQUESTED_TO_BECOME_FRIENDS);
					sm.addString(_name);
