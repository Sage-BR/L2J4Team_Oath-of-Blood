...
public class AutoSpawnHandler
{
	protected static Logger _log = Logger
			.getLogger(AutoSpawnHandler.class.getName());

	private static AutoSpawnHandler _instance;

	private static final int DEFAULT_INITIAL_SPAWN = 30000; // 30 seconds after
															// registration
	...
	private void restoreSpawnData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"SELECT * FROM random_spawn ORDER BY groupId ASC");
			...
			while (rs.next())
			{
				...
				AutoSpawnInstance spawnInst = registerSpawn(rs.getInt("npcId"),
						rs.getInt("initialDelay"), rs.getInt("respawnDelay"),
				...
				statement2 = con.prepareStatement(
						"SELECT * FROM random_spawn_loc WHERE groupId=?");
				...
				while (rs2.next())
				{
					...
					spawnInst.addSpawnLocation(rs2.getInt("x"), rs2.getInt("y"),
							rs2.getInt("z"), rs2.getInt("heading"));
				}
		...
		{
			_log.warning(
					"AutoSpawnHandler: Could not restore spawn data: " + e);
		} finally
		{
			try
			{
	 ...
	 */
	public AutoSpawnInstance registerSpawn(int npcId, int[][] spawnPoints,
			int initialDelay, int respawnDelay, int despawnDelay)
	{
		if (initialDelay < 0)
		...
		AutoSpawnInstance newSpawn = new AutoSpawnInstance(npcId, initialDelay,
				respawnDelay, despawnDelay);
		...
		if (Config.DEBUG)
			_log.config("AutoSpawnHandler: Registered auto spawn for NPC ID "
					+ npcId + " (Object ID = " + newId + ").");
	 ...
	 * @return AutoSpawnInstance spawnInst
	 */
	public AutoSpawnInstance registerSpawn(int npcId, int initialDelay,
			int respawnDelay, int despawnDelay)
	{
		return registerSpawn(npcId, null, initialDelay, respawnDelay,
				despawnDelay);
	}
	...
	public boolean removeSpawn(AutoSpawnInstance spawnInst)
	{
		...
		try
		{
			...
			ScheduledFuture respawnTask = _runningSpawns
					.remove(spawnInst._objectId);
			...
			if (isActive)
			{
				...
				if (spawnInst._desDelay > 0)
					spawnTask = ThreadPoolManager.getInstance()
							.scheduleEffectAtFixedRate(rs, spawnInst._initDelay,
									spawnInst._resDelay);
				else
					spawnTask = ThreadPoolManager.getInstance()
							.scheduleEffect(rs, spawnInst._initDelay);
	 ...
	 * @return AutoSpawnInstance spawnInst
	 */
	public final AutoSpawnInstance getAutoSpawnInstance(int id,
			boolean isObjectId)
	{
		if (isObjectId)
		{
	...
	private class AutoSpawner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				L2NpcTemplate npcTemp = NpcTable.getInstance()
						.getTemplate(spawnInst.getNpcId());
				if (npcTemp == null)
				{
					_log.warning("Couldnt find NPC id" + spawnInst.getNpcId()
							+ " Try to update your DP");
					return;
				}
				...
				if (spawnInst._spawnCount == 1)
				{
					...
					npcInst.setXYZ(npcInst.getX(), npcInst.getY(),
							npcInst.getZ());
				...
				} else
				{
					for (int i = 0; i < spawnInst._spawnCount; i++)
					{
						...
						npcInst.setXYZ(npcInst.getX() + rnd.nextInt(50),
								npcInst.getY() + rnd.nextInt(50),
								npcInst.getZ());
				...
				String nearestTown = MapRegionTable.getInstance()
						.getClosestTownName(npcInst);

				// Announce to all players that the spawn has taken place, with
				// the nearest town location.
				if (spawnInst.isBroadcasting())
					Announcements.getInstance()
							.announceToAll("The " + npcInst.getName()
									+ " has spawned near " + nearestTown + "!");
				...
				if (spawnInst.getDespawnDelay() > 0)
				{
					...
					ThreadPoolManager.getInstance().scheduleAi(rd,
							spawnInst.getDespawnDelay() - 1000);
				}
			} catch (Exception e)
			{
				_log.warning(
	...
	private class AutoDespawner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				{
					_log.info(
							"AutoSpawnHandler: No spawn registered for object ID = "
									+ _objectId + ".");
					return;
				}
				...
				for (L2NpcInstance npcInst : spawnInst.getNPCInstanceList())
				{
					...
					if (Config.DEBUG)
						_log.info(
								"AutoSpawnHandler: Spawns removed for spawn instance (Object ID = "
										+ _objectId + ").");
				}
			} catch (Exception e)
			{
				_log.warning(
						"AutoSpawnHandler: An error occurred while despawning spawn (Object ID = "
								+ _objectId + "): " + e);
			}
		}
	}
	...
	public class AutoSpawnInstance
	{
		...
		protected AutoSpawnInstance(int npcId, int initDelay, int respawnDelay,
				int despawnDelay)
		{
			_npcId = npcId;
