...
public class GameServer
{
	...
	public GameServer() throws Exception
	{
		...
		{
			_log.severe(
					"Could not find the extraced files. Please Check Your Data.");
		...
		_log.config(
				"ItemHandler: Loaded " + _itemHandler.size() + " handlers.");
		...
		_log.config(
				"SkillHandler: Loaded " + _skillHandler.size() + " handlers.");
		...
		_log.config("AdminCommandHandler: Loaded " + _adminCommandHandler.size()
				+ " handlers.");
		...
		_log.config("UserCommandHandler: Loaded " + _userCommandHandler.size()
				+ " handlers.");

		_voicedCommandHandler = VoicedCommandHandler.getInstance();
		_voicedCommandHandler.registerVoicedCommandHandler(new stats());

		_log.config("VoicedCommandHandler: Loaded "
				+ _voicedCommandHandler.size() + " handlers.");
		...
		} catch (Exception ex)
		{
			_log.log(Level.WARNING,
					"DynamicExtension could not be loaded and initialized", ex);
		}
		...
		_log.info("GameServer Started, free memory " + freeMem + " Mb of "
				+ totalMem + " Mb");
