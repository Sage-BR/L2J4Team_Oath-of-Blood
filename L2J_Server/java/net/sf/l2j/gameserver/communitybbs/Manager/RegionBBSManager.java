...
public class RegionBBSManager extends BaseBBSManager
{
		...
		} else
		{
			...
			} else
			{
				ShowBoard sb = new ShowBoard(
						"<html><body><br><br><center>the command: " + command
	...
	private void showOldCommunityPI(L2PcInstance activeChar, String name)
	{
		...
		if (player != null)
		{
			...
			if (player.getLevel() >= 60)
				...
				levelApprox = "medium";
			htmlCode.append("<table border=0><tr><td>" + player.getName() + " ("
					+ sex + " " + player.getTemplate().className
					+ "):</td></tr>");
			htmlCode.append("<tr><td>Level: " + levelApprox + "</td></tr>");
			htmlCode.append("<tr><td><br></td></tr>");

			if (activeChar != null && (activeChar.isGM()
					|| player.getObjectId() == activeChar.getObjectId()
					|| Config.SHOW_LEVEL_COMMUNITYBOARD))
			{
				...
				htmlCode.append("<tr><td>Experience: " + player.getExp() + "/"
						+ nextLevelExp + "</td></tr>");
				htmlCode.append("<tr><td>Experience needed for level up: "
						+ nextLevelExpNeeded + "</td></tr>");
				htmlCode.append("<tr><td><br></td></tr>");
			}
			...
			htmlCode.append("<tr><td>Uptime: " + h + "h " + m + "m " + s
					+ "s</td></tr>");
			htmlCode.append("<tr><td><br></td></tr>");

			if (player.getClan() != null)
			{
				htmlCode.append("<tr><td>Clan: " + player.getClan().getName()
						+ "</td></tr>");
				htmlCode.append("<tr><td><br></td></tr>");
			}
		...
		{
			ShowBoard sb = new ShowBoard(
					"<html><body><br><br><center>No player with name " + name
	...
	{
		TextBuilder htmlCode = new TextBuilder("<html><body><br>");
		Collection<L2PcInstance> players = L2World.getInstance()
				.getAllPlayers();
		String tdClose = "</td>";
		...
		htmlCode.append(tdOpen + "Server Time: " + format.format(cal.getTime())
				+ tdClose);
		htmlCode.append(colSpacer);
		cal.set(Calendar.HOUR_OF_DAY, t / 60);
		cal.set(Calendar.MINUTE, t % 60);
		htmlCode.append(tdOpen + "Game Time: " + format.format(cal.getTime())
				+ tdClose);
		htmlCode.append(colSpacer);
		htmlCode.append(tdOpen + "Server Restarted: "
				+ GameServer.DateTimeServerStarted.getTime() + tdClose);
		...
		htmlCode.append(colSpacer);
		htmlCode.append(
				tdOpen + "Party XP Rate: " + Config.RATE_PARTY_XP + tdClose);
		htmlCode.append(colSpacer);
		htmlCode.append(tdOpen + "XP Exponent: " + Config.ALT_GAME_EXPONENT_XP
				+ tdClose);
		htmlCode.append(trClose);
		...
		htmlCode.append(colSpacer);
		htmlCode.append(
				tdOpen + "Party SP Rate: " + Config.RATE_PARTY_SP + tdClose);
		htmlCode.append(colSpacer);
		htmlCode.append(tdOpen + "SP Exponent: " + Config.ALT_GAME_EXPONENT_SP
				+ tdClose);
		htmlCode.append(trClose);

		htmlCode.append(trOpen);
		htmlCode.append(
				tdOpen + "Drop Rate: " + Config.RATE_DROP_ITEMS + tdClose);
		htmlCode.append(colSpacer);
		htmlCode.append(
				tdOpen + "Spoil Rate: " + Config.RATE_DROP_SPOIL + tdClose);
		htmlCode.append(colSpacer);
		htmlCode.append(
				tdOpen + "Adena Rate: " + Config.RATE_DROP_ADENA + tdClose);
		htmlCode.append(trClose);
		...
		htmlCode.append(trOpen);
		htmlCode.append(
				"<td><img src=\"sek.cbui355\" width=625 height=1><br></td>");
		htmlCode.append(trClose);
		...
		if (activeChar.isGM())
		{
			htmlCode.append(trOpen);
			htmlCode.append(
					tdOpen + L2World.getInstance().getAllVisibleObjectsCount()
							+ " Object count</td>");
			htmlCode.append(trClose);
		}
		...
		for (i = 0; i < startIndex + Config.NAME_PAGE_SIZE_COMMUNITYBOARD; i++)
		{
			...
			htmlCode.append(
					"<td align=left valign=top FIXWIDTH=75><a action=\"bypass _bbsloc;playerinfo;"
							+ player.getName() + "\">");

			if (player.isGM())
				htmlCode.append("<font color=\"LEVEL\">" + player.getName()
						+ "</font>");
			else
		...
		{
			htmlCode.append("<tr><td align=center valign=top>Displaying "
					+ (startIndex + 1) + " - " + i + " player(s)</td></tr>");
			htmlCode.append(
					"<tr><td align=center valign=top><table border=0 width=610>");
			htmlCode.append("<tr>");
			if (startIndex == 0)
				htmlCode.append(
						"<td><button value=\"Prev\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
			else
				htmlCode.append(
						"<td><button value=\"Prev\" action=\"bypass _bbsloc;page;"
								+ (startIndex
			...
			if (players.size() <= startIndex
					+ Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
				htmlCode.append(
						"<td><button value=\"Next\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
			else
				htmlCode.append(
						"<td><button value=\"Next\" action=\"bypass _bbsloc;page;"
								+ (startIndex
										+ Config.NAME_PAGE_SIZE_COMMUNITYBOARD)
								+ "\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
			htmlCode.append("</tr>");
			...
			String ar5, L2PcInstance activeChar)
	{
			...
			try
			{
				...
				if (Config.LOG_CHAT)
				{
					...
					record.setParameters(new Object[]
						{ "TELL", "[" + activeChar.getName() + " to "
								+ receiver.getName() + "]" });
					_logChat.log(record);
				}
				CreatureSay cs = new CreatureSay(activeChar.getObjectId(),
						Say2.TELL, activeChar.getName(), ar3);
				if (receiver != null
						&& !BlockList.isBlocked(receiver, activeChar))
				{
					if (!receiver.getMessageRefusal())
					{
						...
						activeChar.sendPacket(new CreatureSay(
								activeChar.getObjectId(), Say2.TELL,
					...
					} else
					{
						SystemMessage sm = new SystemMessage(
								SystemMessage.THE_PERSON_IS_IN_MESSAGE_REFUSAL_MODE);
						activeChar.sendPacket(sm);
						parsecmd("_bbsloc;playerinfo;" + receiver.getName(),
								activeChar);
					}
				} else
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.S1_IS_NOT_ONLINE);
		...
		{
			ShowBoard sb = new ShowBoard(
					"<html><body><br><br><center>the command: " + ar1
							+ " is not implemented yet</center><br><br></body></html>",
					"101");
			activeChar.sendPacket(sb);
			activeChar.sendPacket(new ShowBoard(null, "102"));
