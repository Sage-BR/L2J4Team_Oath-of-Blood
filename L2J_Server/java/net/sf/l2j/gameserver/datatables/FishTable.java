...
public class FishTable
{
	...
	private FishTable()
	{
		...
		try
		{
			...
			_Fishs_Easy = new FastList<>();
			_Fishs_Normal = new FastList<>();
			_Fishs_Hard = new FastList<>();
			FishData fish;
			PreparedStatement statement = con.prepareStatement(
					"SELECT id, level, name, hp, hpregen, fish_type, fish_group, fish_guts, guts_check_time, wait_time, combat_time FROM fish ORDER BY id");
			...
			while (Fishes.next())
			{
				...
				fish = new FishData(id, lvl, name, hp, hpreg, type, group,
						fish_guts, guts_check_time, wait_time, combat_time);
			...
			Fishes.close();
			statement.close();
			count = _Fishs_Easy.size() + _Fishs_Normal.size()
					+ _Fishs_Hard.size();
		} catch (Exception e)
		{
	...
	public List<FishData> getfish(int lvl, int type, int group)
	{
		...
		if (result.size() == 0)
			_log.warning(
					"Cant Find Any Fish!? - Lvl: " + lvl + " Type: " + type);
		return result;
	}
