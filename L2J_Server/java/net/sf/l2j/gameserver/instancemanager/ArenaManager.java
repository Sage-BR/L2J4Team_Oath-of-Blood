...
public class ArenaManager
{
	protected static Logger _log = Logger
			.getLogger(ArenaManager.class.getName());
	...
	{
		return (getArena(obj) != null);
	}

	/** Return true if object is inside zone */
	public final boolean checkIfInZone(int x, int y)
	{
		return (getArena(x, y) != null);
	}
	...
	public final Arena getArena(L2Object activeObject)
	{
		return getArena(activeObject.getPosition().getX(),
	...
	public final int getArenaIndex(int arenaId)
	{
		...
		for (int i = 0; i < getArenas().size(); i++)
		{
			...
			if (arena != null && arena.getArenaId() == arenaId)
				return i;
		}
		return -1;
	}

	public final int getArenaIndex(L2Object activeObject)
	{
		return getArenaIndex(activeObject.getPosition().getX(),
				activeObject.getPosition().getY());
	}
	...
	public final int getArenaIndex(int x, int y)
	{
		...
		{
			arena = getArenas().get(i);
			if (arena != null && arena.checkIfInZone(x, y))
				return i;
		}
		return -1;
	}
	...
	public final int getArenaIndex(String name)
	{
		...
		{
			arena = getArenas().get(i);
			if (arena != null && arena.getName().equalsIgnoreCase(name.trim()))
				return i;
		}
		return -1;
	}
