...
abstract class AbstractAI implements Ctrl
{
	...
	protected static final Logger _log = Logger
			.getLogger(AbstractAI.class.getName());
	...
	{
		/*
		 * if (Config.DEBUG) _log.warning("AbstractAI: changeIntention -> " +
		 * intention + " " + arg0 + " " + arg1);
		 */
	...
	/**
	 * Launch the L2CharacterAI onIntention method corresponding to the new
	 * Intention.<BR>
	 * <BR>
	 *
			...
			Object arg1)
	{
		...
		/*
		 * if (Config.DEBUG) _log.warning("AbstractAI: setIntention -> " +
		 * intention + " " + arg0 + " " + arg1);
		 */
						...
						&& ((L2Skill) arg0).getId() != 221))
		{
			...
			if (silentMove != null && silentMove.getSkill() != null)
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.EFFECT_S1_DISAPPEARED);
		...
		if (intention != AI_INTENTION_FOLLOW
				&& intention != AI_INTENTION_ATTACK)
			stopFollow();

		// Launch the onIntention method of the L2CharacterAI corresponding to
		// the new Intention
		switch (intention)
		{
		...
		case AI_INTENTION_MOVE_TO_IN_A_BOAT:
			onIntentionMoveToInABoat((L2CharPosition) arg0,
					(L2CharPosition) arg1);
			break;
	...
	public final void notifyEvent(CtrlEvent evt, Object arg0, Object arg1)
	{
		...
		/*
		 * if (Config.DEBUG) _log.warning("AbstractAI: notifyEvent -> " + evt +
		 * " " + arg0 + " " + arg1);
		 */
	...
	protected abstract void onIntentionMoveToInABoat(L2CharPosition destination,
	...
	/**
	 * Sit Down the actor server side AND client side by sending Server->Client
	 * packet ChangeWaitType <I>(broadcast)</I>.<BR>
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	 * subclasses</B></FONT><BR>
	...
	/**
	 * Stand up the actor server side AND client side by sending Server->Client
	 * packet ChangeWaitType <I>(broadcast)</I>.<BR>
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Low level function, used by AI
	...
	protected void moveToPawn(L2Object pawn, int offset)
	{
		...
		if (!_actor.isMovementDisabled())
		{
			...
			// don't send packets too often, check we already moving to this
			// pawn
			...
			// actor to movingObjects of GameTimeController
			_accessor.moveTo(pawn.getX(), pawn.getY(), pawn.getZ(), offset);

			// Send a Server->Client packet MoveToPawn/CharMoveToLocation to the
			// actor and all L2PcInstance in its _knownPlayers
	...
	protected void moveTo(int x, int y, int z)
	{
		...
		if (!_actor.isMovementDisabled())
		{
			...
			// Send a Server->Client packet CharMoveToLocation to the actor and
			// all L2PcInstance in its _knownPlayers
	...
	protected void moveToInABoat(L2CharPosition destination,
			L2CharPosition origin)
	{
		// Chek if actor can move
		if (!_actor.isMovementDisabled())
		{
			/*
			 * // Set AI movement data _client_moving = true;
			 * _client_moving_to_pawn_offset = 0;
			 *
			 * // Calculate movement data for a move to location action and add
			 * the actor to movingObjects of GameTimeController
			 * _accessor.moveTo(((L2PcInstance)_actor).getBoat().getX() -
	...
	protected void clientStopMoving(L2CharPosition pos)
	{
		...
		if (_client_moving || pos != null)
		{
			...
			// Send a Server->Client packet StopMove to the actor and all
			// L2PcInstance in its _knownPlayers
			StopMove msg = new StopMove(_actor);
			_actor.broadcastPacket(msg);

			if (pos != null)
			{
				// Send a Server->Client packet StopRotation to the actor and
				// all L2PcInstance in its _knownPlayers
	...
	public void clientStartAutoAttack()
	{
		if (!isAutoAttacking())
		{
			// Send a Server->Client packet AutoAttackStart to the actor and all
			// L2PcInstance in its _knownPlayers
	...
	public void clientStopAutoAttack()
	{
		if (_actor instanceof L2PcInstance)
		{
			if (!AttackStanceTaskManager.getInstance()
					.getAttackStanceTask(_actor) && isAutoAttacking())
				AttackStanceTaskManager.getInstance()
						.addAttackStanceTask(_actor);
	...
	{
		// Send a Server->Client packet Die to the actor and all L2PcInstance in
		// its _knownPlayers
	 ...
	 * packet MoveToPawn/CharMoveToLocation and AutoAttackStart to the
	...
	public void describeStateToPlayer(L2PcInstance player)
	{
		if (_client_moving)
		{
			...
			{
				// Send a Server->Client packet MoveToPawn to the actor and all
				// L2PcInstance in its _knownPlayers
				MoveToPawn msg = new MoveToPawn(_actor, _follow_target,
						_client_moving_to_pawn_offset);
				player.sendPacket(msg);
			} else
			{
				// Send a Server->Client packet CharMoveToLocation to the actor
				// and all L2PcInstance in its _knownPlayers
				CharMoveToLocation msg = new CharMoveToLocation(_actor);
				player.sendPacket(msg);
			}
		}
	...
	public synchronized void startFollow(L2Character target)
	{
		...
		_follow_task = ThreadPoolManager.getInstance()
				.scheduleAiAtFixedRate(new FollowTask(), 5, FOLLOW_INTERVAL);
	}
	...
	public synchronized void startFollow(L2Character target, int range)
	{
		...
		_follow_target = target;
		_follow_task = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(
				new FollowTask(range), 5, ATTACK_FOLLOW_INTERVAL);
	}
