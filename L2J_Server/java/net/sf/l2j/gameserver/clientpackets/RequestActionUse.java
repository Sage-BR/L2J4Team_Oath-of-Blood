...
public class RequestActionUse extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestActionUse.class.getName());
	...
	/**
	 * packet type id 0x45 format: cddc
	 *
	 * @param rawPacket
	 */
	...
	void runImpl()
	{
		...
		if (Config.DEBUG)
			_log.finest(activeChar.getName() + " request Action use: id "
					+ _actionId + " 2:" + _ctrlPressed + " 3:" + _shiftPressed);
		...
		case 0:
			...
			if (target != null && !activeChar.isSitting()
					...
					&& activeChar.isCastleLord(CastleManager.getInstance()
							.getCastle(target).getCastleId())
					&& activeChar.isInsideRadius(target,
							L2StaticObjectInstance.INTERACTION_DISTANCE, false,
							false))
			{
				ChairSit cs = new ChairSit(activeChar,
			...
			if (Config.DEBUG)
				_log.fine("new wait type: "
						+ (activeChar.isSitting() ? "STANDING" : "SITTING"));
		...
		case 1:
			...
			if (Config.DEBUG)
				_log.fine("new move type: "
						+ (activeChar.isRunning() ? "RUNNING" : "WALKIN"));
			break;
		case 15:
		case 21: // pet follow/stop
			if (pet != null && !pet.isMovementDisabled()
					&& !activeChar.isBetrayed())
				...
				if (target.isAutoAttackable(activeChar) || _ctrlPressed)
				{
					if (target instanceof L2DoorInstance)
					{
						if (((L2DoorInstance) target).isAttackable(activeChar))
							pet.getAI().setIntention(
									CtrlIntention.AI_INTENTION_ATTACK, target);
					}
					// siege golem AI doesn't support attacking other than doors
					// at the moment
		...
		case 19: // pet unsummon
			if (pet != null && !activeChar.isBetrayed())
			{
				...
				{
					activeChar.sendPacket(new SystemMessage(
							SystemMessage.DEAD_PET_CANNOT_BE_RETURNED));
				} else if (pet.isAttackingNow())
				{
					activeChar.sendPacket(new SystemMessage(
							SystemMessage.PET_CANNOT_SENT_BACK_DURING_BATTLE));
				} else
				{
					...
					if (pet instanceof L2PetInstance)
					{
						...
						if (petInst
								.getCurrentFed() > (petInst.getMaxFed() * 0.40))
							pet.unSummon(activeChar);
						else
							activeChar.sendPacket(new SystemMessage(
									SystemMessage.YOU_CANNOT_RESTORE_HUNGRY_PETS));
					}
				}
			}
			break;
		case 38: // pet mount
					...
					&& !activeChar.isBetrayed())
			{
				if (activeChar.isDead())
				{
					...
					SystemMessage msg = new SystemMessage(
							SystemMessage.STRIDER_CANT_BE_RIDDEN_WHILE_DEAD);
				...
				} else if (pet.isDead())
				{
					...
					SystemMessage msg = new SystemMessage(
							SystemMessage.DEAD_STRIDER_CANT_BE_RIDDEN);
					activeChar.sendPacket(msg);
					msg = null;
				} else if (pet.isInCombat())
				{
					...
					SystemMessage msg = new SystemMessage(
							SystemMessage.STRIDER_IN_BATLLE_CANT_BE_RIDDEN);
					activeChar.sendPacket(msg);
					msg = null;
				} else if (activeChar.isInCombat())
				{
					...
					SystemMessage msg = new SystemMessage(
							SystemMessage.STRIDER_CANT_BE_RIDDEN_WHILE_IN_BATTLE);
					activeChar.sendPacket(msg);
					msg = null;
				} else if (activeChar.isSitting() || activeChar.isMoving())
				{
					...
					SystemMessage msg = new SystemMessage(
							SystemMessage.STRIDER_CAN_BE_RIDDEN_ONLY_WHILE_STANDING);
					activeChar.sendPacket(msg);
					msg = null;
				} else if (activeChar.isFishing())
				{
					// You can't mount, dismount, break and drop items while
					// fishing
					SystemMessage msg = new SystemMessage(
							SystemMessage.CANNOT_DO_WHILE_FISHING_2);
					activeChar.sendPacket(msg);
					msg = null;
				...
				{
					Ride mount = new Ride(activeChar.getObjectId(),
							Ride.ACTION_MOUNT, pet.getTemplate().npcId);
			...
			} else if (activeChar.isMounted())
			{
				if (activeChar.isFlying())
					activeChar.removeSkill(
							SkillTable.getInstance().getInfo(4289, 1));
				Ride dismount = new Ride(activeChar.getObjectId(),
		...
		case 48: // Mechanic Golem - Mech. Cannon
			...
			break;
		case 51:
			// Player shouldn't be able to set stores if he/she is alike dead
			// (dead or fake death)
			if (activeChar.isAlikeDead())
			{
				sendPacket(new ActionFailed());
		...
		case 53: // move to target
			if (target != null && pet != null && pet != target
					&& !pet.isMovementDisabled())
			{
				pet.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO,
						new L2CharPosition(target.getX(), target.getY(),
								target.getZ(), 0));
			}
			break;
		...
		case 97: // Request Party Command Channel Info
			// if
			// (!PartyCommandManager.getInstance().isPlayerInChannel(activeChar))
			// return;
		...
		case 1004: // Wind Hatchling/Strider - Wild Defense
			useSkill(4711, activeChar); // TODO use correct skill lvl based on
										// pet lvl
			break;
		...
		case 1006: // Star Hatchling/Strider - Bright Heal
			useSkill(4713, activeChar); // TODO use correct skill lvl based on
										// pet lvl
			break;
		...
		case 1017: // Cursed Man - Cursed Strike/Stun
			...
			break;
		default:
			_log.warning(activeChar.getName() + ": unhandled action type "
					+ _actionId);
		}
	}
	...
	private void useSkill(int skillId, L2Object target)
	{
		...
		{
			Map<Integer, L2Skill> _skills = activeSummon.getTemplate()
					.getSkills();
			...
			if (_skills.size() == 0)
			{
				activeChar.sendPacket(
						new SystemMessage(SystemMessage.SKILL_NOT_AVAILABLE));
				return;
			}
			...
			{
				if (Config.DEBUG)
					_log.warning("Skill " + skillId
							+ " missing from npcskills.sql for a summon id "
							+ activeSummon.getNpcId());
				return;
			}
