...
public class SpawnTable
{
	...
	private Map<Integer, L2Spawn> _spawntable = new FastMap<>();
	...
	private void fillSpawnTable()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT id, count, npc_templateid, locx, locy, locz, heading, respawn_delay, loc_id, periodOfDay FROM spawnlist ORDER BY id");
			...
			while (rset.next())
			{
				template1 = NpcTable.getInstance()
						.getTemplate(rset.getInt("npc_templateid"));
				if (template1 != null)
				{
					...
					} else if (template1.type.equalsIgnoreCase("L2RaidBoss"))
					{
						// Don't spawn raidboss
							...
							&& template1.type.equals("L2ClassMaster"))
					{
					...
					} else
					{
						...
						case 1: // Day
							DayNightSpawnManager.getInstance()
									.addDayCreature(spawnDat);
							_npcSpawnCount++;
							break;
						case 2: // Night
							DayNightSpawnManager.getInstance()
									.addNightCreature(spawnDat);
							_npcSpawnCount++;
							break;
						}
		...
		_log.config("SpawnTable: Loaded " + _spawntable.size()
				+ " Npc Spawn Locations.");

		if (Config.DEBUG)
			_log.fine(
					"SpawnTable: Spawning completed, total number of NPCs in the world: "
							+ _npcSpawnCount);
	...
	public void addNewSpawn(L2Spawn spawn, boolean storeInDb)
	{
		...
		if (storeInDb)
		{
			...
			try
			{
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement(
						"INSERT INTO spawnlist (id,count,npc_templateid,locx,locy,locz,heading,respawn_delay,loc_id) values(?,?,?,?,?,?,?,?,?)");
			...
			} catch (Exception e)
			{
				...
				_log.warning(
						"SpawnTable: Could not store spawn in the DB:" + e);
			} finally
			{
				try
				{
		...
		if (updateDb)
		{
			...
			try
			{
				...
				PreparedStatement statement = con
						.prepareStatement("DELETE FROM spawnlist WHERE id=?");
				statement.setInt(1, spawn.getId());
				statement.execute();
				statement.close();
			} catch (Exception e)
			{
				...
				_log.warning("SpawnTable: Spawn " + spawn.getId()
						+ " could not be removed from DB: " + e);
			} finally
			{
				try
				{
	 ...
	 * @return
	 */
	public void findNPCInstances(L2PcInstance activeChar, int npcId,
			int teleportIndex)
	{
		int index = 0;
			...
			if (npcId == spawn.getNpcid())
			{
				...
				if (teleportIndex > -1)
				{
					if (teleportIndex == index)
						activeChar.teleToLocation(spawn.getLocx(),
								spawn.getLocy(), spawn.getLocz(), true);
				} else
				{
					activeChar.sendMessage(index + " - "
							+ spawn.getTemplate().name + " (" + spawn.getId()
							+ "): " + spawn.getLocx() + " " + spawn.getLocy()
							+ " " + spawn.getLocz());
				}
			}
		}
