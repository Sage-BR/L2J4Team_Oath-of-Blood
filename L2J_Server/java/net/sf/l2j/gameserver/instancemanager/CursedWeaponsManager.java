...
public class CursedWeaponsManager
{
	...
	private final void load()
	{
		...
		try
		{
			...
			File file = new File(
					Config.DATAPACK_ROOT + "/data/cursedWeapons.xml");
			if (!file.exists())
			{
				if (Config.DEBUG)
				...
				if ("list".equalsIgnoreCase(n.getNodeName()))
				{
							...
							.getNextSibling())
					{
						if ("item".equalsIgnoreCase(d.getNodeName()))
						{
							...
							int skillId = Integer.parseInt(attrs
									.getNamedItem("skillId").getNodeValue());
							String name = attrs.getNamedItem("name")
									.getNodeValue();

							CursedWeapon cw = new CursedWeapon(id, skillId,
									name);

							int val;
							for (Node cd = d
									.getFirstChild(); cd != null; cd = cd
											.getNextSibling())
							{
								if ("dropRate"
										.equalsIgnoreCase(cd.getNodeName()))
								{
									attrs = cd.getAttributes();
									val = Integer
											.parseInt(attrs.getNamedItem("val")
													.getNodeValue());
									cw.setDropRate(val);
								} else if ("duration"
										.equalsIgnoreCase(cd.getNodeName()))
	...
	private final void restore()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT itemId, playerId, playerKarma, playerPkKills, nbKills, endTime FROM cursed_weapons");
			...
			for (CursedWeapon cw : _cursedWeapons.values())
			{
				...
				try
				{
					statement = con.prepareStatement(
							"SELECT owner_id FROM items WHERE item_id=?");
					...
					if (rset.next())
					{
						...
						_log.info("PROBLEM : Player " + playerId
								+ " owns the cursed weapon " + itemId
								+ " but he shouldn't.");
						...
						{
							_log.warning("Error while deleting cursed weapon "
									+ itemId + " from userId " + playerId);
						}
						 ...
						 * ); statement.setInt(1, playerId); statement.setInt(2,
						 * cw.getSkillId()); if (statement.executeUpdate() != 1)
						 * { _log.warning("Error while deleting cursed weapon "
						 * +itemId+" skill from userId "+playerId); }
						 */
					}
	...
	public void checkPlayer(L2PcInstance player)
	{
		...
		for (CursedWeapon cw : _cursedWeapons.values())
		{
			if (player.getObjectId() == cw.getPlayerId())
			{
				cw.setPlayer(player);
				cw.setItem(
						player.getInventory().getItemByItemId(cw.getItemId()));
				cw.giveSkill();
				player.setCursedWeaponEquipedId(cw.getItemId());

				SystemMessage sm = new SystemMessage(
						SystemMessage.S2_MINUTE_OF_USAGE_TIME_ARE_LEFT_FOR_S1);
				sm.addString(cw.getName());
				// sm.addItemName(cw.getItemId());
				sm.addNumber((new Long(
						(cw.getEndTime() - System.currentTimeMillis()) / 60000))
								.intValue());
				player.sendPacket(sm);
			}
		}
