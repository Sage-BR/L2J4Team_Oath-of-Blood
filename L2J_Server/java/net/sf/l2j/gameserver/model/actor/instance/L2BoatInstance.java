...
{
	protected static final Logger _logBoat = Logger
			.getLogger(L2BoatInstance.class.getName());
	...
	public class L2BoatTrajet
	{
		 ...
		 * @param sysmessb_1
		 */
		public L2BoatTrajet(int idWaypoint1, int idWTicket1, int ntx1, int nty1,
				int ntz1, String npc1, String sysmess10_1, String sysmess5_1,
		...
		private void loadBoatPath()
		{
			...
			try
			{
				File doorData = new File(Config.DATAPACK_ROOT,
						"data/boatpath.csv");
				lnr = new LineNumberReader(
						new BufferedReader(new FileReader(doorData)));

				String line = null;
				while ((line = lnr.readLine()) != null)
				{
					if (line.trim().length() == 0
							|| !line.startsWith(_IdWaypoint1 + ";"))
						continue;
					parseLine(line);
					return;
				}
		...
		public int state(int state, L2BoatInstance _boat)
		{
			if (state < _max)
			{
				...
				_boat._vd = new VehicleDeparture(_boat, bp.speed1, bp.speed2,
						bp.x, bp.y, bp.z);
				// _boat.getTemplate().baseRunSpd = bp.speed1;
				_boat.moveToLocation(bp.x, bp.y, bp.z, (float) bp.speed1);
				Collection<L2PcInstance> knownPlayers = _boat.getKnownList()
	...
	public void moveToLocation(int x, int y, int z, float speed)
	{
		...
		// destination
		m._ticksToMove = (int) (GameTimeController.TICKS_PER_SECOND * distance
				/ speed);
	...
	public void updatePeopleInTheBoat(int x, int y, int z)
	{
		...
		if (_inboat != null)
		{
			...
			if ((lastx == -1) || (lasty == -1))
			{
				...
				lastx = x;
				lasty = y;
			} else if ((x - lastx) * (x - lastx)
					+ (y - lasty) * (y - lasty) > 2250000) // 1500 * 1500 =
															// 2250000
			{
				check = true;
				lastx = x;
			...
			for (int i = 0; i < _inboat.size(); i++)
			{
				...
				if (check)
				{
					if (needOnVehicleCheckLocation)
					{
						OnVehicleCheckLocation vcl = new OnVehicleCheckLocation(
								this, x, y, z);
	...
	public void begin()
	{
		if (_cycle == 1)
		{
			Collection<L2PcInstance> knownPlayers = getKnownList()
					.getKnownPlayers().values();
			if (knownPlayers != null && !knownPlayers.isEmpty())
			{
				_inboat = new FastMap<>();
				...
				for (L2PcInstance player : knownPlayers)
				{
					...
					{
						L2ItemInstance it;
						it = player.getInventory()
								.getItemByItemId(_t1._IdWTicket1);
						if ((it != null) && (it.getCount() >= 1))
						{
							player.getInventory().destroyItem("Boat",
									it.getObjectId(), 1, player, this);
						...
						{
							player.teleToLocation(_t1._ntx1, _t1._nty1,
									_t1._ntz1, false);
						}
					}
				}
			...
			if (knownPlayers != null && !knownPlayers.isEmpty())
			{
				...
				for (L2PcInstance player : knownPlayers)
				{
					if (player.isInBoat())
					{
						L2ItemInstance it;
						it = player.getInventory()
								.getItemByItemId(_t2._IdWTicket1);
						if ((it != null) && (it.getCount() >= 1))
						{
						...
						{
							player.teleToLocation(_t2._ntx1, _t2._nty1,
									_t2._ntz1, false);
						}
					}
				}
	...
	public void say(int i)
	{
		...
		case 10:
			if (_cycle == 1)
			{
				sm = new CreatureSay(0, Say2.SHOUT, _t1._npc1,
						_t1._sysmess10_1);
			} else
			{
				sm = new CreatureSay(0, Say2.SHOUT, _t2._npc1,
						_t2._sysmess10_1);
			}
			ps = new PlaySound(0, "itemsound.ship_arrival_departure", 1,
					this.getObjectId(), this.getX(), this.getY(), this.getZ());
			if (knownPlayers == null || knownPlayers.isEmpty())
				return;
			for (L2PcInstance player : knownPlayers)
			{
		...
		case 5:
			...
			ps = new PlaySound(0, "itemsound.ship_5min", 1, this.getObjectId(),
		...
		case 1:
			...
			ps = new PlaySound(0, "itemsound.ship_1min", 1, this.getObjectId(),
					this.getX(), this.getY(), this.getZ());
			if (knownPlayers == null || knownPlayers.isEmpty())
				return;
			for (L2PcInstance player : knownPlayers)
	...
	public void SetTrajet1(int idWaypoint1, int idWTicket1, int ntx1, int nty1,
			...
			String sysmess1_1, String sysmess0_1, String sysmessb_1)
	{
		_t1 = new L2BoatTrajet(idWaypoint1, idWTicket1, ntx1, nty1, ntz1,
				idnpc1, sysmess10_1, sysmess5_1, sysmess1_1, sysmess0_1,
				sysmessb_1, _name);
	}
	...
	public void SetTrajet2(int idWaypoint1, int idWTicket1, int ntx1, int nty1,
			...
			String sysmess1_1, String sysmess0_1, String sysmessb_1)
	{
		_t2 = new L2BoatTrajet(idWaypoint1, idWTicket1, ntx1, nty1, ntz1,
				idnpc1, sysmess10_1, sysmess5_1, sysmess1_1, sysmess0_1,
				sysmessb_1, _name);
	}
	 ...
	 * (non-Javadoc)
	 *
	 * @see net.sf.l2j.gameserver.model.L2Object#isAutoAttackable(net.sf.l2j.
	 * gameserver.model.L2Character)
	 */
	@Override
