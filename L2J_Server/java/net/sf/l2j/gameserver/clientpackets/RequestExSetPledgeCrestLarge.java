...
/**
 * Format : chdb c (id) 0xD0 h (subid) 0x11 d data size b raw data (picture i
 * think ;) )
 *
 * @author -Wooden-
 *
...
public class RequestExSetPledgeCrestLarge extends ClientBasePacket
{
	...
	static Logger _log = Logger
			.getLogger(RequestExSetPledgeCrestLarge.class.getName());
	...
	public RequestExSetPledgeCrestLarge(ByteBuffer buf, ClientThread client)
	{
		super(buf, client);
		_size = readD();
		if (_size > 0) // client CAN send a RequestExSetPledgeCrestLarge with
						// the size set to 0 then format is just chd
		{
			_data = readB(_size);
		}
	}
	...
	void runImpl()
	{
		...
		if (_data.length > 2176)
		{
			activeChar.sendMessage(
					"The insignia file size is greater than 2176 bytes.");
			return;
		}

		if ((activeChar.getClanPrivileges()
				& L2Clan.CP_CL_REGISTER_CREST) == L2Clan.CP_CL_REGISTER_CREST)
		{
			if (clan.getHasCastle() == 0 && clan.getHasHideout() == 0)
			{
				activeChar.sendMessage(
						"Only a clan that owns a clan hall or a castle can get their emblem displayed on clan related items"); // there
																																// is
																																// a
																																// system
																																// message
			...
			{
				_log.log(Level.INFO,
						"Error loading large crest of clan:" + clan.getName());
				return;
			}
			...
			try
			{
				...
				PreparedStatement statement = con.prepareStatement(
						"UPDATE clan_data SET crest_large_id = ? WHERE clan_id = ?");
			...
			{
				_log.warning("could not update the large crest id:"
						+ e.getMessage());
			} finally
			{
				try
			...
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.CLAN_EMBLEM_WAS_SUCCESSFULLY_REGISTERED));
