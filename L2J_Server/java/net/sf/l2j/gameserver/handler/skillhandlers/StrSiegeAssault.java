...
public class StrSiegeAssault implements ISkillHandler
{
	...
	L2Object[] targets)
	{
		...
		if (!activeChar.isRiding()
				|| !(player.getTarget() instanceof L2DoorInstance))
			return;
		...
		try
		{
			L2ItemInstance itemToTake = player.getInventory()
			...
			for (L2Object target2 : targets)
			{
				...
				if (damage > 0)
				{
					...
					activeChar.sendPacket(SystemMessage
							.sendString(skill.getName() + " failed."));
			}
		} catch (Exception e)
		{
	...
	{
		return checkIfOkToUseStriderSiegeAssault(activeChar,
				CastleManager.getInstance().getCastle(activeChar), isCheckOnly);
	}
			...
			L2Character activeChar, Castle castle, boolean isCheckOnly)
	{
		...
		if (castle == null || castle.getCastleId() <= 0)
			sm.addString(
					"You must be on castle ground to use strider siege assault");
		else if (!castle.getSiege().getIsInProgress())
			sm.addString(
					"You can only use strider siege assault during a siege.");
		else if (!(player.getTarget() instanceof L2DoorInstance))
			sm.addString(
					"You can only use strider siege assault on doors and walls.");
		else if (!activeChar.isRiding())
			sm.addString(
					"You can only use strider siege assault when on strider.");
		else
			return true;
