...
public class ObjectPosition
{
	private static final Logger _log = Logger
			.getLogger(ObjectPosition.class.getName());
	...
	public final void setXYZ(int x, int y, int z)
	{
		...
		try
		{
			if (L2World.getInstance()
					.getRegion(getWorldPosition()) != getWorldRegion())
				updateWorldRegion();
		} catch (Exception e)
		{
			...
			else if (getActiveObject() instanceof L2PcInstance)
			{
				...
				((L2PcInstance) getActiveObject()).teleToLocation(0, 0, 0,
						false);
				((L2PcInstance) getActiveObject()).sendMessage(
						"Error with your coords, Please ask a GM for help!");
			}
		}
	}
	...
	public void updateWorldRegion()
	{
		...
		L2WorldRegion newRegion = L2World.getInstance()
				.getRegion(getWorldPosition());
