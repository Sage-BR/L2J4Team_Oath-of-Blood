...
public class Siege
{
	...
	// - id=645 msg=[Only clans with Level 4 and higher may register for a
	// castle siege.]
	// id=646 msg=[You do not have the authority to modify the castle defender
	// list.]
	// - id=688 msg=[The clan that owns the castle is automatically registered
	// on the defending side.]
	// id=689 msg=[A clan that owns a castle cannot participate in another
	// siege.]
	// id=690 msg=[You cannot register on the attacking side because you are
	// part of an alliance with the clan that owns the castle.]
	// id=718 msg=[The castle gates cannot be opened and closed during a siege.]
	// - id=295 msg=[$s1's siege was canceled because there were no clans that
	// participated.]
	// id=659 msg=[This is not the time for siege registration and so
	// registrations cannot be accepted or rejected.]
	// - id=660 msg=[This is not the time for siege registration and so
	// registration and cancellation cannot be done.]
	// id=663 msg=[The siege time has been declared for $s. It is not possible
	// to change the time after a siege time has been declared. Do you want to
	// continue?]
	// id=667 msg=[You are registering on the attacking side of the $s1 siege.
	// Do you want to continue?]
	// id=668 msg=[You are registering on the defending side of the $s1 siege.
	// Do you want to continue?]
	// id=669 msg=[You are canceling your application to participate in the $s1
	// siege battle. Do you want to continue?]
	...
	// - id=846 msg=[The siege of $s1 has been canceled due to lack of
	// interest.]
			...
			try
			{
				...
				if (timeRemaining > 3600000)
				{
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleEndSiegeTask(castle),
							timeRemaining - 3600000); // Prepare task for 1 hr
														// left.
				} else if ((timeRemaining <= 3600000)
						&& (timeRemaining > 600000))
				{
					announceToPlayer(Math.round(timeRemaining / 60000)
							+ " minute(s) until " + getCastle().getName()
							+ " siege conclusion.", true);
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleEndSiegeTask(castle),
							timeRemaining - 600000); // Prepare task for 10
														// minute left.
						...
						&& (timeRemaining > 300000))
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleEndSiegeTask(castle),
							timeRemaining - 300000); // Prepare task for 5
														// minute left.
				} else if ((timeRemaining <= 300000) && (timeRemaining > 10000))
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleEndSiegeTask(castle),
							timeRemaining - 10000); // Prepare task for 10
													// seconds count down
				} else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					announceToPlayer(getCastle().getName() + " siege "
							...
							+ " second(s) left!", true);
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleEndSiegeTask(castle), timeRemaining); // Prepare
																				// task
																				// for
																				// second
																				// count
	...
	public class ScheduleStartSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleStartSiegeTask(castle),
							timeRemaining - 86400000); // Prepare task for 24
														// before siege start to
														// end registration
				} else if ((timeRemaining <= 86400000)
						&& (timeRemaining > 13600000))
				{
					announceToPlayer("The registration term for "
							+ getCastle().getName() + " has ended.", false);
					_IsRegistrationOver = true;
					clearSiegeWaitingClan();
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleStartSiegeTask(castle),
							timeRemaining - 13600000); // Prepare task for 1 hr
														// left before siege
														// start.
				...
				} else if ((timeRemaining <= 10000) && (timeRemaining > 0))
				{
					announceToPlayer(getCastle().getName() + " siege "
							+ Math.round(timeRemaining / 1000)
							+ " second(s) to start!", false);
					ThreadPoolManager.getInstance().scheduleGeneral(
							new ScheduleStartSiegeTask(castle), timeRemaining); // Prepare
																				// task
																				// for
																				// second
																				// count
	...
	private List<L2SiegeClan> _Attacker_Clans = new FastList<>(); // L2SiegeClan

	private List<L2SiegeClan> _Defender_Clans = new FastList<>(); // L2SiegeClan

	private List<L2SiegeClan> _Defender_Waiting_Clans = new FastList<>(); // L2SiegeClan
	...
	private boolean _IsNormalSide = true; // true = Atk is Atk, false = Atk is
											// Def
	...
	public void endSiege()
	{
		...
		{
			announceToPlayer(
					"The siege of " + getCastle().getName() + " has finished!",
					false);

			if (getCastle().getOwnerId() <= 0)
				announceToPlayer("The siege of " + getCastle().getName()
						+ " has ended in a draw.", false);
			...
			teleportPlayer(Siege.TeleportWhoType.Attacker,
					MapRegionTable.TeleportWhereType.Town); // Teleport to the
															// second closest
															// town
			// teleportPlayer(Siege.TeleportWhoType.Defender,
			// MapRegionTable.TeleportWhereType.Town); // Teleport to the second
			// closest town
			...
			_SiegeGuardManager.unspawnSiegeGuard(); // Remove all spawned siege
													// guard from this castle
			if (getCastle().getOwnerId() > 0)
	...
	public void midVictory()
	{
		if (getIsInProgress()) // Siege still in progress
		{
			...
			if (getDefenderClans().size() == 0 && // If defender doesn't exist
													// (Pc vs Npc)
					getAttackerClans().size() == 1 // Only 1 attacker
			)
			{
				L2SiegeClan sc_newowner = getAttackerClan(
			...
			if (getCastle().getOwnerId() > 0)
			{
				...
				int allyId = ClanTable.getInstance()
						.getClan(getCastle().getOwnerId()).getAllyId();
				if (getDefenderClans().size() == 0) // If defender doesn't exist
													// (Pc vs Npc)
				// and only an alliance attacks
				{
					...
					if (allyId != 0)
					{
						...
						for (L2SiegeClan sc : getAttackerClans())
						{
							if (sc != null)
							{
								if (ClanTable.getInstance()
										.getClan(sc.getClanId())
										.getAllyId() != allyId)
				...
				teleportPlayer(Siege.TeleportWhoType.Attacker,
						MapRegionTable.TeleportWhereType.SiegeFlag); // Teleport
																		// to
																		// the
																		// second
																		// closest
																		// town
				teleportPlayer(Siege.TeleportWhoType.Spectator,
						MapRegionTable.TeleportWhereType.Town); // Teleport to
																// the second
																// closest town

				removeDefenderFlags(); // Removes defenders' flags
				getCastle().removeUpgrade(); // Remove all castle upgrade
				getCastle().spawnDoor(true); // Respawn door to castle but make
												// them weaker (50% hp)
			}
		}
	}
	...
	public void startSiege()
	{
		if (!getIsInProgress())
		{
			if (getAttackerClans().size() <= 0)
			{
				...
				if (getCastle().getOwnerId() <= 0)
					sm = new SystemMessage(
							SystemMessage.SIEGE_OF_S1_HAS_BEEN_CANCELED_DUE_TO_LACK_OF_INTEREST);
				else
					sm = new SystemMessage(
							SystemMessage.S1_SIEGE_WAS_CANCELED_BECAUSE_NO_CLANS_PARTICIPATED);
			...
			_IsInProgress = true; // Flag so that same siege instance cannot be
									// started again
			...
			MercTicketManager.getInstance()
					.deleteTickets(getCastle().getCastleId()); // remove the
																// tickets from
																// the ground
			...
			_SiegeEndDate.add(Calendar.MINUTE,
					SiegeManager.getInstance().getSiegeLength());
			ThreadPoolManager.getInstance().scheduleGeneral(
					new ScheduleEndSiegeTask(getCastle()), 1000); // Prepare
																	// auto end
																	// task

			announceToPlayer(
					"The siege of " + getCastle().getName() + " has started!",
					false);
		}
	}
	...
	public void announceToPlayer(String message, boolean inAreaOnly)
	{
		...
		{
			if (!inAreaOnly || (inAreaOnly
					&& checkIfInZone(player.getX(), player.getY())))
	...
	{
		return (getIsInProgress() && (getCastle().checkIfInZone(x, y) || // Castle
																			// Zone
				getZone().checkIfInZone(x, y) || // Siege Zone
				(Config.ZONE_TOWN != 0 && getCastle().checkIfInZoneTowns(x, y)) // Town
																				// PVP
																				// Setting
		));
	}
	...
	public void clearSiegeClan()
	{
		...
		try
		{
			...
			if (getCastle().getOwnerId() > 0)
			{
				PreparedStatement statement2 = con.prepareStatement(
						"DELETE FROM siege_clans WHERE clan_id=?");
		...
		{
			System.out
					.println("Exception: clearSiegeClan(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	public void clearSiegeWaitingClan()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"DELETE FROM siege_clans WHERE castle_id=? and type = 2");
			statement.setInt(1, getCastle().getCastleId());
			statement.execute();
			statement.close();
		...
		{
			System.out.println(
					"Exception: clearSiegeWaitingClan(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	public List<L2PcInstance> getDefendersInZone()
	{
		...
		{
			if (player.getClan() != null
					&& player.getClan().getClanId() == ownerId)
				continue;
	...
	/**
	 * Return list of L2PcInstance not registered as attacker or defender in the
	 * zone.
	 */
	public List<L2PcInstance> getSpectatorsInZone()
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			if (checkIfInZone(player.getX(), player.getY()) && (player
					.getClan() == null
					|| (getAttackerClan(player.getClan()) == null
							&& getDefenderClan(player.getClan()) == null)))
				players.add(player);
	...
	{
		_Defender_RespawnDelay_Penalty += SiegeManager.getInstance()
				.getControlTowerLosePenalty(); // Add respawn penalty to
	...
	public void registerAttacker(L2PcInstance player, boolean force)
	{
		...
		int allyId = 0;
		if (getCastle().getOwnerId() != 0)
			allyId = ClanTable.getInstance().getClan(getCastle().getOwnerId())
					.getAllyId();
		if (allyId != 0)
		{
			...
			{
				player.sendMessage(
						"You cannot register as an attacker because your alliance owns the castle");
				return;
			}
		}
	...
	{
		if (getCastle().getOwnerId() <= 0)
			player.sendMessage("You cannot register as a defender because "
	...
	public void removeSiegeClan(int clanId)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"DELETE FROM siege_clans WHERE castle_id=? and clan_id=?");
			statement.setInt(1, getCastle().getCastleId());
	...
	public void removeSiegeClan(L2Clan clan)
	{
		if (clan == null || clan.getHasCastle() == getCastle().getCastleId()
				|| !SiegeManager.getInstance().checkIsRegistered(clan,
	...
	public void startAutoTask()
	{
		...
		_SiegeRegistrationEndDate
				.setTimeInMillis(getCastle().getSiegeDate().getTimeInMillis());
		_SiegeRegistrationEndDate.add(Calendar.DAY_OF_MONTH, -1);

		// Schedule siege auto start
		ThreadPoolManager.getInstance().scheduleGeneral(
				new Siege.ScheduleStartSiegeTask(getCastle()), 1000);
	}
	...
	private void addDefenderWaiting(int clanId)
	{
		getDefenderWaitingClans()
				.add(new L2SiegeClan(clanId, SiegeClanType.DEFENDER_PENDING)); // Add
																				// registered
																				// defender
																				// to
																				// defender
	...
	private boolean checkIfCanRegister(L2PcInstance player)
	{
		if (getIsRegistrationOver())
		...
		else if (player.getClan() == null || player.getClan()
				.getLevel() < SiegeManager.getInstance().getSiegeClanMinLevel())
			...
			player.sendMessage(
					"You cannot register because your clan already own a castle.");
		else if (player.getClan().getClanId() == getCastle().getOwnerId())
			player.sendPacket(new SystemMessage(
					SystemMessage.CLAN_THAT_OWNS_CASTLE_IS_AUTOMATICALLY_REGISTERED_DEFENDING));
	...
	private void correctSiegeDateTime()
	{
		...
		if (getCastle().getSiegeDate().getTimeInMillis() < Calendar
		...
		if (getCastle().getSiegeDate().get(Calendar.DAY_OF_WEEK) != getCastle()
				.getSiegeDayOfWeek())
		{
			corrected = true;
			getCastle().getSiegeDate().set(Calendar.DAY_OF_WEEK,
					getCastle().getSiegeDayOfWeek());
		}

		if (getCastle().getSiegeDate().get(Calendar.HOUR_OF_DAY) != getCastle()
				.getSiegeHourOfDay())
		{
			corrected = true;
			getCastle().getSiegeDate().set(Calendar.HOUR_OF_DAY,
					getCastle().getSiegeHourOfDay());
		}
	...
	private void loadCastleSiege()
	{
		try
		{
			...
			_ControlTowerSpawnList[0] = parseCoords(siegeSettings.getProperty(
					getCastle().getName() + "ControlTower1", "0,0,0"));
			_ControlTowerSpawnList[1] = parseCoords(siegeSettings.getProperty(
					getCastle().getName() + "ControlTower2", "0,0,0"));
			_ControlTowerSpawnList[2] = parseCoords(siegeSettings.getProperty(
					getCastle().getName() + "ControlTower3", "0,0,0"));
			_ControlTowerSpawnList[3] = parseCoords(siegeSettings.getProperty(
					getCastle().getName() + "ControlTower4", "0,0,0"));
		} catch (Exception e)
		{
			System.out
					.println("Exception: loadCastleSiege(): " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void loadSiegeClan()
	{
		...
		try
		{
			...
			// Add castle owner as defender (add owner first so that they are on
			...
			statement = con.prepareStatement(
					"SELECT clan_id,type FROM siege_clans where castle_id=?");
			statement.setInt(1, getCastle().getCastleId());
	...
	private void saveSiegeDate()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"Update castle set siegeDate = ? where id = ?");
	 ...
	 */
	private void saveSiegeClan(L2Clan clan, int typeId,
			boolean isUpdateRegistration)
	{
		...
		try
		{
			...
			{
				if (getDefenderClans().size()
						+ getDefenderWaitingClans().size() >= SiegeManager
								.getInstance().getDefenderMaxClans())
					return;
			} else
			{
				if (getAttackerClans().size() >= SiegeManager.getInstance()
						.getAttackerMaxClans())
					return;
			}
			...
			{
				statement = con.prepareStatement(
						"Update siege_clans set type = ? where castle_id = ? and clan_id = ?");
			...
			if (typeId == 0 || typeId == -1)
			{
				...
				announceToPlayer(
						clan.getName() + " has been registered to defend "
								+ getCastle().getName(),
						false);
			} else if (typeId == 1)
			{
				...
				announceToPlayer(
						clan.getName() + " has been registered to attack "
								+ getCastle().getName(),
						false);
			} else if (typeId == 2)
			{
				addDefenderWaiting(clan.getClanId());
				announceToPlayer(clan.getName() + " has requested to defend "
						+ getCastle().getName(), false);
			}
		} catch (Exception e)
		{
			System.out.println(
					"Exception: saveSiegeClan(L2Clan clan, int typeId, boolean isUpdateRegistration): "
							+ e.getMessage());
			e.printStackTrace();
		} finally
		{
	...
	private void setNextSiegeDate()
	{
				...
				.getInstance().getTimeInMillis())
		{
			...
			getCastle().getSiegeDate().add(Calendar.DAY_OF_MONTH, 14); // Schedule
																		// to
																		// happen
																		// in 14
																		// days
	...
	{
		_Artifact = new L2ArtefactInstance(IdFactory.getInstance().getNextId(),
				NpcTable.getInstance().getTemplate(SiegeManager.getInstance()
						.getArtifactSpawnList()[getCastle().getCastleId()][4]));
		_Artifact.setCurrentHpMp(_Artifact.getMaxHp(), _Artifact.getMaxMp());
		_Artifact.setHeading(SiegeManager.getInstance()
				.getArtifactSpawnList()[getCastle().getCastleId()][3]);
		_Artifact.spawnMe(
				SiegeManager.getInstance().getArtifactSpawnList()[getCastle()
						.getCastleId()][0],
				SiegeManager.getInstance().getArtifactSpawnList()[getCastle()
						.getCastleId()][1],
				SiegeManager.getInstance().getArtifactSpawnList()[getCastle()
	...
	private void spawnControlTower()
	{
		...
		if (_ControlTowers == null
				|| _ControlTowers.size() != _ControlTowerSpawnList.length)
			_ControlTowers = new FastList<>();

		L2ControlTowerInstance ct;
		for (int[] element : _ControlTowerSpawnList)
		{
			StatsSet newStatSet = template.getStatsSet();
			newStatSet.set("baseHpMax", element[4]);
			...
			ct = new L2ControlTowerInstance(IdFactory.getInstance().getNextId(),
					controlTowerTemplate);
			ct.setCurrentHpMp(ct.getMaxHp(), ct.getMaxMp());
			ct.spawnMe(element[0], element[1], element[2] + 20);
	...
	private void spawnSiegeGuard()
	{
		...
		if (getSiegeGuardManager().getSiegeGuardSpawn().size() > 0
				&& _ControlTowers.size() > 0)
		{
	...
	private static int[] parseCoords(String coordString)
	{
		...
		coords[4] = 6000;
		if (st.hasMoreTokens())
			coords[3] = Integer.decode(st.nextToken().trim());
		if (st.hasMoreTokens())
			coords[4] = Integer.decode(st.nextToken().trim());
		return coords;
	}
	...
	public final int getDefenderRespawnDelay()
	{
		return (SiegeManager.getInstance().getDefenderRespawnDelay()
				+ _Defender_RespawnDelay_Penalty);
	}
	...
	public final Zone getZone()
	{
		return ZoneManager.getInstance().getZone(
				ZoneType.getZoneTypeName(
						ZoneType.ZoneTypeEnum.SiegeBattleField),
				getCastle().getName());
	}
