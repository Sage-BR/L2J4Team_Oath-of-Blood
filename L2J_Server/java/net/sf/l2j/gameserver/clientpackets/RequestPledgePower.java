...
public class RequestPledgePower extends ClientBasePacket
{
	...
	void runImpl()
	{
		...
		if (_action == 2)
		{
			if (player.getClan() != null && player.isClanLeader())
			{
				if (_rank == 9)
				{
					...
					// Title management, crest management, master management,
					// level management,
					// bulletin board administration
					// Clan war, right to dismiss, set functions
					// Auction, manage taxes, attack/defend registration,
					// mercenary management
					// => Leaves only CP_CL_VIEW_WAREHOUSE, CP_CH_OPEN_DOOR,
					// CP_CS_OPEN_DOOR?
					_privs = (_privs & L2Clan.CP_CL_VIEW_WAREHOUSE)
							+ (_privs & L2Clan.CP_CH_OPEN_DOOR)
		...
		{
			ManagePledgePower mpp = new ManagePledgePower(
					getClient().getActiveChar().getClan(), _action, _rank);
