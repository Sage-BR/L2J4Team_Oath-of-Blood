...
public class RequestStartPledgeWar extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestStartPledgeWar.class.getName());
	...
	void runImpl()
	{
		...
		if (_clan.getLevel() < 3
				|| _clan.getMembersCount() < Config.ALT_CLAN_MEMBERS_FOR_WAR)
		{
		...
		{
			player.sendMessage(
					"You can't declare war. You are not clan leader.");
			player.sendPacket(new ActionFailed());
			return;
		}
		...
		if (clan == null)
		{
			...
			player.sendPacket(new ActionFailed());
			return;
		} else if (_clan.getAllyId() == clan.getAllyId()
				&& _clan.getAllyId() != 0)
		{
		...
		// else if(clan.getLevel() < 3)
				...
				|| clan.getMembersCount() < Config.ALT_CLAN_MEMBERS_FOR_WAR)
		{
			SystemMessage sm = new SystemMessage(1564);
			player.sendPacket(sm);
			player.sendPacket(new ActionFailed());
		...
		_log.warning("RequestStartPledgeWar, leader: " + clan.getLeaderName()
				+ " clan: " + _clan.getName());

		// L2PcInstance leader =
		// L2World.getInstance().getPlayer(clan.getLeaderName());
		...
		// leader.sendPacket(new
		// StartPledgeWar(_clan.getName(),player.getName()));
		ClanTable.getInstance().storeclanswars(player.getClanId(),
				clan.getClanId());
	}
