...
/**
 * NIO Selector thread. Reads and writes network data. All network messages are
 * encoded and decoded withing this thread. You may send messages using
 * sendMessage() - it push the message into internal queue and return
 * immediatly. A scheduler may read messages using recvMessage(), which returns
 * null when no more messaged left in internal queue.
 *
 * Implementation description: 1. Messages to sent are pushed into outbound
 * queue by external threads. 2. Received messages are pushed into inbound queue
 * by this thread. 3. The loop of Selector is: - try to write mesages from
 * outbound queue - if a message packet is not fully written - store write
 * buffer in the client object - wait for read/write/accept - write data from
 * buffers stored in client objects - read data into a shared read buffer, parse
 * packet and put messages to inbound queue - if a message is not fully received
 * - create a buffer of required size and copy partially read date into it,
 * store this new buffer in the client
 *
 * TODO: Manage thread's priority. The thread has higher then normal priority to
 * handle network transfers quickly. But wakeup of this thread for each small
 * packet may be time-consuming.
 *
 * @author Maxim Kizub
 */
public final class SelectorThread extends Thread
{
	private static Logger _log = Logger
			.getLogger(SelectorThread.class.getName());
	...
	public static Map<Class, Long> packetCount = new FastMap<>();

	public static Map<Class, Long> byteCount = new FastMap<>();

	public static final List<PacketHistory> packetHistory = new FastList<>();

	public static final List<PacketHistory> byteHistory = new FastList<>();

	private static ScheduledFuture packetMonitor = null;

	/**
	 * push counter, currently counts messages in putbound queue, but better it
	 * count size of outbound queue...
	 */
	...
	public static SelectorThread getInstance()
	{
		if (_instance == null)
		{
			_instance = new SelectorThread(Config.GAMESERVER_HOSTNAME,
					Config.PORT_GAME);
		}
		return _instance;
	}
	...
	private SelectorThread(String hostname, int port)
	{
		...
		{
			packetMonitor = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new MonitorPackets(), 10000,
							10000);
		}
	...
	/**
	 * Allocate a write buffer. Take it from a cashed stack, of allocate a new
	 * one, if stack is empty or message is huge.
	 *
	 * @param sz
	...
	private ByteBuffer allocateBuffer(int sz)
	{
		...
		if (sz <= WRITE_BUF_SIZE && numWriteBuffers > 0)
		{
			...
			if (Config.ASSERT)
				assert b.position() == 0;
			if (Config.ASSERT)
				assert b.capacity() == WRITE_BUF_SIZE;
			if (Config.ASSERT)
				assert b.limit() >= sz;
			return b;
		}
	...
	/**
	 * Release a write buffer, store buffers to cashe stack, or just forget it
	...
	private void releaseBuffer(ByteBuffer b)
	{
		...
		if (Config.ASSERT)
			assert b != null;
		if (b.capacity() == WRITE_BUF_SIZE
				&& numWriteBuffers <= WRITE_BUF_HASH_SIZE - 1)
		{
			b.clear();
			if (Config.ASSERT)
				assert b.position() == 0;
			if (Config.ASSERT)
				assert b.limit() == WRITE_BUF_SIZE;
	...
	public void run()
	{
		try
		{
			...
			if ("*".equals(_hostname))
			{
				...
				_log.config("GameServer listening on all available IPs on Port "
						+ _port);
			} else
			{
				isa = new InetSocketAddress(_hostname, _port);
				_log.config("GameServer listening on IP: " + _hostname
						+ " Port " + _port);
			}
			...
			for (;;)
			{
				...
				{
					sleep(30); // sleep some time to prevent thread from eating
								// all cpu time
					continue; // only continue if we don't have any keys to
								// process
				}
				...
				while (it.hasNext())
				{
					...
					try
					{
						...
						default:
							System.err
									.println("Impossible readyOps=" + readyOps);
	...
	/**
	 * Write data to chanell. Writes only buffers stored in clients, i.e. data
	 * that was not written completly. If everything is written - disable
	 * intrest on writing and release write buffer to cache of buffers.
	 *
	 * @param sk
	 */
	private void writeData(SelectionKey sk)
	{
		...
		try
		{
			...
			if (b != null)
			{
				...
				} else if (GameTimeController.getGameTicks()
						- con.writeTimeStamp > 300)
				{
		...
		} catch (IOException e)
		{
			try
			{
				_log.info("Error on network write, player "
						+ con.getClient().getActiveChar().getName()
						+ " disconnected?");
			} catch (NullPointerException npe)
			{
				_log.info(
						"Error on network write, player disconnected? (nullpointer, couldn't get player name)");
			}
	...
	/**
	 * Read data from chanell. If there is an unfinished message (a read buffer
	 * stored on client), then read it. Otherwice read into a shared read
	 * buffer. If there is enough data - parses it and creates a message.
	 *
	 * If only one byte of a new message available - store it into client's
	 * object. If there are two or more bytes - then we know the size of
	 * message, and can allocate a read buffer for it, copy data and store to
	 * client's object.
	 *
	 * @param sk
	 */
	private void readData(SelectionKey sk)
	{
		...
		} catch (IOException e)
		{
			try
			{
				...
				_log.info("Error on network read, player "
						+ con.getClient().getActiveChar().getName()
						+ " disconnected?");
			} catch (NullPointerException npe)
			{
				_log.info(
						"Error on network read, player disconnected? (nullpointer, couldn't get player name)");
			}
	...
	/**
	 * Parse received packet, push a message into inbound queue, to be retrieved
	 * by scheduler.
	 *
	 * @param con
	 *            connection in which we received data
	 * @param buf
	 *            buffer with packet
	 * @param sz
	 *            size of the packet
	...
	private void parse(Connection con, ByteBuffer buf) throws Throwable
	{
		...
		{
			_log.log(Level.SEVERE, "Error on parsing input from client: "
					+ con._client.getLoginName(), e);
			releaseBuffer(buf);
			con.readBuffer = null;
			throw e;
		} catch (Throwable t)
	...
	public void addReceivedPkt(ClientBasePacket pkt)
	{
		...
		// Add a task to one of the pool of thread in function of its priority
		// (HIGH, MEDIUM,LOW)
	...
	/**
	 * Pack (encode) a message into a network buffer. The shared buffer is used.
	 *
	 * @param msg
	 *            message to pack
	...
	{
		if (Config.ASSERT)
			assert msg.getConnection().writeBuffer == null;
		...
		if (Config.ASSERT)
			assert sharedWriteBuffer.position() == 0;
		if (Config.ASSERT)
			assert sharedWriteBuffer.limit() >= 3;
		return true;
	}
	...
	/**
	 * Send a message to client. The message is placed into message queue, and
	...
	/**
	 * Receives incoming messages. Must be called by scheduler, which will read
	 * messages and dispatch their execution to worker threads.
	 *
	 * @return
	 */
	...
	/**
	 * Scan outbound queue. If a client's channel if free (the client has no
	 * write buffer attached, with unfinished message), then remove a message
	 * from queue, encode it into network packet and send. If message was not
	 * sent completly - store write buffer in client's object, and set write
	 * interest for Selector.
	 *
	 * @param selector
	 */
	private void processOutboudQueue()
	{
		...
		while (iter.hasNext())
		{
			...
			if (msg.getLifeTime() > 0)
				if (System.currentTimeMillis() - msg.getTime() > msg
						.getLifeTime())
				{
					iter.remove();
					continue;
			...
			if (Config.ASSERT)
				assert (sk.interestOps() & SelectionKey.OP_READ) != 0;
			if (con.writeBuffer != null)
			{
				if (Config.ASSERT)
					assert (sk.interestOps() & SelectionKey.OP_WRITE) != 0;
			...
			if (sharedWriteBuffer.hasRemaining())
			{
				...
				{
					return; // selection key invalit, what should we do ? just
							// return ?
				}
			} else
			{
	...
	/**
	 * A helper method to close client if connection was closed or error
	 * occured. Frees all buffers, close connections and so on.
	 *
	 * @param c
	 *            a client object
	 */
	protected void closeClient(Connection c, boolean Forced)
	{
		...
		if (player != null)
		{
			...
			if (!Forced)
			{
				...
				if (AttackStanceTaskManager.getInstance()
						.getAttackStanceTask(player))
				{
					c.onlyClose();
					return;
				}
		...
		if (player != null)
		{
					...
					.getAttackStanceTask(player))
			{
				...
				ThreadPoolManager.getInstance()
						.scheduleGeneral(new DisconnectionTask(c), 15000);
				return;
			}
		}
		try
	...
	public static final void startPacketMonitor()
	{
		if (Config.COUNT_PACKETS && packetMonitor == null)
		{
			...
			packetMonitor = ThreadPoolManager.getInstance()
					...
					}, Config.DUMP_INTERVAL_SECONDS * 1000,
							Config.DUMP_INTERVAL_SECONDS * 1000);
		}
	}
	...
	private static final void doPacketHistoryDump()
	{
		...
		try
		{
			synchronized (packetHistory)
			{
				...
				ow.setOutput(new FileOutputStream(
						"log/packetCount_" + currentTimeMillis + ".xml"));
				ow.write(packetHistory);
				ow.setOutput(new FileOutputStream(
						"log/packetBytes_" + currentTimeMillis + ".xml"));
	...
	protected static final void doUpdateHistory()
	{
		synchronized (packetCount)
		{
			...
			packetCount = new FastMap<>();
			byteCount = new FastMap<>();
		}
	}
