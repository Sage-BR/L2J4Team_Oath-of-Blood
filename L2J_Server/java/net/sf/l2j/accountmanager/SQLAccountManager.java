...
public class SQLAccountManager
{
			...
			throws SQLException, IOException, NoSuchAlgorithmException
	{
		...
		LineNumberReader _in = new LineNumberReader(
	...
	private static void printAccInfo() throws SQLException
	{
		...
		PreparedStatement statement = con.prepareStatement(
				"SELECT login, access_level FROM accounts ORDER BY login ASC");
	...
	private static void AddOrUpdateAccount(String account, String password,
			String level)
			throws IOException, SQLException, NoSuchAlgorithmException
	{
		...
		con = L2DatabaseFactory.getInstance().getConnection();
		PreparedStatement statement = con.prepareStatement(
				"REPLACE	accounts (login, password, access_level) VALUES (?,?,?)");
	...
	private static void DeleteAccount(String account) throws SQLException
	{
		...
		} else if (rset.getInt(1) > 0)
		{
			...
			ResultSet rcln;
			statement = con.prepareStatement(
					"SELECT obj_Id, char_name, clanid FROM characters WHERE account_name=?;");
			statement.setEscapeProcessing(true);
			statement.setString(1, account);
			...
			while (rset.next())
			{
				...
				statement = con.prepareStatement(
						"SELECT COUNT(*) FROM clan_data WHERE leader_id=?;");
				...
				if (rcln.getInt(1) > 0)
				{
					...
					// Get Clan Name
					statement = con.prepareStatement(
							"SELECT clan_name FROM clan_data WHERE leader_id=?;");
					statement.setString(1, rset.getString("clanid"));
					rcln = statement.executeQuery();
					rcln.next();

					System.out.println("Deleting clan "
							+ rcln.getString("clan_name") + ".");
					...
					statement = con.prepareStatement(
							"UPDATE characters SET clanid=0 WHERE clanid=?;");
					statement.setString(1, rset.getString("clanid"));
					statement.executeUpdate();

					// Delete Clan
					statement = con.prepareStatement(
							"DELETE FROM clan_data WHERE clan_id=?;");
					statement.setString(1, rset.getString("clanid"));
					statement.executeUpdate();

					statement = con.prepareStatement(
							"DELETE FROM clan_privs WHERE clan_id=?;");
					statement.setString(1, rset.getString("clanid"));
					statement.executeUpdate();
				...
				// friends
				statement = con.prepareStatement(
						"DELETE FROM character_friends WHERE char_id=?;");
				statement.setString(1, rset.getString("obj_Id"));
				statement.executeUpdate();
				...
				// boxaccess
				statement = con.prepareStatement(
						"DELETE FROM boxaccess WHERE charname=?;");
				statement.setString(1, rset.getString("char_name"));
				statement.executeUpdate();

				// characters
				statement = con.prepareStatement(
						"DELETE FROM characters WHERE obj_Id=?;");
				statement.setString(1, rset.getString("obj_Id"));
				statement.executeUpdate();
			...
			// Delete Account
			statement = con
					.prepareStatement("DELETE FROM accounts WHERE login=?;");
			statement.setEscapeProcessing(true);
			statement.setString(1, account);
			statement.executeUpdate();
