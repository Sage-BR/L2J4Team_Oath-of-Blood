...
public class L2FolkInstance extends L2NpcInstance
{
	...
	// Logger.getLogger(L2FolkInstance.class.getName());
	...
	public void showSkillList(L2PcInstance player, ClassId classId)
	{
		...
		L2SkillLearn[] skills = SkillTreeTable.getInstance()
				.getAvailableSkills(player, classId);
		AquireSkillList asl = new AquireSkillList(
				AquireSkillList.skillType.Usual);
		int counts = 0;
		...
		{
			NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			int minlevel = SkillTreeTable.getInstance()
					.getMinLevelForNewSkill(player, classId);

			if (minlevel > 0)
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.DO_NOT_HAVE_FURTHER_SKILLS_TO_LEARN);
	...
	public void showEnchantSkillList(L2PcInstance player, ClassId classId)
	{
		...
		if (counts == 0)
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.THERE_IS_NO_SKILL_THAT_ENABLES_ENCHANT));
			NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		if (command.startsWith("SkillList"))
		{
			if (Config.ALT_GAME_SKILL_LEARN)
			{
				...
				if (id.length() != 0)
				{
					player.setSkillLearningClassId(
							ClassId.values()[Integer.parseInt(id)]);
				...
				} else
				{
					...
					if (!own_class)
					{
						String mages = player.getClassId().isMage() ? "fighters"
								: "mages";
						text += "Skills of your class are the easiest to learn.<br>\n"
								...
								+ "You can also learn skills of " + mages
								+ ", and they are"
					...
					if (_classesToTeach != null)
					{
						...
						while ((count == 0) && (classCheck != null))
						{
							for (ClassId cid : _classesToTeach)
							{
								...
								text += "<a action=\"bypass -h npc_%objectId%_SkillList "
										+ cid.getId() + "\">Learn " + cid
										+ "'s class Skills</a><br>\n";
								count++;
							}
