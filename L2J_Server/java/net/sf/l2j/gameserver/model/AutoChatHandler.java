...
public class AutoChatHandler implements SpawnListener
{
	protected static Logger _log = Logger
			.getLogger(AutoChatHandler.class.getName());
	...
	private void restoreChatData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"SELECT * FROM auto_chat ORDER BY groupId ASC");
			...
			while (rs.next())
			{
				...
				statement2 = con.prepareStatement(
						"SELECT * FROM auto_chat_text WHERE groupId=?");
				...
				registerGlobalChat(rs.getInt("npcId"), chatTexts,
						rs.getLong("chatDelay"));
			...
			if (Config.DEBUG)
				_log.config("AutoChatHandler: Loaded " + numLoaded
						+ " chat group(s) from the database.");
		} catch (Exception e)
		{
	 ...
	 */
	public AutoChatInstance registerGlobalChat(int npcId, String[] chatTexts,
			long chatDelay)
	{
	 ...
	 * @return AutoChatInstance chatInst
	 */
	public AutoChatInstance registerChat(L2NpcInstance npcInst,
			String[] chatTexts, long chatDelay)
	{
		return registerChat(npcInst.getNpcId(), npcInst, chatTexts, chatDelay);
	}

	private final AutoChatInstance registerChat(int npcId,
			L2NpcInstance npcInst, String[] chatTexts, long chatDelay)
	{
		...
		if (_registeredChats.containsKey(npcId))
			...
			chatInst = new AutoChatInstance(npcId, chatTexts, chatDelay,
					(npcInst == null));
	...
	public boolean removeChat(AutoChatInstance chatInst)
	{
		...
		if (Config.DEBUG)
			_log.config("AutoChatHandler: Removed auto chat for NPC ID "
					+ chatInst.getNPCId());
	...
	public class AutoChatInstance
	{
		...
		protected AutoChatInstance(int npcId, String[] chatTexts,
				long chatDelay, boolean isGlobal)
		{
			...
			if (Config.DEBUG)
				_log.config("AutoChatHandler: Registered auto chat for NPC ID "
						+ _npcId + " (Global Chat = " + _globalChat + ").");
		...
		{
			return _chatDefinitions.values().toArray(
					new AutoChatDefinition[_chatDefinitions.values().size()]);
		}
		 ...
		 * @return int objectId
		 */
		public int addChatDefinition(L2NpcInstance npcInst, String[] chatTexts,
				long chatDelay)
		{
			int objectId = npcInst.getObjectId();
			AutoChatDefinition chatDef = new AutoChatDefinition(this, npcInst,
					chatTexts, chatDelay);
		...
		public void setActive(boolean activeValue)
		{
			...
			if (!isGlobal())
			{
				for (AutoChatDefinition chatDefinition : _chatDefinitions
						.values())
			...
			if (isActive())
			{
				...
				_chatTask = ThreadPoolManager.getInstance()
						.scheduleGeneralAtFixedRate(acr, _defaultDelay,
								_defaultDelay);
			} else
			{
		...
		private class AutoChatDefinition
		{
			...
			protected AutoChatDefinition(AutoChatInstance chatInst,
					L2NpcInstance npcInst, String[] chatTexts, long chatDelay)
			{
				...
				if (Config.DEBUG)
					_log.info(
							"AutoChatHandler: Chat definition added for NPC ID "
									+ _npcInstance.getNpcId() + " (Object ID = "
			...
			void setActive(boolean activeValue)
			{
				...
				if (activeValue)
				{
					AutoChatRunner acr = new AutoChatRunner(_npcId,
							_npcInstance.getObjectId());
					_chatTask = ThreadPoolManager.getInstance()
							.scheduleGeneralAtFixedRate(acr, getChatDelay(),
									getChatDelay());
				} else
				{
					_chatTask.cancel(false);
		...
		private class AutoChatRunner implements Runnable
		{
			...
			public synchronized void run()
			{
				...
				{
					AutoChatDefinition chatDef = chatInst
							.getChatDefinition(objectId);

					if (chatDef == null)
					{
						_log.warning(
								"AutoChatHandler: Auto chat definition is NULL for NPC ID "
										+ _npcId + ".");
						return;
					}
				...
				if (Config.DEBUG)
					_log.info("AutoChatHandler: Running auto chat for "
							+ chatDefinitions.length + " instances of NPC ID "
							+ _npcId + "." + " (Global Chat = "
							+ chatInst.isGlobal() + ")");
				...
				for (AutoChatDefinition chatDef : chatDefinitions)
				{
					try
					{
						...
						for (L2Character player : chatNpc.getKnownList()
								.getKnownCharactersInRadius(1500))
						{
						...
						{
							int randomPlayerIndex = rnd
									.nextInt(nearbyPlayers.size());

							L2PcInstance randomPlayer = nearbyPlayers
									.get(randomPlayerIndex);

							final int winningCabal = SevenSigns.getInstance()
							...
							if (text.indexOf("%player_random%") > -1)
								text = text.replaceAll("%player_random%",
							...
							if (text.indexOf("%player_cabal_winner%") > -1)
							{
								...
								{
									if (SevenSigns.getInstance().getPlayerCabal(
											nearbyPlayer) == winningCabal)
									{
						...
						if ((text == null)
								|| text.contains("%player_cabal_loser%")
								|| text.contains("%player_cabal_winner%")
								|| text.contains("%player_random%"))
							return;

						CreatureSay cs = new CreatureSay(chatNpc.getObjectId(),
								0, creatureName, text);
						...
						if (Config.DEBUG)
							_log.fine(
									"AutoChatHandler: Chat propogation for object ID "
											+ chatNpc.getObjectId() + " ("
											+ creatureName + ") with text '"
											+ text + "' sent to "
											+ nearbyPlayers.size()
											+ " nearby players.");
					} catch (Exception e)
