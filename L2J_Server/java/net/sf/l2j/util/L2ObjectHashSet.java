...
/**
 * This class is a highly optimized hashtable, where keys are integers. The main
 * goal of this class is to allow concurent read/iterate and write access to
 * this table, plus minimal used memory.
 *
 * This class uses plain array as the table of values, and keys are used to get
 * position in the table. If the position is already busy, we iterate to the
 * next position, unil we find the needed element or null.
 *
 * To iterate over the table (read access) we may simply iterate throgh table
 * array.
 *
 * In case we remove an element from the table, we check - if the next position
 * is null, we reset table's slot to null, otherwice we assign it to a dummy
 * value
 *
 *
 * @author mkizub
		...
		implements Iterable<T>
{
	...
	private final static int[] primes =
		{ 5, 7, 11, 17, 23, 29, 37, 47, 59, 71, 89, 107, 131, 163, 197, 239,
		...
		do
		{
			...
			if (table[pos] == null)
			{
				...
				if ((collisions[pos >> 5] & (1 << (pos & 31))) == 0)
				{
					...
					count++;
					if (TRACE)
						System.err.println("ht: put obj id=" + hashcode
								+ " at slot=" + slot);
					if (DEBUG)
						check();
					return;
	...
	public synchronized void remove(T obj)
	{
		...
		do
		{
			...
			if (table[pos] == obj)
			{
				...
				count--;
				if (TRACE)
					System.err.println("ht: remove obj id=" + hashcode
							+ " from slot=" + pos);
				if (DEBUG)
					check();
				return;
		...
		next_entry: for (int i = 0; i < table.length; i++)
		{
			...
			do
			{
				...
				if (newTable[pos] == null)
				{
					...
					newTable[pos] = obj;
					if (TRACE)
						System.err.println("ht: move obj id=" + hashcode
								+ " from slot=" + i + " to slot=" + pos);
					continue next_entry;
				}
				// set collision bit
