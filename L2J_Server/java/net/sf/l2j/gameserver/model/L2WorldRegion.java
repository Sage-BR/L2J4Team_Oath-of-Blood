...
public final class L2WorldRegion
{
	private static Logger _log = Logger
			.getLogger(L2WorldRegion.class.getName());

	/**
	 * L2ObjectHashSet(L2PlayableInstance) containing L2PlayableInstance of all
	 * player & summon in game in this L2WorldRegion
	 */
	private L2ObjectSet<L2PlayableInstance> _allPlayable;

	/**
	 * L2ObjectHashSet(L2Object) containing L2Object visible in this
	 * L2WorldRegion
	 */
	...
	{
		_allPlayable = L2ObjectSet.createL2PlayerSet(); // new
														// L2ObjectHashSet<L2PcInstance>();
		_visibleObjects = L2ObjectSet.createL2ObjectSet(); // new
															// L2ObjectHashSet<L2Object>();
		...
		// default a newly initialized region to inactive, unless always on is
		// specified
		...
		if (!isOn)
		{
			for (L2Object o : _visibleObjects)
				if (o instanceof L2Attackable)
				{
					...
					// Stop all active skills effects in progress on the
					// L2Character
					mob.stopAllEffects();
					...
					mob.getAI().setIntention(
							net.sf.l2j.gameserver.ai.CtrlIntention.AI_INTENTION_IDLE);

					// stop the ai tasks
					((L2AttackableAI) mob.getAI()).stopAITask();

					// Stop HP/MP/CP Regeneration task
					// try this: allow regen, but only until mob is 100%
					// full...then stop
	...
	/**
	 * Immediately sets self as active and starts a timer to set neighbors as
	 * active this timer is to avoid turning on neighbors in the case when a
	 * person just teleported into a region and then teleported out
	 * immediately...there is no reason to activate all the neighbors in that
	 * case.
	 */
	private void startActivation()
	{
		...
		_neighborsTask = ThreadPoolManager.getInstance().scheduleGeneral(
				new NeighborsTask(true),
				1000 * Config.GRID_NEIGHBOR_TURNON_TIME);
	}

	/**
	 * starts a timer to set neighbors (including self) as inactive this timer
	 * is to avoid turning off neighbors in the case when a person just moved
	 * out of a region that he may very soon return to. There is no reason to
	 * turn self & neighbors off in that case.
	 */
	private void startDeactivation()
	{
		...
		// suggest means: first check if a neighbor has L2PcInstances in it. If
		// not, deactivate.
		_neighborsTask = ThreadPoolManager.getInstance().scheduleGeneral(
				new NeighborsTask(false),
				1000 * Config.GRID_NEIGHBOR_TURNOFF_TIME);
	}

	/**
	 * Add the L2Object in the L2ObjectHashSet(L2Object) _visibleObjects
	 * containing L2Object visible in this L2WorldRegion <BR>
	 * If L2Object is a L2PcInstance, Add the L2PcInstance in the
	 * L2ObjectHashSet(L2PcInstance) _allPlayable containing L2PcInstance of all
	...
	/**
	 * Remove the L2Object from the L2ObjectHashSet(L2Object) _visibleObjects in
	 * this L2WorldRegion <BR>
	 * <BR>
	 *
	 * If L2Object is a L2PcInstance, remove it from the
	 * L2ObjectHashSet(L2PcInstance) _allPlayable of this L2WorldRegion <BR>
	 * Assert : object.getCurrentWorldRegion() == this ||
	 * object.getCurrentWorldRegion() == null
	 */
	public void removeVisibleObject(L2Object object)
	{
		if (Config.ASSERT)
			assert object.getWorldRegion() == this
					|| object.getWorldRegion() == null;
	...
	/**
	 * Return the FastList _surroundingRegions containing all L2WorldRegion
	 * around the current L2WorldRegion
	 */
	public List<L2WorldRegion> getSurroundingRegions()
	{
		// change to return L2WorldRegion[] ?
		// this should not change after initialization, so maybe changes are not
		// necessary
