...
public class Castle
{
	...
	// =========================================================
	// Data Field
	private List<CropProcure> _procure = new FastList<>();

	private List<SeedProduction> _production = new FastList<>();
	...
	public void addToTreasury(int amount)
	{
		...
		if (_Name.equalsIgnoreCase("Schuttgart")
		...
		if (!_Name.equalsIgnoreCase("aden") && !_Name.equalsIgnoreCase("Rune")
				...
				&& !_Name.equalsIgnoreCase("Goddard")) // If current castle
														...
														// Schuttgart.
		{
			...
			if (aden != null)
			{
				int adenTax = (int) (amount * aden.getTaxRate()); // Find out
																	// what Aden
																	// gets from
																	// the
																	// current
				...
				if (aden.getOwnerId() > 0)
					aden.addToTreasury(adenTax); // Only bother to really add
													// the tax to the treasury
													// if not npc owned

				amount -= adenTax; // Subtract Aden's income from current castle
									// instance's income
			}
		}

		_Treasury += amount; // Add to the current treasury total. Use "-" to
								// substract from treasury
	...
	public void setOwner(L2Clan clan)
	{
				...
				&& (clan == null || clan.getClanId() != getOwnerId()))
		{
			...
			if (oldOwner != null)
			{
				...
				new Announcements().announceToAll(oldOwner.getName()
						+ " has lost " + getName() + " castle!");
			}
		}
	...
	public void setTaxPercent(L2PcInstance activeChar, int taxPercent)
	{
		...
		activeChar.sendMessage(
				getName() + " castle tax changed to " + taxPercent + "%.");
	}
	...
	private void load()
	{
		...
		try
		{
			...
			statement = con
					.prepareStatement("Select * from castle where id = ?");
			...
			statement = con.prepareStatement(
					"Select clan_id from clan_data where hasCastle = ?");
			statement.setInt(1, getCastleId());
			rs = statement.executeQuery();
			...
			if (getOwnerId() > 0)
			{
				...
				ThreadPoolManager.getInstance()
						.scheduleGeneral(new CastleUpdater(clan, 1), 3600000); // Schedule
																				// owner
																				// tasks
																				// to
																				// start
		...
		{
			System.out
					.println("Exception: loadCastleData(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	private void loadDoor()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"Select * from castle_door where castleId = ?");
			statement.setInt(1, getCastleId());
			...
			while (rs.next())
			{
				...
				_DoorDefault.add(rs.getString("name") + ";" + rs.getInt("id")
						...
						+ rs.getInt("z") + ";" + rs.getInt("hp") + ";"
						+ rs.getInt("pDef") + ";" + rs.getInt("mDef"));

				L2DoorInstance door = DoorTable
						.parseList(_DoorDefault.get(_DoorDefault.size() - 1));
				door.spawnMe(door.getX(), door.getY(), door.getZ());
				_Doors.add(door);
			}
		...
		{
			System.out
					.println("Exception: loadCastleDoor(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	private void loadDoorUpgrade()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"Select * from castle_doorupgrade where doorId in (Select Id from castle_door where castleId = ?)");
			statement.setInt(1, getCastleId());
			ResultSet rs = statement.executeQuery();

			while (rs.next())
			{
				upgradeDoor(rs.getInt("id"), rs.getInt("hp"), rs.getInt("pDef"),
						rs.getInt("mDef"));
			}
		...
		{
			System.out.println(
					"Exception: loadCastleDoorUpgrade(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	private void removeDoorUpgrade()
	{
		...
		{
			System.out.println(
					"Exception: removeDoorUpgrade(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	private void saveDoorUpgrade(int doorId, int hp, int pDef, int mDef)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"INSERT INTO castle_doorupgrade (doorId, hp, pDef, mDef) values (?,?,?,?)");
		...
		{
			System.out.println(
					"Exception: saveDoorUpgrade(int doorId, int hp, int pDef, int mDef): "
							+ e.getMessage());
			e.printStackTrace();
		} finally
		{
	...
	private void updateOwnerInDB(L2Clan clan)
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"UPDATE clan_data SET hasCastle=0 WHERE hasCastle=?");
			statement.setInt(1, getCastleId());
			statement.execute();
			statement.close();

			statement = con.prepareStatement(
					"UPDATE clan_data SET hasCastle=? WHERE clan_id=?");
			statement.setInt(1, getCastleId());
			...
			if (clan != null)
			{
				clan.setHasCastle(getCastleId()); // Set has castle flag for new
													// owner
				new Announcements().announceToAll(clan.getName() + " has taken "
						+ getName() + " castle!");
	...
	public final Zone getZone()
	{
		if (_Zone == null)
			_Zone = ZoneManager.getInstance().getZone(
					ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.CastleArea),
					getName());
		return _Zone;
	}
	...
	public final List<Zone> getZoneTowns()
	{
		if (_ZoneTown == null)
		{
			_ZoneTown = new FastList<>();
			// Add towns that belong to castle
			for (Zone zone : ZoneManager.getInstance().getZones(
					ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Town)))
				if (zone != null && zone.getTaxById() == getCastleId())
					_ZoneTown.add(zone);
		}
		return _ZoneTown;
	}
	...
	public void restoreManorData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"SELECT * FROM castle_manor_procure WHERE castleId=?");
			statement.setInt(1, getCastleId());
			rs = statement.executeQuery();

			while (rs.next())
			{
				_procure.add(new CropProcure(rs.getInt("cropId"),
						rs.getInt("canBuy"), rs.getInt("rewardType")));
			}
			...
			statement = con.prepareStatement(
					"SELECT * FROM castle_manor_production WHERE castleId=?");
			statement.setInt(1, getCastleId());
			rs = statement.executeQuery();

			while (rs.next())
			{
				_production.add(new SeedProduction(rs.getInt("seedId"),
						rs.getInt("canProduce")));
			}
		...
		{
			System.out.println(
					"Error restoring manor procure data: " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	public void updateClansReputation()
	{
		if (_formerOwner != null)
		{
			if (_formerOwner != ClanTable.getInstance().getClan(getOwnerId()))
				_formerOwner.setReputationScore(
						_formerOwner.getReputationScore() - 2000, true);
			else
			{
				_formerOwner.setReputationScore(
						_formerOwner.getReputationScore() + 500, true);
			}
		} else
		{
