...
public class L2MonsterInstance extends L2Attackable
{
	...
	public final MonsterKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof MonsterKnownList))
	...
	public void callMinions()
	{
		if (minionList.hasMinions())
		{
			for (L2MinionInstance minion : minionList.getSpawnedMinions())
			{
				...
				{
					// Get the coords of the master to use as a base to move the
					// minion to
					int masterX = getX();
					int masterY = getY();
					int masterZ = getZ();

					// Calculate a new random coord for the minion based on the
					// master's coord
					int minionX = masterX + (Rnd.nextInt(401) - 200);
					int minionY = masterY + (Rnd.nextInt(401) - 200);
					int minionZ = masterZ;
					while (((minionX != (masterX + 30))
							&& (minionX != (masterX - 30)))
							|| ((minionY != (masterY + 30))
									&& (minionY != (masterY - 30))))
					{
					...
					if (!minion.isInCombat() && !minion.isDead()
							&& !minion.isMovementDisabled())
					{
	...
	public void callMinionsToAssist(L2Character attacker)
	{
		if (minionList.hasMinions())
		{
			List<L2MinionInstance> spawnedMinions = minionList
					.getSpawnedMinions();
