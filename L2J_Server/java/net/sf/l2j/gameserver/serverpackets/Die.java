...
/**
 * sample 0b 952a1048 objectId 00000000 00000000 00000000 00000000 00000000
 * 00000000
 *
 * format dddddd rev 377 format ddddddd rev 417
 *
	...
	final void writeImpl()
	{
		...
		if (_clan != null)
		{
			...
			if (castle != null && castle.getSiege().getIsInProgress())
			{
				...
				if (siegeClan == null
						&& castle.getSiege().checkIsDefender(_clan))
				{
					isInDefense = true;
				}
			}

			writeD(_clan.getHasHideout() > 0 ? 0x01 : 0x00); // 6d 01 00 00 00 -
																// to hide away
			writeD(_clan.getHasCastle() > 0 || isInDefense ? 0x01 : 0x00); // 6d
																			// 02
																			// 00
																			// 00
																			// 00
																			...
																			// to
																			// castle
			writeD(siegeClan != null && !isInDefense
					&& siegeClan.getFlag().size() > 0 ? 0x01 : 0x00); // 6d 03
																		// 00 00
																		// 00 -
																		// to
																		// siege
		...
		writeD(_access >= REQUIRED_LEVEL ? 0x01 : 0x00); // 6d 04 00 00 00 - to
															// FIXED
	}
