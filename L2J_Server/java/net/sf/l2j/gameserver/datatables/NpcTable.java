...
public class NpcTable
{
		...
		try
		{
			try
			{
				...
				statement = con.prepareStatement("SELECT " + L2DatabaseFactory
						.getInstance().safetyString(new String[]
						{ "id", "idTemplate", "name", "serverSideName", "title",
								"serverSideTitle", "class", "collision_radius",
								"collision_height", "level", "sex", "type",
								"attackrange", "hp", "mp", "hpreg", "mpreg",
								"str", "con", "dex", "int", "wit", "men", "exp",
								"sp", "patk", "pdef", "matk", "mdef", "atkspd",
								"aggro", "matkspd", "rhand", "lhand", "armor",
								"walkspd", "runspd", "faction_id",
								"faction_range", "isUndead", "absorb_level" })
						+ " FROM npc");
			...
			try
			{
				...
				PreparedStatement statement = con.prepareStatement(
						"SELECT npcid, skillid, level FROM npcskills");
				...
				while (npcskills.next())
				{
					...
					{
					case 4084:
						int resistPAtk = npcDat.getResist(Stats.POWER_DEFENCE)
								- 20;
					...
					case 4276:
						...
						break;
					case 4273:
						int resistDagger = npcDat
								.getResist(Stats.DAGGER_WPN_RES) - 80;
						...
						break;
					case 4274:
						int resistBlunt = npcDat.getResist(Stats.BLUNT_WPN_RES)
								+ 20;
			...
			try
			{
				...
				while (dropData.next())
				{
					...
					if (npcDat == null)
					{
						_log.severe("NPCTable: No npc correlating with id : "
								+ mobId);
						continue;
					}
			...
			try
			{
				...
				while (learndata.next())
				{
					...
					if (npc == null)
					{
						_log.warning("NPCTable: Error getting NPC template ID "
								+ npcId
	...
	private void fillNpcTable(ResultSet NpcData) throws Exception
	{
		while (NpcData.next())
		{
			...
			npcDat.set("serverSideTitle",
					NpcData.getBoolean("serverSideTitle"));
			npcDat.set("collision_radius",
					NpcData.getDouble("collision_radius"));
			npcDat.set("collision_height",
					NpcData.getDouble("collision_height"));
			...
			npcDat.set("baseHpReg",
					NpcData.getFloat("hpreg") > 0 ? NpcData.getFloat("hpreg")
							: 1.5 + ((level - 1) / 10));
			npcDat.set("baseMpReg",
					NpcData.getFloat("mpreg") > 0 ? NpcData.getFloat("mpreg")
							: 0.9 + 0.3 * ((level - 1) / 10));
	...
	public void reloadNpc(int id)
	{
		...
		try
		{
			...
			PreparedStatement st = con.prepareStatement("SELECT "
					...
					{ "id", "idTemplate", "name", "serverSideName", "title",
							"serverSideTitle", "class", "collision_radius",
							"collision_height", "level", "sex", "type",
							"attackrange", "hp", "mp", "hpreg", "mpreg", "str",
							"con", "dex", "int", "wit", "men", "exp", "sp",
							"patk", "pdef", "matk", "mdef", "atkspd", "aggro",
							"matkspd", "rhand", "lhand", "armor", "walkspd",
							"runspd", "faction_id", "faction_range", "isUndead",
							"absorb_level" })
					+ " FROM npc WHERE id=?");
			st.setInt(1, id);
		...
		{
			_log.warning(
					"NPCTable: Could not reload data for NPC " + id + ": " + e);
		} finally
		{
			try
			{
	...
	public void saveNpc(StatsSet npc)
	{
		...
		{
			_log.warning(
					"NPCTable: Could not store new NPC data in database: " + e);
		} finally
		{
			try
			{
