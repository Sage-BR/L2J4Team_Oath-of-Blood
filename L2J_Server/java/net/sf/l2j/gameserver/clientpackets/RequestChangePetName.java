	...
	void runImpl()
	{
		...
		if (pet.getName() != null)
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.NAMING_YOU_CANNOT_SET_NAME_OF_THE_PET));
			return;
		} else if (PetNameTable.getInstance().doesPetNameExist(_name,
				pet.getTemplate().npcId))
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.NAMING_ALREADY_IN_USE_BY_ANOTHER_PET));
			return;
		...
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.NAMING_PETNAME_CONTAINS_INVALID_CHARS));
			return;
		}
