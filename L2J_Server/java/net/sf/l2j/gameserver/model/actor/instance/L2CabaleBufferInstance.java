...
public class L2CabaleBufferInstance extends L2NpcInstance
{
	...
	private class CabalaAI implements Runnable
	{
		...
		public void run()
		{
			...
			/**
			 * For each known player in range, cast either the positive or
			 * negative buff. <BR>
			 * The stats affected depend on the player type, either a fighter or
			 * a mystic. <BR>
			 * <BR>
			 ...
			 * <BR>
			 * <BR>
			 * Blessing of Prophecy (Winner) - Fighters: +25% Max Load, +25%
			...
			for (L2PcInstance player : getKnownList().getKnownPlayers()
					.values())
			{
				final int playerCabal = SevenSigns.getInstance()
		...
		private boolean handleCast(L2PcInstance player, int skillId)
		{
			...
			if (player.isDead() || !player.isVisible()
					|| !isInsideRadius(player, getDistanceToWatchObject(player),
							false, false))
				return false;
			...
			if (player.getEffect(skill) == null)
			{
				...
				broadcastPacket(new MagicSkillUser(_caster, player,
						skill.getId(), skillLevel, skill.getSkillTime(), 0));
	...
	public L2CabaleBufferInstance(int objectId, L2NpcTemplate template)
	{
		...
		aiTask = ThreadPoolManager.getInstance()
				.scheduleAiAtFixedRate(new CabalaAI(this), 3000, 3000);
	}
