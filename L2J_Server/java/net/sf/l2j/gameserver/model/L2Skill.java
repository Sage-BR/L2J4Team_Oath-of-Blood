...
public abstract class L2Skill
{
	...
	{
		// Damage
		PDAM, MDAM, MANADAM, DOT, MDOT, DRAIN_SOUL, DRAIN(
				L2SkillDrain.class), DEATHLINK,

		// Disablers
		BLEED, POISON, STUN, ROOT, CONFUSION, FEAR, SLEEP, CONFUSE_MOB_ONLY, MUTE, PARALYZE, WEAKNESS,

		// hp, mp, cp
		HEAL, HOT, BALANCE_LIFE, HEAL_PERCENT, HEAL_STATIC, COMBATPOINTHEAL, CPHOT, MANAHEAL, MANA_BY_LEVEL, MANAHEAL_PERCENT, MANARECHARGE, MPHOT,
		...
		// MISC
		UNLOCK, ENCHANT_ARMOR, ENCHANT_WEAPON, SOULSHOT, SPIRITSHOT, SIEGEFLAG, TAKECASTLE, WEAPON_SA,

		// Creation
		COMMON_CRAFT, DWARVEN_CRAFT, CREATE_ITEM(
				L2SkillCreateItem.class), SUMMON_TREASURE_KEY,

		// Summons
		SUMMON(L2SkillSummon.class), FEED_PET, DEATHLINK_PET, STRSIEGEASSAULT, ERASE, BETRAY,

		// Cancel
		CANCEL, MAGE_BANE, WARRIOR_BANE, NEGATE,

		BUFF, DEBUFF, PASSIVE, CONT,

		RESURRECT, CHARGE(L2SkillCharge.class), MHOT, CHARGEDAM(
				L2SkillChargeDmg.class), DETECT_WEAKNESS, LUCK, RECALL, SUMMON_FRIEND, REFLECT, SPOIL, SWEEP, FAKE_DEATH, UNBLEED, UNPOISON, UNDEAD_DEFENSE, SEED(
						L2SkillSeed.class),
	...
	protected L2Skill(StatsSet set)
	{
		...
		_magicLevel = set.getInteger("magicLvl",
			...
			while (st.hasMoreTokens())
			{
				...
				} catch (Throwable t)
				{
					_log.log(Level.SEVERE,
							"Bad class " + cls + " to learn skill", t);
				}
			}
		}
		...
		} else
		{
			...
			while (st.hasMoreTokens())
			{
				...
				{
					_log.log(Level.SEVERE,
							"Bad teacher id " + npcid + " to teach skill", t);
				}
			}
		}
	}
	...
	public final double getPower(L2Character activeChar)
	{
		if (_skillType == SkillType.DEATHLINK && activeChar != null)
			return _power * Math.pow(
					1.7165 - activeChar.getCurrentHp() / activeChar.getMaxHp(),
					2) * 0.577;
		else
			return _power;
	}
	...
	public final boolean getWeaponDependancy(L2Character activeChar)
	{
		...
		if (activeChar.getActiveWeaponItem() != null)
		{
			...
			{
				playerWeapon = activeChar.getSecondaryWeaponItem()
						.getItemType();
	...
	public boolean checkCondition(L2Character activeChar, boolean itemOrWeapon)
	{
		...
		if ((getCondition() & L2Skill.COND_SHIELD) != 0)
		{
			 ...
			 * activeChar.getInventory().getPaperdollItem(Inventory.
			 * PAPERDOLL_RHAND); armorPiece = (L2Armor) dummy.getItem();
			 */
	 ...
	 * the caster if it's a L2PcInstance and no L2PcInstance targeted</li>
	 * <li>SELF</li>
	 * <li>HOLY, UNDEAD</li>
	 * <li>PET</li>
			...
			boolean onlyFirst)
	{
		...
		case TARGET_AURA:
		{
			...
			boolean srcInArena = (ArenaManager.getInstance()
					.getArena(activeChar) != null);
					...
					.values())
			{
				...
				{
					// Don't add this target if this is a Pc->Pc pvp casting and
					// pvp condition not met
					if (obj == activeChar || obj == src)
						continue;
					if (src != null)
					{
						if (!GeoData.getInstance().canSeeTarget(activeChar,
								obj))
							continue;
						// check if both attacker and target are L2PcInstances
						// and if they are in same party
						if (obj instanceof L2PcInstance)
						{
							if (!src.checkPvpSkill(obj, this) || ((src
									.getParty() != null
									&& ((L2PcInstance) obj).getParty() != null)
									&& src.getParty()
											.getPartyLeaderOID() == ((L2PcInstance) obj)
													.getParty()
													.getPartyLeaderOID()))
								continue;
							if (!srcInArena && ArenaManager.getInstance()
									.getArena(obj) == null)
							{
								if (src.getClanId() != 0 && src
										.getClanId() == ((L2PcInstance) obj)
												.getClanId())
									continue;
							}
						}
						if (obj instanceof L2Summon)
						{
							...
							if ((trg == src) || !src.checkPvpSkill(trg, this))
								continue;
		...
		case TARGET_AREA:
		{
			if ((!(target instanceof L2Attackable
					|| target instanceof L2PlayableInstance)) || // Target is
																	// not
																	// L2Attackable
																	// or
																	// L2PlayableInstance
					(this.getCastRange() >= 0 && (target == null
							|| target == activeChar || target.isAlikeDead()))) // target
																				// is
																				// null
																				// or
																				// self
					...
					.values())
			{
				...
				if (!target.isAlikeDead() && (target != activeChar))
				{
					...
					{
						if (effectOriginIsL2PlayableInstance && // If effect
																// starts at
																// L2PlayableInstance
																// and
		...
		case TARGET_MULTIFACE:
		{
					...
					.values())
			{
				...
				{
					activeChar.sendPacket(
							new SystemMessage(SystemMessage.TARGET_CANT_FOUND));
					return null;
				}
			}
			return targetList.toArray(new L2Character[targetList.size()]);
			// TODO multiface targets all around right now. need it to just get
			// targets
		...
		case TARGET_PARTY:
		{
			...
			if (activeChar.getParty() != null)
			{
				...
				for (L2PcInstance partyMember : partyList)
				{
					...
					if (!partyMember.isDead() && Util.checkIfInRange(
							getSkillRadius(), activeChar, partyMember, true))
					{
		...
		{
			if ((target != null && target == activeChar) || (target != null
							...
							&& activeChar == target.getPet()))
			{
				if (!target.isDead())
				{
					// If a target is found, return it in a table else send a
					// system message TARGET_IS_INCORRECT
					return new L2Character[]
						{ target };
				} else
					return null;
				...
				if (clan != null)
				{
							...
							.getKnownObjects().values())
					{
						...
						if ((targetType == SkillTargetType.TARGET_CORPSE_ALLY
								...
								|| !Util.checkIfInRange(radius, activeChar,
										newTarget, true)
			...
			if (activeChar instanceof L2PcInstance)
			{
				...
				if (clan != null)
				{
					...
					for (L2ClanMember member : clan.getMembers())
					{
						...
						if ((newTarget == null)
								|| (targetType == SkillTargetType.TARGET_CORPSE_CLAN
										&& !newTarget.isDead())
								|| !Util.checkIfInRange(radius, activeChar,
										newTarget, true))
							continue;
						...
						&& (targetPlayer != null || targetPet != null))
				{
					...
					if (getId() == 1016) // Greater Resurrection
					{
						...
						if (targetPlayer != null)
							castle = CastleManager.getInstance().getCastle(
									targetPlayer.getX(), targetPlayer.getY());
						else if (targetPet != null)
							castle = CastleManager.getInstance().getCastle(
									targetPet.getX(), targetPet.getY());
						...
						if (castle != null)
							if (castle.getSiege().getIsInProgress())
							{
								...
								player.sendPacket(new SystemMessage(
										SystemMessage.CANNOT_BE_RESURRECTED_DURING_SIEGE));
							}
						...
						if (targetPlayer != null)
						{
							if (targetPlayer.isReviveRequested())
							{
								if (targetPlayer.isRevivingPet())
									player.sendPacket(new SystemMessage(1511)); // While
																				// a
																				// pet
																				// is
																				// attempting
									...
									player.sendPacket(new SystemMessage(1513)); // Resurrection
																				// is
																				// already
																				// been
																				// proposed.
					...
					if (condGood)
					{
						if (!onlyFirst)
						{
							targetList.add(target);
							return targetList
									.toArray(new L2Object[targetList.size()]);
						} else
							return new L2Character[]
								{ target };
			...
			if (activeChar.getKnownList() != null)
						...
						.values())
				{
					...
					if (!Util.checkIfInRange(radius, target, obj, true)
							|| !GeoData.getInstance().canSeeTarget(activeChar,
									obj))
						continue;
		...
		case TARGET_AREA_UNDEAD:
		{
			...
			if (cha != null && cha.getKnownList() != null)
						...
						.values())
				{
					...
					if (!target.isAlikeDead()) // If target is not dead/fake
												// death and not self
					{
						if (!target.isUndead()
								|| !Util.checkIfInRange(radius, cha, obj, true)) // Go
																					// to
																					// next
																					// obj
																					// if
						...
						if (!onlyFirst)
							targetList.add((L2Character) obj); // Add obj to
																// target lists
						else
							return new L2Character[]
								{ (L2Character) obj };
		...
		case TARGET_ENEMY_SUMMON:
		{
			if (target != null && target instanceof L2Summon)
			{
				...
				if (activeChar instanceof L2PcInstance
						...
						|| (targetSummon.getOwner().getInPvpZone()
								&& ((L2PcInstance) activeChar).getInPvpZone()))
					return new L2Character[]
						{ targetSummon };
			}
			return null;
	...
	L2Effect effect, L2Character player)
	{
		if ((!(player instanceof L2PcInstance)
				...
				&& !(player instanceof L2Summon)) || (_funcTemplates == null))
			...
			L2Character effected)
	{
		if (isPassive() || (_effectTemplates == null)
				|| ((effector != effected) && effected.isInvul()))
	...
	public final L2Effect[] getEffectsSelf(L2Character effector)
	{
		...
		for (EffectTemplate et : _effectTemplatesSelf)
		{
			...
			if (e != null)
			{
				...
				{
					env._skill = SkillTable.getInstance().getInfo(8,
							effector.getSkillLevel(8));
					EffectCharge effect = (EffectCharge) env._target
							.getEffect(L2Effect.EffectType.CHARGE);
					if (effect != null)
					{
