...
public class AdminCursedWeapons implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_cw_infos", "admin_cw_remove", "admin_cw_goto",
				"admin_cw_reload" };
		...
		if (command.startsWith("admin_cw_infos"))
		{
			...
			for (CursedWeapon cw : cwm.getCursedWeapons())
			{
				activeChar.sendMessage(
						"> " + cw.getName() + " (" + cw.getItemId() + ")");
				if (cw.isActivated())
				{
					L2PcInstance pl = cw.getPlayer();
					activeChar.sendMessage("  Player holding: " + pl.getName());
					activeChar.sendMessage(
							"    Player karma: " + cw.getPlayerKarma());
					activeChar.sendMessage("    Time Remaing: "
							+ (cw.getTimeLeft() / 60000) + " min.");
					activeChar
							.sendMessage("    Kills done: " + cw.getNbKills());
