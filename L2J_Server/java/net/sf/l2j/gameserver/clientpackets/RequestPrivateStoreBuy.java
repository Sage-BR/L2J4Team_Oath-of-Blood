...
//	private static final String _C__79_SENDPRIVATESTOREBUYLIST = "[C] 79 SendPrivateStoreBuyList";
	...
	private static Logger _log = Logger
			.getLogger(RequestPrivateStoreBuy.class.getName());
	...
	void runImpl()
	{
		...
		for (ItemRequest ir : _items)
		{
			...
			{
				String msgErr = "[RequestPrivateStoreBuy] player "
						+ getClient().getActiveChar().getName()
						+ " tried an overflow exploit, ban this player!";
				Util.handleIllegalPlayerAction(getClient().getActiveChar(),
						msgErr, Config.DEFAULT_PUNISH);
				return;
			}
		...
		if (!storeList.PrivateStoreBuy(player, _items, (int) priceTotal))
		{
			...
			_log.warning(
					"PrivateStore buy has failed due to invalid list or request. Player: "
							+ player.getName() + ", Private store of: "
							+ storePlayer.getName());
			return;
		}
		...
		/*
		 * Lease holders are currently not implemented else if (_seller != null)
		 * { // lease shop sell L2MerchantInstance seller =
		 * (L2MerchantInstance)_seller; L2ItemInstance ladena =
		 * seller.getLeaseAdena(); for (TradeItem ti : buyerlist) {
		 * L2ItemInstance li = seller.getLeaseItemByObjectId(ti.getObjectId());
		 * if (li == null) { if (ti.getObjectId() == ladena.getObjectId()) {
		 * buyer.addAdena(ti.getCount());
		 * ladena.setCount(ladena.getCount()-ti.getCount());
		 * ladena.updateDatabase(); } continue; } int cnt = li.getCount(); if
		 * (cnt < ti.getCount()) ti.setCount(cnt); if (ti.getCount() <= 0)
		 * continue; L2ItemInstance inst =
		 * ItemTable.getInstance().createItem(li.getItemId());
		 * inst.setCount(ti.getCount());
		 * inst.setEnchantLevel(li.getEnchantLevel());
		 * buyer.getInventory().addItem(inst);
		 * li.setCount(li.getCount()-ti.getCount()); li.updateDatabase();
