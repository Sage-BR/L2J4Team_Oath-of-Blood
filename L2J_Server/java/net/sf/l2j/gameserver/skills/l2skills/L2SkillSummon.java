...
public class L2SkillSummon extends L2Skill
{
	...
	public boolean checkCondition(L2Character activeChar)
	{
		if (activeChar instanceof L2PcInstance)
		{
			...
			if (isCubic)
			{
				...
				{
					return true; // Player is always able to cast mass cubic
									// skill
				}
	...
	public void useSkill(L2Character caster, L2Object[] targets)
	{
		...
		if (isCubic)
		{
			if (targets.length > 1) // Mass cubic skill
			{
				for (L2Object obj : targets)
				{
					...
					int mastery = player
							.getSkillLevel(L2Skill.SKILL_CUBIC_MASTERY);
					if (mastery < 0)
						mastery = 0;
			...
			} else // normal cubic skill
			{
				...
				if (activeChar.getCubics().size() > mastery)
				{
					if (Config.DEBUG)
						_log.fine(
								"player can't summon any more cubics. ignore summon skill");
					activeChar.sendPacket(new SystemMessage(
							SystemMessage.CUBIC_SUMMONING_FAILED));
					return;
				}
		...
		L2NpcTemplate summonTemplate = NpcTable.getInstance()
				.getTemplate(npcId);
		L2SummonInstance summon = new L2SummonInstance(
				IdFactory.getInstance().getNextId(), summonTemplate, activeChar,
				this);
		...
		{
			summon.getStat()
					.setExp(Experience.LEVEL[Experience.LEVEL.length - 1]);
			_log.warning("Summon (" + summon.getName() + ") NpcID: "
					+ summon.getNpcId()
					+ " has a level above 75. Please rectify.");
		} else
		{
			summon.getStat().setExp(Experience.LEVEL[(summon.getLevel()
		...
		summon.setShowSummonAnimation(false); // addVisibleObject created the
												// info packets with summon
												// animation
												// if someone comes into range
												// now, the animation shouldnt
												// show any more
