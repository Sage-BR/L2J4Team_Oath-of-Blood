...
public class PcKnownList extends PlayableKnownList
{
	...
	// Method - Public
	/**
	 * Add a visible L2Object to L2PcInstance _knownObjects and _knownPlayer (if
	 * necessary) and send Server-Client Packets needed to inform the
	 * L2PcInstance of its state and actions in progress.<BR>
	 * <BR>
	 *
	 * <B><U> object is a L2ItemInstance </U> :</B><BR>
	 * <BR>
	 * <li>Send Server-Client Packet DropItem/SpawnItem to the L2PcInstance
	 * </li><BR>
	 * <BR>
	 *
	 * <B><U> object is a L2DoorInstance </U> :</B><BR>
	 * <BR>
	 * <li>Send Server-Client Packets DoorInfo and DoorStatusUpdate to the
	 * L2PcInstance</li>
	 * <li>Send Server->Client packet MoveToPawn/CharMoveToLocation and
	 ...
	 * <BR>
	 * <li>Send Server-Client Packet NpcInfo/PetItemList (if the L2PcInstance is
	 * the owner) to the L2PcInstance</li>
	 * <li>Send Server->Client packet MoveToPawn/CharMoveToLocation and
	 * AutoAttackStart to the L2PcInstance</li><BR>
	 * <BR>
	 *
	 ...
	 * <li>If the object has a private store, Send Server-Client Packet
	 * PrivateStoreMsgSell to the L2PcInstance</li>
	 * <li>Send Server->Client packet MoveToPawn/CharMoveToLocation and
	 * AutoAttackStart to the L2PcInstance</li><BR>
	 * <BR>
	 *
	...
	public boolean addKnownObject(L2Object object, L2Character dropper)
	{
		...
		} else
		{
			...
			{
				if (dropper != null)
					getActiveChar().sendPacket(new DropItem(
							(L2ItemInstance) object, dropper.getObjectId()));
				else
					getActiveChar()
			...
			} else if (object instanceof L2DoorInstance)
			{
				...
				getActiveChar().sendPacket(
						new DoorStatusUpdate((L2DoorInstance) object));
			} else if (object instanceof L2BoatInstance)
			{
				if (!getActiveChar().isInBoat())
					...
					{
						getActiveChar().sendPacket(
								new VehicleInfo((L2BoatInstance) object));
						((L2BoatInstance) object)
								.sendVehicleDeparture(getActiveChar());
					}
			} else if (object instanceof L2StaticObjectInstance)
			{
				getActiveChar().sendPacket(
						new StaticObject((L2StaticObjectInstance) object));
			} else if (object instanceof L2NpcInstance)
			{
				if (Config.CHECK_KNOWN)
					getActiveChar().sendMessage(
							"Added NPC: " + ((L2NpcInstance) object).getName());
				getActiveChar().sendPacket(
						new NpcInfo((L2NpcInstance) object, getActiveChar()));
			} else if (object instanceof L2Summon)
			{
				...
				if (getActiveChar().equals(summon.getOwner()))
				{
					...
					{
						getActiveChar().sendPacket(
								new PetItemList((L2PetInstance) summon));
					}
				} else
					getActiveChar()
							.sendPacket(new NpcInfo(summon, getActiveChar()));
			} else if (object instanceof L2PcInstance)
			{
				L2PcInstance otherPlayer = (L2PcInstance) object;
				if (otherPlayer.isInBoat())
				{
					otherPlayer.getPosition().setWorldPosition(otherPlayer
							.getBoat().getPosition().getWorldPosition());
					getActiveChar().sendPacket(new CharInfo(otherPlayer));
					getActiveChar().sendPacket(
							new GetOnVehicle(otherPlayer, otherPlayer.getBoat(),
									otherPlayer.getInBoatPosition().getX(),
									otherPlayer.getInBoatPosition().getY(),
									otherPlayer.getInBoatPosition().getZ()));
					/*
					 * if(otherPlayer.getBoat().GetVehicleDeparture() == null) {
					 *
					 * int xboat = otherPlayer.getBoat().getX(); int yboat=
					 * otherPlayer.getBoat().getY(); double modifier =
					 * Math.PI/2; if (yboat == 0) { yboat = 1; } if(yboat < 0) {
					 * modifier = -modifier; } double angleboat = modifier -
					 * Math.atan(xboat/yboat); int xp = otherPlayer.getX(); int
					 * yp = otherPlayer.getY(); modifier = Math.PI/2; if (yp ==
					 * 0) { yboat = 1; } if(yboat < 0) { modifier = -modifier; }
					 * double anglep = modifier - Math.atan(yp/xp);
					 *
					 * double finx = Math.cos(anglep - angleboat)*Math.sqrt(xp
					 * *xp +yp*yp ) + Math.cos(angleboat)*Math.sqrt(xboat *xboat
					 * +yboat*yboat ); double finy = Math.sin(anglep -
					 * angleboat)*Math.sqrt(xp *xp +yp*yp ) +
					 * Math.sin(angleboat)*Math.sqrt(xboat *xboat +yboat*yboat
					 * );
					 * //otherPlayer.getPosition().setWorldPosition(otherPlayer.
					 * getBoat().getX() -
					 * otherPlayer.getInBoatPosition().x,otherPlayer.getBoat().
					 * getY() -
					 * otherPlayer.getInBoatPosition().y,otherPlayer.getBoat().
					 * getZ()- otherPlayer.getInBoatPosition().z);
					 * otherPlayer.getPosition().setWorldPosition((int)finx,(int
					 * )finy,otherPlayer.getBoat().getZ()-
					 * otherPlayer.getInBoatPosition().z);
					 *
					 * }
					 */
				} else
				...
				if (otherPlayer
					...
					getActiveChar()
						...
						.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_BUY)
					getActiveChar()
			...
			{
				// Update the state of the L2Character object client side by
				// sending Server->Client packet MoveToPawn/CharMoveToLocation
				// and AutoAttackStart to the L2PcInstance
	...
	/**
	 * Remove a L2Object from L2PcInstance _knownObjects and _knownPlayer (if
	 * necessary) and send Server-Client Packet DeleteObject to the
	 * L2PcInstance.<BR>
	 * <BR>
	 *
	 * @param object
	...
	public boolean removeKnownObject(L2Object object)
	{
		...
		if (Config.CHECK_KNOWN && object instanceof L2NpcInstance)
			getActiveChar().sendMessage(
					"Removed NPC: " + ((L2NpcInstance) object).getName());
		return true;
	}
