...
public class RequestAutoSoulShot extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestAutoSoulShot.class.getName());
			...
			if (item != null)
			{
				if (_type == 1)
				{
					...
					if (_itemId < 6535 || _itemId > 6540)
					{
						...
						if (_itemId == 6645 || _itemId == 6646
								|| _itemId == 6647)
						{
							activeChar.addAutoSoulShot(_itemId);
							ExAutoSoulShot atk = new ExAutoSoulShot(_itemId,
									_type);
							activeChar.sendPacket(atk);

							// start the auto soulshot use
							SystemMessage sm = new SystemMessage(
									SystemMessage.USE_OF_S1_WILL_BE_AUTO);
						...
						{
							if (activeChar.getActiveWeaponItem() != activeChar
									.getFistsWeaponItem()
									&& item.getItem()
											.getCrystalType() == activeChar
													.getActiveWeaponItem()
													.getCrystalType())
							{
								activeChar.addAutoSoulShot(_itemId);
								...
								activeChar.rechargeAutoSoulShot(true, true,
										false);
							} else
							{
								if ((_itemId >= 2509 && _itemId <= 2514)
									...
									activeChar.sendPacket(new SystemMessage(
											SystemMessage.SPIRITSHOTS_GRADE_MISMATCH));
								else
									activeChar.sendPacket(new SystemMessage(
