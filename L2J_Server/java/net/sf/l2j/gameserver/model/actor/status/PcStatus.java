...
public class PcStatus extends PlayableStatus
{
			...
			boolean awake)
	{
		...
		if (attacker != null && attacker != getActiveChar())
		{
			...
			{
				int tDmg = (int) value
						* (int) getActiveChar().getStat().calcStat(
								Stats.TRANSFER_DAMAGE_PERCENT, 0, null, null)
						/ 100;
				...
				if (tDmg > 0)
				{
					...
					value -= tDmg;
					fullValue = (int) value; // reduce the annouced value here
												// as player will get a message
												// about summon dammage
				}
			}
			...
			if (attacker instanceof L2PlayableInstance)
			{
				...
				{
					setCurrentCp(getCurrentCp() - value); // Set Cp to diff of
															// Cp vs value
					value = 0; // No need to subtract anything from Hp
				} else
				{
					value -= getCurrentCp(); // Get diff from value vs Cp; will
												// apply diff to Hp
