...
public class L2Party
{
	...
	private int _pendingInvitation = 0; // Number of players that already have
										// been invited (but not replied yet)
	...
	// getPartyMembers().get(Rnd.get(getPartyMembers().size())); }
	private L2PcInstance getCheckedRandomMember(int ItemId, L2Character target)
	{
		...
		for (L2PcInstance member : getPartyMembers())
		{
			if (member.getInventory().validateCapacityByItemId(ItemId)
					&& Util.checkIfInRange(1150, target, member, true))
	 ...
	 */
	/*
	 * private L2PcInstance getNextLooter() { _itemLastLoot++; if (_itemLastLoot
	 * > getPartyMembers().size() -1) _itemLastLoot = 0;
	 *
	 * return (getPartyMembers().size() > 0) ?
	 * getPartyMembers().get(_itemLastLoot) : null; }
	 */
	 ...
	 * @return
	 */
	private L2PcInstance getActualLooter(L2PcInstance player, int ItemId,
			boolean spoil, L2Character target)
	{
	 ...
	 * <BR>
	 */
	public void broadcastToPartyMembers(L2PcInstance player,
	...
	public void addPartyMember(L2PcInstance player)
	{
		...
		SystemMessage msg = new SystemMessage(
				SystemMessage.YOU_JOINED_S1_PARTY);
		...
		if (checkMemberValidity(player))
			_validMembers.add(player);
	...
	public void removePartyMember(L2PcInstance player)
	{
		if (getPartyMembers().contains(player))
		{
			...
			if (_validMembers.contains(player))
				_validMembers.remove(player);
			...
			if (getPartyMembers().contains(player))
			{
				if (isLeader(player))
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.YOU_CANNOT_TRANSFER_RIGHTS_TO_YOURSELF));
				} else
				{
					// Swap party members
					L2PcInstance temp;
					...
					SystemMessage msg = new SystemMessage(
							SystemMessage.S1_HAS_BECOME_A_PARTY_LEADER);
					msg.addString(getPartyMembers().get(0).getName());
					broadcastToPartyMembers(msg);
					broadcastToPartyMembers(new PartySmallWindowUpdate(
							getPartyMembers().get(0)));
				}
			} else
			{
				player.sendPacket(new SystemMessage(
						SystemMessage.YOU_CAN_TRANSFER_RIGHTS_ONLY_TO_ANOTHER_PARTY_MEMBER));
			}
		}
	 ...
	 */
	/*
	 * [DEPRECATED] private void dissolveParty() { SystemMessage msg = new
	 * SystemMessage(SystemMessage.PARTY_DISPERSED); for(int i = 0; i <
	 * _members.size(); i++) { L2PcInstance temp = _members.get(i);
	 * temp.sendPacket(msg); temp.sendPacket(new PartySmallWindowDeleteAll());
	 * temp.setParty(null); } }
	 */
	...
	public void distributeItem(L2PcInstance player, L2ItemInstance item)
	{
		...
		L2PcInstance target = getActualLooter(player, item.getItemId(), false,
				player);
		...
		{
			SystemMessage msg = new SystemMessage(
					SystemMessage.S1_PICKED_UP_S2_S3);
	 ...
	 * @param item
	 */
	public void distributeItem(L2PcInstance player,
			L2Attackable.RewardItem item, boolean spoil, L2Attackable target)
	{
		if (item == null)
			return;
		...
		L2PcInstance looter = getActualLooter(player, item.getItemId(), spoil,
				target);

		looter.addItem(spoil ? "Sweep" : "Party", item.getItemId(),
				item.getCount(), player, true);

		// Send messages to other aprty members about reward
		if (item.getCount() > 1)
		{
			SystemMessage msg = spoil
					? new SystemMessage(SystemMessage.S1_SWEEPED_UP_S2_S3)
		...
		{
			SystemMessage msg = spoil
					? new SystemMessage(SystemMessage.S1_SWEEPED_UP_S2)
	 ...
	 * @param adena
	 */
	public void distributeAdena(L2PcInstance player, int adena,
			L2Character target)
	{
		...
		List<L2PcInstance> ToReward = new FastList<>();
		for (L2PcInstance member : membersList)
		{
			if (!Util.checkIfInRange(1150, target, member, true))
				continue;
			ToReward.add(member);
		}
	...
	/**
	 * Distribute Experience and SP rewards to L2PcInstance Party members in the
	 * known area of the last attacker.<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the L2PcInstance owner of the L2SummonInstance (if necessary)
	 * </li>
	 ...
	 *            The list of L2PcInstance to reward and LSummonInstance whose
	 *            owner must be reward
	 * @param lastAttacker
	 *            The L2Character that has killed the L2Attackable
	 *
	 */
	public void distributeXpAndSp(long xpReward, int spReward,
			List<L2Character> rewardedMembers, L2Character lastAttacker)
	{
		...
		List<L2Character> ToRemove = new FastList<>();
		...
		synchronized (rewardedMembers)
		{
			...
			{
				if (ToRemove != null && ToRemove.contains(member))
					continue;

				// Check if members are near the last Attacker
				if ((member instanceof L2PlayableInstance
						&& lastAttacker.getKnownList().knowsObject(member))
						|| lastAttacker.getKnownList()
								.knowsThePlayer((L2PcInstance) member))
				{
					penalty = 0;

					// The reward can only be given to a L2PcInstance that owns
					// the L2SummonInstance
					// The Summon and the L2PcInstance are in the rewarded
					// members table, but it's impossible to know which one is
					// first
					// That's why only one of them must be used (remove the
					// other during the calculation of the first)
					...
					} else if (member instanceof L2PcInstance)
					{
						...
						if (owner.getPet() instanceof L2SummonInstance)
						{
							...
							// Remove the L2SummonInstance from the rewarded
							// members
					...
					if (_validMembers.contains(member))
					{
						...
						member.addExpAndSp(
								Math.round(member.calcStat(Stats.EXPSP_RATE,
										xpReward * preCalculation, null, null)),
								(int) member.calcStat(Stats.EXPSP_RATE,
										spReward * preCalculation, null, null));
					}
				}
			}
		}
	 ...
	 * This method takes in consideration number of members, members' levels,
	 * rewarder's level and bonus modifier for the actual party.<BR>
	 * <BR>
	 *
	 * @param member
	 *            is the L2Character to be rewarded
	 * @param xpReward
	 *            is the total amount of XP to be "splited" and given to the
	 *            member
	 * @param spReward
	 *            is the total amount of SP to be "splited" and given to the
	 *            member
	 * @param penalty
	 *            is the penalty that must be applied to the XP rewards of the
	 *            requested member
	 */
	...
	public void recalculateValidMembers()
	{
		...
		for (L2PcInstance member : getPartyMembers())
		{
			if (checkMemberValidity(member))
				_validMembers.add(member);
		}
	}
...
//		Fixed LevelDiff cutoff point
		...
		{
			return (getLevel()
					- member.getLevel() <= Config.PARTY_XP_CUTOFF_LEVEL);
		}
//		Fixed MinPercentage cutoff point
		else if (Config.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("percentage"))
		{
			...
			return (sqLevel * 100 >= sqLevelSum
					* Config.PARTY_XP_CUTOFF_PERCENT);
		}
//		Automatic cutoff method
		else if (Config.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("auto"))
		{
			...
			return (sqLevel >= sqLevelSum
					* (1 - 1 / (1 + _bonusExpSp[i] - _bonusExpSp[i - 1])));
		}
		return true;
	}
