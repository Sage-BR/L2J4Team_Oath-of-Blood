...
public class ItemsAutoDestroy
{
	...
	protected class CheckItemsForDestroy extends Thread
	{
		...
		{
			if (Config.DEBUG)
				_log.info("[ItemsAutoDestroy] : " + _items.size()
						+ " items to check.");
			...
			{
				if (item == null || item.getDropTime() == 0 || item
				...
				else
				{
					...
					{
						if ((curtime - item
								.getDropTime()) > Config.HERB_AUTO_DESTROY_TIME)
						{
							L2World.getInstance().removeVisibleObject(item,
									item.getWorldRegion());
