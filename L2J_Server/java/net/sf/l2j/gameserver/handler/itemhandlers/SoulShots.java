...
public class SoulShots implements IItemHandler
{
			...
			L2ItemInstance item)
	{
		...
		if (weaponInst == null || weaponItem.getSoulShotCount() == 0)
		{
			if (!activeChar.getAutoSoulShot().containsKey(itemId))
				activeChar.sendPacket(
						new SystemMessage(SystemMessage.CANNOT_USE_SOULSHOTS));
			return;
		}
				...
				|| (weaponGrade == L2Item.CRYSTAL_S && itemId != 1467))
		{
			if (!activeChar.getAutoSoulShot().containsKey(itemId))
				activeChar.sendPacket(new SystemMessage(
						SystemMessage.SOULSHOTS_GRADE_MISMATCH));
			return;
		}

		// Consume Soulshots if player has enough of them
		int saSSCount = (int) activeChar.getStat()
				.calcStat(Stats.SOULSHOT_COUNT, 0, null, null);
		int SSCount = saSSCount == 0 ? weaponItem.getSoulShotCount()
				: saSSCount;

		if (!activeChar.destroyItem("Consume", item.getObjectId(), SSCount,
				null, false))
		{
			if (activeChar.getAutoSoulShot().containsKey(itemId))
			{
				...
				activeChar.sendPacket(
						new SystemMessage(SystemMessage.NOT_ENOUGH_SOULSHOTS));
			return;
		}
		...
		activeChar
				.sendPacket(new SystemMessage(SystemMessage.ENABLED_SOULSHOT));
		Broadcast
				.toSelfAndKnownPlayersInRadius(activeChar,
						new MagicSkillUser(activeChar, activeChar,
								_skillIds[weaponGrade], 1, 0, 0),
						360000/* 600 */);
	}
