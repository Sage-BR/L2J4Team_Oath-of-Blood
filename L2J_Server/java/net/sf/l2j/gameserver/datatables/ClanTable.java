...
public class ClanTable
{
	...
	private ClanTable()
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("SELECT clan_id FROM clan_data");
			...
			while (result.next())
			{
				_clans.put(Integer.parseInt(result.getString("clan_id")),
						new L2Clan(
								Integer.parseInt(result.getString("clan_id"))));
				clan = getClan(Integer.parseInt(result.getString("clan_id")));
				if (clan.getDissolvingExpiryTime() != 0)
				{
					if (clan.getDissolvingExpiryTime() < System
							.currentTimeMillis())
					{
	...
	public L2Clan createClan(L2PcInstance player, String clanName)
	{
		...
		if (10 > player.getLevel())
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.YOU_DO_NOT_MEET_CRITERIA_IN_ORDER_TO_CREATE_A_CLAN));
			return null;
		}
		if (0 != player.getClanId())
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.FAILED_TO_CREATE_CLAN));
			return null;
		}
		if (System.currentTimeMillis() < player.getClanCreateExpiryTime())
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.YOU_MUST_WAIT_XX_DAYS_BEFORE_CREATING_A_NEW_CLAN));
			return null;
		}
		...
		L2ClanMember leader = new L2ClanMember(clan, player.getName(),
	...
	public synchronized void destroyClan(int clanId)
	{
		...
		clan.broadcastToOnlineMembers(
				new SystemMessage(SystemMessage.CLAN_HAS_DISPERSED));

		if (AuctionManager.getInstance()
				.getAuction(clan.getAuctionBiddedAt()) != null)
		{
			AuctionManager.getInstance().getAuction(clan.getAuctionBiddedAt())
					.cancelBid(clan.getClanId());
		}

		if (clan.getHasHideout() != 0)
		{
			ClanHallManager.getInstance().getClanHall(clan.getHasHideout())
					.setOwner(null);
		}
		...
		if (leaderMember == null)
			...
			clan.getWarehouse().destroyAllItems("ClanRemove",
					clan.getLeader().getPlayerInstance(), null);
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement("DELETE FROM clan_data WHERE clan_id=?");
			statement.setInt(1, clanId);
			statement.execute();
			statement.close();

			statement = con
					.prepareStatement("DELETE FROM clan_privs WHERE clan_id=?");
			statement.setInt(1, clanId);
			statement.execute();
			statement.close();

			statement = con.prepareStatement(
					"DELETE FROM clan_skills WHERE clan_id=?");
			statement.setInt(1, clanId);
			statement.execute();
			statement.close();

			statement = con.prepareStatement(
					"DELETE FROM clan_subpledges WHERE clan_id=?");
			statement.setInt(1, clanId);
			statement.execute();
			statement.close();

			statement = con.prepareStatement(
					"DELETE FROM clan_wars WHERE clan1=? OR clan2=?");
			statement.setInt(1, clanId);
			...
			{
				statement = con.prepareStatement(
						"UPDATE castle SET taxPercent = 0 WHERE id = ?");
	...
	public void scheduleRemoveClan(final int clanId)
	{
		...
		}, getClan(clanId).getDissolvingExpiryTime()
	...
	public void storeclanswars(int clanId1, int clanId2)
	{
		...
		// SystemMessage(SystemMessage.WAR_WITH_THE_S1_CLAN_HAS_BEGUN);
		//
	...
	public void deleteclanswars(int clanId1, int clanId2)
	{
		...
		try
		{
			...
			PreparedStatement statement;
			statement = con.prepareStatement(
					"DELETE FROM clan_wars WHERE clan1=? AND clan2=?");
			statement.setInt(1, clanId1);
			statement.setInt(2, clanId2);
			statement.execute();
			// statement = con.prepareStatement("DELETE FROM clan_wars WHERE
			// clan1=? AND clan2=?");
		...
		// SystemMessage msg = new
		// SystemMessage(SystemMessage.WAR_WITH_THE_S1_CLAN_HAS_ENDED);
	...
	public void CheckSurrender(L2Clan clan1, L2Clan clan2)
	{
		...
		{
			if (player != null
					&& player.getPlayerInstance().getWantsPeace() == 1)
				count++;
		}
