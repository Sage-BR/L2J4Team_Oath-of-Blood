...
public final class QuestState
{
	 ...
	 *            : state of the quest
	 * @param completed
	 *            : boolean for completion of the quest
	 */
	...
	public Object setState(State state)
	{
		...
		if (getDrops() != null)
		{
			for (Iterator<List<L2DropData>> i = getDrops().values()
					.iterator(); i.hasNext();)
			{
				...
				for (Iterator<L2DropData> ds = lst.iterator(); ds.hasNext();)
				{
					...
					{
						if (getState().getName().equals(state2))
						{
							ds.remove();
							break;
						}
	...
	public int getInt(String var)
	{
		...
		{
			_log.finer(getPlayer().getName() + ": variable " + var
					+ " isn't an integer: " + varint + e);
...
//		exitQuest(true);
	 ...
	 *
	 * @param npcId
	 *            : int pointing out the NPC given the drop
	 * @param itemId
	 *            : int pointing out the ID of the item dropped
	 * @param chance
	 *            : int pointing out the chance to get the drop
	 */
	 ...
	 *            : L2Attackable killed
	 * @param drops
	 *            : List of drops of the L2Attackable
	 */
	public void fillQuestDrops(L2NpcInstance npc, List<L2DropData> drops)
	{
		...
		// If drops of the NPC in class variable "drops" exist, add them to
		// parameter "drops"
		if (lst != null)
			drops.addAll(lst);
		// Get drops for all NPC recorded in class variable "drops"
		lst = getDrops().get(0); // all mobs
		// If drops for all NPC in class variable "drops" exist, add them to
		// parameter "drops"
		if (lst != null)
			drops.addAll(lst);
	}
	...
	{
		L2ItemInstance enchanteditem = getPlayer().getInventory()
	...
	public void giveItems(int itemId, int count, int enchantlevel)
	{
		...
		// If item for reward is gold (ID=57), modify count with rate for quest
		// reward
		if (itemId == 57
				&& !(getQuest().getQuestIntId() >= 217
						&& getQuest().getQuestIntId() <= 233)
				&& !(getQuest().getQuestIntId() >= 401
						&& getQuest().getQuestIntId() <= 418))
			count = (int) (count * Config.RATE_QUESTS_REWARD);
		// Set quantity of item

		// Add items to player's inventory
		L2ItemInstance item = getPlayer().getInventory().addItem("Quest",
				itemId, count, getPlayer(), getPlayer().getTarget());
	 ...
	 * @param count(minCount,
	 *            maxCount) : int Quantity of items to be dropped
	 * @param neededCount
	 *            : Quantity of items needed for quest
	 * @param dropChance
	 *            : int Base chance of drop, same as in droplist
	 * @param sound
	 *            : boolean indicating whether to play sound
	 * @return boolean indicating whether player has requested number of items
	 */
	public boolean dropQuestItems(int itemId, int count, int neededCount,
			int dropChance, boolean sound)
	{
		return dropQuestItems(itemId, count, count, neededCount, dropChance,
				sound);
	}

	public boolean dropQuestItems(int itemId, int minCount, int maxCount,
			int neededCount, int dropChance, boolean sound)
	{
		dropChance *= Config.RATE_DROP_QUEST / ((getPlayer().getParty() != null)
				? getPlayer().getParty().getMemberCount()
				: 1);
		...
		if (itemCount > 0)
		{
			...
			getPlayer().addItem("Quest", itemId, itemCount,
					getPlayer().getTarget(), true);

			if (sound)
				playSound((currentCount + itemCount < neededCount)
...
// END STUFF THAT WILL PROBABLY BE CHANGED
	...
	public void takeItems(int itemId, int count)
	{
		...
		if (itemId == 57)
			...
			getPlayer().destroyItemByItemId("Quest", itemId, count, getPlayer(),
					true);
	}
	...
	{
		getPlayer().addExpAndSp(
				(int) getPlayer().calcStat(Stats.EXPSP_RATE,
						exp * Config.RATE_QUESTS_REWARD, null, null),
				(int) getPlayer().calcStat(Stats.EXPSP_RATE,
						sp * Config.RATE_QUESTS_REWARD, null, null));
	}
	 ...
	 *
	 * @param name<BR>
	 *            The name of the timer. Will also be the value for event of
	 *            onEvent
	 * @param time<BR>
	...
	{
		for (QuestTimer element : getQuestTimers())
	 ...
	 * Quest.getPcSpawn(L2PcInstance) For now, I shall leave it as is Return a
	 * QuestPcSpawn for curren player instance
	 */
	...
	public QuestState exitQuest(boolean repeatable)
	{
		...
		if (getDrops() != null)
		{
			// Go through values of class variable "drops" pointing out mobs
			// that drop for quest
			for (List<L2DropData> lst : getDrops().values())
			{

				// Go through values of mobs that drop for quest pointing out
				// drops of the mob
				for (L2DropData d : lst)
				{
					int itemId = d.getItemId();

					// Get [item from] / [presence of the item in] the inventory
					// of the player
					L2ItemInstance item = getPlayer().getInventory()
							.getItemByItemId(itemId);
					...
					// If player has the item in inventory, destroy it (if not
					// gold)

					getPlayer().destroyItemByItemId("Quest", itemId, count,
							getPlayer(), true);
				}
			}
