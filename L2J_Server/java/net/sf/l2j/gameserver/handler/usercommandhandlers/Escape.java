...
public class Escape implements IUserCommandHandler
{
	...
	int id, L2PcInstance activeChar)
	{
		...
		int unstuckTimer = (activeChar.getAccessLevel() >= REQUIRED_LEVEL ? 5000
		...
		if (activeChar.isFestivalParticipant())
		{
			activeChar.sendPacket(SystemMessage.sendString(
					"You may not use an escape command in a festival."));
			return false;
		}
		...
		sm.addString("After " + unstuckTimer / 60000
				+ " min. you be returned to near village.");
		...
		MagicSkillUser msk = new MagicSkillUser(activeChar, 1050, 1,
				unstuckTimer, 0);
		Broadcast.toSelfAndKnownPlayersInRadius(activeChar, msk,
				810000/* 900 */);
		...
		activeChar.setSkillCast(ThreadPoolManager.getInstance()
				.scheduleGeneral(ef, unstuckTimer));
