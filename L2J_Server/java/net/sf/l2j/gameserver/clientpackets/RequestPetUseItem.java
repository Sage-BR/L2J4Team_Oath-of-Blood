...
public class RequestPetUseItem extends ClientBasePacket
{
	private static Logger _log = Logger
			.getLogger(RequestPetUseItem.class.getName());
	...
	void runImpl()
	{
		...
		if (Config.DEBUG)
			_log.finest(
					activeChar.getObjectId() + ": pet use item " + _objectId);
		...
		if (item.isEquipable())
		{
			...
			} else
			{
				activeChar.sendPacket(
						new SystemMessage(SystemMessage.ITEM_NOT_FOR_PETS));
				return;
			}
		} else if (L2PetDataTable.isPetFood(itemId))
		{
			if (L2PetDataTable.isWolf(pet.getNpcId())
					&& L2PetDataTable.isWolfFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			} else if (L2PetDataTable.isHatchling(pet.getNpcId())
					&& L2PetDataTable.isHatchlingFood(itemId))
			{
				feed(activeChar, pet, item);
				return;
			...
			L2PcInstance activeChar)
	{
		...
		} else
		{
			...
			if (handler == null)
				_log.warning("no itemhandler registered for itemId:"
						+ item.getItemId());
			else
	 ...
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : 1 food = 100 points of
	 * currentFed</B></FONT><BR>
	 * <BR>
	 */
	private void feed(L2PcInstance player, L2PetInstance pet,
			L2ItemInstance item)
	{
