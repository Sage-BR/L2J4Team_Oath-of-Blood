...
public class IDFactoryTest extends TestCase
{
	...
	protected AtomicInteger count = new AtomicInteger(0),
			adds = new AtomicInteger(0), removes = new AtomicInteger(0);
	...
	protected void setUp() throws Exception
	{
		super.setUp();
		System.out
				.println("Initial Free ID's: " + IdFactory.FREE_OBJECT_ID_SIZE);
		System.out.println("IdFactoryType: " + Config.IDFACTORY_TYPE.name());
		/*
		 * long startMemoryUse = getMemoryUse(); BitSet freeIds = new
		 * BitSet(0x6FFFFFFF); long endMemoryUse = getMemoryUse();
		 * freeIds.clear(); long approximateSize = ( endMemoryUse -
		 * startMemoryUse ) / (1024*1024);
		 *
	...
	public final void testFactory()
	{
		...
		System.out.println("Used ID's: "
				+ (IdFactory.FREE_OBJECT_ID_SIZE - idFactory.size()));
	...
	CountDownLatch latch = new CountDownLatch(
			REQUESTER_THREADS + RELEASER_THREADS);

	protected static Vector<Integer> map = new Vector<>();
	...
	public class RequestID implements Runnable
	{
		...
		public void run()
		{
			...
			if (debug)
				System.out.println(
						getName() + " myCount is " + myCount.get() + "/100.");
			latch.countDown();
		}
	}
		...
		public void run()
		{
			for (int i = 0; i < RELEASER_THREAD_RELEASES; i++)
			{
				synchronized (map)
				{
					...
					if (random.nextInt(10) == 0)
					{
						System.out.println("Total ID releases: " + removes.get()
								+ ". " + time2 + "ns");
					}
					// }
				}
			...
			if (debug)
				System.out.println(
						getName() + " count is " + myCount.get() + "/100.");
