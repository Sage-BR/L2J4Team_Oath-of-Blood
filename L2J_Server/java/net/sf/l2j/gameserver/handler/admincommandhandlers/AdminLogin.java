...
public class AdminLogin implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
			...
			if (st.countTokens() > 1)
			{
				...
				try
				{
					LoginServerThread.getInstance()
							.setMaxPlayer(new Integer(number).intValue());
		...
		} else if (command.startsWith("admin_server_list_clock"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				if (mode.equals("on"))
				{
					LoginServerThread.getInstance().sendServerStatus(
							ServerStatus.SERVER_LIST_CLOCK, ServerStatus.ON);
					activeChar.sendMessage(
							"A clock will now be displayed next to the server name");
					Config.SERVER_LIST_CLOCK = true;
					showWindow(activeChar);
				} else if (mode.equals("off"))
				{
					LoginServerThread.getInstance().sendServerStatus(
							ServerStatus.SERVER_LIST_CLOCK, ServerStatus.OFF);
				...
				{
					activeChar.sendMessage(
							"Format is server_list_clock <on/off>");
				}
			} else
			{
	...
	private void showWindow(L2PcInstance activeChar)
	{
		...
		html.replace("%server_name%",
				LoginServerThread.getInstance().getServerName());
		html.replace("%status%",
				LoginServerThread.getInstance().getStatusString());
		...
		html.replace("%max_players%",
				String.valueOf(LoginServerThread.getInstance().getMaxPlayer()));
	...
	{
		LoginServerThread.getInstance()
				.setServerStatus(ServerStatus.STATUS_AUTO);
	...
	{
		LoginServerThread.getInstance()
				.setServerStatus(ServerStatus.STATUS_GM_ONLY);
