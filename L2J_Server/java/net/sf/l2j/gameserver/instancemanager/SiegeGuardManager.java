...
public class SiegeGuardManager
{
	...
	private static Logger _log = Logger
			.getLogger(SiegeGuardManager.class.getName());
	...
	public void addSiegeGuard(L2PcInstance activeChar, int npcId)
	{
		if (activeChar == null)
			return;
		addSiegeGuard(activeChar.getX(), activeChar.getY(), activeChar.getZ(),
				activeChar.getHeading(), npcId);
	}
	...
	public void hireMerc(L2PcInstance activeChar, int npcId)
	{
		if (activeChar == null)
			return;
		hireMerc(activeChar.getX(), activeChar.getY(), activeChar.getZ(),
				activeChar.getHeading(), npcId);
	}
	...
	public void removeMerc(int npcId, int x, int y, int z)
	{
		...
		{
			_log.warning("Error deleting hired siege guard at " + x + ',' + y
					+ ',' + z + ":" + e1);
		} finally
		{
			try
	...
	public void removeMercs()
	{
		...
		{
			_log.warning("Error deleting hired siege guard for castle "
					+ getCastle().getName() + ":" + e1);
		} finally
		{
			try
			{
	...
	private void loadSiegeGuard()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT * FROM castle_siege_guards Where castleId = ? And isHired = ?");
			statement.setInt(1, getCastle().getCastleId());
			...
			while (rs.next())
			{
				...
				{
					_log.warning("Missing npc data in npc table for id: "
							+ rs.getInt("npcId"));
				}
			}
			statement.close();
		...
		{
			_log.warning("Error loading siege guard for castle "
					+ getCastle().getName() + ":" + e1);
		} finally
		{
			try
	 ...
	 * <BR>
	 */
	private void saveSiegeGuard(int x, int y, int z, int heading, int npcId,
			int isHire)
	{
		java.sql.Connection con = null;
		try
		...
		{
			_log.warning("Error adding siege guard for castle "
					+ getCastle().getName() + ":" + e1);
		} finally
		{
			try
