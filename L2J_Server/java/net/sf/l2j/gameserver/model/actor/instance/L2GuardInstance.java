...
public final class L2GuardInstance extends L2Attackable
{
	...
	private static Logger _log = Logger
			.getLogger(L2GuardInstance.class.getName());
	...
	public final GuardKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof GuardKnownList))
	 ...
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * Guard will always try to return to this location after it has killed all
	 * PK's in range <BR>
	 * <BR>
	 *
	 */
	public void getHomeLocation()
	{
		...
		if (Config.DEBUG)
			_log.finer(getObjectId() + ": Home location set to" + " X:" + _homeX
					+ " Y:" + _homeY + " Z:" + _homeZ);
	}
	...
	public void returnHome()
	{
		...
		{
			if (Config.DEBUG)
				_log.fine(getObjectId() + ": moving hometo" + " X:" + _homeX
						+ " Y:" + _homeY + " Z:" + _homeZ);

			clearAggroList();

			getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO,
					new L2CharPosition(_homeX, _homeY, _homeZ, 0));
		}
	}
	 ...
	 * L2GuardInstance Identifier and of the page number.<BR>
	 * <BR>
	 *
	 ...
	 * number)</li>
	 * <li>if page number > 0 : <B>data/html/guard/12006-1.htm</B> (npcId-page
	 * number)</li><BR>
	 * <BR>
	 *
	 * @param npcId
	 ...
	 * L2PcInstance Intention to AI_INTENTION_ATTACK</li>
	 * <li>If L2PcInstance is NOT in the _aggroList of the L2GuardInstance, set
	 * the L2PcInstance Intention to AI_INTENTION_INTERACT (after a distance
		...
		} else
		{
			...
			} else
			{
				...
				} else
				{
					...
					// Set the L2PcInstance Intention to AI_INTENTION_IDLE
					player.getAI().setIntention(
							CtrlIntention.AI_INTENTION_INTERACT, null);
