...
public class RequestBypassToServer extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestBypassToServer.class.getName());
	...
	void runImpl()
	{
		...
		try
		{
			...
			{
				if (Config.ALT_PRIVILEGES_ADMIN && !AdminCommandHandler
						.getInstance().checkPrivileges(activeChar, _command))
				{
					_log.info("<GM>" + activeChar
							+ " does not have sufficient privileges for command '"
							+ _command + "'.");
					return;
				}

				IAdminCommandHandler ach = AdminCommandHandler.getInstance()
						.getAdminCommandHandler(_command);
				...
				if (ach != null)
					...
					_log.warning("No handler registered for bypass '" + _command
							+ "'");
			} else if (_command.equals("come_here")
					&& activeChar.getAccessLevel() >= Config.GM_ACCESSLEVEL)
			{
				comeHere(activeChar);
			...
			} else if (_command.startsWith("npc_"))
			{
				...
				try
				{
					L2Object object = L2World.getInstance()
							.findObject(Integer.parseInt(id));
					if (_command.substring(endOfId + 1)
							.startsWith("event_participate"))
						L2Event.inscribePlayer(activeChar);
					if (object != null && object instanceof L2NpcInstance
							&& endOfId > 0
							&& activeChar.isInsideRadius(object,
									L2NpcInstance.INTERACTION_DISTANCE, false,
									false))
					{
						((L2NpcInstance) object).onBypassFeedback(activeChar,
								_command.substring(endOfId + 1));
					}
			...
			} else if (_command.startsWith("Quest "))
			{
				...
				if (idx < 0)
					...
					player.processQuestEvent(p.substring(0, idx),
							p.substring(idx).trim());
			}
		} catch (Exception e)
		{
	...
	private void comeHere(L2PcInstance activeChar)
	{
		...
		if (obj instanceof L2NpcInstance)
		{
			...
			temp.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO,
					new L2CharPosition(activeChar.getX(), activeChar.getY(),
							activeChar.getZ(), 0));
