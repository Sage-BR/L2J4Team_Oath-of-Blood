	...
	void runImpl()
	{
		...
		if (activeChar.isClanLeader())
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.CLAN_LEADER_CANNOT_WITHDRAW));
			return;
		}
		if (activeChar.isInCombat())
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.YOU_CANNOT_LEAVE_DURING_COMBAT));
			return;
		}

		L2Clan clan = activeChar.getClan();

		clan.removeClanMember(activeChar.getName(), System.currentTimeMillis()
				+ Config.ALT_CLAN_JOIN_DAYS * 86400000); // 24*60*60*1000 =
															// 86400000

		SystemMessage sm = new SystemMessage(
				SystemMessage.S1_HAS_WITHDRAWN_FROM_THE_CLAN);
		...
		activeChar.sendPacket(
				new SystemMessage(SystemMessage.YOU_HAVE_WITHDRAWN_FROM_CLAN));
		activeChar.sendPacket(new SystemMessage(
				SystemMessage.YOU_MUST_WAIT_BEFORE_JOINING_ANOTHER_CLAN));
	}
