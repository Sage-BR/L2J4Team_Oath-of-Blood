...
public class RequestJoinParty extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestJoinParty.class.getName());
	...
	void runImpl()
	{
		...
		if (target.isInParty())
		{
			SystemMessage msg = new SystemMessage(
					SystemMessage.S1_IS_ALREADY_IN_PARTY);
			msg.addString(target.getName());
			requestor.sendPacket(msg);
			return;
		}

		if ((target == requestor) || target.isCursedWeaponEquiped()
				|| requestor.isCursedWeaponEquiped())
		{
			requestor.sendPacket(
	...
	private void addTargetToParty(L2PcInstance target, L2PcInstance requestor)
	{
		...
		if (requestor.getParty().getMemberCount()
				+ requestor.getParty().getPendingInvitationNumber() >= 9)
		{
		...
		{
			requestor.sendPacket(
					new SystemMessage(SystemMessage.ONLY_LEADER_CAN_INVITE));
			return;
		}
		...
		if (!target.isProcessingRequest())
		{
			...
			target.sendPacket(
					new AskJoinParty(requestor.getName(), _itemDistribution));
		...
		} else
		{
			...
			if (Config.DEBUG)
				_log.warning(requestor.getName()
						+ " already received a party invitation");
		}
		msg = null;
	}
