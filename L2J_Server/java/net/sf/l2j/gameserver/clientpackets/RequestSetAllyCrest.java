...
public class RequestSetAllyCrest extends ClientBasePacket
{
	...
	void runImpl()
	{
		...
		{
			L2Clan leaderclan = ClanTable.getInstance()
					.getClan(activeChar.getAllyId());

			if (activeChar.getClanId() != leaderclan.getClanId()
			...
			{
				_log.log(Level.INFO, "Error loading crest of ally:"
						+ leaderclan.getAllyName());
				return;
			}
			...
			try
			{
				...
				PreparedStatement statement = con.prepareStatement(
						"UPDATE clan_data SET ally_crest_id = ? WHERE ally_id = ?");
			...
			{
				_log.warning(
						"could not update the ally crest id:" + e.getMessage());
			} finally
			{
				try
				{
