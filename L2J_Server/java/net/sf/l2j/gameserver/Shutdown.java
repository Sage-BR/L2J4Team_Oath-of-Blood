...
public class Shutdown extends Thread
{
	...
	public void startTelnetShutdown(String IP, int seconds, boolean restart)
	{
		...
		if (shutdownMode > 0)
		{
			...
			if (shutdownMode == 1 || shutdownMode == 2)
			{
				...
				_an.announceToAll("during server " + _modeText[shutdownMode]
						+ " procedure.");
			}
		}
	 ...
	 *            true if the server will restart after shutdown
	 */
	public void startShutdown(L2PcInstance activeChar, int seconds,
			boolean restart)
	{
		Announcements _an = Announcements.getInstance();
		_log.warning("GM: " + activeChar.getName() + "("
				+ activeChar.getObjectId() + ") issued shutdown command. "
				+ _modeText[shutdownMode] + " in " + seconds + " seconds!");
...
//		 the main instance should only run for shutdown hook, so we start a new instance
	...
	public void abort(L2PcInstance activeChar)
	{
		Announcements _an = Announcements.getInstance();
		_log.warning("GM: " + activeChar.getName() + "("
				+ activeChar.getObjectId() + ") issued shutdown ABORT. "
				+ _modeText[shutdownMode] + " has been stopped!");
		_an.announceToAll("Server aborts " + _modeText[shutdownMode]
				+ " and continues normal operation!");
	...
	private void countdown()
	{
		...
		try
		{
			while (secondsShut > 0)
			{
				...
				{
				case 540:
					_an.announceToAll("The server is " + _modeText[shutdownMode]
							+ " in 9 minutes.");
					break;
				case 480:
					_an.announceToAll("The server is " + _modeText[shutdownMode]
				...
				case 60:
					LoginServerThread.getInstance()
							.setServerStatus(ServerStatus.STATUS_DOWN); // avoids
																		// new
																		// players
																		// from
																		// logging
	...
	private void saveData()
	{
		...
		if (Config.SAVE_DROPPED_ITEM)
		{
			...
			System.err.println(
					"ItemsOnGroundManager: All items on ground saved!!");
		}
		System.err.println(
