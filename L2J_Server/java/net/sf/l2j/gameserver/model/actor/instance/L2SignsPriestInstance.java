...
public class L2SignsPriestInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		} else if (command.startsWith("SevenSigns"))
		{
			...
			int stoneType = 0;
			L2ItemInstance ancientAdena = player.getInventory()
					.getItemByItemId(SevenSigns.ANCIENT_ADENA_ID);
			int ancientAdenaAmount = ancientAdena == null ? 0
					: ancientAdena.getCount();
			...
			if (command.length() > 13)
			{
				...
				} catch (Exception e)
				{
					...
					{
						cabal = Integer
								.parseInt(command.substring(13, 14).trim());
					} catch (Exception e2)
					{
						try
						{
						...
						{
							_log.warning(
									"Failed to retrieve cabal from bypass command. NpcId: "
											+ this.getNpcId() + "; Command: "
											+ command);
						}
					}
				}
			...
			case 2: // Purchase Record of the Seven Signs
				...
				{
					player.sendPacket(
							new SystemMessage(SystemMessage.SLOTS_FULL));
					break;
				}

				L2ItemInstance adenaItem = player.getInventory()
						.getAdenaInstance();
				if (!player.reduceAdena("SevenSigns",
						SevenSigns.RECORD_SEVEN_SIGNS_COST, this, true))
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.YOU_NOT_ENOUGH_ADENA));
					break;
				}
				L2ItemInstance recordSevenSigns = player.getInventory().addItem(
						"SevenSigns", SevenSigns.RECORD_SEVEN_SIGNS_ID, 1,
						player, this);
			...
			case 10: // Teleport Locations List
				showChatWindow(player, val, SevenSigns.getCabalShortName(cabal),
						false);
				break;
			case 4: // Join a Cabal - SevenSigns 4 [0]1 x
				...
				{
					player.sendMessage(
							"You must have already completed your first class transfer.");
					break;
				} else if (player.getClassId().level() >= 2)
				{
					if (Config.ALT_GAME_REQUIRE_CASTLE_DAWN)
					{
						if (getPlayerAllyHasCastle(player))
						{
							...
							{
								player.sendMessage(
										"You must not be a member of a castle-owning clan to join the Revolutionaries of Dusk.");
								return;
							}
						}
						/*
						...
						else
						{
							 ...
							 * certificate.
							 *
							 * If not then try to take the required amount of
							 * adena instead.
							 */
							if (cabal == SevenSigns.CABAL_DAWN)
							{
								...
								if (player.destroyItemByItemId("SevenSigns",
										...
										false))
								{
									...
									sm.addNumber(
											SevenSigns.ADENA_JOIN_DAWN_COST);
									player.sendPacket(sm);
									allowJoinDawn = true;
								}
								...
								{
									player.sendMessage(
											"You must be a member of a castle-owning clan, have a Certificate of Lord's Approval, or pay 50000 adena to join the Lords of Dawn.");
									return;
								}
							}
						}
				...
				if (cabal == SevenSigns.CABAL_DAWN)
					player.sendPacket(new SystemMessage(
							SystemMessage.SEVENSIGNS_PARTECIPATION_DAWN)); // Joined
																			// Dawn
				else
					player.sendPacket(new SystemMessage(
							SystemMessage.SEVENSIGNS_PARTECIPATION_DUSK)); // Joined
																			// Dusk
				...
				case SevenSigns.SEAL_AVARICE:
					player.sendPacket(
							new SystemMessage(SystemMessage.FIGHT_FOR_AVARICE));
					break;
				case SevenSigns.SEAL_GNOSIS:
					player.sendPacket(
							new SystemMessage(SystemMessage.FIGHT_FOR_GNOSIS));
					break;
				case SevenSigns.SEAL_STRIFE:
					player.sendPacket(
							new SystemMessage(SystemMessage.FIGHT_FOR_STRIFE));
					break;
				}

				showChatWindow(player, 4, SevenSigns.getCabalShortName(cabal),
						false);
				break;
			case 6: // Contribute Seal Stones - SevenSigns 6 x
				stoneType = Integer.parseInt(command.substring(13));
				L2ItemInstance redStones = player.getInventory()
						.getItemByItemId(SevenSigns.SEAL_STONE_RED_ID);
				int redStoneCount = redStones == null ? 0
						: redStones.getCount();
				L2ItemInstance greenStones = player.getInventory()
						.getItemByItemId(SevenSigns.SEAL_STONE_GREEN_ID);
				int greenStoneCount = greenStones == null ? 0
						: greenStones.getCount();
				L2ItemInstance blueStones = player.getInventory()
						.getItemByItemId(SevenSigns.SEAL_STONE_BLUE_ID);
				int blueStoneCount = blueStones == null ? 0
						: blueStones.getCount();
				int contribScore = SevenSigns.getInstance()
						.getPlayerContribScore(player);
				boolean stonesFound = false;

				if (contribScore == Config.ALT_MAXIMUM_PLAYER_CONTRIB)
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.CONTRIB_SCORE_EXCEEDED));
					break;
				} else
				{
					...
					case 3:
						redContribCount = (Config.ALT_MAXIMUM_PLAYER_CONTRIB
								- contribScore) / SevenSigns.RED_CONTRIB_POINTS;
					...
					{
						if (player.destroyItemByItemId("SevenSigns",
								SevenSigns.SEAL_STONE_RED_ID, redContribCount,
								this, false))
							stonesFound = true;
					}
					...
					{
						if (player.destroyItemByItemId("SevenSigns",
								SevenSigns.SEAL_STONE_BLUE_ID, blueContribCount,
								this, false))
							stonesFound = true;
					}
					...
					{
						player.sendMessage(
								"You do not have any seal stones of that type.");
						break;
					} else
						contribScore = SevenSigns.getInstance()
								.addPlayerStoneContrib(player, blueContribCount,
										greenContribCount, redContribCount);
			...
			case 7: // Exchange Ancient Adena for Adena - SevenSigns 7 xxxxxxx
				...
				try
				{
					ancientAdenaConvert = Integer
							.parseInt(command.substring(13).trim());
				...
				player.reduceAncientAdena("SevenSigns", ancientAdenaConvert,
						this, true);
				player.addAdena("SevenSigns", ancientAdenaConvert, this, true);

				iu = new InventoryUpdate();
				iu.addModifiedItem(
						player.getInventory().getAncientAdenaInstance());
				iu.addModifiedItem(player.getInventory().getAdenaInstance());
				player.sendPacket(iu);
				break;
			case 9: // Receive Contribution Rewards
				int playerCabal = SevenSigns.getInstance()
						.getPlayerCabal(player);
				int winningCabal = SevenSigns.getInstance()
						.getCabalHighestScore();

				if (SevenSigns.getInstance().isSealValidationPeriod()
						&& playerCabal == winningCabal)
				{
					int ancientAdenaReward = SevenSigns.getInstance()
							.getAncientAdenaReward(player, true);
					...
					player.addAncientAdena("SevenSigns", ancientAdenaReward,
							this, true);
					...
					// Update current load as well
					su = new StatusUpdate(player.getObjectId());
					su.addAttribute(StatusUpdate.CUR_LOAD,
			...
			case 11: // Teleport to Hunting Grounds
				try
				{
					...
					{
						if (!player.reduceAncientAdena("SevenSigns",
								ancientAdenaCost, this, true))
							break;
					}
				...
				{
					_log.warning(
							"SevenSigns: Error occurred while teleporting player: "
									+ e);
				}
				break;
			case 17: // Exchange Seal Stones for Ancient Adena (Type Choice) -
				...
				L2ItemInstance stoneInstance = player.getInventory()
						.getItemByItemId(stoneId);
				...
				if (content != null)
				{
					...
					content = content.replaceAll("%stoneValue%",
							String.valueOf(stoneValue));
					content = content.replaceAll("%stoneCount%",
							String.valueOf(stoneCount));
					content = content.replaceAll("%stoneItemId%",
							String.valueOf(stoneId));
				...
				break;
			case 18: // Exchange Seal Stones for Ancient Adena - SevenSigns 18
						// xxxx xxxxxx
				int convertStoneId = Integer
						.parseInt(command.substring(14, 18));
				int convertCount = 0;

				try
				{
					convertCount = Integer
							.parseInt(command.substring(19).trim());
				...
				L2ItemInstance convertItem = player.getInventory()
						.getItemByItemId(convertStoneId);
				...
				if (convertCount <= totalCount && convertCount > 0)
				{
					...
					case SevenSigns.SEAL_STONE_BLUE_ID:
						ancientAdenaReward = SevenSigns
								.calcAncientAdenaReward(convertCount, 0, 0);
						break;
					case SevenSigns.SEAL_STONE_GREEN_ID:
						ancientAdenaReward = SevenSigns
								.calcAncientAdenaReward(0, convertCount, 0);
						break;
					case SevenSigns.SEAL_STONE_RED_ID:
						ancientAdenaReward = SevenSigns
								.calcAncientAdenaReward(0, 0, convertCount);
						break;
					}

					if (player.destroyItemByItemId("SevenSigns", convertStoneId,
							convertCount, this, true))
					{
						player.addAncientAdena("SevenSigns", ancientAdenaReward,
								this, true);
						...
						// Send inventory update packet
						iu = new InventoryUpdate();
						iu.addModifiedItem(player.getInventory()
				...
				{
					player.sendMessage(
							"You do not have that many seal stones.");
				}
				break;
			...
			case 20: // Seal Status (for when joining a cabal)
				...
				for (int i = 1; i < 4; i++)
				{
					...
					if (sealOwner != SevenSigns.CABAL_NULL)
						...
						contentBuffer
								.append("[" + SevenSigns.getSealName(i, false)
										+ ": Nothingness]<br>");
				}
