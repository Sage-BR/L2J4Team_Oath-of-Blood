...
public class QuestPcSpawnManager
{
		...
		public void run()
		{
			try
			{
				...
				ThreadPoolManager.getInstance()
						.scheduleGeneral(new ScheduleTimerTask(), 60000);
			} catch (Throwable t)
			{
			}
		}
	...
	private List<QuestPcSpawn> _PcSpawns = new FastList<>();

	// =========================================================
	// Constructor
	public QuestPcSpawnManager()
	{
		ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleTimerTask(),
				60000);
	}
	...
	{
		return addSpawn(player, npcId, player.getX(), player.getY(),
				player.getZ());
	}
	...
	/**
	 * Add spawn that will despawn after the specified spawn lenth for the
	 * specified player instance at the coords of the player Return object id of
	 * newly spawned npc
	 */
	public int addSpawn(L2PcInstance player, int npcId, int spawnLength)
	{
		return addSpawn(player, npcId, player.getX(), player.getY(),
				player.getZ(), spawnLength);
	}

	/**
	 * Add spawn that will despawn after the specified spawn lenth for the
	 * specified player instance at the specified coords Return object id of
	 * newly spawned npc
	 */
	public int addSpawn(L2PcInstance player, int npcId, int x, int y, int z,
			int spawnLength)
	...
	/**
	 * Add random spawn for the specified player instance Return object id of
	 * newly spawned npc
	 */
	public int addRandomSpawn(L2PcInstance player, int npcId, int count,
			int randomSpawnDelay, int spawnDelay, int offset)
	{
		return getPcSpawn(player).addRandomSpawn(npcId, count, randomSpawnDelay,
				spawnDelay, offset);
	}
	...
	{
		getPcSpawn(player).addRandomSpawnLoc(objectId, player.getX(),
	 ...
	 * Add random spawn location for npc of the the specified
	 */
	public void addRandomSpawnLoc(L2PcInstance player, int objectId, int x,
			int y, int z)
	{
	...
	{
		for (QuestPcSpawn element : getPcSpawns())
		{
			if (element.getPlayer() != null && element.getPlayer()
	...
	public void removeAllSpawn(L2PcInstance player)
	{
		if (contains(player))
		{
			removeAllSpawn(getPcSpawn(player));
		}
	}
	...
	public void removeSpawn(L2PcInstance player, int objectId)
	{
		if (contains(player))
		{
			getPcSpawn(player).removeSpawn(objectId);
		}
	}
	...
	public void removeSpawns(L2PcInstance player, int[] objectIds)
	{
		if (contains(player))
		{
			getPcSpawn(player).removeSpawns(objectIds);
		}
	}
