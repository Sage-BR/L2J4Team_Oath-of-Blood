...
public class AdminPetition implements IAdminCommandHandler
{
	private static String[] _adminCommands =
		{ "admin_view_petitions", "admin_view_petition",
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		} else if (command.startsWith("admin_accept_petition"))
		{
					...
					.isPlayerInConsultation(activeChar))
			{
				activeChar.sendMessage(
						"You may only deal with one active petition at a time.");
				return true;
			}
			...
			if (!PetitionManager.getInstance().acceptPetition(activeChar,
					petitionId))
				activeChar.sendMessage(
						"Invalid petition specified or error occurred.");
		} else if (command.startsWith("admin_reject_petition"))
		{
			if (!PetitionManager.getInstance().rejectPetition(activeChar,
					petitionId))
				activeChar.sendMessage(
						"Invalid petition specified or error occurred.");
		} else if (command.equals("admin_reset_petitions"))
		{
			...
			{
				activeChar.sendMessage(
						"You cannot clear the petition queue when a petition is being dealt with.");
				return false;
			}
