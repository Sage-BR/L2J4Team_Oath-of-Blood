...
public class AdminKill implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if (command.startsWith("admin_kill"))
		{
			...
			if (st.hasMoreTokens())
			{
				...
				if (plyr != null)
				{
					if (st.hasMoreTokens())
					{
						try
						{
							...
							for (L2Character knownChar : plyr.getKnownList()
							...
							activeChar.sendMessage(
									"Killed all characters within a " + radius
											+ " unit radius.");
							return true;
				...
				} else
				{
					try
					{
						...
						for (L2Character knownChar : activeChar.getKnownList()
								.getKnownCharactersInRadius(radius))
						{
							if (knownChar == null
									|| knownChar instanceof L2ControllableMobInstance
					...
					{
						activeChar.sendMessage(
								"Enter a valid player name or radius.");
						return false;
					}
				}
			} else
			{
				...
				if (obj == null || obj instanceof L2ControllableMobInstance
	...
	private void kill(L2PcInstance activeChar, L2Character target)
	{
		if (target instanceof L2PcInstance)
			target.reduceCurrentHp(target.getMaxHp() + target.getMaxCp() + 1,
					activeChar);
		else
