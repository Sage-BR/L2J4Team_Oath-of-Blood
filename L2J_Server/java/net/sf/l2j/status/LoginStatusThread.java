...
public class LoginStatusThread extends Thread
{
	private static final Logger _log = Logger
			.getLogger(LoginStatusThread.class.getName());
	...
	public LoginStatusThread(Socket client, int uptime) throws IOException
	{
		...
		{
			telnetOutput(1, "Connection attempt from "
		...
		try
		{
					...
					&& _usrCommand.compareTo("exit") != 0)
			{
				...
				if (_usrCommand.equals("help"))
				{
					...
					_print.println(
							"unblockip <ip>      - removes <ip> from hacking protection list.");
					_print.println("shutdown			- shuts down server.");
					_print.println(
							"restart				- restarts the server.");
					_print.println(
							"RedirectLogger		- Telnet will give you some info about server in real time.");
					_print.println(
				...
				} else if (_usrCommand.startsWith("unblock"))
				{
					try
					{
						...
						{
							_log.warning(
									"IP removed via TELNET by host: " + _csocket
											.getInetAddress().getHostAddress());
							_print.println("The IP " + _usrCommand
									+ " has been removed from the hack protection list!");
						} else
						{
							_print.println(
									"IP not found in hack protection list...");
						}
				...
				{
					/* Do Nothing Again - Same reason as the quit part */ } else
				{
