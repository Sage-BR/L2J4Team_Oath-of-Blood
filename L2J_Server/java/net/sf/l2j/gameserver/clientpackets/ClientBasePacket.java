...
public abstract class ClientBasePacket extends BasePacket implements Runnable
{
	private static Logger _log = Logger
			.getLogger(ClientBasePacket.class.getName());

	protected ClientBasePacket(ByteBuffer buf, ClientThread client)
	{
		super(client);
		if (Config.DEBUG)
			_log.fine(getType() + " <<< " + client.getLoginName());
		_buf = buf;
		if (Config.ASSERT)
			assert _buf.position() == 1;
	}
	...
	public final void run()
	{
		...
		} catch (Throwable e)
		{
			...
			if (player != null)
			{
				_log.log(Level.SEVERE, "Character " + player.getName()
						+ " of account " + player.getAccountName()
						+ " caused the following error at packet-handling: "
						+ getType(), e);
			} else
				_log.log(Level.SEVERE,
						"error handling client message " + getType(), e);
		}
	}
