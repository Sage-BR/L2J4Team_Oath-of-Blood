...
public abstract class L2Effect
{
	...
	public void setFirstTime(int newfirsttime)
	{
		...
		{
			_periodStartTicks = GameTimeController.getGameTicks()
					- newfirsttime * GameTimeController.TICKS_PER_SECOND;
	...
	private synchronized void startEffectTask(int duration)
	{
		...
		_currentFuture = ThreadPoolManager.getInstance()
				.scheduleEffect(_currentTask, duration);
	...
	private synchronized void startEffectTaskAtFixedRate(int delay, int rate)
	{
		...
		_currentFuture = ThreadPoolManager.getInstance()
				.scheduleEffectAtFixedRate(_currentTask, delay, rate);
		if (_state == EffectState.ACTING)
			_effected.addEffect(this);
		updateEffects(_effected);
	}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Cancel the effect in the the abnormal effect map of the L2Character
	 * </li>
	 ...
	 * <li>Stop and remove L2Effect from L2Character and update client magic
	 * icone</li><BR>
	 * <BR>
	 *
	 */
	...
	public final void scheduleEffect()
	{
		...
		{
			// Cancel the effect in the the abnormal effect map of the
			// L2Character
			onExit();
	...
	public final void addIcon(MagicEffectIcons mi)
	{
		...
		if (task == null || future == null || _state == EffectState.FINISHING
				|| _state == EffectState.CREATED)
			return;
		L2Skill sk = getSkill();
		if (task.rate > 0)
			...
			mi.addEffect(sk.getId(), getLevel(),
					(int) future.getDelay(TimeUnit.MILLISECONDS));
	}
	...
	public final void addPartySpelledIcon(PartySpelled ps)
	{
		...
		if (task == null || future == null || _state == EffectState.FINISHING
			...
			return;
		L2Skill sk = getSkill();
		ps.addPartySpelledEffect(sk.getId(), getLevel(),
				(int) future.getDelay(TimeUnit.MILLISECONDS));
	}
	...
	public final void addOlympiadSpelledIcon(ExOlympiadSpelledInfo os)
	{
		...
		if (task == null || future == null || _state == EffectState.FINISHING
			...
			return;
		L2Skill sk = getSkill();
		os.addEffect(sk.getId(), getLevel(),
				(int) future.getDelay(TimeUnit.MILLISECONDS));
	}
