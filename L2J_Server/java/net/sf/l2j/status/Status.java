...
public class Status extends Thread
{
	...
	public void run()
	{
		while (true)
		{
			try
			{
				...
				{
					LoginStatusThread lst = new LoginStatusThread(connection,
							_uptime);
					if (lst.isAlive())
					{
	...
	public Status(int mode) throws IOException
	{
		...
		_StatusPort = Integer
				.parseInt(telnetSettings.getProperty("StatusPort", "12345"));
		...
		for (int i = 0; i < length; i++)
		{
			...
			case 0:
				password.append(lowerChar
						.charAt(randInt.nextInt(lowerChar.length() - 1)));
				break;
			case 1:
				password.append(upperChar
						.charAt(randInt.nextInt(upperChar.length() - 1)));
				break;
			case 2:
				password.append(
						digits.charAt(randInt.nextInt(digits.length() - 1)));
				break;
			}
		}
