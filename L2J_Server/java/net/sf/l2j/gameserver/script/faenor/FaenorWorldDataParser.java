...
public class FaenorWorldDataParser extends FaenorParser
{
	static Logger _log = Logger
			.getLogger(FaenorWorldDataParser.class.getName());
	...
	public void parseScript(Node eventNode, BSFManager context)
	{
		...
		for (Node node = eventNode.getFirstChild(); node != null; node = node
				.getNextSibling())
		{
	...
	private void parsePetData(Node petNode, BSFManager context)
	{
		...
		try
		{
			...
			for (Node node = petNode.getFirstChild(); node != null; node = node
					.getNextSibling())
			{
			...
			bridge.addPetData(context, petData.petID, petData.levelStart,
					petData.levelEnd, petData.statValues);
		} catch (Exception e)
		{
			petData.petID = -1;
	...
	private void parseStat(Node stat, PetData petData)
	{
		...
		try
		{
			...
			for (Node node = stat.getFirstChild(); node != null; node = node
					.getNextSibling())
			{
	...
	static
	{
		ScriptEngine.parserFactories.put(getParserName("WorldData"),
				new FaenorWorldDataParserFactory());
	}
}
