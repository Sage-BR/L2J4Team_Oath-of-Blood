...
public class Pdam implements ISkillHandler
{
			...
			L2Object[] targets)
	{
		...
		if (Config.DEBUG)
			if (Config.DEBUG)
				_log.fine("Begin Skill processing in Pdam.java "
						+ skill.getSkillType());
		...
		for (L2Object target2 : targets)
		{
			...
			if (activeChar instanceof L2PcInstance
					&& target instanceof L2PcInstance && target.isAlikeDead()
					&& target.isFakeDeath())
			{
			...
			boolean crit = Formulas.getInstance()
					.calcCrit(activeChar.getCriticalHit(target, skill));
			boolean soul = (weapon != null && weapon
			...
			if (!crit && (skill.getCondition() & L2Skill.COND_CRIT) != 0)
				...
				damage = (int) Formulas.getInstance().calcPhysDam(activeChar,
						target, skill, shld, crit, dual, soul);
			...
			if (damage > 5000 && activeChar instanceof L2PcInstance)
			{
				...
				Log.add(activeChar.getName() + "(" + activeChar.getObjectId()
						...
						+ skill.getId() + ") to " + name, "damage_pdam");
			}
			...
			if (damage > 0)
			{
				if (activeChar instanceof L2PcInstance)
				{
					if (crit)
						activeChar.sendPacket(
								new SystemMessage(SystemMessage.CRITICAL_HIT));
				...
				if (skill.hasEffects())
				{
					...
					if (Formulas.getInstance().calcSkillSuccess(activeChar,
							target, skill, false, false, false))
					{
				...
				int chance;
				if (!target.isRaid()
						&& (chance = Rnd.get(100)) < skill.getLethalChance1())
				{
					// 1st lethal effect activate (cp to 1 or if target is npc
					// then hp to 50%)
					if (chance >= skill.getLethalChance2())
					{
						...
						} else if (target instanceof L2MonsterInstance) // If is
																		// a
																		// monster
																		// remove
																		// first
																		...
																		// hp
						{
							...
							target.reduceCurrentHp(target.getCurrentHp() / 2,
									activeChar);
						}
					} else // 2nd lethal effect activate (cp,hp to 1 or if
							// target is npc then hp to 1)
					{
						// If is a monster damage is (CurrentHp - 1) so HP = 1
						if (target instanceof L2NpcInstance)
							target.reduceCurrentHp(target.getCurrentHp() - 1,
									activeChar);
						...
						if (target instanceof L2PcInstance)
						{
							...
							if (!player.isInvul())
							{
								if (damage >= player.getCurrentHp())
								{
									...
									player.setCurrentHp(
											player.getCurrentHp() - damage);
							}

							SystemMessage smsg = new SystemMessage(
									SystemMessage.S1_GAVE_YOU_S2_DMG);
			...
			if (skill.getId() == 345 || skill.getId() == 346) // Sonic Rage or
																// Raging Force
			{
				...
				if (effect != null)
				{
					...
					if (effectcharge < 7)
					{
						...
						SystemMessage sm = new SystemMessage(
								SystemMessage.FORCE_INCREASED_TO_S1);
						sm.addNumber(effectcharge);
						activeChar.sendPacket(sm);
					} else
					{
						SystemMessage sm = new SystemMessage(
								SystemMessage.FORCE_MAXLEVEL_REACHED);
						activeChar.sendPacket(sm);
					}
				} else
				{
					...
					{
						L2Skill dummy = SkillTable.getInstance().getInfo(8, 7); // Lv7
																				// Sonic
																				// Focus
						dummy.getEffects(activeChar, activeChar);
					} else if (skill.getId() == 346) // Raging Force
					{
						L2Skill dummy = SkillTable.getInstance().getInfo(50, 7); // Lv7
																					// Focused
																					// Force
						dummy.getEffects(activeChar, activeChar);
					}
