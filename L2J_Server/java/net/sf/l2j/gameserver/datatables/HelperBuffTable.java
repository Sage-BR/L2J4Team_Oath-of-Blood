...
public class HelperBuffTable
{
	 ...
	 * Used to generate message : "Only novice character of level ... or less
	 * can receive my support magic.")
	 */
	...
	/**
	 * Create and Load the Newbie Helper Buff list from SQL Table
	 * helper_buff_list
	 */
		...
		try
		{
			try
			{
				...
				PreparedStatement statement = con
						.prepareStatement("SELECT * FROM helper_buff_list");
			...
			{
				_log.severe(
						"Table helper_buff_list not found : Update your DataPack"
								+ e);
				e.printStackTrace();
			}
	...
	private void fillHelperBuffTable(ResultSet HelperBuffData) throws Exception
	{
		...
		while (HelperBuffData.next())
		{
			...
			helperBuffDat.set("skillLevel",
					HelperBuffData.getInt("skill_level"));
			helperBuffDat.set("lowerLevel",
					HelperBuffData.getInt("lower_level"));
			helperBuffDat.set("upperLevel",
					HelperBuffData.getInt("upper_level"));
			helperBuffDat.set("isMagicClass",
					HelperBuffData.getString("is_magic_class"));

			// Calulate the range level in wich player must be to obtain buff
			// from Newbie Helper
			if ("false".equals(HelperBuffData.getString("is_magic_class")))
			{
				if (HelperBuffData
						.getInt("lower_level") < _physicClassLowestLevel)
					_physicClassLowestLevel = HelperBuffData
							.getInt("lower_level");

				if (HelperBuffData
						.getInt("upper_level") > _physicClassHighestLevel)
					_physicClassHighestLevel = HelperBuffData
							.getInt("upper_level");
			} else
			{
				if (HelperBuffData
						.getInt("lower_level") < _magicClassLowestLevel)
					_magicClassLowestLevel = HelperBuffData
							.getInt("lower_level");

				if (HelperBuffData
						.getInt("upper_level") > _magicClassHighestLevel)
					_magicClassHighestLevel = HelperBuffData
							.getInt("upper_level");
			}
		...
		_log.config("Helper Buff Table: Loaded " + _helperBuff.size()
				+ " Templates.");
	...
	/**
	 * @param magicClassHighestLevel
	 *            The magicClassHighestLevel to set.
	 */
	...
	/**
	 * @param magicClassLowestLevel
	 *            The magicClassLowestLevel to set.
	 */
	...
	/**
	 * @param physicClassHighestLevel
	 *            The physicClassHighestLevel to set.
	 */
	...
	/**
	 * @param physicClassLowestLevel
	 *            The physicClassLowestLevel to set.
	 */
