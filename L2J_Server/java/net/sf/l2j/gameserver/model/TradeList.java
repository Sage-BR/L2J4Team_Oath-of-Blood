...
public class TradeList
{
				...
				if (exclItem.getItem().getItemId() == item.getItemId())
					if (item.getCount() <= exclItem.getCount())
						...
						return new TradeItem(item,
								item.getCount() - exclItem.getCount(),
	...
	public synchronized TradeItem addItem(int objectId, int count, int price)
	{
		...
		{
			_log.warning(
					_owner.getName() + ": Attempt to modify locked TradeList!");
			return null;
		}
		...
		{
			_log.warning(_owner.getName()
					+ ": Attempt to add invalid item to TradeList!");
			return null;
		}
		...
		{
			_log.warning(_owner.getName()
					+ ": Attempt to add non-stackable item to TradeList with count > 1!");
			return null;
		}
		...
		// If Player has already confirmed this trade, invalidate the
		// confirmation
	 ...
	 * @return
	 */
	public synchronized TradeItem addItemByItemId(int itemId, int count,
			int price)
	{
		if (isLocked())
		{
	 ...
	 * @return
	 */
	public synchronized TradeItem removeItem(int objectId, int itemId,
			int count)
	{
		if (isLocked())
		{
		...
		for (TradeItem titem : _items)
					...
					|| titem.getItem().getItemId() == itemId)
			{
				...
				if (_partner != null)
				{
					...
					if (partnerList == null)
					{
						_log.warning(_partner.getName() + ": Trading partner ("
								+ _partner.getName()
	...
	public synchronized void updateItems()
	{
		for (TradeItem titem : _items)
		{
			L2ItemInstance item = _owner.getInventory()
					.getItemByObjectId(titem.getObjectId());
	...
	public boolean Confirm()
	{
		...
		if (_partner != null)
		{
			...
			{
				_log.warning(_partner.getName() + ": Trading partner ("
						+ _partner.getName() + ") is invalid in this trade!");
				return false;
			}
	...
	private boolean Validate()
	{
		...
		if (_owner == null || L2World.getInstance()
				.findObject(_owner.getObjectId()) == null)
		{
		...
		for (TradeItem titem : _items)
		{
			L2ItemInstance item = _owner.checkItemManipulation(
					titem.getObjectId(), titem.getCount(), "transfer");
			if (item == null || titem.getCount() < 1)
			{
			...
			InventoryUpdate partnerIU)
	{
		for (TradeItem titem : _items)
		{
			L2ItemInstance oldItem = _owner.getInventory()
					.getItemByObjectId(titem.getObjectId());
			if (oldItem == null)
				return false;
			L2ItemInstance newItem = _owner.getInventory().transferItem("Trade",
					titem.getObjectId(), titem.getCount(),
					partner.getInventory(), _owner, _partner);
			if (newItem == null)
				return false;
	...
	public int countItemsSlots(L2PcInstance partner)
	{
		...
		for (TradeItem item : _items)
		{
			if (item == null)
				continue;
			L2Item template = ItemTable.getInstance()
					.getTemplate(item.getItem().getItemId());
			if (template == null)
				continue;
			if (!template.isStackable())
				slots += item.getCount();
			else if (partner.getInventory()
					.getItemByItemId(item.getItem().getItemId()) == null)
				slots++;
		}
	...
	private void doExchange(TradeList partnerList)
	{
		...
		if ((!this.getOwner().getInventory()
				.validateWeight(partnerList.calcItemsWeight()))
				|| !(partnerList.getOwner().getInventory()
						.validateWeight(this.calcItemsWeight())))
		{
			partnerList.getOwner().sendPacket(
					new SystemMessage(SystemMessage.WEIGHT_LIMIT_EXCEEDED));
			this.getOwner().sendPacket(
					new SystemMessage(SystemMessage.WEIGHT_LIMIT_EXCEEDED));
		} else if ((!this.getOwner().getInventory()
				.validateCapacity(partnerList.countItemsSlots(this.getOwner())))
				|| (!partnerList.getOwner().getInventory().validateCapacity(
						this.countItemsSlots(partnerList.getOwner()))))
		{
		...
		} else
		{
			...
			InventoryUpdate ownerIU = Config.FORCE_INVENTORY_UPDATE ? null
					: new InventoryUpdate();
			InventoryUpdate partnerIU = Config.FORCE_INVENTORY_UPDATE ? null
					: new InventoryUpdate();
	 ...
	 */
	public synchronized boolean PrivateStoreBuy(L2PcInstance player,
			ItemRequest[] items, int price)
	{
		if (_locked)
			return false;
		if (!Validate())
		...
		for (ItemRequest item : items)
		{
			...
			L2ItemInstance oldItem = _owner.checkItemManipulation(
					item.getObjectId(), item.getCount(), "sell");
			if (oldItem == null)
			{
				Lock();
				return false;
			...
			L2ItemInstance newItem = ownerInventory.transferItem("PrivateStore",
					item.getObjectId(), item.getCount(), playerInventory,
					_owner, player);
			if (newItem == null)
				return false;
			...
			if (newItem.isStackable())
			{
				SystemMessage msg = new SystemMessage(
						SystemMessage.S1_PURCHASED_S3_S2_s);
				...
				msg = new SystemMessage(
						SystemMessage.PURCHASED_S3_S2_s_FROM_S1);
	 ...
	 * @return : boolean true if success
	 */
	public synchronized boolean PrivateStoreSell(L2PcInstance player,
			ItemRequest[] items, int price)
	{
		if (_locked)
			return false;
		...
		// we must check item are available before begining transaction, TODO:
		// should we remove that check when transfering items as it's done here?
		// (there might be synchro problems if player clicks fast if we remove
		// it)
		for (ItemRequest item : items)
		{
			// Check if requested item is available for manipulation
			L2ItemInstance oldItem = player.checkItemManipulation(
					item.getObjectId(), item.getCount(), "sell");
			if (oldItem == null)
				return false;
		}
		...
		for (ItemRequest item : items)
		{
			...
			// Proceed with item transfer
			L2ItemInstance newItem = playerInventory.transferItem(
					"PrivateStore", item.getObjectId(), item.getCount(),
					ownerInventory, player, _owner);
			if (newItem == null)
				return false;
			...
			{
				SystemMessage msg = new SystemMessage(
						SystemMessage.PURCHASED_S2_FROM_S1);
				msg.addString(player.getName());
				msg.addItemName(newItem.getItemId());
				_owner.sendPacket(msg);
