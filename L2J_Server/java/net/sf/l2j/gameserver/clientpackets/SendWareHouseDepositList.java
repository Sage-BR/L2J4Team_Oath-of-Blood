...
public class SendWareHouseDepositList extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(SendWareHouseDepositList.class.getName());
	...
	void runImpl()
	{
		...
		if ((manager == null || !player.isInsideRadius(manager,
				L2NpcInstance.INTERACTION_DISTANCE, false, false))
				&& !player.isGM())
			return;

		if ((warehouse instanceof PcFreight
				|| warehouse instanceof ClanWarehouse)
				&& Config.GM_DISABLE_TRANSACTION
		...
		int fee = _count * (warehouse instanceof PcFreight
				? Config.ALT_GAME_FREIGHT_PRICE
				: 30);
		...
		for (int i = 0; i < _count; i++)
		{
			...
			if (item == null)
			{
				_log.warning("Error depositing a warehouse object for char "
						+ player.getName());
				_items[i * 2 + 0] = 0;
				_items[i * 2 + 1] = 0;
				continue;
		...
		if (currentAdena < fee || !player.reduceAdena("Warehouse", fee,
		...
		InventoryUpdate playerIU = Config.FORCE_INVENTORY_UPDATE ? null
				: new InventoryUpdate();
		for (int i = 0; i < _count; i++)
		{
			int objectId = _items[i * 2 + 0];
			...
			L2ItemInstance oldItem = player.getInventory()
			...
			L2ItemInstance newItem = player.getInventory().transferItem(
					"Warehouse", objectId, count, warehouse, player,
