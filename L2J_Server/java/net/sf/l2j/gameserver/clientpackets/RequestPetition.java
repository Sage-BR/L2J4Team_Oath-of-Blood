	...
	void runImpl()
	{
		...
		if (!GmListTable.getInstance().isGmOnline())
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.NO_GM_PROVIDING_SERVICE_NOW));
			return;
		}
		...
		if (PetitionManager.getInstance()
				.getPendingPetitionCount() == Config.MAX_PETITIONS_PENDING)
		{
			activeChar.sendPacket(new SystemMessage(602));
			return;
		}

		int totalPetitions = PetitionManager.getInstance()
				.getPlayerTotalPetitionCount(activeChar) + 1;
		...
		int petitionId = PetitionManager.getInstance()
				.submitPetition(activeChar, _content, _type);
