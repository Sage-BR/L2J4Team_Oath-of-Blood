...
public class Disablers implements ISkillHandler
{
	protected SkillType[] _skillIds =
		{ L2Skill.SkillType.STUN, L2Skill.SkillType.ROOT,
				...
				L2Skill.SkillType.ERASE, L2Skill.SkillType.MAGE_BANE,
				L2Skill.SkillType.WARRIOR_BANE, L2Skill.SkillType.BETRAY };
			...
			L2Object[] targets)
	{
		...
		if (activeChar instanceof L2PcInstance)
		{
			if (weaponInst == null && skill.isOffensive())
			{
				...
				sm2.addString(
						"You must equip a weapon before casting a spell.");
		...
		if (weaponInst != null)
		{
					...
					.getChargedSpiritshot() == L2ItemInstance.CHARGED_SPIRITSHOT)
			{
				...
				if (skill.getId() != 1020) // vitalize
					...
					.getChargedSoulshot() == L2ItemInstance.CHARGED_SOULSHOT)
			{
				ss = true;
				if (skill.getId() != 1020) // vitalize
		...
		for (L2Object target2 : targets)
		{
			...
			case BETRAY:
			{
				if (Formulas.getInstance().calcSkillSuccess(activeChar, target,
						skill, false, sps, bss))
			...
			case AGGREDUCE:
			{
				...
				if (target instanceof L2Attackable)
				{
					...
					double aggdiff = ((L2Attackable) target)
							.getHating(activeChar)
							- target.calcStat(
									Stats.AGGRESSION, ((L2Attackable) target)
											.getHating(activeChar),
									target, skill);

					if (skill.getPower() > 0)
						target.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION,
								null, -(int) skill.getPower());
					else if (aggdiff > 0)
						target.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION,
								null, -(int) aggdiff);
				}
			...
			case AGGREDUCE_CHAR:
			{
						...
						skill, false, sps, bss))
				{
					if (target instanceof L2Attackable)
					{
						target.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION,
								activeChar,
								-((L2Attackable) target).getHating(activeChar));
					}
					skill.getEffects(activeChar, target);
				} else
				{
			...
			case AGGREMOVE:
			{
				...
				if (target instanceof L2Attackable)
				{
					...
					{
						if (skill
								.getTargetType() == L2Skill.SkillTargetType.TARGET_UNDEAD)
						{
							if (target.isUndead())
								target.getAI().notifyEvent(
										CtrlEvent.EVT_AGGRESSION, null,
										-((L2Attackable) target).getHating(
												((L2Attackable) target)
														.getMostHated()));
						} else
							target.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION,
			...
			case ERASE:
			{
				if (Formulas.getInstance().calcSkillSuccess(activeChar, target,
						...
						&& !(((L2Summon) target)
								.getNpcId() == L2Summon.SIEGE_GOLEM_ID)
						&& !(((L2Summon) target)
								.getNpcId() == L2Summon.HOG_CANNON_ID))
				{
			...
			case MAGE_BANE:
			{
				for (L2Object t : targets)
				{
					...
					for (L2Effect e : effects)
					{
						...
						{
							if (f._stat == Stats.MAGIC_ATTACK
									|| f._stat == Stats.MAGIC_ATTACK_SPEED)
							{
								e.exit();
								break;
							}
			...
			case WARRIOR_BANE:
			{
				for (L2Object t : targets)
				{
					...
					for (L2Effect e : effects)
					{
						for (Func f : e.getStatFuncs())
						{
							if (f._stat == Stats.RUN_SPEED
									|| f._stat == Stats.POWER_ATTACK_SPEED)
							{
								e.exit();
								break;
							}
			...
			case NEGATE:
			{
				...
				if (skill.getId() == 1056)
				{
					...
					landrate *= (100 - activeChar.calcStat(Stats.CANCEL_RES, 0,
							target, null)) / 100;
					...
					if (Rnd.get(100) < landrate)
					{
						...
						for (L2Effect e : effects)
						{
							...
							{
								if (e.getSkill()
										.getSkillType() != SkillType.BUFF) // sleep,
																			// slow,
																			// surrenders
																			// etc
									e.exit();
				...
				else if (skill.getId() == 2275)
				{
					...
					negateEffect(target, SkillType.BUFF, _negatePower,
							_negateId);
				}
				...
				else
				{
					...
					for (String stat : _negateStats)
					{
						...
						if (stat == "poison")
							negateEffect(target, SkillType.POISON,
									_negatePower);
						if (stat == "bleed")
						...
						if (stat == "heal")
						{
							ISkillHandler Healhandler = SkillHandler
									.getInstance()
									.getSkillHandler(SkillType.HEAL);
							if (Healhandler == null)
							{
								_log.severe(
										"Couldn't find skill handler for HEAL.");
								continue;
							}
			...
			int skillId)
	{
		...
		for (L2Effect e : effects)
			...
			{
				if (e.getSkill().getSkillType() == type
						|| (e.getSkill().getEffectType() != null
								&& e.getSkill().getEffectType() == type))
				{
					if (skillId != 0)
					{
