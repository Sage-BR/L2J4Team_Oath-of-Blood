...
abstract class DocumentBase
{
	...
	{
		Stats stat = Stats.valueOfXml(
				n.getAttributes().getNamedItem("stat").getNodeValue());
		String order = n.getAttributes().getNamedItem("order").getNodeValue();
		Lambda lambda = getLambda(n, template);
		int ord = getNumber(order, template).intValue();
		Condition applayCond = parseCondition(n.getFirstChild(), template);
		FuncTemplate ft = new FuncTemplate(attachCond, applayCond, name, stat,
				ord, lambda);
	...
	protected void attachLambdaFunc(Node n, Object template, LambdaCalc calc)
	{
		...
		name = sb.toString();
		Lambda lambda = getLambda(n, template);
		FuncTemplate ft = new FuncTemplate(null, null, name, null,
				calc._funcs.length, lambda);
	...
	protected void attachEffect(Node n, Object template, Condition attachCond)
	{
		...
		if (attrs.getNamedItem("count") != null)
			count = getNumber(attrs.getNamedItem("count").getNodeValue(),
					template).intValue();
		if (attrs.getNamedItem("time") != null)
			time = getNumber(attrs.getNamedItem("time").getNodeValue(),
					template).intValue();
		else
		...
		{
			if (getNumber(attrs.getNamedItem("self").getNodeValue(), template)
					.intValue() == 1)
				self = true;
		}
		Lambda lambda = getLambda(n, template);
		...
		if (attrs.getNamedItem("stackOrder") != null)
			stackOrder = getNumber(
					attrs.getNamedItem("stackOrder").getNodeValue(), template)
							.floatValue();
		EffectTemplate lt = new EffectTemplate(attachCond, applayCond, name,
				lambda, count, time, abnormal, stackType, stackOrder);
		parseTemplate(n, lt);
		if (template instanceof L2Item)
	...
	protected void attachSkill(Node n, Object template, Condition attachCond)
	{
		...
		if (attrs.getNamedItem("id") != null)
			id = getNumber(attrs.getNamedItem("id").getNodeValue(), template)
					.intValue();
		if (attrs.getNamedItem("lvl") != null)
			lvl = getNumber(attrs.getNamedItem("lvl").getNodeValue(), template)
					.intValue();
		...
		if (attrs.getNamedItem("chance") != null)
		{
			if (template instanceof L2Weapon || template instanceof L2Item)
				skill.attach(new ConditionGameChance(
						getNumber(attrs.getNamedItem("chance").getNodeValue(),
								template).intValue()),
						true);
			else
				skill.attach(new ConditionGameChance(
		...
		if (template instanceof L2Weapon)
		{
			if (attrs.getNamedItem("onUse") != null
					|| (attrs.getNamedItem("onCrit") == null
							&& attrs.getNamedItem("onCast") == null))
				((L2Weapon) template).attach(skill); // Attach as skill
														// triggered on use
			if (attrs.getNamedItem("onCrit") != null)
				((L2Weapon) template).attachOnCrit(skill); // Attach as skill
															// triggered on
															// critical hit
			if (attrs.getNamedItem("onCast") != null)
				((L2Weapon) template).attachOnCast(skill); // Attach as skill
															// triggered on cast
		} else if (template instanceof L2Item)
		{
			((L2Item) template).attach(skill); // Attach as skill triggered on
												// use
		}
	}
	...
	protected Condition parsePlayerCondition(Node n)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			} else if ("resting".equalsIgnoreCase(a.getNodeName()))
			{
				...
				cond = joinAnd(cond, new ConditionPlayerState(
						CheckPlayerState.RESTING, val));
			} else if ("flying".equalsIgnoreCase(a.getNodeName()))
			{
				boolean val = Boolean.valueOf(a.getNodeValue());
				cond = joinAnd(cond,
						new ConditionPlayerState(CheckPlayerState.FLYING, val));
			} else if ("moving".equalsIgnoreCase(a.getNodeName()))
			{
				boolean val = Boolean.valueOf(a.getNodeValue());
				cond = joinAnd(cond,
						new ConditionPlayerState(CheckPlayerState.MOVING, val));
			} else if ("running".equalsIgnoreCase(a.getNodeName()))
			{
				boolean val = Boolean.valueOf(a.getNodeValue());
				cond = joinAnd(cond, new ConditionPlayerState(
						CheckPlayerState.RUNNING, val));
			} else if ("behind".equalsIgnoreCase(a.getNodeName()))
			{
				boolean val = Boolean.valueOf(a.getNodeValue());
				cond = joinAnd(cond,
						new ConditionPlayerState(CheckPlayerState.BEHIND, val));
	...
	protected Condition parseSkillCondition(Node n)
	{
		NamedNodeMap attrs = n.getAttributes();
		Stats stat = Stats
				.valueOfXml(attrs.getNamedItem("stat").getNodeValue());
	...
	protected Condition parseUsingCondition(Node n)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			} else if ("slotitem".equalsIgnoreCase(a.getNodeName()))
			{
				...
				cond = joinAnd(cond,
						new ConditionSlotItemId(slot, id, enchant));
			}
		}
		if (cond == null)
	...
	protected Condition parseGameCondition(Node n)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			if ("night".equalsIgnoreCase(a.getNodeName()))
			{
				boolean val = Boolean.valueOf(a.getNodeValue());
				cond = joinAnd(cond,
						new ConditionGameTime(CheckGameTime.NIGHT, val));
			}
	...
	protected void parseBeanSet(Node n, StatsSet set, Integer level)
	{
		String name = n.getAttributes().getNamedItem("name").getNodeValue()
				.trim();
		String value = n.getAttributes().getNamedItem("val").getNodeValue()
				.trim();
	...
	protected Lambda getLambda(Node n, Object template)
	{
		...
		if (nval != null)
		{
			...
			} else if (val.charAt(0) == '$')
			{
				...
				if (field != null)
					return new LambdaConst(
							getNumber(field, template).doubleValue());
				// failed
	...
	protected Number getNumber(String value, Object template)
	{
		...
		if (value.indexOf('.') == -1)
		{
			int radix = 10;
			if (value.length() > 2
					&& value.substring(0, 2).equalsIgnoreCase("0x"))
			{
