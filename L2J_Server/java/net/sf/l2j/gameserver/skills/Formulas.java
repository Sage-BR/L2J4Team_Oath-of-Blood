...
public final class Formulas
{
	...
	// These values are 100% matching retail tables, no need to change and no
	// need add
	// calculation into the stat bonus when accessing (not efficient),
	// better to have everything precalculated and use values directly (saves
	// CPU)
	static
	{
		for (int i = 0; i < STRbonus.length; i++)
			STRbonus[i] = Math.floor(
					Math.pow(STRCompute[0], i - STRCompute[1]) * 100 + .5d)
					/ 100;
		for (int i = 0; i < INTbonus.length; i++)
			INTbonus[i] = Math.floor(
					Math.pow(INTCompute[0], i - INTCompute[1]) * 100 + .5d)
					/ 100;
		for (int i = 0; i < DEXbonus.length; i++)
			DEXbonus[i] = Math.floor(
					Math.pow(DEXCompute[0], i - DEXCompute[1]) * 100 + .5d)
					/ 100;
		for (int i = 0; i < WITbonus.length; i++)
			WITbonus[i] = Math.floor(
					Math.pow(WITCompute[0], i - WITCompute[1]) * 100 + .5d)
					/ 100;
		for (int i = 0; i < CONbonus.length; i++)
			CONbonus[i] = Math.floor(
					Math.pow(CONCompute[0], i - CONCompute[1]) * 100 + .5d)
					/ 100;
		for (int i = 0; i < MENbonus.length; i++)
			MENbonus[i] = Math.floor(
					Math.pow(MENCompute[0], i - MENCompute[1]) * 100 + .5d)
					/ 100;
	}
	...
	static class FuncMultRegenResting extends Func
	{
		...
		private FuncMultRegenResting(Stats stat)
		{
			super(stat, 0x20, null);
			setCondition(
					new ConditionPlayerState(CheckPlayerState.RESTING, true));
		}
	...
	static class FuncMDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			if (env._player instanceof L2PcInstance)
			{
				...
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_LFINGER) != null)
					env.value -= 5;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_RFINGER) != null)
					env.value -= 5;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_LEAR) != null)
					env.value -= 9;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_REAR) != null)
					env.value -= 9;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_NECK) != null)
					env.value -= 13;
			}
			env.value *= MENbonus[env._player.getMEN()]
					* env._player.getLevelMod();
		}
	}
	...
	static class FuncPDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			if (env._player instanceof L2PcInstance)
			{
				L2PcInstance p = (L2PcInstance) env._player;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_HEAD) != null)
					env.value -= 12;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_CHEST) != null)
					env.value -= ((p.getClassId().isMage()) ? 15 : 31);
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_LEGS) != null)
					env.value -= ((p.getClassId().isMage()) ? 8 : 18);
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_GLOVES) != null)
					env.value -= 8;
				if (p.getInventory()
						.getPaperdollItem(Inventory.PAPERDOLL_FEET) != null)
					env.value -= 7;
			}
			env.value *= env._player.getLevelMod();
		}
		...
		public void calc(Env env)
		{
			...
			if (p instanceof L2Summon)
				env.value = 40;
					...
					&& p.getActiveWeaponInstance() == null)
				env.value = 40;
			else
			{
	...
	public Calculator[] getStdNPCCalculators()
	{
		...
		std[Stats.ACCURACY_COMBAT.ordinal()]
				.addFunc(FuncAtkAccuracy.getInstance());
	...
	public final double calcHpRegen(L2Character cha)
	{
		...
		double hpRegenMultiplier = cha.isRaid()
				? Config.RAID_HP_REGEN_MULTIPLIER
				: Config.HP_REGEN_MULTIPLIER;
		...
		if (cha instanceof L2PcInstance)
		{
			...
			init += (player.getLevel() > 10) ? ((player.getLevel() - 1) / 10)
					: 0.5;
			...
			if (player.getIsInClanHall())
			{
				...
				if (clanHallIndex > 0)
				{
					...
					if (clansHall != null)
						if (clansHall
								.getFunction(ClanHall.FUNC_RESTORE_HP) != null)
							hpRegenMultiplier *= 1 + clansHall
		...
		return cha.calcStat(Stats.REGENERATE_HP_RATE, init, null, null)
				* hpRegenMultiplier + hpRegenBonus;
	}
	...
	public final double calcMpRegen(L2Character cha)
	{
		...
		double mpRegenMultiplier = cha.isRaid()
				? Config.RAID_MP_REGEN_MULTIPLIER
				: Config.MP_REGEN_MULTIPLIER;
		...
		if (cha instanceof L2PcInstance)
		{
			...
			if (player.getIsInClanHall())
			{
				...
				if (clanHallIndex > 0)
				{
					...
					if (clansHall != null)
						if (clansHall
								.getFunction(ClanHall.FUNC_RESTORE_MP) != null)
							mpRegenMultiplier *= 1 + clansHall
									.getFunction(ClanHall.FUNC_RESTORE_MP)
									.getLvl() / 100;
				}
			}
		...
		return cha.calcStat(Stats.REGENERATE_MP_RATE, init, null, null)
				* mpRegenMultiplier + mpRegenBonus;
	}
	...
	public final double calcCpRegen(L2Character cha)
	{
		...
		return cha.calcStat(Stats.REGENERATE_CP_RATE, init, null, null)
				* cpRegenMultiplier + cpRegenBonus;
	}

	@SuppressWarnings("deprecation")
	public final double calcFestivalRegenModifier(L2PcInstance activeChar)
	{
		final int[] festivalInfo = SevenSignsFestival.getInstance()
				.getFestivalForPlayer(activeChar);
		final int oracle = festivalInfo[0];
		final int festivalId = festivalInfo[1];
		int[] festivalCenter;

		// If the player isn't found in the festival, leave the regen rate as it
		// is.
		if (festivalId < 0)
			return 0;

		// Retrieve the X and Y coords for the center of the festival arena the
		// player is in.
		...
		// Check the distance between the player and the player spawn point, in
		// the center of the arena.
		double distToCenter = activeChar.getDistance(festivalCenter[0],
				festivalCenter[1]);

		if (Config.DEBUG)
			_log.info("Distance: " + distToCenter + ", RegenMulti: "
					+ (distToCenter * 2.5) / 50);

		return 1.0 - (distToCenter * 0.0005); // Maximum Decreased Regen of ~
												// -65%;
	}
	...
	public final double calcSiegeRegenModifer(L2PcInstance activeChar)
	{
		...
		Siege siege = SiegeManager.getInstance().getSiege(
				activeChar.getPosition().getX(),
				activeChar.getPosition().getY());
		if (siege == null || !siege.getIsInProgress())
			return 0;

		L2SiegeClan siegeClan = siege
				.getAttackerClan(activeChar.getClan().getClanId());
		if (siegeClan == null || siegeClan.getFlag().size() == 0
				|| !Util.checkIfInRange(200, activeChar,
						siegeClan.getFlag().get(0), true))
			return 0;
	 ...
	 */
	public final double calcPhysDam(L2Character attacker, L2Character target,
			L2Skill skill, boolean shld, boolean crit, boolean dual, boolean ss)
	{
		if (attacker instanceof L2PcInstance)
		{
			L2PcInstance pcInst = (L2PcInstance) attacker;
			if (pcInst.isGM()
					&& pcInst.getAccessLevel() < Config.GM_CAN_GIVE_DAMAGE)
				return 0;
		}
		...
		if (skill != null)
		{
			...
			// damage += skill.getPower() * 0.7 *
			// attacker.getPAtk(target)/defence;
		}
		// In C5 summons make 10 % less dmg in PvP.
		if (attacker instanceof L2Summon && target instanceof L2PcInstance)
			damage *= 0.9;

		// damage = damage * attacker.getSTR()*(1 -
		// attacker.getLevel()/100)/60*1.15;
		if (target instanceof L2NpcInstance)
		{
			Integer resistPAtk = ((L2NpcInstance) target).getTemplate()
					.getResist(Stats.POWER_DEFENCE);
		...
		if (weapon != null)
		{
			...
			{
			case BOW:
				defence = target.calcStat(Stats.BOW_WPN_RES, defence, target,
						null);
				if (target instanceof L2NpcInstance)
				{
					Integer resistBow = ((L2NpcInstance) target).getTemplate()
							.getResist(Stats.BOW_WPN_RES);
					damage *= resistBow.doubleValue() / 100;
				}
				break;
			case BLUNT:
			case BIGBLUNT:
				defence = target.calcStat(Stats.BLUNT_WPN_RES, defence, target,
						null);
				if (target instanceof L2NpcInstance)
				{
					Integer resistBlunt = ((L2NpcInstance) target).getTemplate()
							.getResist(Stats.BLUNT_WPN_RES);
					damage *= resistBlunt.doubleValue() / 100;
				}
				break;
			case DAGGER:
				defence = target.calcStat(Stats.DAGGER_WPN_RES, defence, target,
						null);
				if (target instanceof L2NpcInstance)
				{
					Integer resistDagger = ((L2NpcInstance) target)
							.getTemplate().getResist(Stats.DAGGER_WPN_RES);
					damage *= resistDagger.doubleValue() / 100;
				}
				break;
			case DUAL:
				defence = target.calcStat(Stats.DUAL_WPN_RES, defence, target,
						null);
				break;
			case DUALFIST:
				defence = target.calcStat(Stats.DUALFIST_WPN_RES, defence,
						target, null);
				break;
			case ETC:
				defence = target.calcStat(Stats.ETC_WPN_RES, defence, target,
						null);
				break;
			case FIST:
				defence = target.calcStat(Stats.FIST_WPN_RES, defence, target,
						null);
				break;
			case POLE:
				defence = target.calcStat(Stats.POLE_WPN_RES, defence, target,
						null);
				break;
			case SWORD:
				defence = target.calcStat(Stats.SWORD_WPN_RES, defence, target,
						null);
				break;
		...
		// Sami: These values are a quick fix to balance dagger gameplay and
		// give
		...
		if (target instanceof L2PcInstance && weapon != null
				&& weapon.getItemType() == L2WeaponType.DAGGER && skill != null)
		{
		...
		{
			if (skill == null)
				damage *= attacker.calcStat(Stats.PVP_PHYSICAL_DMG, 1, null,
						null);
			else
				damage *= attacker.calcStat(Stats.PVP_PHYS_SKILL_DMG, 1, null,
						null);
		}
	...
	public final double calcMagicDam(L2Character attacker, L2Character target,
			L2Skill skill, boolean ss, boolean bss, boolean mcrit)
	{
		if (attacker instanceof L2PcInstance)
		{
			L2PcInstance pcInst = (L2PcInstance) attacker;
		...
		// if(attacker instanceof L2PcInstance && target instanceof
		// L2PcInstance) damage *= 0.9; // PvP modifier (-10%)

		// Failure calculation
		if (Config.ALT_GAME_MAGICFAILURES
				&& !calcMagicSuccess(attacker, target, skill))
		{
			if (attacker instanceof L2PcInstance)
			{
						...
						&& (target.getLevel() - attacker.getLevel()) <= 9)
				{
					if (skill.getSkillType() == SkillType.DRAIN)
						attacker.sendPacket(new SystemMessage(
								SystemMessage.DRAIN_HALF_SUCCESFUL));
					else
						attacker.sendPacket(
								new SystemMessage(SystemMessage.ATTACK_FAILED));

					damage /= 2;
				} else
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.S1_WAS_UNAFFECTED_BY_S2);
			...
			if (target instanceof L2PcInstance)
			{
				...
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.RESISTED_S1_DRAIN);
					sm.addString(attacker.getName());
					target.sendPacket(sm);
				} else
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.RESISTED_S1_MAGIC);
					sm.addString(attacker.getName());
					target.sendPacket(sm);
				}
			}
		...
		{
			if (skill.isMagic())
				damage *= attacker.calcStat(Stats.PVP_MAGICAL_DMG, 1, null,
						null);
			else
				damage *= attacker.calcStat(Stats.PVP_PHYS_SKILL_DMG, 1, null,
						null);
	...
	L2Character target, double rate)
	{
		...
		if (rate < 2)
			...
			return (int) (460000 / rate);
	}

	/** Calculate delay (in milliseconds) for skills cast */
	public final int calcMAtkSpd(L2Character attacker,
	...
	/** Calculate delay (in milliseconds) for skills cast */
	public final int calcMAtkSpd(L2Character attacker, L2Skill skill,
			double skillTime)
	{
		if (skill.isMagic())
			return (int) (skillTime * 333 / attacker.getMAtkSpd());
	...
	public boolean calcMagicAffected(L2Character actor, L2Character target,
			L2Skill skill)
	{
		SkillType type = skill.getSkillType();
		if (target.isRaid() && (type == SkillType.CONFUSION
				|| type == SkillType.MUTE || type == SkillType.PARALYZE
				|| type == SkillType.ROOT || type == SkillType.FEAR
				|| type == SkillType.SLEEP || type == SkillType.STUN
				|| type == SkillType.DEBUFF || type == SkillType.AGGDEBUFF))
			return false; // these skills should have only 1/1000 chance on
							// raid, now it's 0.
	...
	public boolean calcSkillSuccessOld(L2Character player, L2Character target,
			L2Skill skill, boolean ss, boolean sps, boolean bss)
	{
		...
		case STUN:
			...
			value = 4800 + (int) (50 * (player.getLevel() - target.getLevel())
					+ 5100 * ((float) skill.getLevel() / maxLevel));
			if (pDef > 0)
				value /= pDef;
		...
		case CONFUSION:
			...
			// _log.fine(player.getName()+"
			// matk:"+mAtk+",mdef="+mDef+",value="+value+",modifier="+modifier+",maxlevel="+maxLevel+",level="+skill.getLevel());
			break;
		case MUTE:
			mAtk = player.getMAtk(target, skill);
			mDef = target.getMDef(player, skill);
		...
		default:
			...
			pDef = 1;
			pDef = target.calcStat(Stats.STUN_RES, pDef, target, null);
			value = 5000 + (int) (50 * (player.getLevel() - target.getLevel())
					+ 5000 * ((float) skill.getLevel() / maxLevel));
			if (pDef > 0)
				value /= pDef;
			modifier = 10 * target.getCON() / 3;
			if (modifier > 0)
			...
			// pDef = target.calcStat(Stats.STUN_RES,pDef,target,null);
			// value = 30 * 100 + (int)(70 * 100 *
			// ((float)skill.getLevel()/maxLevel));
			// value = 100 * 100;
			// if (pDef > 0 && pAtk > 0)
			// value *= 0.6 * pAtk/pDef;
		...
		if (rate == 0)
			rate = (int) (((player.getLevel() - target.getLevel())
					+ (int) (value / modifier)) * ssmodifier);
			...
			L2Skill skill, boolean ss, boolean sps, boolean bss)
	{
		...
		// Uncomment this if you want to revert to old skill success
		// calculation:
		...
		if (type == SkillType.PDAM || type == SkillType.MDAM) // For additional
																// effects on
																// PDAM skills
																// (like STUN,
																// SHOCK,...)
		...
		if (value == 0)
			value = (type == SkillType.PARALYZE) ? 50 : 80;
		if (lvlDepend == 0)
			lvlDepend = (type == SkillType.PARALYZE) ? 1 : 2;
		...
		int lvlmodifier = ((skill.getMagicLevel() > 0 ? skill.getMagicLevel()
				: attacker.getLevel()) - target.getLevel()) * lvlDepend;
		...
		if (Config.DEVELOPER)
			System.out.println(skill.getName() + ": " + value + ", "
					+ statmodifier + ", " + lvlmodifier + ", " + resmodifier
					+ ", "
					+ ((int) (Math
							.pow((double) attacker.getMAtk(target, skill)
	...
	public boolean calcMagicSuccess(L2Character attacker, L2Character target,
			L2Skill skill)
	{
		double lvlDifference = (target.getLevel()
				- (skill.getMagicLevel() > 0 ? skill.getMagicLevel()
						: attacker.getLevel()));
	...
	public boolean calcAltSkillSuccess(L2Character activeChar,
			L2Character target, L2Skill skill)
	{
		...
		int baseRate = (attackerLevel - targetLevel) + powerModifier
				+ levelModifier;
		int rate = baseRate;
		int check = Rnd.get(100);
		...
		case PARALYZE:
		case SLEEP:
			rate += WITModifier; // uses WIT Modifier for SLEEP and PARALYZE
									// types
			break;
		default:
			rate += CONModifier; // uses CON Modifier for any other types (like
									// PDAM ones)
			break;
		}

		if (rate > 100)
			rate = 100; // We shouldn't have more than 100% success rate
		if (rate < 1)
			rate = 1; // We shouldn't have less than 1% success rate
	...
	public double calcManaDam(L2Character attacker, L2Character target,
			L2Skill skill, boolean ss, boolean bss)
	{
		...
		double damage = (Math.sqrt(mAtk) * skill.getPower(attacker) * (mp / 97))
				/ mDef;

		return damage;
	}

	public double calculateSkillResurrectRestorePercent(
			double baseRestorePercent, int casterWIT)
	{
