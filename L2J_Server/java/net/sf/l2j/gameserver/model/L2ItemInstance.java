...
public final class L2ItemInstance extends L2Object
{
	private static final Logger _log = Logger
			.getLogger(L2ItemInstance.class.getName());
	...
	public static final int CHARGED_BLESSED_SOULSHOT = 2; // It's a realy
															// exists? ;-)
	 ...
	 */
	public void setOwnerId(String process, int owner_id, L2PcInstance creator,
			L2Object reference)
	{
		setOwnerId(owner_id);
	 ...
	 *            : ItemLocation (enumeration)
	 * @param loc_data
	 *            : int designating the slot where the item is stored or the
	 *            village for freights
	 */
	 ...
	 *            item or previous item in transformation
	 */
	public void changeCount(String process, int count, L2PcInstance creator,
			L2Object reference)
	{
		if (count == 0)
			return;
	...
	{
		if (Config.ASSERT)
			assert _loc == ItemLocation.PAPERDOLL
					|| _loc == ItemLocation.PET_EQUIP
					|| _loc == ItemLocation.FREIGHT;
		return _loc_data;
	}
	...
	{
		return (isConsumable()
				? (int) (_price_sell * Config.RATE_CONSUMABLE_COST)
				: _price_sell);
	}
	...
	{
		return (isConsumable()
				? (int) (_price_buy * Config.RATE_CONSUMABLE_COST)
				: _price_buy);
	}
	...
	public boolean isAvailable(L2PcInstance player, boolean allowAdena)
	{
		return ((!isEquipped()) // Not equipped
				...
				&& (getItem().getType2() != 4 || getItem().getType1() != 1) // TODO:
																			// what
																			// does
																			// this
																			// mean?
				&& (player.getPet() == null
						|| getObjectId() != player.getPet().getControlItemId()) // Not
																				// Control
																				// item
																				// of
																				// currently
																				// summoned
																				// pet
				&& (player.getActiveEnchantItem() != this) // Not momentarily
															// used enchant
															// scroll
				&& (allowAdena || getItemId() != 57)
				&& (player.getCurrentSkill() == null || player.getCurrentSkill()
						.getSkill().getItemConsumeId() != getItemId())
				&& (isTradeable()));
	}
	 ...
	 *
	 * @see
	 * net.sf.l2j.gameserver.model.L2Object#onAction(net.sf.l2j.gameserver.model
	 * .L2PcInstance) also check constraints: only soloing castle owners may
	 * pick up mercenary tickets of their castle
	 */
	@Override
	public void onAction(L2PcInstance player)
	{
		// this causes the validate position handler to do the pickup if the
		// location is reached.
		// mercenary tickets can only be picked up by the castle owner.
		if ((_itemId >= 3960 && _itemId <= 4021 && player.isInParty())
				|| (_itemId >= 3960 && _itemId <= 3969
						&& !player.isCastleLord(1))
				|| (_itemId >= 3973 && _itemId <= 3982
						&& !player.isCastleLord(2))
				|| (_itemId >= 3986 && _itemId <= 3995
						&& !player.isCastleLord(3))
				|| (_itemId >= 3999 && _itemId <= 4008
						&& !player.isCastleLord(4))
				|| (_itemId >= 4012 && _itemId <= 4021
						&& !player.isCastleLord(5))
				|| (_itemId >= 5205 && _itemId <= 5214
						&& !player.isCastleLord(6))
				|| (_itemId >= 6779 && _itemId <= 6788
						&& !player.isCastleLord(7))
				|| (_itemId >= 7973 && _itemId <= 7982
						&& !player.isCastleLord(8))
				|| (_itemId >= 7918 && _itemId <= 7927
						&& !player.isCastleLord(9)))
		{
			if (player.isInParty()) // do not allow owner who is in party to
									// pick tickets up
				player.sendMessage(
						"You cannot pickup mercenaries while in a party.");
			else
				player.sendMessage(
						"Only the castle lord can pickup mercenaries.");
			...
			player.getAI().setIntention(CtrlIntention.AI_INTENTION_PICK_UP,
					this);
	}
	 ...
	 *
	 * @param type
	 *            : int (CHARGED_NONE, CHARGED_SPIRITSHOT,
	 *            CHARGED_BLESSED_SPIRITSHOT)
	 */
	...
	public void updateDatabase()
	{
		...
		if (_existsInDb)
		{
			if (_owner_id == 0 || _loc == ItemLocation.VOID
					|| (_count == 0 && _loc != ItemLocation.LEASE))
				removeFromDb();
			else
				updateInDb();
		} else
		{
			if ((_count == 0 && _loc != ItemLocation.LEASE)
					|| _loc == ItemLocation.VOID || _owner_id == 0)
				return;
			insertIntoDb();
		}
	}
	...
	public static L2ItemInstance restoreFromDb(int objectId)
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT owner_id, object_id, item_id, count, enchant_level, loc, loc_data, price_sell, price_buy, custom_type1, custom_type2 FROM items WHERE object_id = ?");
			...
			if (rs.next())
			{
				...
				if (item == null)
				{
					_log.severe("Item item_id=" + item_id
							+ " not known, object_id=" + objectId);
					return null;
				}
		...
		} catch (Exception e)
		{
			_log.log(Level.SEVERE,
					"Could not restore item " + objectId + " from DB:", e);
		} finally
		{
			try
			{
	 ...
	 * object to _allObjects of L2World </B></FONT><BR>
	 * <BR>
	 *
	 * <B><U> Assert </U> :</B><BR>
	 * <BR>
	...
	public final void dropMe(L2Character dropper, int x, int y, int z)
	{
		...
		synchronized (this)
		{
			// Set the x,y,z position of the L2ItemInstance dropped and update
			// its _worldregion
			setIsVisible(true);
			getPosition().setWorldPosition(x, y, z);
			getPosition().setWorldRegion(L2World.getInstance()
					.getRegion(getPosition().getWorldPosition()));
		...
		L2World.getInstance().addVisibleObject(this,
				getPosition().getWorldRegion(), dropper);
	...
	private void updateInDb()
	{
		...
		{
			_log.log(Level.SEVERE, "Could not update item " + getObjectId()
					+ " in DB: Reason: " + "Duplicate itemId");
		} finally
		{
			try
			{
	...
	private void insertIntoDb()
	{
		...
		{
			_log.log(Level.SEVERE, "Could not insert item " + getObjectId()
					+ " into DB: Reason: " + "Duplicate itemId");
		} finally
		{
			try
			{
	...
	private void removeFromDb()
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("DELETE FROM items WHERE object_id=?");
		...
		{
			_log.log(Level.SEVERE,
					"Could not delete item " + getObjectId() + " in DB:", e);
		} finally
		{
			try
			{
