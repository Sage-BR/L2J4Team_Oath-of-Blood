 ...
 * sample
 *
 * 27 00 00 01 00 // item count
 *
 * 04 00 // itemType1 0-weapon/ring/earring/necklace 1-armor/shield
 * 4-item/questitem/adena c6 37 50 40 // objectId cd 09 00 00 // itemId 05 00 00
 * 00 // count 05 00 // itemType2 0-weapon 1-shield/armor
 * 2-ring/earring/necklace 3-questitem 4-adena 5-item 00 00 // always 0 ?? 00 00
 * // equipped 1-yes 00 00 // slot 0006-lr.ear 0008-neck 0030-lr.finger
 * 0040-head 0080-?? 0100-l.hand 0200-gloves 0400-chest 0800-pants 1000-feet
 * 2000-?? 4000-r.hand 8000-r.hand 00 00 // always 0 ?? 00 00 // always 0 ??
 *
 *
 * format h (h dddhhhh hh) revision 377 format h (h dddhhhd hh) revision 415
 *
...
public class ItemList extends ServerBasePacket
{
	...
	final void runImpl()
	{
		...
		if (Config.DEBUG)
		{
			...
			{
				_log.fine("item:" + temp.getItem().getName() + " type1:"
						+ temp.getItem().getType1() + " type2:"
	...
	final void writeImpl()
	{
		...
		for (L2ItemInstance temp : _items)
		{
			...
			writeD(temp.getItem().getBodyPart()); // rev 415 slot 0006-lr.ear
