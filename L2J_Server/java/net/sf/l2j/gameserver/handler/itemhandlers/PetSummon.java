...
public class PetSummon implements IItemHandler
{
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		L2PetInstance newpet = L2PetInstance.spawnPet(petTemplate, activeChar,
				item);
		if (newpet == null)
			return;
		...
		MagicSkillUser msk = new MagicSkillUser(activeChar, 2046, 1, 1000,
				600000);
		...
		newpet.spawnMe(activeChar.getX() + 50, activeChar.getY() + 100,
				activeChar.getZ());
		...
		ThreadPoolManager.getInstance()
				.scheduleGeneral(new SummonFinalizer(activeChar, newpet), 900);
		...
		if (newpet.getCurrentFed() <= 0)
		{
			...
			ThreadPoolManager.getInstance()
					.scheduleGeneral(new FeedWait(activeChar, newpet), 60000);
		} else
		{
	...
	static class SummonFinalizer implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				// addVisibleObject created the info packets with summon
				// animation
				// if someone comes into range now, the animation shouldnt show
				// any more
