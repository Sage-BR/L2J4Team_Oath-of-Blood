...
public final class L2ChestInstance extends L2Attackable
{
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker)
	{
		...
		{
			id = id + 18265;
			super.doItemDrop(NpcTable.getInstance().getTemplate(id),
					lastAttacker);
		} else
		{
			id = id + 21801;
	...
	private boolean handleCast(L2Character player, int skillId)
	{
		...
		if (player.isDead() || !player.isVisible()
				|| !player.isInsideRadius(this,
						getDistanceToWatchObject(player), false, false))
			return false;
		...
		if (player.getEffect(skill) == null)
		{
			...
			broadcastPacket(new MagicSkillUser(this, player, skill.getId(),
					skillLevel, skill.getSkillTime(), 0));
			return true;
		}
		return false;
	}
