...
public final class TaskManager
{
	...
	public class ExecutedTask implements Runnable
	{
				...
				throws SQLException
		{
			...
			_params = new String[]
				{ rset.getString("param1"), rset.getString("param2"),
		...
		public void run()
		{
			...
			try
			{
				...
				PreparedStatement statement = con
						.prepareStatement(SQL_STATEMENTS[1]);
	...
	private void startAllTasks()
	{
		...
		try
		{
			try
			{
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con
						.prepareStatement(SQL_STATEMENTS[0]);
				ResultSet rset = statement.executeQuery();

				while (rset.next())
				{
					Task task = _tasks.get(rset.getString("task").trim()
					...
					if (type != TYPE_NONE)
					{
						ExecutedTask current = new ExecutedTask(task, type,
								rset);
	...
	private boolean launchTask(ExecutedTask task)
	{
		...
		} else if (type == TYPE_TIME)
		{
			try
			{
				Date desired = DateFormat.getInstance()
						.parse(task.getParams()[0]);
		...
		} else if (type == TYPE_GLOBAL_TASK)
		{
			...
			{
				_log.warning(
						"Task " + task.getId() + " has incorrect parameters");
				return false;
			}
			...
			String param1, String param2, String param3, long lastActivation)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement(SQL_STATEMENTS[2]);
			...
			String param2, String param3, long lastActivation)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement(SQL_STATEMENTS[3]);
			statement.setString(1, task);
			statement.setString(2, type.toString());
			statement.setLong(3, lastActivation);
			statement.setString(4, param1);
