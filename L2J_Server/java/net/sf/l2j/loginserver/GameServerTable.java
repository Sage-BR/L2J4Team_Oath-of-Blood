...
public class GameServerTable
{
	protected static Logger _log = Logger
			.getLogger(GameServerTable.class.getName());
	...
	{
		_gameServerList = new FastList<>();
		load();
		_last_IP_Update = System.currentTimeMillis();
		try
		{
			_keyGen = KeyPairGenerator.getInstance("RSA");
			RSAKeyGenParameterSpec spec = new RSAKeyGenParameterSpec(512,
	...
	public void load()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				// for(int i = 1;id-i > previousID; i++) //fill with dummy
				// servers to keep
				// {
				...
				GameServer gs = new GameServer(
						stringToHex(rset.getString("hexid")), id);
				_gameServerList.add(gs);
				// previousID = id;
				number++;
			}
			_log.info("GameServerTable: Loaded " + number + " servers (max id:"
					+ id + ")");
	...
	private void loadServerNames()
	{
		...
		try
		{
			...
			for (int e = xpp
					.getEventType(); e != XMLStreamConstants.END_DOCUMENT; e = xpp
							.next())
			{
	...
	public void addServer(GameServerThread gst)
	{
		...
		/*
		 * gst.setAuthed(true); _gameServerList.add(gameServer);
		 */
	...
	public void createServer(GameServer gs)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("INSERT INTO gameservers "
					+ "(hexid,server_id,host)" + "values (?,?,?)");
	...
	public ServerList makeServerList(boolean isGM, boolean _internalip)
	{
		...
		{
			if (Config.DEBUG)
				System.out.println("updtime:" + Config.IP_UPDATE_TIME
						+ " , current:" + _last_IP_Update + " so:"
						+ (System.currentTimeMillis() - _last_IP_Update));
			if (System.currentTimeMillis()
					- _last_IP_Update > Config.IP_UPDATE_TIME * 60000)
			{
				if (gs.gst != null)
				{
					gs.gst.setGameHosts(gs.gst.getGameExternalHost(),
							gs.gst.getGameInternalHost());
			...
			if (_internalip)
			{
				...
				sl.addServer(gs.internal_ip, gs.port, gs.pvp, gs.testServer,
						(gs.gst == null ? 0 : gs.gst.getCurrentPlayers()),
						gs.maxPlayers, gs.brackets, gs.clock, status,
						gs.server_id);
			} else
			{
				int status = gs.status;
	...
	private static final Comparator<GameServer> gsComparator = new Comparator<GameServer>()
	{
		...
		{
			return (gs1.server_id < gs2.server_id ? -1
					: gs1.server_id == gs2.server_id ? 0 : 1);
		}
	};
	...
	public class GameServer
	{
		...
		{
			return "GameServer: " + serverNames.get(server_id) + " id:"
					+ server_id + " hex:" + hexToString(hexID) + " ip:" + ip
					+ ":" + port + " status: "
					+ ServerStatus.statusString[status];
		}
