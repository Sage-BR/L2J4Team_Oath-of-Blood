...
//		{
		...
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TELEPORT && (itemId == 736
				|| itemId == 1538 || itemId == 1829 || itemId == 1830
				|| itemId == 3958 || itemId == 5858 || itemId == 5859)
		...
		if (activeChar.isFishing() && (itemId < 6535 || itemId > 6540))
		{
			...
			SystemMessage sm = new SystemMessage(
					SystemMessage.CANNOT_DO_WHILE_FISHING_3);
		...
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_CANNOT_BE_USED);
		...
		{
			SystemMessage sm = new SystemMessage(600); // You cannot equip a pet
														// item.
			sm.addItemName(itemId);
			getClient().getActiveChar().sendPacket(sm);
			sm = null;
		...
		if (item.isEquipable())
		{
			...
			if (activeChar.isMounted() && (bodyPart == L2Item.SLOT_LR_HAND
			...
			if ((activeChar.isStunned() || activeChar.isSleeping()
					|| activeChar.isAttackingNow() || activeChar.isCastingNow()
			...
			/*
			 * Since c5 you can equip weapon again // Don't allow weapon/shield
			 * equipment if wearing formal wear if
			 * (activeChar.isWearingFormalWear() && (bodyPart ==
			 * L2Item.SLOT_LR_HAND || bodyPart == L2Item.SLOT_L_HAND || bodyPart
			 * == L2Item.SLOT_R_HAND)) { SystemMessage sm = new
			 * SystemMessage(SystemMessage.
			 * CANNOT_USE_ITEMS_SKILLS_WITH_FORMALWEAR);
			...
			if (activeChar.isCursedWeaponEquiped()
					&& ((bodyPart == L2Item.SLOT_LR_HAND
							|| bodyPart == L2Item.SLOT_L_HAND
							|| bodyPart == L2Item.SLOT_R_HAND)
							|| itemId == 6408)) // Don't allow to put formal
												// wear
			{
				return;
			}
			...
			L2ItemInstance old = activeChar.getInventory()
					.getPaperdollItem(Inventory.PAPERDOLL_LRHAND);
			if (old == null)
				old = activeChar.getInventory()
			...
			if (isEquiped)
			{
				if (item.getEnchantLevel() > 0)
				{
					sm = new SystemMessage(
							SystemMessage.EQUIPMENT_S1_S2_REMOVED);
				...
				items = activeChar.getInventory()
						.unEquipItemInBodySlotAndRecord(bodyPart);
			} else
			{
				if (item.getEnchantLevel() > 0)
				{
		...
		} else
		{
							...
							|| (itemid >= 8505 && itemid <= 8513)))
			{
				activeChar.getInventory()
						.setPaperdollItem(Inventory.PAPERDOLL_LHAND, item);
			...
			} else
			{
				...
				if (handler == null)
					_log.warning("No item handler registered for item ID "
							+ item.getItemId() + ".");
				else
