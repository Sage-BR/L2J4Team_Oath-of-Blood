...
//	    L2DoorInstance[] currentDoors = getDoors();
	...
	private void parseData()
	{
		...
		try
		{
			...
			while ((line = lnr.readLine()) != null)
			{
				...
				ClanHall clanhall = ClanHallManager.getInstance()
						.getClanHall(door.getX(), door.getY(), 1500);
				if (clanhall != null)
				{
					clanhall.getDoors().add(door);
					door.setClanHall(clanhall);
					if (Config.DEBUG)
						_log.warning("door " + door.getDoorName()
								+ " attached to ch " + clanhall.getName());
				}
			}

			_log.config("DoorTable: Loaded " + _staticItems.size()
					+ " Door Templates.");
	...
	public static L2DoorInstance parseList(String line)
	{
		...
		L2DoorInstance door = new L2DoorInstance(
				IdFactory.getInstance().getNextId(), template, id, name,
				unlockable);
	...
	public L2DoorInstance[] getDoors()
	{
		L2DoorInstance[] _allTemplates = _staticItems.values()
				.toArray(new L2DoorInstance[_staticItems.size()]);
