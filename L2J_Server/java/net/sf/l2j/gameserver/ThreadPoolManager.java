 ...
 * <li>{@link #_urgentPacketsThreadPool UrgentPackets} where all the urgent
 * packets are executed.</li>
...
public class ThreadPoolManager
{
	...
	private ThreadPoolManager()
	{
		...
		_urgentPacketsThreadPool = new ThreadPoolExecutor(
				Config.URGENT_PACKET_THREAD_CORE_SIZE, Integer.MAX_VALUE, 5L,
				TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
				new PriorityThreadFactory("High Packet Pool",
						Thread.NORM_PRIORITY + 3));

		_generalPacketsThreadPool = new ThreadPoolExecutor(
				Config.GENERAL_PACKET_THREAD_CORE_SIZE,
				Config.GENERAL_PACKET_THREAD_CORE_SIZE + 2, 15L,
				TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
		...
		_generalThreadPool = new ThreadPoolExecutor(
				...
				Config.GENERAL_THREAD_CORE_SIZE + 2, 5L, TimeUnit.SECONDS,
			...
			long delay)
	{
		try
		{
			...
			if (initial < 0)
				initial = 0;
			return _aiScheduledThreadPool.scheduleAtFixedRate(r, initial, delay,
					TimeUnit.MILLISECONDS);
		} catch (RejectedExecutionException e)
		{
			return null;
	...
	public String[] getStats()
	{
		return new String[]
			{ "STP:", " + Effects:",
					...
					" |- PoolSize:        "
							+ _aiScheduledThreadPool.getPoolSize(),
					" |- MaximumPoolSize: "
					...
					" | -------", " + Urgent Packets:",
					" |- ActiveThreads:   "
							+ _urgentPacketsThreadPool.getActiveCount(),
					" |- getCorePoolSize: "
							+ _urgentPacketsThreadPool.getCorePoolSize(),
					" |- MaximumPoolSize: "
							+ _urgentPacketsThreadPool.getMaximumPoolSize(),
					" |- LargestPoolSize: "
							+ _urgentPacketsThreadPool.getLargestPoolSize(),
					" |- PoolSize:        "
							+ _urgentPacketsThreadPool.getPoolSize(),
					" |- CompletedTasks:  "
							+ _urgentPacketsThreadPool.getCompletedTaskCount(),
					" |- QueuedTasks:     "
							+ _urgentPacketsThreadPool.getQueue().size(),
					" | -------", " + General Tasks:",
					" |- ActiveThreads:   "
							+ _generalThreadPool.getActiveCount(),
					" |- getCorePoolSize: "
							+ _generalThreadPool.getCorePoolSize(),
					" |- MaximumPoolSize: "
