...
public final class L2VillageMasterInstance extends L2FolkInstance
{
	...
	// Logger.getLogger(L2VillageMasterInstance.class.getName());
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		} else if (actualCommand.equalsIgnoreCase("create_royal"))
		{
			...
			createSubPledge(player, cmdParams, cmdParams2,
					L2Clan.SUBUNIT_ROYAL1, 6);
		...
		} else if (command.startsWith("Subclass"))
		{
			...
			try
			{
				...
				paramOne = Integer
						.parseInt(command.substring(11, endIndex).trim());
			...
			case 1: // Add Subclass - Initial
				// Avoid giving player an option to add a new sub class, if they
				// have three already.
				if (player.getTotalSubClasses() == 3)
				{
					player.sendMessage(
							"You can now only change one of your current sub classes.");
					return;
				}
				...
				{
					content.append(
							"Add Subclass:<br>Which sub class do you wish to add?<br>");

					for (PlayerClass subClass : subsAvailable)
						content.append("<a action=\"bypass -h npc_"
								+ getObjectId() + "_Subclass 4 "
			...
			case 2: // Change Class - Initial
				...
				{
					content.append(
							"Which class would you like to switch to?<br>");

					if (baseClassId == player.getActiveClass())
						content.append(CharTemplateTable
								.getClassNameById(baseClassId)
								+ "&nbsp;<font color=\"LEVEL\">(Base Class)</font><br><br>");
					else
						content.append("<a action=\"bypass -h npc_"
								+ getObjectId() + "_Subclass 5 0\">"
								+ CharTemplateTable.getClassNameById(
										baseClassId)
								+ "</a>&nbsp;"
								+ "<font color=\"LEVEL\">(Base Class)</font><br><br>");

					for (Iterator<SubClass> subList = iterSubClasses(
							player); subList.hasNext();)
					{
						...
						if (subClassId == player.getActiveClass())
							content.append(CharTemplateTable
									.getClassNameById(subClassId) + "<br>");
						else
							content.append("<a action=\"bypass -h npc_"
									+ getObjectId() + "_Subclass 5 "
						...
						player); subList.hasNext();)
				{
					...
					content.append(
							"<a action=\"bypass -h npc_" + getObjectId()
									+ "_Subclass 6 " + subClass.getClassIndex()
									+ "\">"
									+ CharTemplateTable.getClassNameById(
											subClass.getClassId())
									+ "</a><br>");
			...
			case 4: // Add Subclass - Action (Subclass 4 x[x])
				...
				/*
				 * If quest checking is enabled, verify if the character has
				 * completed the Mimir's Elixir (Path to Subclass) and Fate's
				 * Whisper (A Grade Weapon) quests by checking for instances of
				 * their unique reward items.
				 *
				 * If they both exist, remove both unique items and continue
				...
				if (!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS)
				{
					...
					{
						player.sendMessage(
								"You must have completed the Fate's Whisper quest to continue adding your sub class.");
						return;
					}
				}
				...
				if (allowAddition)
				{
					String className = CharTemplateTable
							.getClassNameById(paramOne);

					if (!player.addSubClass(paramOne,
							player.getTotalSubClasses() + 1))
					{
					...
					player.sendPacket(new SystemMessage(1308)); // Transfer to
																// new class.
				} else
				{
			...
			case 5: // Change Class - Action
				...
				player.sendPacket(new SystemMessage(1270)); // Transfer
															// completed.
				break;
			case 6: // Change/Cancel Subclass - Choice
				...
				if (!subsAvailable.isEmpty())
				{
					for (PlayerClass subClass : subsAvailable)
						content.append("<a action=\"bypass -h npc_"
								+ getObjectId() + "_Subclass 7 " + paramOne
								+ " " + subClass.ordinal() + "\">"
								+ formatClassForDisplay(subClass) + "</a><br>");
				} else
				{
			...
			case 7: // Change Subclass - Action
				/*
				 * Warning: the information about this subclass will be removed
				 * from the subclass list even if false!
				 */
				if (player.modifySubClass(paramOne, paramTwo))
				{
					...
					player.sendPacket(new SystemMessage(1269)); // Subclass
																// added.
				} else
				{
					/*
					 * This isn't good! modifySubClass() removed subclass from
					 * memory we must update _classIndex! Else
					 * IndexOutOfBoundsException can turn up some place down the
					 * line along with other seemingly unrelated problems.
					 */
					player.setActiveClass(0); // Also updates _classIndex plus
												// switching _classid to
												// baseclass.
	...
	public void dissolveClan(L2PcInstance player, int clanId)
	{
		if (Config.DEBUG)
			_log.fine(player.getObjectId() + "(" + player.getName()
					+ ") requested dissolve a clan from " + getObjectId() + "("
					+ getName() + ")");
		...
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_DISPERSE_THE_CLANS_IN_ALLY));
			return;
		}
		if (clan.isAtWar() != 0)
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_DISSOLVE_WHILE_IN_WAR));
			return;
		}
		if (clan.getHasCastle() != 0 || clan.getHasHideout() != 0)
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_DISSOLVE_WHILE_OWNING_CLAN_HALL_OR_CASTLE));
			return;
		}
		for (Castle castle : CastleManager.getInstance().getCastles())
		{
			if (SiegeManager.getInstance().checkIsRegistered(clan,
					castle.getCastleId()))
			{
				player.sendPacket(new SystemMessage(
						SystemMessage.CANNOT_DISSOLVE_CAUSE_CLAN_WILL_PARTICIPATE_IN_CASTLE_SIEGE));
				return;
			}
		}
		...
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_DISSOLVE_WHILE_IN_SIEGE));
			return;
		}
		if (clan.getDissolvingExpiryTime() > System.currentTimeMillis())
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.DISSOLUTION_IN_PROGRESS));
			return;
		}

		clan.setDissolvingExpiryTime(System.currentTimeMillis()
				+ Config.ALT_CLAN_DISSOLVE_DAYS * 86400000); // 24*60*60*1000 =
																// 86400000
		clan.updateClanInDB();
	...
	public void recoverClan(L2PcInstance player, int clanId)
	{
		if (Config.DEBUG)
			_log.fine(player.getObjectId() + "(" + player.getName()
					+ ") requested recover a clan from " + getObjectId() + "("
					+ getName() + ")");
	...
	public void changeClanLeader(L2PcInstance player, String target)
	{
		if (Config.DEBUG)
			_log.fine(player.getObjectId() + "(" + player.getName()
					+ ") requested change a clan leader from " + getObjectId()
					+ "(" + getName() + ")");
		...
		if (member == null)
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_DOES_NOT_EXIST);
			sm.addString(target);
		...
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.INVITED_USER_NOT_ONLINE));
			return;
		}
		clan.setNewLeader(member);
	}

	public void createSubPledge(L2PcInstance player, String clanName,
			String leaderName, int pledgeType, int minClanLvl)
	{
		// if (Config.DEBUG)
		_log.fine(player.getObjectId() + "(" + player.getName()
				+ ") requested sub clan creation from " + getObjectId() + "("
				+ getName() + ")");
		...
		if (clan.getLevel() < minClanLvl)
		{
			...
			{
				player.sendPacket(new SystemMessage(
						SystemMessage.YOU_DO_NOT_MEET_CRITERIA_IN_ORDER_TO_CREATE_A_CLAN_ACADEMY));
			} else
			{
				player.sendPacket(new SystemMessage(
						SystemMessage.YOU_DO_NOT_MEET_CRITERIA_IN_ORDER_TO_CREATE_A_MILITARY_UNIT));
			}
			return;
		}
		...
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.CLAN_NAME_INCORRECT));
			return;
		}
		if (clanName.length() > 16)
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.CLAN_NAME_TOO_LONG));
			return;
		}
		for (L2Clan tempClan : ClanTable.getInstance().getClans())
		{
			if (tempClan.getSubPledge(clanName) != null)
			{
				...
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.S1_ALREADY_EXISTS);
				...
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.ANOTHER_MILITARY_UNIT_IS_ALREADY_USING_THAT_NAME));
				}
				return;
			}
		...
		if (pledgeType != L2Clan.SUBUNIT_ACADEMY)
					...
					|| clan.getClanMember(leaderName).getPledgeType() != 0)
			{
				...
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.CAPTAIN_OF_ORDER_OF_KNIGHTS_CANNOT_BE_APPOINTED));
				} else if (pledgeType >= L2Clan.SUBUNIT_ROYAL1)
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.CAPTAIN_OF_ROYAL_GUARD_CANNOT_BE_APPOINTED));
				}
				return;
			}
		...
		if (clan.createSubPledge(pledgeType, leaderName, clanName) == null)
		{
			...
			{
				player.sendPacket(new SystemMessage(
						SystemMessage.CLAN_HAS_ALREADY_ESTABLISHED_A_CLAN_ACADEMY));
			} else
				player.sendMessage(
						"You can't create any more sub-units of this type");
			return;
		}
		...
		if (pledgeType == L2Clan.SUBUNIT_ACADEMY)
		{
			sm = new SystemMessage(
					SystemMessage.THE_S1S_CLAN_ACADEMY_HAS_BEEN_CREATED);
			sm.addString(player.getClan().getName());
		} else if (pledgeType >= L2Clan.SUBUNIT_KNIGHT1)
		{
			sm = new SystemMessage(
					SystemMessage.THE_KNIGHTS_OF_S1_HAVE_BEEN_CREATED);
			sm.addString(player.getClan().getName());
		} else if (pledgeType >= L2Clan.SUBUNIT_ROYAL1)
		{
			sm = new SystemMessage(
					SystemMessage.THE_ROYAL_GUARD_OF_S1_HAVE_BEEN_CREATED);
			sm.addString(player.getClan().getName());
		} else
	...
	public void assignSubPledgeLeader(L2PcInstance player, String clanName,
			String leaderName)
	{
		if (Config.DEBUG)
			_log.fine(player.getObjectId() + "(" + player.getName()
					+ ") requested to assign sub clan" + clanName + "leader "
					+ "(" + leaderName + ")");
		...
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.NAMING_CHARNAME_UP_TO_16CHARS));
			return;
		}
		...
		if ((null == subPledge)
				|| (subPledge.getId() == L2Clan.SUBUNIT_ACADEMY))
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.CLAN_NAME_INCORRECT));
			return;
		...
		if (null == clan.getClanMember(leaderName)
				|| 0 != clan.getClanMember(leaderName).getPledgeType())
		{
		...
		SystemMessage sm = new SystemMessage(
				SystemMessage.S1_HAS_BEEN_SELECTED_AS_CAPTAIN_OF_S2);
			...
			for (PlayerClass availSub : availSubs)
			{
						...
						player); subList.hasNext();)
				{
					...
					if (availSub.ordinal() == prevSubClass.getClassId()
						...
						availSubs.remove(
								PlayerClass.values()[availSub.ordinal()]);
				}

				if ((npcRace == PlayerRace.Human
						|| npcRace == PlayerRace.LightElf))
				{
					// If the master is human or light elf, ensure that
					// fighter-type
					// masters only teach fighter classes, and priest-type
					// masters
				...
				} else
				{
					...
					// then remove any classes not of the same race as the
					// master.
					if ((npcRace != PlayerRace.Human
							&& npcRace != PlayerRace.LightElf)
	...
	public void showPledgeSkillList(L2PcInstance player)
	{
		...
		L2PledgeSkillLearn[] skills = SkillTreeTable.getInstance()
				.getAvailablePledgeSkills(player);
		AquireSkillList asl = new AquireSkillList(
				AquireSkillList.skillType.Clan);
		int counts = 0;
	...
	private final String formatClassForDisplay(PlayerClass className)
	{
		...
		for (int i = 1; i < charArray.length; i++)
			if (Character.isUpperCase(charArray[i]))
				classNameStr = classNameStr.substring(0, i) + " "
						+ classNameStr.substring(i);
