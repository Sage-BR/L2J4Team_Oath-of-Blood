...
public abstract class L2Character extends L2Object
{
	...
	private boolean _IsSleeping = false; // Cannot move/attack until sleep timed
											// out or monster is attacked
	 ...
	 * slot</li>
	 * <li>If L2Character is a L2PcInstance or L2Summon, copy basic Calculator
	 * set to object</li><BR>
	 * <BR>
	 *
	 * @param objectId
	...
	public void onTeleported()
	{
		...
		spawnMe(getPosition().getX(), getPosition().getY(),
				getPosition().getZ());
		...
		if (getPet() != null)
			getPet().teleToLocation(getPosition().getX() + Rnd.get(-100, 100),
					getPosition().getY() + Rnd.get(-100, 100),
					getPosition().getZ(), false);
	}
	 ...
	 * _KnownPlayers of the L2Character.<BR>
	 * <BR>
	 *
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	...
	public final void broadcastPacket(ServerBasePacket mov)
	{
		...
		if (getKnownList().getKnownPlayers() == null)
			return;
		...
		if (Config.DEBUG)
			_log.fine("players to notify:" + knownPlayers.size() + " packet:"
					+ mov.getType());
		...
		{
		} // this npe catch might not be necessary any more
	}
	 ...
	 * other L2PcInstance to inform.<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 ...
	 * all L2Character called _statusListener that must be informed of HP/MP
	 ...
	 * <BR>
	 * <li>L2PcInstance : Send current HP,MP and CP to the L2PcInstance and only
	 * current HP, MP and Level to all other L2PcInstance of the Party</li><BR>
	 * <BR>
	 *
	 */
	public void broadcastStatusUpdate()
	{
		if (getStatus().getStatusListener() == null
				|| getStatus().getStatusListener().isEmpty()
				|| !needHpUpdate(352))
			return;

		if (Config.DEBUG)
			_log.fine("Broadcast Status Update for " + getObjectId() + "("
					+ getName() + "). HP: " + getCurrentHp());
	 ...
	 * <li>Set the x,y,z position of the L2Object and if necessary modify its
	 * _worldRegion</li>
	 * <li>Send a Server->Client packet TeleportToLocationt to the L2Character
	 * AND to all L2PcInstance in its _KnownPlayers</li>
	...
	public void teleToLocation(int x, int y, int z, boolean allowRandomOffset)
	{
		...
		{
			x += Rnd.get(-Config.RESPAWN_RANDOM_MAX_OFFSET,
					Config.RESPAWN_RANDOM_MAX_OFFSET);
			y += Rnd.get(-Config.RESPAWN_RANDOM_MAX_OFFSET,
					Config.RESPAWN_RANDOM_MAX_OFFSET);
		}
	...
	{
		teleToLocation(MapRegionTable.getInstance().getTeleToLocation(this,
				teleportWhere), true);
	}
	 ...
	 * necessary, equip the L2PcInstance with arrows in left hand)</li>
	 * <li>If weapon is a bow, consumme MP and set the new period of bow non
	 * re-use</li><BR>
	 * <BR>
	 * <li>Get the Attack Speed of the L2Character (delay (in milliseconds)
	 * before next attack)</li>
	 ...
	 * in the _KnownPlayers of the L2Character</li>
	...
	protected void doAttack(L2Character target)
	{
		...
		if (isAlikeDead() || target == null
				|| (this instanceof L2NpcInstance && target.isAlikeDead())
				|| (this instanceof L2PcInstance && target.isDead()
						&& !target.isFakeDeath())
		...
		if (this instanceof L2PcInstance)
		{
			...
			if (target instanceof L2PcInstance)
			{
				if (((L2PcInstance) target).isCursedWeaponEquiped()
						&& ((L2PcInstance) this).getLevel() <= 20)
				{
					((L2PcInstance) this).sendMessage(
							"Cant attack a cursed Player under twenty");
					sendPacket(new ActionFailed());
					return;
				}

				if (((L2PcInstance) this).isCursedWeaponEquiped()
						&& ((L2PcInstance) target).getLevel() <= 20)
				{
					((L2PcInstance) this).sendMessage(
							"Cant attack a newbie player with Zarike");
					sendPacket(new ActionFailed());
					return;
				}
			}
		...
		if (weaponItem != null && weaponItem.getItemType() == L2WeaponType.ROD)
		{
			...
			((L2PcInstance) this).sendPacket(new SystemMessage(
					SystemMessage.CANNOT_ATTACK_WITH_FISHING_POLE));
		...
		// Check for a bow
		if ((weaponItem != null
				&& weaponItem.getItemType() == L2WeaponType.BOW))
		{
			// Check for arrows and MP
			if (this instanceof L2PcInstance)
			{
				// Verify if the bow can be use
				if (_disableBowAttackEndTime <= GameTimeController
						.getGameTicks())
				{
					// Verify if L2PcInstance owns enough MP
					int saMpConsume = (int) getStat().calcStat(Stats.MP_CONSUME,
							0, null, null);
					int mpConsume = saMpConsume == 0 ? weaponItem.getMpConsume()
							: saMpConsume;
					...
					if (getCurrentMp() < mpConsume)
					{
						...
						ThreadPoolManager.getInstance().scheduleAi(
								new NotifyAITask(CtrlEvent.EVT_READY_TO_ACT),
								1000);

						sendPacket(
								new SystemMessage(SystemMessage.NOT_ENOUGH_MP));
						sendPacket(new ActionFailed());
						return;
					}
				...
				} else
				{
					...
					// moment
					ThreadPoolManager.getInstance().scheduleAi(
							new NotifyAITask(CtrlEvent.EVT_READY_TO_ACT), 1000);
				...
				if (!checkAndEquipArrows())
				{
					...
					sendPacket(new ActionFailed());
					sendPacket(
							new SystemMessage(SystemMessage.NOT_ENOUGH_ARROWS));
					return;
				}
		...
		if (this instanceof L2PcInstance)
			...
			((L2Summon) this).getOwner().rechargeAutoSoulShot(true, false,
					true);
		...
		if (this instanceof L2Summon && !(this instanceof L2PetInstance))
			wasSSCharged = (((L2Summon) this)
					.getChargedSoulShot() != L2ItemInstance.CHARGED_NONE);
		else
			wasSSCharged = (weaponInst != null && weaponInst
					.getChargedSoulshot() != L2ItemInstance.CHARGED_NONE);
			...
			if (player != null)
			{
				...
				} else if (player.isHero())
				{
					if (target instanceof L2PcInstance
						...
						// If a cursed weapon is hitted by a Hero, Cp is reduced
						// to 0
						target.setCurrentCp(0);
				}
			}
		...
		ThreadPoolManager.getInstance().scheduleAi(
				new NotifyAITask(CtrlEvent.EVT_READY_TO_ACT), sAtk + reuse);
	}
	 ...
	 * <li>If the L2Character is a L2PcInstance, Send a Server->Client packet
	 * SetupGauge</li>
	 ...
	 *
	 */
	private boolean doAttackHitByBow(Attack attack, L2Character target,
			int sAtk, int reuse)
	{
		int damage1 = 0;
		boolean shld1 = false;
		...
		if (!miss1)
		{
			...
			crit1 = Formulas.getInstance()
					.calcCrit(getStat().getCriticalHit(target, null));

			// Calculate physical damages
			damage1 = (int) Formulas.getInstance().calcPhysDam(this, target,
					null, shld1, crit1, false, attack._soulshot);
		}
		...
		if (this instanceof L2PcInstance)
		{
			...
			sendPacket(new SystemMessage(
					SystemMessage.GETTING_READY_TO_SHOOT_AN_ARROW));
		...
		ThreadPoolManager.getInstance().scheduleAi(new HitTask(target, damage1,
				crit1, miss1, attack._soulshot, shld1), sAtk);
			...
			int sAtk)
	{
		...
		if (!miss1)
		{
			...
			damage1 = (int) Formulas.getInstance().calcPhysDam(this, target,
					null, shld1, crit1, true, attack._soulshot);
			damage1 /= 2;
		}
		...
		if (!miss2)
		{
			...
			crit2 = Formulas.getInstance()
					.calcCrit(getStat().getCriticalHit(target, null));

			// Calculate physical damages of hit 2
			damage2 = (int) Formulas.getInstance().calcPhysDam(this, target,
					null, shld2, crit2, true, attack._soulshot);
			damage2 /= 2;
		}

		// Create a new hit task with Medium priority for hit 1
		ThreadPoolManager.getInstance().scheduleAi(new HitTask(target, damage1,
				crit1, miss1, attack._soulshot, shld1), sAtk / 2);

		// Create a new hit task with Medium priority for hit 2 with a higher
		// delay
		ThreadPoolManager.getInstance().scheduleAi(new HitTask(target, damage2,
				crit2, miss2, attack._soulshot, shld2), sAtk);
	...
	private boolean doAttackHitByPole(Attack attack, int sAtk)
	{
		...
		int maxRadius = (int) getStat().calcStat(Stats.POWER_ATTACK_RANGE, 66,
				null, null);
		int maxAngleDiff = (int) getStat().calcStat(Stats.POWER_ATTACK_ANGLE,
				120, null, null);
		...
		this.setHeading((int) ((angleTarget / 9.0) * 1610.0)); // =
																// this.setHeading((int)((angleTarget
																// / 360.0) *
																// 64400.0));
		...
		int attackcountmax = (int) getStat().calcStat(Stats.ATTACK_COUNT_MAX, 3,
				null, null);
		int attackcount = 0;
		...
		for (L2Object obj : getKnownList().getKnownObjects().values())
		{
			...
			if (obj instanceof L2Character)
			{
				if ((obj instanceof L2PetInstance
						...
						&& ((L2PetInstance) obj)
								.getOwner() == ((L2PcInstance) this))
						|| !Util.checkIfInRange(maxRadius, this, obj, false))
					continue;

				// otherwise hit too high/low. 650 because mob z coord sometimes
				// wrong on hills
				if (Math.abs(obj.getZ() - this.getZ()) > 650)
					continue;
				angleTarget = Util.calculateAngleFrom(this, obj);
				if (Math.abs(angleChar - angleTarget) > maxAngleDiff
						&& Math.abs(
								(angleChar + 360) - angleTarget) > maxAngleDiff
						&& // Example: char is at 1 degree and target is at 359
							// degree
						Math.abs(angleChar - (angleTarget + 360)) > maxAngleDiff // Example:
																					// target
																					// is
																					// at
																					// 1
				...
				if (!target.isAlikeDead())
				{
					...
					{
						if (target == getAI().getAttackTarget()
								|| target.isAutoAttackable(this))
						{
	 ...
	 * launch the Magic Skill at the end of the casting time</li><BR>
	 * <BR>
	 *
	 * @param skill
	 *            The L2Skill to use
	...
	public void doCast(L2Skill skill)
	{
		...
		if (isSkillDisabled(skill.getId()))
		{
			if (this instanceof L2PcInstance)
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.S1_PREPARED_FOR_REUSE);
		...
		// Check if the skill is a magic spell and if the L2Character is not
		// muted
		...
		// Check if the skill is psychical and if the L2Character is not
		// psychical_muted
		...
		} else if (skill.useSpiritShot())
		{
			if (this instanceof L2PcInstance)
				...
				((L2Summon) this).getOwner().rechargeAutoSoulShot(false, true,
						true);
		}
		...
		// Get all possible targets of the skill in a table in function of the
		// skill target type
				...
				|| skill.getTargetType() == L2Skill.SkillTargetType.TARGET_ALLY)
		{
							...
							.getIntention() == CtrlIntention.AI_INTENTION_ATTACK)
			{
				...
				if (((L2PcInstance) this).isInParty() && skill
						.getTargetType() == L2Skill.SkillTargetType.TARGET_PARTY)
				{
					for (L2PcInstance member : ((L2PcInstance) this).getParty()
							.getPartyMembers())
		...
		// Calculate the Interrupt Time of the skill (base + modifier) if the
		// skill is a spell else 0
		if (skill.isMagic())
			skillInterruptTime = Formulas.getInstance().calcMAtkSpd(this, skill,
					skillInterruptTime);
		else
				...
				&& ((skill.getTargetType()) != (SkillTargetType.TARGET_SELF)))
		{
							...
							.getChargedSpiritshot() == L2ItemInstance.CHARGED_SPIRITSHOT))
			{
				...
				// Because the following are magic skills that do not actively
				// 'eat' BSpS/SpS,
				// I must 'eat' them here so players don't take advantage of
		...
		_castEndTime = 10 + GameTimeController.getGameTicks()
				+ skillTime / GameTimeController.MILLIS_IN_TICK;
		_castInterruptTime = GameTimeController.getGameTicks()
				+ skillInterruptTime / GameTimeController.MILLIS_IN_TICK;

		// Init the reuse time of the skill
		int reuseDelay = (int) (skill.getReuseDelay()
				* getStat().getMReuseRate(skill));
		if (skill.isMagic())
		...
		// to the L2Character AND to all L2PcInstance in the _KnownPlayers of
		// the L2Character
		broadcastPacket(new MagicSkillUser(this, target, displayId, level,
				skillTime, reuseDelay));
		...
		if (initmpcons > 0)
		{
			StatusUpdate su = new StatusUpdate(getObjectId());
			getStatus().reduceMp(
					calcStat(Stats.MP_CONSUME_RATE, initmpcons, null, null));
			su.addAttribute(StatusUpdate.CUR_MP, (int) getCurrentMp());
			sendPacket(su);
		}

		// Disable the skill during the re-use delay and create a task
		// EnableSkill with Medium priority to enable it at the end of the
		// re-use delay
		if (reuseDelay > 10)
		{
		...
		if (skillTime > 50)
		{
			...
			_skillCast = ThreadPoolManager.getInstance().scheduleEffect(
					new MagicUseTask(targets, skill), skillTime);
		} else
		{
	 ...
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2NpcInstance : Create a DecayTask to remove the corpse of the
	 * L2NpcInstance after 7 seconds</li>
	 * <li>L2Attackable : Distribute rewards (EXP, SP, Drops...) and notify
	 * Quest Engine</li>
	 * <li>L2PcInstance : Apply Death Penalty, Manage gain/loss Karma and Item
	 * Drop</li><BR>
	 * <BR>
	 *
	 * @param killer
	...
	public void doDie(L2Character killer)
	{
		...
		// if the Character isn't a Noblesse Blessed L2PlayableInstance and
		// killed by a raid boss
		if (this instanceof L2PlayableInstance
				&& ((L2PlayableInstance) this).isNoblesseBlessed())
	...
	protected void useMagic(L2Skill skill)
	{
		...
		// ??
		if (skill == null || isDead() || isAllSkillsDisabled()
				|| skill.isPassive())
			return;
		...
		{
		case TARGET_AURA: // AURA, SELF should be cast even if no target has
							// been found
		case TARGET_SELF:
			target = this;
			break;
	...
	public final L2Skill getAttackingCharSkill()
	{
		return _AttackingCharSkill;
	}

	/**
	 * Set _attackingCharSkill to the L2Skill used against this L2Character.<BR>
	 * <BR>
	 *
	 * @param skill
	 *            The L2Skill used against this L2Character
	 */
	public void setAttackingCharSkill(L2Skill skill)
	{
		_AttackingCharSkill = skill;
	}
	...
	public final void setIsAffraid(boolean value)
	{
		_IsAffraid = value;
	}
	...
	{
		return isStunned() || isSleeping()
				|| _attackEndTime > GameTimeController.getGameTicks()
				|| isFakeDeath() || isParalyzed();
	}
	...
	{
		return isStunned() || isRooted() || isSleeping() || isOverloaded()
				|| isParalyzed() || isImobilised() || isFakeDeath();
	}
	...
	public CharKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof CharKnownList))
	...
	class HitTask implements Runnable
	{
		...
		public HitTask(L2Character target, int damage, boolean crit,
	...
	class QueuedMagicUseTask implements Runnable
	{
		...
		public QueuedMagicUseTask(L2PcInstance currPlayer, L2Skill queuedSkill,
				boolean isCtrlPressed, boolean isShiftPressed)
		{
		...
		public void run()
		{
			try
			{
				_currPlayer.useMagic(_queuedSkill, _isCtrlPressed,
						_isShiftPressed);
			} catch (Throwable e)
			{
				_log.log(Level.SEVERE, "", e);
		...
		public void run()
		{
			try
			{
				...
				if (Math.abs(System.currentTimeMillis()
						- getlastPvpAttack()) > Config.PVP_TIME)
				{
					// _log.fine("Stopping PvP");
					stopPvPFlag();
				} else if (Math.abs(System.currentTimeMillis()
						- getlastPvpAttack()) > Config.PVP_TIME - 5000)
				{
					updatePvPFlag(2);
				} else
				{
	 ...
	 * in ConcurrentHashMap(Integer,L2Effect) <B>_effects</B>. The Integer key
	 * of _effects is the L2Skill Identifier that has created the L2Effect.<BR>
	 * <BR>
	 *
	 ...
	 * its priority order) will be preserve.<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 ...
	 * Calculator set of the L2Character (remove the old one if necessary)</li>
	 * <li>If this effect has higher priority in its Stack Group, add its Funcs
	 * to the Calculator set of the L2Character (remove previous stacked effect
	...
	public final void addEffect(L2Effect newEffect)
	{
		...
		synchronized (_effects)
		{
			...
			for (L2Effect _effect : _effects)
			{
				if (_effect.getSkill().getId() == newEffect.getSkill().getId())
					return;
			}

			// Remove first Buff if number of buffs > 19
			L2Skill tempskill = newEffect.getSkill();
			if (getBuffCount() > Config.BUFFS_MAX_AMOUNT
					&& !doesStack(tempskill)
					&& ((tempskill.getSkillType() == L2Skill.SkillType.BUFF
							|| tempskill
									.getSkillType() == L2Skill.SkillType.DEBUFF
							|| tempskill
									.getSkillType() == L2Skill.SkillType.REFLECT
							|| tempskill
									.getSkillType() == L2Skill.SkillType.HEAL_PERCENT
							|| tempskill
									.getSkillType() == L2Skill.SkillType.MANAHEAL_PERCENT)
			...
			if (!newEffect.getSkill().isToggle())
			{
				...
				for (L2Effect _effect : _effects)
				{
					if (_effect != null)
					{
						...
						if (!_effect.getSkill().isToggle()
								&& (skillid != 4267 && skillid != 4270
			...
			List<L2Effect> stackQueue = _stackedEffects
					.get(newEffect.getStackType());
	 ...
	 *
	 */
	private List<L2Effect> effectQueueInsert(L2Effect newStackedEffect,
			List<L2Effect> stackQueue)
	{
		...
		// petrify.
		if (Config.EFFECT_CANCELING && !newStackedEffect.isHerbEffect()
				&& stackQueue.size() > 1)
		{
	 ...
	 * L2Character and update client magic icone.<BR>
	 * <BR>
	 *
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove Func added by this effect from the L2Character Calculator
	 * (Stop L2Effect)</li>
						...
						if (stackQueue.size() > 0)
						{
							...
							for (L2Effect _effect : _effects)
							{
								if (_effect == stackQueue.get(0))
								{
									...
									addStatFuncs(_effect.getStatFuncs());
									// Set the effect to In Use
									_effect.setInUse(true);
									break;
								}
							}
						}
	...
	public final void startFakeDeath()
	{
		...
		broadcastPacket(
				new ChangeWaitType(this, ChangeWaitType.WT_START_FAKEDEATH));
	}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Calculate the success rate of the Stun Abnormal Effect on this
	 * L2Character</li>
	 * <li>If Stun succeed, active the abnormal effect Stun flag, notify the
	 * L2Character AI and send Server->Client UserInfo/CharInfo packet</li>
	 * <li>If Stun NOT succeed, send a system message Failed to the L2PcInstance
	 * attacker</li><BR>
	 * <BR>
	 *
	 */
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Confused abnormal L2Effect
	 * from L2Character and update client magic icone</li>
	...
	/**
	 * Stop and remove the L2Effect corresponding to the L2Skill Identifier and
	 ...
	 * DMG_OVER_TIME...) from the L2Character and update client magic icone.<BR>
	 * <BR>
	 *
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Fake Death abnormal L2Effect
	 * from L2Character and update client magic icone</li>
	...
	public final void stopFakeDeath(L2Effect effect)
	{
		...
		ChangeWaitType revive = new ChangeWaitType(this,
				ChangeWaitType.WT_STOP_FAKEDEATH);
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Fear abnormal L2Effect from
	 * L2Character and update client magic icone</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Muted abnormal L2Effect from
	 * L2Character and update client magic icone</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Root abnormal L2Effect from
	 * L2Character and update client magic icone</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Sleep abnormal L2Effect from
	 * L2Character and update client magic icone</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Stun abnormal L2Effect from
	 * L2Character and update client magic icone</li>
	...
	public final void updateEffectIcons(boolean partyOnly)
	{
		...
		if (os != null)
		{
			if (Olympiad.getInstance()
					.getSpectators(player.getOlympiadGameId()) != null)
			{
				for (L2PcInstance spectator : Olympiad.getInstance()
						.getSpectators(player.getOlympiadGameId()))
				{
					if (spectator == null)
						continue;
	 ...
	 * <BR>
	 * All active skills effects in progress on the L2Character are identified
	 * in <B>_effects</B>. The Integer key of _effects is the L2Skill Identifier
	 ...
	 *
	 * @param index
	 *            The L2Skill Identifier of the L2Effect to return from the
	 *            _effects
	 *
...
//	private int _floatingRunSpeed;
	 ...
	 *
	 * @param f
	 *            The Func object to add to the Calculator corresponding to the
	 *            state affected
	 */
	 ...
	 *
	 * @param funcs
	 *            The list of Func objects to add to the Calculator
	 ...
	 *
	 * @param f
	 *            The Func object to remove from the Calculator corresponding to
	 *            the state affected
	 */
	public final synchronized void removeStatFunc(Func f)
	{
		...
		if (this instanceof L2NpcInstance)
		{
			...
			{
				if (!Calculator.equalsCals(_Calculators[i],
						NPC_STD_CALCULATOR[i]))
					break;
			}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove all Func objects of the selected owner from
	 * _calculators</li><BR>
	 * <BR>
	 * <li>If L2Character is a L2NPCInstance and _calculators is equal to
	 * NPC_STD_CALCULATOR, free cache memory and just create a link on
	...
	public Zone getZone()
	{
		for (Zone zone : ZoneManager.getInstance()
				.getZones(ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Town)))
			if (zone.checkIfInZone(this))
				return zone;
		return null;
	}
		...
		if (isCastingNow())
		{
			...
			if (this instanceof L2PcInstance)
				getAI().notifyEvent(CtrlEvent.EVT_FINISH_CASTING); // setting
																	...
																	// previous
																	// intention
			broadcastPacket(new MagicSkillCanceld(getObjectId())); // broadcast
																	// packet to
																	// stop
																	// animations
																	// client-side
			sendPacket(new ActionFailed()); // send an "action failed" packet to
											// the caster
		}
	}
	...
	public boolean updatePosition(int gameTicks)
	{
		...
		if (elapsed >= m._ticksToMove)
		{
			...
			if (this instanceof L2BoatInstance)
			{
				super.getPosition().setXYZ(m._xDestination, m._yDestination,
						m._zDestination);
				((L2BoatInstance) this).updatePeopleInTheBoat(m._xDestination,
						m._yDestination, m._zDestination);
			} else
			{
				super.getPosition().setXYZ(m._xDestination, m._yDestination,
						m._zDestination);
		...
		if (this instanceof L2BoatInstance)
		{
			super.getPosition().setXYZ(
					m._xMoveFrom + (int) (elapsed * m._xSpeedTicks),
					m._yMoveFrom + (int) (elapsed * m._ySpeedTicks),
					super.getZ());
			((L2BoatInstance) this).updatePeopleInTheBoat(
					m._xMoveFrom + (int) (elapsed * m._xSpeedTicks),
					m._yMoveFrom + (int) (elapsed * m._ySpeedTicks),
					super.getZ());
		} else
	 ...
	 * <li>Set the current position (x,y,z), its current L2WorldRegion if
	 * necessary and its heading</li>
	 * <li>Remove the L2Object object from _gmList** of GmListTable</li>
	 * <li>Remove object from _knownObjects and _knownPlayer* of all surrounding
	 * L2WorldRegion L2Characters</li><BR>
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T send
	 * Server->Client packet StopMove/StopRotation </B></FONT><BR>
	 * <BR>
	 *
	 */
	public void stopMove(L2CharPosition pos)
	...
	public void stopMove(L2CharPosition pos, boolean updateKnownObjects)
	{
		...
		if (updateKnownObjects)
			ThreadPoolManager.getInstance()
					.executeTask(new KnownListAsynchronousUpdateTask(this));
	}
	 ...
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * The L2Object (including L2Character) targeted is identified in
	 * <B>_target</B> of the L2Character<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 ...
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2PcInstance : Remove the L2PcInstance from the old target
	 * _statusListener and add it to the new target if it was a
	 * L2Character</li><BR>
	 * <BR>
	 *
	 * @param object
	...
	public void setTarget(L2Object object)
	{
		...
		if (object == null)
		{
			 ...
			 * getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
			 *
			 * if (this instanceof L2PcInstance) { sendPacket(new
			 * ActionFailed()); SystemMessage sm = new
			 * SystemMessage(SystemMessage.S1_S2);
			 * sm.addString("Attack is aborted"); sendPacket(sm); } }
			 *
			 * if (isCastingNow() && canAbortCast() && getAI().getCastTarget()
			 * == _target) { abortCast();
			 * getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE); if (this
			 * instanceof L2PcInstance) { sendPacket(new ActionFailed());
			 * SystemMessage sm = new SystemMessage(SystemMessage.S1_S2);
			 * sm.addString("Casting is aborted"); sendPacket(sm); } }
			 */
		}
	 ...
	 * <li>Create a task to notify the AI that L2Character arrives at a check
	 ...
	 * onIntentionInteract(L2Object)</li>
	 * <li>FollowTask</li><BR>
	 * <BR>
	 *
	 * @param x
	...
	protected void moveToLocation(int x, int y, int z, int offset)
	{
		...
		// destination
		m._ticksToMove = (int) (GameTimeController.TICKS_PER_SECOND
				* (distance + 25) / speed);

		// Calculate the xspeed and yspeed in unit/ticks in function of the
		// movement speed
		m._xSpeedTicks = (float) (cos * speed
				/ GameTimeController.TICKS_PER_SECOND);
		m._ySpeedTicks = (float) (sin * speed
				/ GameTimeController.TICKS_PER_SECOND);

		// Calculate and set the heading of the L2Character
		int heading = (int) (Math.atan2(-sin, -cos)
				* 10430.378350470452724949566316381);
		heading += 32768;
		setHeading(heading);

		if (Config.DEBUG)
			_log.fine("dist:" + distance + "speed:" + speed + " ttt:"
					+ m._ticksToMove + " dx:" + (int) m._xSpeedTicks + " dy:"
					+ (int) m._ySpeedTicks + " heading:" + heading);
		...
		if (tm > 3000)
			ThreadPoolManager.getInstance().scheduleAi(
					new NotifyAITask(CtrlEvent.EVT_ARRIVED_REVALIDATE), 2000);
...
//		if (_move._heading < heading - 5 || _move._heading > heading  5)
	 ...
	 *
	 * @see net.sf.l2j.gameserver.model.L2Character.isInsideRadius(int x, int y,
	 *      int z, int radius, boolean checkZ, boolean strictCheck)
	 */
	public final boolean isInsideRadius(L2Object object, int radius,
			boolean checkZ, boolean strictCheck)
	{
		return isInsideRadius(object.getX(), object.getY(), object.getZ(),
				radius, checkZ, strictCheck);
	}
	 ...
	 * @return true is the L2Character is inside the radius.
	 */
	public final boolean isInsideRadius(int x, int y, int z, int radius,
			boolean checkZ, boolean strictCheck)
	{
		double dx = x - getX();
		double dy = y - getY();
...
//
			...
			boolean miss, boolean soulshot, boolean shld)
	{
		...
		// L2PcInstance)
		if (target == null || isAlikeDead() || (this instanceof L2NpcInstance
				&& ((L2NpcInstance) this).isEventMob))
		{
			getAI().notifyEvent(CtrlEvent.EVT_CANCEL);
			return;
		}
		...
		if ((this instanceof L2NpcInstance && target.isAlikeDead())
				|| target.isDead() || (!getKnownList().knowsObject(target)
		...
		if (miss)
		{
			...
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.AVOIDED_S1s_ATTACK);
		...
		if (!isAttackAborted())
		{
			...
			if (this instanceof L2PcInstance)
			{
				...
				{
					// ((L2PcInstance)this).sendMessage("You hit the target's
					// armor.");
				} else
				{
					SystemMessage sm = new SystemMessage(
			...
			} else if (this instanceof L2Summon)
			{
				...
				// Prevents the double spam of system messages, if the target is
				// the owning player.
				if (target.getObjectId() != activeSummon.getOwner()
						.getObjectId())
				{
					if (crit)
						activeSummon.getOwner().sendPacket(new SystemMessage(
								SystemMessage.PET_CRITICAL_HIT));

					if (miss)
					{
						activeSummon.getOwner()
								.sendMessage("The pet missed the target.");
					} else
					{
						SystemMessage sm = new SystemMessage(
								SystemMessage.PET_DID_S1_DMG);
						sm.addNumber(damage);
			...
			if (target instanceof L2PcInstance)
			{
				...
				if (shld)
					enemy.sendPacket(new SystemMessage(
							SystemMessage.SHIELD_DEFENCE_SUCCESSFULL));
			...
			} else if (target instanceof L2Summon)
			{
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.PET_RECEIVED_DAMAGE_OF_S2_BY_S1);
				sm.addString(getName());
				sm.addNumber(damage);
				activeSummon.getOwner().sendPacket(sm);
			}
			...
			if (!miss && damage > 0)
			{
				...
				boolean isBow = (weapon != null && weapon.getItemType()
						.toString().equalsIgnoreCase("Bow"));
				...
				if (!isBow) // Do not reflect or absorb if weapon is of type bow
				{
					...
					double reflectPercent = target.getStat().calcStat(
							Stats.REFLECT_DAMAGE_PERCENT, 0, null, null);

					if (reflectPercent > 0)
					{
						int reflectedDamage = (int) (reflectPercent / 100.
								* damage);
						...
						/*
						 * if (target instanceof L2PcInstance)
						 * ((L2PcInstance)target).sendMessage("You reflected " +
						 * reflectedDamage + " damage."); else if (target
						 * instanceof L2Summon) ((L2Summon)target).getOwner().
						 * sendMessage("Summon reflected " + reflectedDamage +
						 * " damage.");
						 *
						 * if (this instanceof L2PcInstance)
						 * ((L2PcInstance)this).
						 * sendMessage("Target reflected to you " +
						 * reflectedDamage + " damage."); else if (this
						 * instanceof L2Summon) ((L2Summon)this).getOwner().
						 * sendMessage("Target reflected to your summon " +
						 * reflectedDamage + " damage.");
						 */
					}

					// Absorb HP from the damage inflicted
					double absorbPercent = this.getStat().calcStat(
							Stats.ABSORB_DAMAGE_PERCENT, 0, null, null);

					if (absorbPercent > 0)
					{
						int maxCanAbsorb = (int) (this.getMaxHp()
								- this.getCurrentHp());
						int absorbDamage = (int) (absorbPercent / 100.
								* damage);
						...
						if (absorbDamage > 0)
						{
							...
							/*
							 * if (this instanceof L2PcInstance)
							 * ((L2PcInstance)this).sendMessage("You absorbed "
							 * + absorbDamage + " damage."); else if (this
							 * instanceof L2Summon) ((L2Summon)this).getOwner().
							 * sendMessage("Summon absorbed " + absorbDamage +
							 * " damage."); else if (Config.DEBUG)
							 * _log.info(getName() + " absorbed " + absorbDamage
							 * + " damage.");
							 */
						}
					}
			...
			if (target.isRaid())
			{
				...
				if (level > target.getLevel() + 8)
				{
					...
					if (skill != null)
						...
						_log.warning(
								"Skill 4515 at level 99 is missing in DP.");
				}
			}
			 ...
			 *
			 * We must not discharge the soulshouts at the onHitTimer method, as
			 * this can cause unwanted soulshout consumption if the attacker
			 ...
			 *
			 * if (!miss) { if (this instanceof L2Summon && !(this instanceof
			 * L2PetInstance)) { if (((L2Summon)this).getChargedSoulShot() !=
			 * L2ItemInstance.CHARGED_NONE)
			 * ((L2Summon)this).setChargedSoulShot(L2ItemInstance.CHARGED_NONE);
			 * } else { if (weapon != null && weapon.getChargedSoulshot() !=
			 * L2ItemInstance.CHARGED_NONE)
	...
	public void breakCast()
	{
		if (isCastingNow() && canAbortCast())
		{
			...
			if (this instanceof L2PcInstance)
			{
				// Send a system message
				sendPacket(
						new SystemMessage(SystemMessage.CASTING_INTERRUPTED));
			}
		}
	}
	 ...
	 * Notify AI with AI_INTENTION_ATTACK</li><BR>
	 * <BR>
	 *
	 * @param player
	...
	public void onForcedAttack(L2PcInstance player)
	{
		...
		} else
		{
			...
			player.sendPacket(new MyTargetSelected(getObjectId(),
	...
	{
		return ((attacker.getAccessLevel() < Config.GM_PEACEATTACK)
				&& isInsidePeaceZone((L2Object) attacker, target));
	}

	public boolean isInsidePeaceZone(L2Object attacker, L2Object target)
	{
		return (target != null && !(target instanceof L2MonsterInstance) && // Target
																			// is
																			// not
																			// a
																			// monster
																			// and
				( // (
				ZoneManager.getInstance().checkIfInZonePeace(attacker) || // Player
																			// is
																			// inside
																			// peace
																			// zone
																			// or
						ZoneManager.getInstance().checkIfInZonePeace(target) // Target
																				// is
																				// inside
																				// peace
																				// zone
				...
				( // (
				!(target instanceof L2PcInstance) || // Target is not a player
														// or
						(!Config.ALT_GAME_KARMA_PLAYER_CAN_BE_KILLED_IN_PEACEZONE)
						|| // Target is a player and killable in peace zone is
							// false or
						(((L2PcInstance) target).getKarma() <= 0) // Target is a
																	// player,
																	// killable
																	// in peace
																	// zone is
	 ...
	 * <BR>
	 */
	public int calculateAttackSpeed(L2Character target,
	 ...
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * All skills own by a L2Character are identified in <B>_skills</B> the
	 * L2Character <BR>
	 * <BR>
	 *
	 */
	...
	public boolean doesStack(L2Skill checkSkill)
	{
		...
		for (L2Effect _effect : _effects)
		{
			if (_effect.getStackType() != null
					&& _effect.getStackType().equals(stackType))
				return true;
		}
		return false;
	}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Send a Server->Client packet MagicSkillLaunched (to display magic
	 * skill animation) to all L2PcInstance of L2Charcater _knownPlayers</li>
	 * <li>Consumme MP, HP and Item if necessary</li>
	 * <li>Send a Server->Client packet StatusUpdate with MP modification to the
	 * L2PcInstance</li>
	 * <li>Launch the magic skill in order to calculate its effects</li>
	 * <li>If the skill type is PDAM, notify the AI of the target with
	...
	public void onMagicUseTimer(L2Object[] targets, L2Skill skill)
	{
		...
		// Escaping from under skill's radius. First version, not perfect in AoE
		// skills.
		int escapeRange = 0;
		...
		if (escapeRange > 0)
		{
			...
			for (L2Object target : targets)
			{
				...
				{
					if (!this.isInsideRadius(target, escapeRange, true, false))
						continue;
					else
				...
				// {
				// if (Config.DEBUG)
				// _log.warning("Class cast bad:
				// "+targets[i].getClass().toString());
				// }
			}
			if (targetList.isEmpty())
			{
				...
				targets = targetList
						.toArray(new L2Character[targetList.size()]);
		}
		...
		if (isCastingNow())
		{
			...
			for (L2Object target2 : targets)
			{
				if (target2 instanceof L2Character)
				{
					...
					if (skill.getSkillType() == L2Skill.SkillType.BUFF
							|| skill.getSkillType() == L2Skill.SkillType.SEED)
					{
						SystemMessage smsg = new SystemMessage(
								SystemMessage.YOU_FEEL_S1_EFFECT);
						smsg.addString(skill.getName());
						target.sendPacket(smsg);
					}

					if (Config.DEBUG)
						_log.fine("msl: " + getName() + " " + magicId + " "
								+ level + " " + target.getTitle());

					// Send a Server->Client packet MagicSkillLaunched to the
					// L2Character AND to all L2PcInstance in the _KnownPlayers
					// of the L2Character
					broadcastPacket(new MagicSkillLaunched(this, magicId, level,
							target));
							...
							&& target instanceof L2Summon)
					{
						if (this.equals(((L2Summon) target).getOwner()))
							...
							this.sendPacket(
									new NpcInfo((L2Summon) target, this));
					}
				}
			}
			...
			{
				getStatus().reduceMp(
						calcStat(Stats.MP_CONSUME_RATE, mpConsume, null, null));
				su.addAttribute(StatusUpdate.CUR_MP, (int) getCurrentMp());
				isSendStatus = true;
			}
			...
			if (skill.getHpConsume() > 0)
			{
				...
				consumeHp = calcStat(Stats.HP_CONSUME_RATE,
						skill.getHpConsume(), null, null);
			...
			// If the skill type is PDAM or DRAIN_SOUL, notify the AI of the
			// target with AI_INTENTION_ATTACK
	 ...
	 * <B>_disabledSkills</B> of the L2Character <BR>
	 * <BR>
	 *
	 * @param skillId
	...
	{
		if (_disabledSkills == null)
			_disabledSkills = Collections
					.synchronizedList(new FastList<Integer>());
	...
	public void disableSkill(int skillId, long delay)
	{
		...
		if (delay > 10)
			ThreadPoolManager.getInstance().scheduleAi(new EnableSkill(skillId),
					delay);
	}
	...
	public void callSkill(L2Skill skill, L2Object[] targets)
	{
		try
		{
			...
			// Do initial checkings for skills and set pvp flag/draw aggro when
			// needed
			for (L2Object target : targets)
			{
				if (target instanceof L2Character)
				{
					...
					{
						if (activeWeapon.getSkillEffects(this, player,
								skill).length > 0
								&& this instanceof L2PcInstance)
							this.sendPacket(SystemMessage.sendString(
									"Target affected by weapon special ability!"));
					}

					// Check Raidboss attack
					if (player.isRaid() && getLevel() > player.getLevel() + 8)
					{
						L2Skill tempSkill = SkillTable.getInstance()
								.getInfo(4515, 99);
						if (tempSkill != null)
						{
					...
					if (activeChar != null)
					{
						if (skill.isOffensive())
						{
							if ((player instanceof L2PcInstance
									...
									&& activeChar.checkIfPvP(player))
							{
								player.getAI().notifyEvent(
										CtrlEvent.EVT_ATTACKED, activeChar);
						...
						} else
						{
							if (player instanceof L2PcInstance)
							{
								// Casting non offensive skill on player with
								// pvp flag set or with karma
							...
							} else if (player instanceof L2Attackable && !(skill
									.getSkillType() == L2Skill.SkillType.SUMMON))
								activeChar.updatePvPStatus();
						}
					}
				}
			...
			if (skill.isToggle())
			{
				// Check if the skill effects are already in progress on the
				// L2Character
				if (getEffect(skill.getId()) != null)
				{
					handler = SkillHandler.getInstance()
			...
			if (skill.isOverhit())
			{
				// Set the "over-hit enabled" flag on each of the possible
				// targets
				for (L2Object target : targets)
				{
					L2Character player = (L2Character) target;
	...
	public boolean isBehindTarget()
	{
		...
		if (getTarget() instanceof L2Character)
		{
			...
			{
				if (Config.DEBUG)
					_log.info("Char " + this.getName() + " is behind "
							+ target.getName());
				return true;
			}
		} else
	...
	public void startPvPFlag()
	{
		...
		_PvPRegTask = ThreadPoolManager.getInstance()
				.scheduleGeneralAtFixedRate(new PvPFlag(), 1000, 1000);
	}
...
//
	...
	{
		double reflect = calcStat(skill.isMagic() ? Stats.REFLECT_SKILL_MAGIC
				: Stats.REFLECT_SKILL_PHYSIC, 0, null, null);
