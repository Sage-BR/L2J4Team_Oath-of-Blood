...
public class L2Event
{
	...
	{ // this will return top N players sorted by kills, first element in the
		// array will be the one with more kills
	...
	public static void showEventHtml(L2PcInstance player, String objectid)
	{
		try
		{
			...
			if (L2Event.participatingPlayers.contains(player.getName()))
				replyMSG.append(
						"<br><center>You are already in the event players list !!</center></body></html>");
			else
				replyMSG.append(
	...
	public static void spawn(L2PcInstance target, int npcid)
	{
		...
		try
		{
			...
			spawn.getLastSpawn().spawnMe(spawn.getLastSpawn().getX(),
					spawn.getLastSpawn().getY(), spawn.getLastSpawn().getZ());

			spawn.getLastSpawn().broadcastPacket(new MagicSkillUser(
					spawn.getLastSpawn(), spawn.getLastSpawn(), 1034, 1, 1, 1));
	...
	public static void restoreChar(L2PcInstance player)
	{
		try
		{
			...
			player.eventkarma = connectionLossData
					.get(player.getName()).eventkarma;
			player.eventpvpkills = connectionLossData
					.get(player.getName()).eventpvpkills;
			player.eventpkkills = connectionLossData
					.get(player.getName()).eventpkkills;
			player.eventTitle = connectionLossData
					.get(player.getName()).eventTitle;
			player.kills = connectionLossData.get(player.getName()).kills;
			player.eventSitForced = connectionLossData
					.get(player.getName()).eventSitForced;
			player.atEvent = true;
		} catch (Exception e)
		{
		}
