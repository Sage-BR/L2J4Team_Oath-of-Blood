...
public class RequestRestartPoint extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestRestartPoint.class.getName());
	...
	class DeathTask implements Runnable
	{
		...
		public void run()
		{
			// _log.warning(activeChar.getName()+" request restartpoint
			// "+requestedPointType);
			try
			{
				Location loc = null;
				...
				else if (requestedPointType == 1) // to clanhall
				{
					...
					loc = MapRegionTable.getInstance().getTeleToLocation(
							activeChar,
							MapRegionTable.TeleportWhereType.ClanHall);
					if (ClanHallManager.getInstance()
							.getClanHallByOwner(activeChar.getClan()) != null
							&& ClanHallManager.getInstance()
									.getClanHallByOwner(activeChar.getClan())
									.getFunction(
											ClanHall.FUNC_RESTORE_EXP) != null)
						activeChar.restoreExp(ClanHallManager.getInstance()
								.getClanHallByOwner(activeChar.getClan())
				...
				} else if (requestedPointType == 2) // to castle
				{
					...
					Castle castle = CastleManager.getInstance()
							.getCastle(activeChar);
					if (castle != null && castle.getSiege().getIsInProgress())
					{
						// siege in progress
						if (castle.getSiege()
								.checkIsDefender(activeChar.getClan()))
							isInDefense = true;
					}
				...
				} else if (requestedPointType == 3) // to siege HQ
				{
					...
					Castle castle = CastleManager.getInstance()
							.getCastle(activeChar);
					if (castle != null && castle.getSiege().getIsInProgress())
						siegeClan = castle.getSiege()
								.getAttackerClan(activeChar.getClan());
					...
					loc = MapRegionTable.getInstance().getTeleToLocation(
						...
						activeChar.isFestivalParticipant()) // Player is a
															// festival
															// participant
				{
					if (!activeChar.isGM()
					...
					loc = MapRegionTable.getInstance().getTeleToLocation(
							activeChar, MapRegionTable.TeleportWhereType.Town);
	...
	void runImpl()
	{
		...
		{
			_log.warning("Living player [" + activeChar.getName()
					+ "] called RestartPointPacket! Ban this player!");
			return;
		}

		Castle castle = CastleManager.getInstance().getCastle(activeChar.getX(),
				activeChar.getY());
		if (castle != null && castle.getSiege().getIsInProgress())
		{
					...
					&& castle.getSiege().checkIsAttacker(activeChar.getClan()))
			{
				...
				ThreadPoolManager.getInstance().scheduleGeneral(
						new DeathTask(activeChar),
						castle.getSiege().getAttackerRespawnDelay());
				sm.addString("You will be re-spawned in "
						+ castle.getSiege().getAttackerRespawnDelay() / 1000
						+ " seconds");
			...
			} else
			{
				...
				ThreadPoolManager.getInstance().scheduleGeneral(
						new DeathTask(activeChar),
						castle.getSiege().getDefenderRespawnDelay());
				sm.addString("You will be re-spawned in "
						+ castle.getSiege().getDefenderRespawnDelay() / 1000
						+ " seconds");
				activeChar.sendPacket(sm);
			}
			sm = null;
		...
		ThreadPoolManager.getInstance()
				.scheduleGeneral(new DeathTask(activeChar), 1);
	}
