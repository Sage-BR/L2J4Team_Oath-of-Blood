...
public class Ride extends ServerBasePacket
{
	...
	final void runImpl()
	{
		...
		if ((cha == null) || cha.isCursedWeaponEquiped())
			return;

		// Unequip the weapon
		L2ItemInstance wpn = cha.getInventory()
				.getPaperdollItem(Inventory.PAPERDOLL_RHAND);
		if (wpn == null)
			wpn = cha.getInventory()
					.getPaperdollItem(Inventory.PAPERDOLL_LRHAND);
		if (wpn != null)
		{
			L2ItemInstance[] unequiped = cha.getInventory()
					.unEquipItemInBodySlotAndRecord(
							wpn.getItem().getBodyPart());
		...
		// Unequip the shield
		L2ItemInstance sld = cha.getInventory()
				.getPaperdollItem(Inventory.PAPERDOLL_LHAND);
		if (sld != null)
		{
			L2ItemInstance[] unequiped = cha.getInventory()
					.unEquipItemInBodySlotAndRecord(
							sld.getItem().getBodyPart());
			InventoryUpdate iu = new InventoryUpdate();
			for (L2ItemInstance element : unequiped)
				iu.addModifiedItem(element);
			cha.sendPacket(iu);
