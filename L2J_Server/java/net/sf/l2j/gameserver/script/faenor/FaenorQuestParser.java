...
{
	@Override
	public void parseScript(Node questNode, @SuppressWarnings("unused")
	BSFManager context)
	{
		if (DEBUG)
		...
		for (Node node = questNode.getFirstChild(); node != null; node = node
				.getNextSibling())
		{
			...
			throws NullPointerException
	{
		...
		for (Node node = dropList.getFirstChild(); node != null; node = node
				.getNextSibling())
		{
	...
	private void parseQuestDrop(Node drop, String questID)// throws
	...
	static
	{
		ScriptEngine.parserFactories.put(getParserName("Quest"),
				new FaenorQuestParserFactory());
	}
}
