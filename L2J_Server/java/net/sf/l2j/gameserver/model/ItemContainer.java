...
public abstract class ItemContainer
{
	protected static final Logger _log = Logger
			.getLogger(ItemContainer.class.getName());
		...
		for (L2ItemInstance item : _items)
			if (item.getItemId() == itemId
				...
				if (item.isAvailable((L2PcInstance) getOwner(), true)
						|| item.getItem().getType2() == 3)// available or quest
															// item
					if (item.isStackable())
						count = item.getCount();
					else
				...
				if (template == null)
				{
					_log.log(Level.WARNING,
							(actor != null ? "[" + actor.getName() + "] " : "")
									+ "Invalid ItemId requested: ",
							itemId);
					return null;
				}

				item = ItemTable.getInstance().createItem(process, itemId,
						template.isStackable() ? count : 1, actor, reference);
	 ...
	 */
	public L2ItemInstance addWearItem(String process, int itemId,
			L2PcInstance actor, L2Object reference)
	{
		...
		item = ItemTable.getInstance().createItem(process, itemId, 1, actor,
				reference);
			...
			ItemContainer target, L2PcInstance actor, L2Object reference)
	{
		...
		if (sourceitem == null)
			return null;
		L2ItemInstance targetitem = sourceitem.isStackable()
				? target.getItemByItemId(sourceitem.getItemId())
				: null;
		...
		synchronized (sourceitem)
		{
			...
			} else
			{
				...
				if (targetitem != null) // If possible, only update counts
					...
					targetitem = target.addItem(process, sourceitem.getItemId(),
	 ...
	 *            item or previous item in transformation
	 */
	public synchronized void destroyAllItems(String process, L2PcInstance actor,
			L2Object reference)
	{
		for (L2ItemInstance item : _items)
