...
public class L2DoormenInstance extends L2FolkInstance
{
	...
	// Logger.getLogger(L2DoormenInstance.class.getName());
	...
	public final ClanHall getClanHall()
	{
		...
		if (_ClanHall == null)
			_ClanHall = ClanHallManager.getInstance().getClanHall(getX(),
					getY(), 1500);
		// if (_ClanHall != null)
	...
	{
		player.sendPacket(new ActionFailed());
		String filename = "data/html/doormen/" + getTemplate().npcId
				+ "-no.htm";

		int condition = validateCondition(player);
		if (condition == Cond_Busy_Because_Of_Siege)
			filename = "data/html/doormen/" + getTemplate().npcId + "-busy.htm"; // Busy
																					// because
																					// of
																					// siege
		else if (condition == Cond_Castle_Owner) // Clan owns castle
			filename = "data/html/doormen/" + getTemplate().npcId + ".htm"; // Owner
																			// message
																			// window
		...
		if (getClanHall() != null)
		{
			if (condition == Cond_Hall_Owner)
			{
				str = "<html><body>Hello!<br><font color=\"55FFFF\">"
						+ getName()
			...
			{
				L2Clan owner = ClanTable.getInstance()
						.getClan(getClanHall().getOwnerId());
				if (owner != null && owner.getLeader() != null)
				{
					str = "<html><body>Hello there!<br>This clan hall is owned by <font color=\"55FFFF\">"
							+ owner.getLeader().getName()
					...
					str += "I am sorry, but only the clan members who belong to the <font color=\"55FFFF\">"
							+ owner.getName()
							+ "</font> clan can enter the clan hall.</body></html>";
				} else
					str = "<html><body>" + getName()
							+ ":<br1>Clan hall <font color=\"LEVEL\">"
							+ getClanHall().getName()
