...
public class QuestPcSpawn
{
	protected static Logger _log = Logger
			.getLogger(QuestPcSpawn.class.getName());
	...
	private List<AutoSpawnInstance> _autoSpawns = new FastList<>();

	private List<L2Spawn> _Spawns = new FastList<>();
	...
	{
		return addSpawn(npcId, getPlayer().getX(), getPlayer().getY(),
				getPlayer().getZ(), true);
	}
	...
	{
		return addSpawn(npcId, getPlayer().getX(), getPlayer().getY(),
				getPlayer().getZ(), true, spawnLength);
	}

	/**
	 * Add spawn for player instance Will random message when chat delay time
	 * reached Return object id of newly spawned npc
	 */
	public int addSpawn(int npcId, String[] messages, int chatDelay)
	{
		return addSpawn(npcId, getPlayer().getX(), getPlayer().getY(),
				getPlayer().getZ(), true, messages, chatDelay);
	}

	/**
	 * Add spawn for player instance Will despawn after the spawn length expires
	 * Will random message when chat delay time reached Return object id of
	 * newly spawned npc
	 */
	public int addSpawn(int npcId, int spawnLength, String[] messages,
			int chatDelay)
	{
		return addSpawn(npcId, getPlayer().getX(), getPlayer().getY(),
				getPlayer().getZ(), true, spawnLength, messages, chatDelay);
	}
	...
	public int addSpawn(int npcId, int x, int y, int z, boolean randomOffset)
	{
		try
		{
			...
			if (template1 != null)
			{
				...
				if ((x == 0) && (y == 0))
				{
					_log.log(Level.WARNING, getPlayer().getName()
							+ " requested quest spawn with loc 0,0.  Loc is being adjusted");

					// attempt to use the player's xyz as a the default spawn
					// location.
					x = getPlayer().getX();
					y = getPlayer().getY();
					z = getPlayer().getZ();
					// if the fail-safe also did not help, abort this spawning
					// and give a severe log
					if ((x == 0) && (y == 0))
					{
						_log.log(Level.SEVERE,
								"Failed to adjust bad locks for quest spawn requested by "
										+ getPlayer().getName()
										+ "!  Spawn aborted!");
						return 0;
					}
	 ...
	 * reached Return object id of newly spawned npc
	 */
	public int addSpawn(int npcId, int x, int y, int z, String[] messages,
			int chatDelay)
	{
	...
	public int addSpawn(int npcId, int x, int y, int z, int spawnLength,
			String[] messages, int chatDelay)
	{
		return addSpawn(npcId, x, y, z, false, spawnLength, messages,
				chatDelay);
	}
	 ...
	 */
	public int addSpawn(int npcId, int x, int y, int z, boolean randomOffset,
			int spawnLength, String[] messages, int chatDelay)
	{
	...
	/**
	 * Add random spawn for player instance Return object id of newly spawned
	 * npc
	 */
	public int addRandomSpawn(int npcId, int count, int initialDelay,
			int respawnDelay, int despawnDelay)
	{
		_autoSpawns.add(AutoSpawnHandler.getInstance().registerSpawn(npcId,
				initialDelay, respawnDelay, despawnDelay));
	 ...
	 */
	public void addRandomSpawnChat(int objectId, String[] messages,
			int chatDelay)
	{
	...
	// =========================================================
	// Method - Private
	private void addChatTask(L2NpcInstance npcInst, String[] messages,
			int chatDelay)
	{
		if ((messages != null) && (npcInst != null))
			AutoChatHandler.getInstance().registerChat(npcInst, messages,
					chatDelay);
	}
	...
	private void addDeSpawnTask(int objectId, int spawnLength)
	{
		if (spawnLength > 0)
			ThreadPoolManager.getInstance().scheduleGeneral(
					new DeSpawnScheduleTimerTask(objectId), spawnLength);
	}
	...
	{
		return AutoChatHandler.getInstance().getAutoChatInstance(objectId,
				true);
	}
