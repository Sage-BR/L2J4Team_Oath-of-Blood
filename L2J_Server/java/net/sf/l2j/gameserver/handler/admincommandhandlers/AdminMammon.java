...
public class AdminMammon implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_mammon_find", "admin_mammon_respawn", "admin_list_spawns",
				"admin_msg" };
		...
		if (command.startsWith("admin_mammon_find"))
		{
			...
			} catch (Exception NumberFormatException)
			{
				activeChar.sendMessage(
						"Command format is //mammon_find <teleportIndex> (where 1 = Blacksmith, 2 = Merchant)");
			}

			if (!_isSealValidation)
			{
				activeChar.sendMessage(
						"The competition period is currently in effect.");
				return true;
			}
			...
			{
				activeChar.sendMessage("Blacksmith of Mammon: "
						+ blackInst[0].getX() + " " + blackInst[0].getY() + " "
						+ blackInst[0].getZ());

				if (teleportIndex == 0)
					activeChar.teleToLocation(blackInst[0].getX(),
							blackInst[0].getY(), blackInst[0].getZ(), true);
			}

			if (merchInst.length > 0)
			{
				activeChar.sendMessage("Merchant of Mammon: "
						+ merchInst[0].getX() + " " + merchInst[0].getY() + " "
						+ merchInst[0].getZ());

				if (teleportIndex == 1)
					activeChar.teleToLocation(merchInst[0].getX(),
							merchInst[0].getY(), merchInst[0].getZ(), true);
			}
		}
		...
		else if (command.startsWith("admin_list_spawns"))
		{
			...
			} catch (Exception e)
			{
				activeChar.sendPacket(SystemMessage.sendString(
						"Command format is //list_spawns <NPC_ID> <TELE_INDEX>"));
			}
