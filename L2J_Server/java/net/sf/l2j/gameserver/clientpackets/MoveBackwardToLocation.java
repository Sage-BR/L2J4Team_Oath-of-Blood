...
public class MoveBackwardToLocation extends ClientBasePacket
{
	...
	// Logger.getLogger(MoveBackwardToLocation.class.getName());
	// cdddddd
	...
	public MoveBackwardToLocation(ByteBuffer buf, ClientThread client)
	{
		...
		_originY = readD();
		_originZ = readD();
		if (Config.allowL2Walker(client.getActiveChar())
				&& client.getRevision() == Config.L2WALKER_REVISION)
		{
			_moveMovement = 1;
		} else
			_moveMovement = readD(); // is 0 if cursor keys are used 1 if mouse
										// is used
	}
	...
	void runImpl()
	{
		...
		if (_moveMovement == 0 && !Config.GEODATA) // activeChar.isCastingNow()
													// || cursor movement is
													// temporary disabled
		{
			activeChar.sendPacket(new ActionFailed());
				...
				&& (activeChar.getActiveWeaponItem()
		...
		} else
		{
			...
			if (_moveMovement == 0)
			{
				Location destiny = GeoData.getInstance().moveCheck(_CurX, _CurY,
						_CurZ, _targetX, _targetY, _targetZ);
