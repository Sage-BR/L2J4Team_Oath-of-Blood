...
{
	protected static Logger _logPet = Logger
			.getLogger(L2PetInstance.class.getName());
	...
	{
		if (_Data == null)
			_Data = L2PetDataTable.getInstance().getPetData(getTemplate().npcId,
					getStat().getLevel());
	...
	class FeedTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				} else
				{
					...
					getOwner().sendMessage(
							"Your pet is too hungry to stay summoned.");
				}
					...
					if (destroyItem("Feed", food.getObjectId(), 1, null, false))
					{
						...
						if (getOwner() != null)
						{
							SystemMessage sm = new SystemMessage(
									SystemMessage.PET_TOOK_S1_BECAUSE_HE_WAS_HUNGRY);
			...
			{
				if (Config.DEBUG)
					_logPet.warning("Pet [#" + getObjectId()
							+ "] a feed task error has occurred: " + e);
			}
		}
	}

	public static L2PetInstance spawnPet(L2NpcTemplate template,
			L2PcInstance owner, L2ItemInstance control)
	{
	...
	public void onAction(L2PcInstance player)
	{
		...
		boolean thisIsTarget = player.getTarget() != null
				&& player.getTarget().getObjectId() == this.getObjectId();
	...
	{
		L2ItemInstance item = _inventory.destroyItem(process, objectId, count,
				getOwner(), reference);
	...
	public boolean destroyItemByItemId(String process, int itemId, int count,
			L2Object reference, boolean sendMessage)
	{
		L2ItemInstance item = _inventory.destroyItemByItemId(process, itemId,
				count, getOwner(), reference);
	...
	protected void doPickupItem(L2Object object)
	{
		...
		if (Config.DEBUG)
			_logPet.fine("Pet pickup pos: " + object.getX() + " "
		...
		synchronized (target)
		{
			...
			if (target.getOwnerId() != 0
					&& target.getOwnerId() != getOwner().getObjectId()
		...
		// FIXME Just send the updates if possible (old way wasn't working
		// though)
	...
	public void deleteMe(L2PcInstance owner)
	{
		...
		destroyControlItem(owner); // this should also delete the pet from the
									// db
	}
	...
	public void reduceCurrentHp(double damage, L2Character attacker)
	{
		...
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.S1_GAME_PET_S2_DMG);
	...
	public L2ItemInstance transferItem(String process, int objectId, int count,
			Inventory target, L2PcInstance actor, L2Object reference)
	{
		...
		if (target instanceof PcInventory)
		{
			...
			StatusUpdate playerSU = new StatusUpdate(
					targetPlayer.getObjectId());
			...
			for (int i = 0; (i < items.length); i++)
			{
				...
				if (((giveit.getItem().getWeight() * giveit.getCount())
						+ getOwner().getInventory()
								.getTotalWeight()) < getOwner().getMaxLoad())
				{
	...
	public void giveItemToOwner(L2ItemInstance item)
	{
		try
		{
			getInventory().transferItem("PetTransfer", item.getObjectId(),
					item.getCount(), getOwner().getInventory(), getOwner(),
					this);
			PetInventoryUpdate petiu = new PetInventoryUpdate();
	...
	public void destroyControlItem(L2PcInstance owner)
	{
		...
		try
		{
			L2ItemInstance removedItem = owner.getInventory().destroyItem(
					"PetDestroy", getControlItemId(), 1, getOwner(), this);
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("DELETE FROM pets WHERE item_obj_id=?");
	...
	{
		dropit = getInventory().dropItem("Drop", dropit.getObjectId(),
				dropit.getCount(), getOwner(), this);

		if (dropit != null)
		{
			_logPet.finer("Item id to drop: " + dropit.getItemId() + " amount: "
					+ dropit.getCount());
...
//
	...
	private static L2PetInstance restore(L2ItemInstance control,
			L2NpcTemplate template, L2PcInstance owner)
	{
		java.sql.Connection con = null;
		try
		{
			L2PetInstance pet = new L2PetInstance(
					IdFactory.getInstance().getNextId(), template, owner,
					control);
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT item_obj_id, name, level, curHp, curMp, exp, sp, karma, pkkills, fed FROM pets WHERE item_obj_id=?");
	...
	public void store()
	{
		...
		if (itemInst != null
				&& itemInst.getEnchantLevel() != getStat().getLevel())
		{
	...
	public synchronized void startFeed(boolean battleFeed)
	{
		...
		if (!isDead())
		{
			...
			// pet feed time must be different than 0. Changing time to bypass
			// divide by 0
			if (_feedTime <= 0)
			{
				_feedTime = 1;
			...
			_feedTask = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new FeedTask(),
							60000 / _feedTime, 60000 / _feedTime);
		}
	}
	...
	public void restoreExp(double restorePercent)
	{
		if (_expBeforeDeath > 0)
		{
			...
			getStat().addExp(Math.round((_expBeforeDeath - getStat().getExp())
					* restorePercent / 100));
			_expBeforeDeath = 0;
		}
	}
	...
	private void deathPenalty()
	{
		...
		long lostExp = Math.round((getStat().getExpForLevel(lvl + 1)
	...
	{
		getStat().addExpAndSp(Math.round(addToExp * Config.PET_XP_RATE),
				addToSp);
	}
