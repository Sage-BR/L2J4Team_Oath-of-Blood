...
public class RequestBuyItem extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestBuyItem.class.getName());
	 ...
	 * sample
	 *
	 * 1f 44 22 02 01 // list id 02 00 00 00 // items to buy
	 *
	 * 27 07 00 00 // item id 06 00 00 00 // count
	 *
	 * 83 06 00 00 01 00 00 00
	 *
	 * format: cdd (dd)
	 *
	 * @param decrypt
	...
	void runImpl()
	{
		...
		if ((player == null) || (!Config.ALT_GAME_KARMA_PLAYER_CAN_SHOP
				&& player.getKarma() > 0))
			return;
		...
		if (!player.isGM() && (target == null // No target (ie GM Shop)
				|| !(target instanceof L2MerchantInstance
						...
						|| target instanceof L2MercManagerInstance) // Target
																	// not a
																	// merchant,
																	// fisherman
																	// or
																	// mercmanager
				|| !player.isInsideRadius(target,
						L2NpcInstance.INTERACTION_DISTANCE, false, false) // Distance
																			// is
																			// too
																			// far
		))
		...
		if (merchant != null)
		{
			List<L2TradeList> lists = TradeController.getInstance()
					.getBuyListByNpcId(merchant.getNpcId());
		...
		{
			if (merchant != null
					&& merchant.getTemplate().npcId != _listId - 1000000)
			{
		...
		for (int i = 0; i < _count; i++)
		{
			...
			/*
			 * TODO: Disabled until Leaseholders are rewritten ;-) } else {
			 * L2ItemInstance li = merchant.findLeaseItem(itemId, 0); if (li ==
			 * null || li.getCount() < cnt) { cnt = li.getCount(); if (cnt <= 0)
			 * { items.remove(i); continue; } items.get(i).setCount((int)cnt); }
			...
			if (subTotal + tax > Integer.MAX_VALUE)
			{
				Util.handleIllegalPlayerAction(player,
						"Warning!! Character " + player.getName()
								+ " of account " + player.getAccountName()
								+ " tried to purchase over " + Integer.MAX_VALUE
		...
		if (weight > Integer.MAX_VALUE || weight < 0
				|| !player.getInventory().validateWeight((int) weight))
		{
		...
		if (slots > Integer.MAX_VALUE || slots < 0
				|| !player.getInventory().validateCapacity((int) slots))
		{
		...
		if ((subTotal < 0) || !player.reduceAdena("Buy", (int) (subTotal + tax),
				player.getLastFolkNPC(), false))
		{
		...
		for (int i = 0; i < _count; i++)
		{
			...
			player.getInventory().addItem("Buy", itemId, count, player,
					merchant);

			/*
			 * TODO: Disabled until Leaseholders are rewritten ;-) // Update
			 * Leaseholder list if (_listId >= 1000000) { L2ItemInstance li =
			 * merchant.findLeaseItem(item.getItemId(), 0); if (li == null)
			 * continue; if (li.getCount() < item.getCount())
			 * item.setCount(li.getCount()); li.setCount(li.getCount() -
			 * item.getCount()); li.updateDatabase(); price = item.getCount() +
			 * li.getPriceToSell(); L2ItemInstance la =
			 * merchant.getLeaseAdena(); la.setCount(la.getCount() + price);
			 *
			 * la.updateDatabase(); player.getInventory().addItem(item);
			 * item.updateDatabase(); }
			 */
		}
		...
		if (merchant != null)
		{
			String html = HtmCache.getInstance().getHtm("data/html/"
					+ htmlFolder + "/" + merchant.getNpcId() + "-bought.htm");

			if (html != null)
			{
				NpcHtmlMessage boughtMsg = new NpcHtmlMessage(
						merchant.getObjectId());
				boughtMsg.setHtml(html.replaceAll("%objectId%",
						String.valueOf(merchant.getObjectId())));
