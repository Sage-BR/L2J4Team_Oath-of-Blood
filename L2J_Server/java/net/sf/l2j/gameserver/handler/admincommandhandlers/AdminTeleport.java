...
/**
 * This class handles following admin commands: - show_moves - show_teleport -
 * teleport_to_character - move_to - teleport_character
 *
...
public class AdminTeleport implements IAdminCommandHandler
{
	private static final Logger _log = Logger
			.getLogger(AdminTeleport.class.getName());

	private static String[] _adminCommands =
		{ "admin_show_moves", "admin_show_moves_other", "admin_show_teleport",
				"admin_teleport_to_character", "admin_teleportto",
				"admin_move_to", "admin_teleport_character", "admin_recall",
				"admin_walk", "admin_explore", "teleportto", "recall",
				"admin_recall_npc", "admin_gonorth", "admin_gosouth",
				"admin_goeast", "admin_gowest", "admin_goup", "admin_godown",
				"admin_tele", "admin_teleto", "admin_failed" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		} else if (command.equals("admin_explore")
		...
		} else if (command.startsWith("admin_walk"))
		{
			try
			{
				...
				activeChar.getAI()
						.setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, pos);
			} catch (Exception e)
			{
				if (Config.DEBUG)
		...
		} else if (command.startsWith("admin_recall "))
		{
			try
			{
				...
				L2PcInstance player = L2World.getInstance()
						.getPlayer(targetName);
				if (activeChar.getAccessLevel() >= REQUIRED_LEVEL2)
					teleportCharacter(player, activeChar.getX(),
	...
	private void teleportToCharacter(L2PcInstance activeChar, L2Object target)
	{
		...
		} else
		{
			...
			activeChar.sendMessage("You have teleported to character "
					+ player.getName() + ".");
		}
	}
	...
	private void recallNPC(L2PcInstance activeChar)
	{
		...
		if ((obj != null) && (obj instanceof L2NpcInstance))
		{
			...
			L2NpcTemplate template1 = NpcTable.getInstance()
					.getTemplate(monsterTemplate);
			if (template1 == null)
			{
				activeChar.sendMessage("Incorrect monster template.");
				_log.warning("ERROR: NPC " + target.getObjectId()
						+ " has a 'null' template.");
				return;
			}
			...
			try
			{
				...
				if (Config.DEBUG)
				{
					_log.fine("Spawn at X=" + spawn.getLocx() + " Y="
							+ spawn.getLocy() + " Z=" + spawn.getLocz());
					_log.warning("GM: " + activeChar.getName() + "("
							+ activeChar.getObjectId() + ") moved NPC "
							+ target.getObjectId());
				}
			} catch (Exception e)
			{
