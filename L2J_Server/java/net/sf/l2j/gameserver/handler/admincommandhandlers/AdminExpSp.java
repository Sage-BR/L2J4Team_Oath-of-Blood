...
/**
 * This class handles following admin commands: - add_exp_sp_to_character = show
 * menu - add_exp_sp exp sp = adds exp & sp to target
 *
...
public class AdminExpSp implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_add_exp_sp_to_character", "admin_add_exp_sp",
				"admin_remove_exp_sp" };
	...
	private void addExpSp(L2PcInstance activeChar)
	{
		...
		replyMSG.append("<tr><td>Class: " + player.getTemplate().className
				+ "</td></tr>");
		replyMSG.append("<tr><td>Exp: " + player.getExp() + "</td></tr>");
		replyMSG.append("<tr><td>SP: " + player.getSp() + "</td></tr></table>");
		replyMSG.append(
				"<table width=270><tr><td>Note: Fill BOTH values before saving the modifications</td></tr>");
		replyMSG.append(
				"<tr><td>and use 0 if no changes are needed.</td></tr></table><br>");
		...
		replyMSG.append(
				"<td>&nbsp;<center><button value=\"Add\" action=\"bypass -h admin_add_exp_sp $exp_to_add $sp_to_add\" width=50 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center></td>");
		replyMSG.append(
				"<td>&nbsp;<center><button value=\"Remove\" action=\"bypass -h admin_remove_exp_sp $exp_to_add $sp_to_add\" width=70 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center></td>");
	...
	private void removeExp(L2PcInstance activeChar)
	{
		...
		replyMSG.append("<center><table><tr>");
		replyMSG.append(
				"<td>remove Exp: <edit var=\"exp_to_remove\" width=50></td>");
		replyMSG.append(
				"<td>remove SP:  <edit var=\"sp_to_remove\" width=50></td>");
		replyMSG.append(
				"<td>&nbsp;<button value=\"Save Changes\" action=\"bypass -h admin_remove_exp_sp $exp_to_remove $sp_to_remove\" width=80 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
		replyMSG.append("</tr></table></center>");
		replyMSG.append("</body></html>");
	...
	private void adminAddExpSp(L2PcInstance activeChar, String ExpSp)
	{
		...
		} else
		{
			...
			if (expval != 0 || spval != 0)
			{
				...
				SystemMessage sm = new SystemMessage(614);
				sm.addString("Admin is adding you " + expval + " xp and "
						+ spval + " sp.");
				...
				// Admin information
				SystemMessage smA = new SystemMessage(614);
				smA.addString("Added " + expval + " xp and " + spval + " sp to "
						+ player.getName() + ".");
				activeChar.sendPacket(smA);
				if (Config.DEBUG)
				{
					_log.fine("GM: " + activeChar.getName() + "("
							+ activeChar.getObjectId() + ") added " + expval
							+ " xp and " + spval + " sp to "
							+ player.getObjectId() + ".");
				}
	...
	private void adminRemoveExpSP(L2PcInstance activeChar, String ExpSp)
	{
		...
		} else
		{
			...
			if (expval != 0 || spval != 0)
			{
				// Common character information
				SystemMessage sm = new SystemMessage(614);
				sm.addString("Admin is removing you " + expval + " xp and "
						+ spval + " sp.");
				player.sendPacket(sm);
				...
				// Admin information
				SystemMessage smA = new SystemMessage(614);
				smA.addString("Removed " + expval + " xp and " + spval
						+ " sp from " + player.getName() + ".");
				activeChar.sendPacket(smA);
				if (Config.DEBUG)
				{
					_log.fine("GM: " + activeChar.getName() + "("
