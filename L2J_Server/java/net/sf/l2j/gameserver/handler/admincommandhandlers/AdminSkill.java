...
public class AdminSkill implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_show_skills", "admin_remove_skills", "admin_skill_list",
				"admin_skill_index", "admin_add_skill", "admin_remove_skill",
				"admin_get_skills", "admin_reset_skills",
				"admin_give_all_skills", "admin_remove_all_skills" };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		String target = (activeChar.getTarget() != null
		...
		else if (command.equals("admin_remove_all_skills"))
		{
			if (activeChar.getTarget() instanceof L2PcInstance)
			{
				...
				activeChar.sendMessage(
						"You removed all skills from " + player.getName());
	...
	private void adminGiveAllSkills(L2PcInstance activeChar)
	{
		...
		activeChar.sendMessage(
				"You gave " + skillCounter + " skills to " + player.getName());
	}
	...
	private void removeSkillsPage(L2PcInstance activeChar, int page)
	{
		...
		replyMSG.append(
				"<td width=40><button value=\"Back\" action=\"bypass -h admin_show_skills\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td>");
		replyMSG.append("</tr></table>");
		replyMSG.append("<br><br>");
		replyMSG.append("<center>Editing <font color=\"LEVEL\">"
				+ player.getName() + "</font></center>");
		replyMSG.append("<br><table width=270><tr><td>Lv: " + player.getLevel()
				+ " " + player.getTemplate().className + "</td></tr></table>");
		replyMSG.append(
		...
		{
			int pagenr = x + 1;
			pages += "<td><a action=\"bypass -h admin_remove_skills " + x
					+ "\">Page " + pagenr + "</a></td>";
		}
		...
		for (int i = SkillsStart; i < SkillsEnd; i++)
		{
			replyMSG.append(
					"<tr><td width=80><a action=\"bypass -h admin_remove_skill "
							+ skills[i].getId() + "\">" + skills[i].getName()
							+ "</a></td><td width=60>" + skills[i].getLevel()
							+ "</td><td width=40>" + skills[i].getId()
							+ "</td></tr>");
		}
		...
		replyMSG.append(
				"<center><button value=\"Remove skill\" action=\"bypass -h admin_remove_skill $id_to_remove\" width=110 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></center>");
		replyMSG.append(
				"<br><center><button value=\"Back\" action=\"bypass -h admin_current_player\" width=40 height=15></center>");
	...
	private void showSkillsPage(L2PcInstance activeChar)
	{
		...
		replyMSG.append(
				"<tr><td><button value=\"Give All Skills\" action=\"bypass -h admin_give_all_skills\" width=70 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr>");
		replyMSG.append("</table></center>");
		replyMSG.append("</body></html>");
	...
	private void adminResetSkills(L2PcInstance activeChar)
	{
		...
		if (adminSkills == null)
		{
			SystemMessage smA = new SystemMessage(614);
			smA.addString(
					"You must first get the skills of someone to do this.");
			activeChar.sendPacket(smA);
		} else
		{
			L2Skill[] skills = player.getAllSkills();
			...
			SystemMessage sm = new SystemMessage(614);
			sm.addString("[GM]" + activeChar.getName()
					+ " has updated your skills.");
			player.sendPacket(sm);
			SystemMessage smA = new SystemMessage(614);
	...
	private void adminAddSkill(L2PcInstance activeChar, String val)
	{
		...
		} else
		{
			...
			if (skill != null)
			{
				SystemMessage sm = new SystemMessage(614);
				sm.addString(
						"Admin gave you the skill " + skill.getName() + ".");
				player.sendPacket(sm);
				...
				// Admin information
				SystemMessage smA = new SystemMessage(614);
				smA.addString("You gave the skill " + skill.getName() + " to "
						+ player.getName() + ".");

				activeChar.sendPacket(smA);
				if (Config.DEBUG)
					_log.fine("[GM]" + activeChar.getName() + "gave the skill "
							+ skill.getName() + " to " + player.getName()
							+ ".");
			} else
			{
				SystemMessage smA = new SystemMessage(614);
	...
	private void adminRemoveSkill(L2PcInstance activeChar, int idval)
	{
		...
		L2Skill skill = SkillTable.getInstance().getInfo(idval,
				player.getSkillLevel(idval));
		...
		if (skill != null)
		{
			...
			// Admin information
			SystemMessage smA = new SystemMessage(614);
			smA.addString("You removed the skill " + skill.getName() + " from "
					+ player.getName() + ".");

			activeChar.sendPacket(smA);
			if (Config.DEBUG)
				_log.fine("[GM]" + activeChar.getName() + "removed the skill "
						+ skill.getName() + " from " + player.getName() + ".");
		} else
		{
			SystemMessage smA = new SystemMessage(614);
			smA.addString("Error: there is no such skill.");
	...
	public void showSkill(L2PcInstance activeChar, String val)
	{
		...
		if (skill != null)
		{
			if (skill.getTargetType() == L2Skill.SkillTargetType.TARGET_SELF)
			{
				...
				MagicSkillUser msk = new MagicSkillUser(activeChar, skillid, 1,
						skill.getSkillTime(), skill.getReuseDelay());
				activeChar.broadcastPacket(msk);
				if (Config.DEBUG)
					_log.fine("showing self skill, id: " + skill.getId()
							+ " named: " + skill.getName());
			} else if (skill
					.getTargetType() == L2Skill.SkillTargetType.TARGET_ONE)
			{
				if (Config.DEBUG)
					_log.fine("showing ATTACK skill, id: " + skill.getId()
							+ " named: " + skill.getName());
			}
		} else
		{
