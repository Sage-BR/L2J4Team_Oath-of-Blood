...
public class AdminBan implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if (command.startsWith("admin_ban"))
		{
			...
			if (plyr != null && !plyr.equals(activeChar)) // you cannot ban
															// yourself!
			{
				account_name = plyr.getAccountName();
				LoginServerThread.getInstance().sendAccessLevel(account_name,
						-100);
				plyr.logout();
			}
		...
		} else if (command.startsWith("admin_jail"))
		{
			try
			{
				...
				if (playerObj != null)
				{
					...
					activeChar.sendMessage("Character " + player
							+ " jailed for "
			...
			try
			{
				...
				if (playerObj != null)
				{
					...
					activeChar.sendMessage(
							"Character " + player + " removed from jail");
				} else
	...
	private void jailOfflinePlayer(L2PcInstance activeChar, String name,
			int delay)
	{
		Connection con = null;
		try
		{
			...
			if (count == 0)
				...
				activeChar.sendMessage("Character " + name + " jailed for "
						+ (delay > 0 ? delay + " minutes." : "ever!"));
	...
	private void unjailOfflinePlayer(L2PcInstance activeChar, String name)
	{
		...
		try
		{
			...
			if (count == 0)
				...
				activeChar.sendMessage(
						"Character " + name + " removed from jail");
		} catch (SQLException se)
		{
			activeChar.sendMessage("SQLException while jailing player");
			if (Config.DEBUG)
