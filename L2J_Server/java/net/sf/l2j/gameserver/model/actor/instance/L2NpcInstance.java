...
public class L2NpcInstance extends L2Character
{
	...
	// Logger.getLogger(L2NpcInstance.class.getName());
	 ...
	 * L2Character (copy skills from template to object and link _calculators to
	...
	public L2NpcInstance(int objectId, L2NpcTemplate template)
	{
		...
		{
			_log.severe(
					"No template for Npc. Please check your datapack is setup correctly.");
			return;
		}
		...
		// calculated delay if the server allow it
	...
	public NpcKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof NpcKnownList))
	...
	public NpcStatus getStatus()
	{
		if (super.getStatus() == null
				|| !(super.getStatus() instanceof NpcStatus))
			this.setStatus(new NpcStatus(this));
		return (NpcStatus) super.getStatus();
	}

	/**
	 * Check if the server allow Random Animation.<BR>
	 * <BR>
	 */
	...
	/**
	 * Return the generic Identifier of this L2NpcInstance contained in the
	 * L2NpcTemplate.<BR>
	 * <BR>
	 */
	public int getNpcId()
	 ...
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * If a NPC belows to a Faction, other NPC of the faction inside the Faction
	 * range will help it if it's attacked<BR>
	 * <BR>
	 *
	 */
	 ...
	 * in the _KnownPlayers of the L2NpcInstance.<BR>
	 * <BR>
	 */
	@Override
	...
	public int getDistanceToWatchObject(L2Object object)
	{
		...
		if (object instanceof L2FolkInstance
	 ...
	 * _knownObject in function of the object type.<BR>
	 * <BR>
	 *
	 * <B><U> Values </U> :</B><BR>
	 * <BR>
	 ...
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2MonsterInstance : Check if the attacker is not another
	 * L2MonsterInstance</li>
	 ...
	 * contained in the L2NpcTemplate.<BR>
	 * <BR>
	 */
	 ...
	 * necessary)</li>
	 * <li>Send a Server->Client packet MyTargetSelected to the L2PcInstance
	 * player (display the select window)</li>
	 * <li>If L2NpcInstance is autoAttackable, send a Server->Client packet
	 * StatusUpdate to the L2PcInstance in order to update L2NpcInstance HP bar
	 * </li>
	 * <li>Send a Server->Client packet ValidateLocation to correct the
	 * L2NpcInstance position and heading on the client</li><BR>
	 * <BR>
	 *
	 ...
	 * <li>Send a Server->Client packet MyTargetSelected to the L2PcInstance
	 * player (display the select window)</li>
	 * <li>If L2NpcInstance is autoAttackable, notify the L2PcInstance AI with
	 * AI_INTENTION_ATTACK (after a height verification)</li>
	 * <li>If L2NpcInstance is NOT autoAttackable, notify the L2PcInstance AI
	 * with AI_INTENTION_INTERACT (after a distance verification) and show
	 * message</li><BR>
	 * <BR>
	 *
	 ...
	 * client wait an other packet</B></FONT><BR>
	 * <BR>
	 *
	 ...
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2ArtefactInstance : Manage only fisrt click to select
	 * Artefact</li><BR>
	 * <BR>
	...
	public void onAction(L2PcInstance player)
	{
		...
		if (this != player.getTarget())
		{
			...
			// player
			// The player.getLevel() - getLevel() permit to display the correct
			// color in the select window
			MyTargetSelected my = new MyTargetSelected(getObjectId(),
		...
		} else
		{
			...
			if (!isAutoAttackable(player))
			{
				...
				if (!isInsideRadius(player, INTERACTION_DISTANCE, false, false))
				{
					...
					// Send a Server->Client packet ActionFailed (target is out
					// of interaction range) to the L2PcInstance player
					player.sendPacket(new ActionFailed());
				} else
				{
	 ...
	 * StatusUpdate to the L2PcInstance in order to update L2NpcInstance HP bar
	 * </li>
	 * <li>Send a Server->Client NpcHtmlMessage() containing the GM console
	 * about this L2NpcInstance</li><BR>
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Each group of Server->Client
	 * packet must be terminated by a ActionFailed packet in order to avoid that
	 ...
	 *
	 * @param client
	 *            The thread that manage the player that pessed Shift and click
	...
	public void onActionShift(ClientThread client)
	{
		...
		if (player.getAccessLevel() >= Config.GM_ACCESSLEVEL)
		{
			...
			// Send a Server->Client NpcHtmlMessage() containing the GM console
			...
			html1.append("Instance Type: " + className + "<br1>Faction: "
					+ getFactionId() + "<br1>Location ID: "
					+ (getSpawn() != null ? getSpawn().getLocation() : 0)
					+ "<br1>");

			if (this instanceof L2ControllableMobInstance)
				html1.append("Mob Group: " + MobGroupTable.getInstance()
						.getGroupForMob((L2ControllableMobInstance) this)
						.getGroupId() + "<br>");
			else
				html1.append("Respawn Time: "
						+ (getSpawn() != null
								? (getSpawn().getRespawnDelay() / 1000)
										+ "  Seconds<br>"
								: "?  Seconds<br>"));

			html1.append("<table border=\"0\" width=\"100%\">");
			html1.append("<tr><td>Object ID</td><td>" + getObjectId()
					+ "</td><td>NPC ID</td><td>" + getTemplate().npcId
					+ "</td></tr>");
			html1.append("<tr><td>Castle</td><td>" + getCastle().getCastleId()
					+ "</td><td>Coords</td><td>" + getX() + "," + getY() + ","
					+ getZ() + "</td></tr>");
			html1.append("<tr><td>Level</td><td>" + getLevel()
					+ "</td><td>Aggro</td><td>"
					+ ((this instanceof L2Attackable)
							? ((L2Attackable) this).getAggroRange()
							: 0)
					+ "</td></tr>");
			...
			html1.append("<table border=\"0\" width=\"100%\">");
			html1.append("<tr><td>Current HP</td><td>" + getCurrentHp()
					+ "</td><td>Current MP</td><td>" + getCurrentMp()
					+ "</td></tr>");
			html1.append("<tr><td>Max.HP</td><td>"
					+ (int) (getMaxHp()
							/ getStat().calcStat(Stats.MAX_HP, 1, this, null))
					+ "*" + getStat().calcStat(Stats.MAX_HP, 1, this, null)
					+ "</td><td>Max.MP</td><td>" + getMaxMp() + "</td></tr>");
			html1.append("<tr><td>P.Atk.</td><td>" + getPAtk(null)
					+ "</td><td>M.Atk.</td><td>" + getMAtk(null, null)
					+ "</td></tr>");
			html1.append("<tr><td>P.Def.</td><td>" + getPDef(null)
					+ "</td><td>M.Def.</td><td>" + getMDef(null, null)
					+ "</td></tr>");
			html1.append("<tr><td>Accuracy</td><td>" + getAccuracy()
					+ "</td><td>Evasion</td><td>" + getEvasionRate(null)
					+ "</td></tr>");
			html1.append("<tr><td>Critical</td><td>"
					+ getCriticalHit(null, null) + "</td><td>Speed</td><td>"
					+ getRunSpeed() + "</td></tr>");
			html1.append("<tr><td>Atk.Speed</td><td>" + getPAtkSpd()
					+ "</td><td>Cast.Speed</td><td>" + getMAtkSpd()
					+ "</td></tr>");
			html1.append("</table><br>");

			html1.append("<font color=\"LEVEL\">Basic Stats</font>");
			html1.append("<table border=\"0\" width=\"100%\">");
			html1.append("<tr><td>STR</td><td>" + getSTR()
					+ "</td><td>DEX</td><td>" + getDEX()
					+ "</td><td>CON</td><td>" + getCON() + "</td></tr>");
			html1.append("<tr><td>INT</td><td>" + getINT()
					+ "</td><td>WIT</td><td>" + getWIT()
					+ "</td><td>MEN</td><td>" + getMEN() + "</td></tr>");
			html1.append("</table>");

			html1.append(
					"<br><center><table><tr><td><button value=\"Edit NPC\" action=\"bypass -h admin_edit_npc "
							+ getTemplate().npcId
							+ "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"><br1></td>");
			html1.append(
					"<td><button value=\"Kill\" action=\"bypass -h admin_kill\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td><br1></tr>");
			html1.append(
			...
			html1.append(
					"<td><button value=\"Delete\" action=\"bypass -h admin_delete\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr>");
		...
		} else if (Config.ALT_GAME_VIEWNPC)
		{
			...
			html1.append("<tr><td>Max.HP</td><td>"
					+ (int) (getMaxHp()
							/ getStat().calcStat(Stats.MAX_HP, 1, this, null))
					+ "*"
					+ (int) getStat().calcStat(Stats.MAX_HP, 1, this, null)
					+ "</td><td>Max.MP</td><td>" + getMaxMp() + "</td></tr>");
			html1.append("<tr><td>P.Atk.</td><td>" + getPAtk(null)
					+ "</td><td>M.Atk.</td><td>" + getMAtk(null, null)
					+ "</td></tr>");
			...
			html1.append("<tr><td>Atk.Speed</td><td>" + getPAtkSpd()
					+ "</td><td>Cast.Speed</td><td>" + getMAtkSpd()
					+ "</td></tr>");
			html1.append("<tr><td>Race</td><td>" + getTemplate().race
					+ "</td><td></td><td></td></tr>");
			html1.append("</table>");
			...
			html1.append(
					"Rates legend: <font color=\"ff0000\">50%+</font> <font color=\"00ff00\">30%+</font> <font color=\"0000ff\">less than 30%</font>");
			html1.append("<table border=0 width=\"100%\">");

			for (L2DropCategory cat : getTemplate().getDropData())
				for (L2DropData drop : cat.getAllDrops())
				{
					String name = ItemTable.getInstance()
							.getTemplate(drop.getItemId()).getName();

					if (drop.getChance() >= 600000)
						html1.append("<tr><td><font color=\"ff0000\">" + name
								+ "</font></td><td>"
								+ (drop.isQuestDrop() ? "Quest"
										: (cat.isSweep() ? "Sweep" : "Drop"))
								+ "</td></tr>");
					else if (drop.getChance() >= 300000)
						html1.append("<tr><td><font color=\"00ff00\">" + name
								+ "</font></td><td>"
								+ (drop.isQuestDrop() ? "Quest"
										: (cat.isSweep() ? "Sweep" : "Drop"))
								+ "</td></tr>");
					else
						html1.append("<tr><td><font color=\"0000ff\">" + name
								+ "</font></td><td>"
								+ (drop.isQuestDrop() ? "Quest"
										: (cat.isSweep() ? "Sweep" : "Drop"))
								+ "</td></tr>");
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		} else
		{
			if (isBusy() && getBusyMessage().length() > 0)
			{
				...
				player.sendPacket(html);
			} else if (Config.ALLOW_WYVERN_UPGRADER
					&& command.startsWith("upgrade") && player.getClan() != null
					&& player.getClan().getHasCastle() != 0)
			{
				...
				{
					L2NpcTemplate wind = NpcTable.getInstance()
							.getTemplate(L2PetDataTable.getStriderWindId());
					L2NpcTemplate star = NpcTable.getInstance()
							.getTemplate(L2PetDataTable.getStriderStarId());
					L2NpcTemplate twilight = NpcTable.getInstance()
							.getTemplate(L2PetDataTable.getStriderTwilightId());

					L2Summon summon = player.getPet();
					L2NpcTemplate myPet = summon.getTemplate();

					if ((myPet.equals(wind) || myPet.equals(star)
							|| myPet.equals(twilight))
							&& player.getAdena() >= 20000000
							&& (player.getInventory().getItemByObjectId(
									summon.getControlItemId()) != null))
					{
						int exchangeItem = L2PetDataTable.getWyvernItemId();
						if (!player.reduceAdena("PetUpdate", 20000000, this,
								true))
							return;
						player.getInventory().destroyItem("PetUpdate",
								summon.getControlItemId(), 1, player, this);

						L2NpcTemplate template1 = NpcTable.getInstance()
								.getTemplate(629);
						try
						{
							...
							spawn.getLastSpawn().spawnMe(
									spawn.getLastSpawn().getX(),
									spawn.getLastSpawn().getY(),
							...
							spawn.getLastSpawn()
									.broadcastPacket(new MagicSkillUser(
											spawn.getLastSpawn(),
											spawn.getLastSpawn(), 1034, 1, 1,
											1));
							spawn.getLastSpawn()
									.broadcastPacket(new MagicSkillUser(
											spawn.getLastSpawn(), summon, 1034,
											1, 1, 1));

							if (Rnd.nextInt(100) < chance)
							{
								ThreadPoolManager.getInstance().scheduleGeneral(
										new destroyTemporalSummon(summon,
												player),
										6000);
								player.getInventory().addItem("PetUpdate",
										exchangeItem, 1, player, this);
								...
								TextBuilder replyMSG = new TextBuilder(
										"<html><body>");
								replyMSG.append(
										"Congratulations, the evolution suceeded.");
								replyMSG.append("</body></html>");
								adminReply.setHtml(replyMSG.toString());
								player.sendPacket(adminReply);
							} else
							{
								summon.reduceCurrentHp(summon.getCurrentHp(),
										player);
							}
							ThreadPoolManager.getInstance().scheduleGeneral(
									new destroyTemporalNPC(spawn), 15000);
					...
					} else
					{
						...
						replyMSG.append(
								"You will need 20.000.000 and have the pet summoned for the ceremony ...");
						replyMSG.append("</body></html>");
				...
				{
					L2NpcTemplate wind = NpcTable.getInstance()
							.getTemplate(L2PetDataTable.getHatchlingWindId());
					L2NpcTemplate star = NpcTable.getInstance()
							.getTemplate(L2PetDataTable.getHatchlingStarId());
					L2NpcTemplate twilight = NpcTable.getInstance().getTemplate(
							L2PetDataTable.getHatchlingTwilightId());
					...
					if ((myPet.equals(wind) || myPet.equals(star)
							...
							&& (player.getInventory().getItemByObjectId(
									summon.getControlItemId()) != null))
					{
						int exchangeItem = L2PetDataTable
								.getStriderTwilightItemId();
						if (myPet.equals(wind))
							exchangeItem = L2PetDataTable
									.getStriderWindItemId();
						else if (myPet.equals(star))
							exchangeItem = L2PetDataTable
									.getStriderStarItemId();

						if (!player.reduceAdena("PetUpdate", 6000000, this,
								true))
							return;
						player.getInventory().destroyItem("PetUpdate",
								summon.getControlItemId(), 1, player, this);
					...
					} else
					{
						...
						replyMSG.append(
								"You will need 6.000.000 and have the pet summoned for the ceremony ...");
						replyMSG.append("</body></html>");
			...
			} else if (command.equalsIgnoreCase("TerritoryStatus"))
			{
				...
				if (getIsInTown())
				{
					...
					html.replace("%taxpercent%",
							"" + getCastle().getTaxPercent());
			...
			} else if (command.startsWith("multisell"))
			{
				try
				{
					MultiSellList multisell = new MultiSellList(
							Integer.parseInt(command.substring(9).trim()),
							this);
			...
			{
				player.sendPacket(new MultiSellList(
						Integer.parseInt(command.substring(13).trim()), this,
						true, player));
			} else if (command.startsWith("npcfind_byid"))
			{
				try
				{
					L2Spawn spawn = SpawnTable.getInstance().getTemplate(
							Integer.parseInt(command.substring(12).trim()));

					if (spawn != null)
						player.sendPacket(
								new RadarControl(1, 1, spawn.getLocx(),
	 ...
	 * the L2NpcInstance and create a new RandomAnimation Task.<BR>
	 * <BR>
	 */
	public void onRandomAnimation()
	{
		...
		// Create a new RandomAnimation Task that will be launched after the
		// calculated delay
		startRandomAnimation();
	}
	...
	public void startRandomAnimation()
	{
		...
		ThreadPoolManager.getInstance()
				.scheduleGeneral(new RandomAnimationTask(), interval);
	}
	...
	{
		// Get the weapon identifier equiped in the right hand of the
		// L2NpcInstance
	...
	public String getHtmlPath(int npcId, int val)
	{
		...
		// If the file is not found, the standard message "I have nothing to say
		// to you" is returned
	 ...
	 * L2NpcInstance to the L2PcInstance</li><BR>
	 * <BR>
	 *
	 * @param player
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the quest state in the folder
	 * data/jscript/quests/questId/stateId.htm</li>
	 * <li>Send a Server->Client NpcHtmlMessage containing the text of the
	 * L2NpcInstance to the L2PcInstance</li>
	 * <li>Send a Server->Client ActionFailed to the L2PcInstance in order to
	 * avoid that the client wait another packet</li><BR>
	 * <BR>
	 *
	 * @param player
	 *            The L2PcInstance that talk with the L2NpcInstance
	...
	public void showQuestWindow(L2PcInstance player, String questId)
	{
		...
		} else
		{
			...
			if (q != null)
			{
				...
				Quest[] qlst = getTemplate()
						.getEventQuests(Quest.QuestEventType.QUEST_START);
				...
				if (qlst != null && qlst.length > 0)
				{
					for (Quest element : qlst)
					{
						if (element == q)
						{
							...
							// disabled by mr. becouse quest dialog only show on
							// second click.
		...
		} else
		{
			...
			String path = "data/jscript/quests/" + questId + "/" + stateId
					+ ".htm";
			...
			if (Config.DEBUG)
			{
				...
				{
					_log.fine("File not exists for quest " + questId
							+ " html path: " + path);
				}
			}
		}
	 ...
	 * several available) or QuestWindow.<BR>
	 * <BR>
	 *
	 * @param player
	 *            The L2PcInstance that talk with the L2NpcInstance
	...
	public void showQuestWindow(L2PcInstance player)
	{
		...
		Quest[] starts = getTemplate()
				.getEventQuests(Quest.QuestEventType.QUEST_START);

		// Quests are limited between 1 and 999 because those are the quests
		// that are supported by the client.
		// By limitting them there, we are allowed to create custom quests at
		...
		if (options.size() > 1)
		{
			showQuestChooseWindow(player,
					options.toArray(new Quest[options.size()]));
	...
	public void showLotoWindow(L2PcInstance player, int val)
	{
		...
		if (val == 0) // 0 - first buy lottery ticket window
		{
			...
			html.setFile(filename);
		} else if (val >= 1 && val <= 21) // 1-20 - buttons, 21 - second buy
			...
			for (int i = 0; i < 5; i++)
				if (player.getLoto(i) > 0)
				{
					...
					if (player.getLoto(i) < 10)
						button = "0" + button;
					String search = "fore=\"L2UI.lottoNum" + button
							+ "\" back=\"L2UI.lottoNum" + button + "a_check\"";
					String replace = "fore=\"L2UI.lottoNum" + button
							+ "a_check\" back=\"L2UI.lottoNum" + button + "\"";
		...
		} else if (val == 22) // 22 - selected ticket with 5 numbers
		{
			...
			L2ItemInstance item = new L2ItemInstance(
					IdFactory.getInstance().getNextId(), 4442);
			item.setCount(1);
			...
			iu.addItem(item);
			L2ItemInstance adenaupdate = player.getInventory()
					.getItemByItemId(57);
		...
		} else if (val == 24) // 24 - Previous winning numbers/Prize claim
		{
			...
			for (L2ItemInstance item : player.getInventory().getItems())
			{
				if (item == null)
					continue;
				if (item.getItemId() == 4442
						&& item.getCustomType1() < lotonumber)
				{
					message = message
							+ "<a action=\"bypass -h npc_%objectId%_Loto "
							+ item.getObjectId() + "\">" + item.getCustomType1()
							+ " Event Number ";
					int[] numbers = Lottery.getInstance().decodeNumbers(
							item.getEnchantLevel(), item.getCustomType2());
					for (int i = 0; i < 5; i++)
					{
						message += numbers[i] + " ";
					}
		...
		} else if (val > 24) // >24 - check lottery ticket by item object id
		{
			...
			if (item == null || item.getItemId() != 4442
					|| item.getCustomType1() >= lotonumber)
				return;
			int[] check = Lottery.getInstance().checkTicket(item);
		...
		html.replace("%enddate%", "" + DateFormat.getDateInstance()
				.format(Lottery.getInstance().getEndDate()));
		player.sendPacket(html);
	...
	public void makeCPRecovery(L2PcInstance player)
	{
		...
		int neededmoney = 100;
		SystemMessage sm;
		if (!player.reduceAdena("RestoreCP", neededmoney,
				player.getLastFolkNPC(), true))
			return;
	...
	public void makeSupportMagic(L2PcInstance player)
	{
		...
		// Calculate the min and max level between wich the player must be to
		// obtain buff
		if (player.isMageClass())
		{
			lowestLevel = HelperBuffTable.getInstance()
					.getMagicClassLowestLevel();
			higestLevel = HelperBuffTable.getInstance()
					.getMagicClassHighestLevel();
		} else
		{
			lowestLevel = HelperBuffTable.getInstance()
					.getPhysicClassLowestLevel();
			higestLevel = HelperBuffTable.getInstance()
					.getPhysicClassHighestLevel();
		}
		...
		{
			String content = "<html><body>Come back here when you have reached level "
					+ lowestLevel
		...
		L2Skill skill = null;
		// Go through the Helper Buff list define in sql table helper_buff_list
		// and cast skill
		for (L2HelperBuff helperBuffItem : HelperBuffTable.getInstance()
				.getHelperBuffTable())
		{
			if (helperBuffItem.isMagicClassBuff() == player.isMageClass())
			{
						...
						&& player_level <= helperBuffItem.getUpperLevel())
				{
					skill = SkillTable.getInstance().getInfo(
							helperBuffItem.getSkillID(),
							helperBuffItem.getSkillLevel());
	...
	private boolean showPkDenyChatWindow(L2PcInstance player, String type)
	{
		String html = HtmCache.getInstance()
				.getHtm("data/html/" + type + "/" + getNpcId() + "-pk.htm");
	...
	public void showChatWindow(L2PcInstance player, int val)
	{
		if (player.getKarma() > 0)
		{
			if (this instanceof L2MerchantInstance
					&& !Config.ALT_GAME_KARMA_PLAYER_CAN_SHOP)
			{
				if (showPkDenyChatWindow(player, "merchant"))
					return;
			} else if (this instanceof L2TeleporterInstance
					&& !Config.ALT_GAME_KARMA_PLAYER_CAN_USE_GK)
			{
			...
			} else if (this instanceof L2WarehouseInstance)
			{
				if (showPkDenyChatWindow(player, "warehouse")
						&& !Config.ALT_GAME_KARMA_PLAYER_CAN_USE_WAREHOUSE)
					return;
			} else if (this instanceof L2FishermanInstance
					&& !Config.ALT_GAME_KARMA_PLAYER_CAN_SHOP)
			{
		...
		int sealAvariceOwner = SevenSigns.getInstance()
				.getSealOwner(SevenSigns.SEAL_AVARICE);
		int sealGnosisOwner = SevenSigns.getInstance()
				.getSealOwner(SevenSigns.SEAL_GNOSIS);
		int playerCabal = SevenSigns.getInstance().getPlayerCabal(player);
		boolean isSealValidationPeriod = SevenSigns.getInstance()
				...
				if (playerCabal != compWinner || sealAvariceOwner != compWinner)
				{
					...
					case SevenSigns.CABAL_DAWN:
						player.sendPacket(new SystemMessage(
								SystemMessage.CAN_BE_USED_BY_DAWN));
						filename += "necro_no.htm";
						break;
					case SevenSigns.CABAL_DUSK:
						player.sendPacket(new SystemMessage(
								SystemMessage.CAN_BE_USED_BY_DUSK));
						filename += "necro_no.htm";
						break;
		...
		html.replace("%objectId%", String.valueOf(getObjectId()));
		html.replace("%festivalMins%",
				SevenSignsFestival.getInstance().getTimeToNextFestivalStr());
		player.sendPacket(html);
	 ...
	 * L2NpcTemplate (modified by RATE_XP).<BR>
	 * <BR>
	 */
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove the L2NpcInstance from the world and update its spawn object
	 * </li>
	 * <li>Remove all L2Object from _knownObjects and _knownPlayer of the
	 * L2NpcInstance then cancel Attak or Cast and notify AI</li>
