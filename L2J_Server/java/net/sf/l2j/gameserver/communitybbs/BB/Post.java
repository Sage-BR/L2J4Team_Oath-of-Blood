...
public class Post
{
	...
	public void deleteme(Topic t)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"DELETE FROM posts WHERE post_forum_id=? AND post_topic_id=?");
	...
	private void load(Topic t)
	{
		...
		try
		{
			...
			while (result.next())
			{
				...
				cp._PostOwnerID = Integer
						.parseInt(result.getString("post_ownerid"));
				cp._PostDate = Long.parseLong(result.getString("post_date"));
				cp._PostTopicID = Integer
						.parseInt(result.getString("post_topic_id"));
				cp._PostForumID = Integer
						.parseInt(result.getString("post_forum_id"));
