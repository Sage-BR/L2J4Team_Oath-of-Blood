...
public class SevenSigns
{
	...
	// The quest event and seal validation periods last for approximately one
	// week
	// with a 15 minutes "interval" period sandwiched between them.
	public static final int PERIOD_MINOR_LENGTH = 900000;

	public static final int PERIOD_MAJOR_LENGTH = 604800000
			- PERIOD_MINOR_LENGTH;
	...
	public SevenSigns()
	{
		...
		_log.info("SevenSigns: Currently in the " + getCurrentPeriodName()
				+ " period!");
		initializeSeals();
		...
		if (isSealValidationPeriod())
			if (getCabalHighestScore() == CABAL_NULL)
				...
				_log.info("SevenSigns: The "
						+ getCabalName(getCabalHighestScore())
						+ " were victorious last week.");
		else if (getCabalHighestScore() == CABAL_NULL)
			_log.info(
					"SevenSigns: The competition, if the current trend continues, will end in a tie this week.");
		else
			_log.info("SevenSigns: The " + getCabalName(getCabalHighestScore())
					+ " are in the lead this week.");
		...
		synchronized (this)
		{
			...
			ThreadPoolManager.getInstance().scheduleGeneral(sspc,
					milliToChange);
			...
			_log.info("SevenSigns: Next period begins in " + numDays + " days, "
					+ numHours + " hours and " + numMins + " mins.");
		}
	}
	...
	{
		_merchantSpawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(MAMMON_MERCHANT_ID, false);
		_blacksmithSpawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(MAMMON_BLACKSMITH_ID, false);
		_marketeerSpawns = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstances(MAMMON_MARKETEER_ID);
		_spiritInSpawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(SPIRIT_IN_ID, false);
		_spiritOutSpawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(SPIRIT_OUT_ID, false);
		_lilithSpawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(LILITH_NPC_ID, false);
		_anakimSpawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(ANAKIM_NPC_ID, false);
		_crestofdawnspawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(CREST_OF_DAWN_ID, false);
		_crestofduskspawn = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstance(CREST_OF_DUSK_ID, false);
		_oratorSpawns = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstances(ORATOR_NPC_ID);
		_preacherSpawns = AutoSpawnHandler.getInstance()
				.getAutoSpawnInstances(PREACHER_NPC_ID);
		...
		if (isSealValidationPeriod() || isCompResultsPeriod())
		{
			...
			if (getSealOwner(SEAL_GNOSIS) == getCabalHighestScore()
					&& getSealOwner(SEAL_GNOSIS) != CABAL_NULL)
			{
				if (!Config.ANNOUNCE_MAMMON_SPAWN)
					_blacksmithSpawn.setBroadcast(false);

				if (!AutoSpawnHandler.getInstance().getAutoSpawnInstance(
						_blacksmithSpawn.getObjectId(), true).isSpawnActive())
					AutoSpawnHandler.getInstance()
							.setSpawnActive(_blacksmithSpawn, true);

				for (AutoSpawnInstance spawnInst : _oratorSpawns.values())
					if (!AutoSpawnHandler.getInstance()
							.getAutoSpawnInstance(spawnInst.getObjectId(), true)
							.isSpawnActive())
						AutoSpawnHandler.getInstance().setSpawnActive(spawnInst,
								true);
				...
				if (!AutoChatHandler.getInstance()
						.getAutoChatInstance(PREACHER_NPC_ID, false).isActive()
						&& !AutoChatHandler.getInstance()
								.getAutoChatInstance(ORATOR_NPC_ID, false)
								.isActive())
					AutoChatHandler.getInstance().setAutoChatActive(true);
			} else
			{
				AutoSpawnHandler.getInstance().setSpawnActive(_blacksmithSpawn,
						false);
			...
			if (getSealOwner(SEAL_AVARICE) == getCabalHighestScore()
					&& getSealOwner(SEAL_AVARICE) != CABAL_NULL)
			{
				if (!Config.ANNOUNCE_MAMMON_SPAWN)
					_merchantSpawn.setBroadcast(false);

				if (!AutoSpawnHandler.getInstance().getAutoSpawnInstance(
						_merchantSpawn.getObjectId(), true).isSpawnActive())
					AutoSpawnHandler.getInstance()
							.setSpawnActive(_merchantSpawn, true);

				if (!AutoSpawnHandler.getInstance().getAutoSpawnInstance(
						_spiritInSpawn.getObjectId(), true).isSpawnActive())
					AutoSpawnHandler.getInstance()
							.setSpawnActive(_spiritInSpawn, true);

				if (!AutoSpawnHandler.getInstance().getAutoSpawnInstance(
						_spiritOutSpawn.getObjectId(), true).isSpawnActive())
					AutoSpawnHandler.getInstance()
							.setSpawnActive(_spiritOutSpawn, true);

				switch (getCabalHighestScore())
				{
				case CABAL_DAWN:
					if (!AutoSpawnHandler.getInstance().getAutoSpawnInstance(
							_lilithSpawn.getObjectId(), true).isSpawnActive())
						AutoSpawnHandler.getInstance()
								.setSpawnActive(_lilithSpawn, true);

					AutoSpawnHandler.getInstance().setSpawnActive(_anakimSpawn,
							false);
					if (!AutoSpawnHandler.getInstance()
							.getAutoSpawnInstance(
									_crestofdawnspawn.getObjectId(), true)
							.isSpawnActive())
						AutoSpawnHandler.getInstance()
								.setSpawnActive(_crestofdawnspawn, true);

					AutoSpawnHandler.getInstance()
							.setSpawnActive(_crestofduskspawn, false);
					break;

				case CABAL_DUSK:
					if (!AutoSpawnHandler.getInstance().getAutoSpawnInstance(
							_anakimSpawn.getObjectId(), true).isSpawnActive())
						AutoSpawnHandler.getInstance()
								.setSpawnActive(_anakimSpawn, true);

					AutoSpawnHandler.getInstance().setSpawnActive(_lilithSpawn,
							false);
					if (!AutoSpawnHandler.getInstance()
							.getAutoSpawnInstance(
									_crestofduskspawn.getObjectId(), true)
							.isSpawnActive())
						AutoSpawnHandler.getInstance()
								.setSpawnActive(_crestofduskspawn, true);

					AutoSpawnHandler.getInstance()
							.setSpawnActive(_crestofdawnspawn, false);
					break;
				}
			} else
			{
				AutoSpawnHandler.getInstance().setSpawnActive(_merchantSpawn,
						false);
				AutoSpawnHandler.getInstance().setSpawnActive(_lilithSpawn,
						false);
				AutoSpawnHandler.getInstance().setSpawnActive(_anakimSpawn,
						false);
				AutoSpawnHandler.getInstance().setSpawnActive(_crestofdawnspawn,
						false);
				AutoSpawnHandler.getInstance().setSpawnActive(_crestofduskspawn,
						false);
				AutoSpawnHandler.getInstance().setSpawnActive(_spiritInSpawn,
						false);
				AutoSpawnHandler.getInstance().setSpawnActive(_spiritOutSpawn,
						false);
			}
		} else
		{
	...
	public static int calcContributionScore(int blueCount, int greenCount,
			int redCount)
	{
	...
	public static int calcAncientAdenaReward(int blueCount, int greenCount,
			int redCount)
	{
			...
			if (daysToChange == 7)
				if (_calendar.get(Calendar.HOUR_OF_DAY) < PERIOD_START_HOUR)
					daysToChange = 0;
						...
						.get(Calendar.HOUR_OF_DAY) == PERIOD_START_HOUR
	...
	public final int getCurrentScore(int cabal)
	{
		...
		case CABAL_NULL:
			return 0;
		case CABAL_DAWN:
			return Math.round((float) (_dawnStoneScore
					/ ((float) totalStoneScore == 0 ? 1 : totalStoneScore))
					* 500) + _dawnFestivalScore;
		case CABAL_DUSK:
			return Math.round((float) (_duskStoneScore
					/ ((float) totalStoneScore == 0 ? 1 : totalStoneScore))
					* 500) + _duskFestivalScore;
		}
	...
	protected void restoreSevenSignsData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"SELECT char_obj_id, cabal, seal, red_stones, green_stones, blue_stones, "
			...
			while (rset.next())
			{
				...
				sevenDat.set("ancient_adena_amount",
						rset.getDouble("ancient_adena_amount"));
				sevenDat.set("contribution_score",
						rset.getDouble("contribution_score"));

				if (Config.DEBUG)
					_log.info("SevenSigns: Loaded data from DB for char ID "
							+ charObjId + " (" + sevenDat.getString("cabal")
							+ ")");
			...
			statement = con.prepareStatement(
					"SELECT * FROM seven_signs_status WHERE id=0");
			rset = statement.executeQuery();
			...
			while (rset.next())
			{
				...
				_signsSealOwners.put(SEAL_AVARICE,
						rset.getInt("avarice_owner"));
				_signsSealOwners.put(SEAL_GNOSIS, rset.getInt("gnosis_owner"));
				_signsSealOwners.put(SEAL_STRIFE, rset.getInt("strife_owner"));

				_signsDawnSealTotals.put(SEAL_AVARICE,
						rset.getInt("avarice_dawn_score"));
				_signsDawnSealTotals.put(SEAL_GNOSIS,
						rset.getInt("gnosis_dawn_score"));
				_signsDawnSealTotals.put(SEAL_STRIFE,
						rset.getInt("strife_dawn_score"));
				_signsDuskSealTotals.put(SEAL_AVARICE,
						rset.getInt("avarice_dusk_score"));
				_signsDuskSealTotals.put(SEAL_GNOSIS,
						rset.getInt("gnosis_dusk_score"));
				_signsDuskSealTotals.put(SEAL_STRIFE,
						rset.getInt("strife_dusk_score"));
			}
			...
			statement = con.prepareStatement(
					"UPDATE seven_signs_status SET date=? WHERE id=0");
			statement.setInt(1,
		...
		{
			_log.severe(
					"SevenSigns: Unable to load Seven Signs data from database: "
							+ e);
		} finally
		{
			try
	...
	/**
	 * Saves all Seven Signs data, both to the database and properties file (if
	 * updateSettings = True). Often called to preserve data integrity and
	 * synchronization with DB, in case of errors. <BR>
	 * If player != null, just that player's data is updated in the database,
	 * otherwise all player's data is sequentially updated.
	 *
	 * @param player
	...
	public void saveSevenSignsData(L2PcInstance player, boolean updateSettings)
	{
		...
		try
		{
			...
			{
				if (player != null)
					if (sevenDat.getInteger("char_obj_id") != player
							.getObjectId())
						continue;
				...
				statement.setDouble(6,
						sevenDat.getDouble("ancient_adena_amount"));
				statement.setDouble(7,
						sevenDat.getDouble("contribution_score"));
				...
				if (Config.DEBUG)
					_log.info(
							"SevenSigns: Updated data in database for char ID "
			...
			if (updateSettings)
			{
				...
				statement.setInt(17, SevenSignsFestival.getInstance()
						.getCurrentFestivalCycle());

				for (int i = 0; i < SevenSignsFestival.FESTIVAL_COUNT; i++)
					statement.setInt(18 + i, SevenSignsFestival.getInstance()
							.getAccumulatedBonus(i));
	...
	protected void resetPlayerData()
	{
		if (Config.DEBUG)
			_log.info(
					"SevenSigns: Resetting player data for new event period.");
	...
	/**
	 * Used to specify cabal-related details for the specified player. This
	 * method checks to see if the player has registered before and will update
	 * the database if necessary. <BR>
	 ...
	 * @return int cabal
	 */
	public int setPlayerInfo(L2PcInstance player, int chosenCabal,
			int chosenSeal)
	{
		...
		} else
		{
			...
			try
			{
				...
				if (Config.DEBUG)
					_log.info("SevenSigns: Inserted data in DB for char ID "
							+ currPlayerData.getInteger("char_obj_id") + " ("
		...
		if (currPlayerData.getString("cabal") == "dawn")
			_signsDawnSealTotals.put(chosenSeal,
					_signsDawnSealTotals.get(chosenSeal) + 1);
		else
			_signsDuskSealTotals.put(chosenSeal,
					_signsDuskSealTotals.get(chosenSeal) + 1);

		saveSevenSignsData(player, true);

		if (Config.DEBUG)
			_log.info("SevenSigns: " + player.getName() + " has joined the "
					+ getCabalName(chosenCabal) + " for the "
	...
	/**
	 * Returns the amount of ancient adena the specified player can claim, if
	 * any.<BR>
	 * If removeReward = True, all the ancient adena owed to them is removed,
	 * then DB is updated.
	 *
	 * @param player
	 * @param removeReward
	 ...
	 */
	public int addPlayerStoneContrib(L2PcInstance player, int blueCount,
			int greenCount, int redCount)
	{
		StatsSet currPlayer = getPlayerData(player);

		int contribScore = calcContributionScore(blueCount, greenCount,
				redCount);
		int totalAncientAdena = currPlayer.getInteger("ancient_adena_amount")
				+ calcAncientAdenaReward(blueCount, greenCount, redCount);
		int totalContribScore = currPlayer.getInteger("contribution_score")
				+ contribScore;

		if (totalContribScore > Config.ALT_MAXIMUM_PLAYER_CONTRIB)
			return -1;

		currPlayer.set("red_stones",
				currPlayer.getInteger("red_stones") + redCount);
		currPlayer.set("green_stones",
				currPlayer.getInteger("green_stones") + greenCount);
		currPlayer.set("blue_stones",
				currPlayer.getInteger("blue_stones") + blueCount);
		...
		if (Config.DEBUG)
			_log.info("SevenSigns: " + player.getName() + " contributed "
					+ contribScore + " seal stone points to their cabal.");
	...
	/**
	 * Sends the built-in system message specified by sysMsgId to all online
	 * players.
	 *
	 * @param sysMsgId
	 */
	...
	protected void initializeSeals()
	{
		for (Integer currSeal : _signsSealOwners.keySet())
		{
			...
			if (sealOwner != CABAL_NULL)
				if (isSealValidationPeriod())
					_log.info("SevenSigns: The " + getCabalName(sealOwner)
							+ " have won the " + getSealName(currSeal, false)
							+ ".");
				else
					_log.info("SevenSigns: The " + getSealName(currSeal, false)
	...
	protected void calcNewSealOwners()
	{
		if (Config.DEBUG)
		{
			_log.info("SevenSigns: (Avarice) Dawn = "
					+ _signsDawnSealTotals.get(SEAL_AVARICE) + ", Dusk = "
					+ _signsDuskSealTotals.get(SEAL_AVARICE));
			_log.info("SevenSigns: (Gnosis) Dawn = "
					+ _signsDawnSealTotals.get(SEAL_GNOSIS) + ", Dusk = "
					+ _signsDuskSealTotals.get(SEAL_GNOSIS));
			_log.info("SevenSigns: (Strife) Dawn = "
					+ _signsDawnSealTotals.get(SEAL_STRIFE) + ", Dusk = "
		...
		for (Integer currSeal : _signsDawnSealTotals.keySet())
		{
			...
			int totalDawnMembers = getTotalMembers(CABAL_DAWN) == 0 ? 1
					: getTotalMembers(CABAL_DAWN);
			int dawnPercent = Math.round(
					((float) dawnProportion / (float) totalDawnMembers) * 100);
			int duskProportion = getSealProportion(currSeal, CABAL_DUSK);
			int totalDuskMembers = getTotalMembers(CABAL_DUSK) == 0 ? 1
					: getTotalMembers(CABAL_DUSK);
			int duskPercent = Math.round(
					((float) duskProportion / (float) totalDuskMembers) * 100);

			/*
			 * - If a Seal was already closed or owned by the opponent and the
			 * new winner wants to assume ownership of the Seal, 35% or more of
			 * the members of the Cabal must have chosen the Seal. If they chose
			 * less than 35%, they cannot own the Seal.
			 *
			 * - If the Seal was owned by the winner in the previous Seven
			 * Signs, they can retain that seal if 10% or more members have
			 * chosen it. If they want to possess a new Seal, at least 35% of
			 * the members of the Cabal must have chosen the new Seal.
			 */
	...
	protected void teleLosingCabalFromDungeons(String compWinner)
	{
		for (L2PcInstance onlinePlayer : L2World.getInstance().getAllPlayers())
		{
			...
			{
				if (!onlinePlayer.isGM() && onlinePlayer.isIn7sDungeon()
						&& !currPlayer.getString("cabal").equals(""))
				{
					onlinePlayer.teleToLocation(
							MapRegionTable.TeleportWhereType.Town);
					onlinePlayer.setIsIn7sDungeon(false);
	...
	protected class SevenSignsPeriodChange implements Runnable
	{
		...
		public void run()
		{
			...
			case PERIOD_COMPETITION: // Results Calculation
				...
				SevenSignsFestival.getInstance().getFestivalManagerSchedule()
						.cancel(false);
			...
			case PERIOD_COMP_RESULTS: // Seal Validation
				...
				_log.info("SevenSigns: The " + getCabalName(_previousWinner)
			...
			case PERIOD_SEAL_VALIDATION: // Reset for New Cycle
				...
				// Reset all Festival-related data and remove any unused blood
				// offerings.
				// NOTE: A full update of Festival data in the database is also
				// performed.
			...
			teleLosingCabalFromDungeons(
					getCabalShortName(getCabalHighestScore()));
			...
			_log.info("SevenSigns: The " + getCurrentPeriodName()
					+ " period has begun!");
