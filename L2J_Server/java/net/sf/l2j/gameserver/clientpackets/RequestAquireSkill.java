...
public class RequestAquireSkill extends ClientBasePacket
{
	...
	void runImpl()
	{
		...
		if (!player.isInsideRadius(trainer, L2NpcInstance.INTERACTION_DISTANCE,
		...
		if (_skillType == 0)
		{
			...
			L2SkillLearn[] skills = SkillTreeTable.getInstance()
					.getAvailableSkills(player,
							player.getSkillLearningClassId());
			...
			{
				player.sendMessage(
						"You are trying to learn skill that u can't..");
									...
									// system messages.
		{
			if (!player.isClanLeader())
			{
				...
				player.sendMessage(
						"This feature is available only for the clan leader");
				return;
			}
			...
			{
				if (!player.destroyItemByItemId("Consume", itemId, 1, trainer,
						false))
				{
					// Haven't spellbook
					player.sendPacket(new SystemMessage(
			...
			if (Config.DEBUG)
				_log.fine("Learned pledge skill " + _id + " for " + _requiredSp
						+ " SP.");

			player.getClan().setReputationScore(
					player.getClan().getReputationScore() - repCost, true);
			...
			((L2VillageMasterInstance) trainer).showPledgeSkillList(player); // Maybe
																				// we
																				// shoud
																				// add
																				// a
		...
		if (_level > 1)
		{
			...
			for (L2ShortCut sc : allShortCuts)
			{
				...
				{
					L2ShortCut newsc = new L2ShortCut(sc.getSlot(),
							sc.getPage(), sc.getType(), sc.getId(), _level, 1);
