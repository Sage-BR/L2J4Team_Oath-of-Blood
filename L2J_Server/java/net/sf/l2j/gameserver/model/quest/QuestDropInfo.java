...
public class QuestDropInfo
{
	public class DropInfo
	{
		 ...
		 * @param itemCount
		 *            : int designating the quantity of items needed for quest
		 * @param itemObjID
		 *            : int designating the ID of the item in the inventory of
		 *            the player
		 */
	 ...
	 *
	 * @param pcInstance
	 *            : L2PcInstance designating the player
	 * @param questName
		...
		if (questState.getDrops() != null)
		{
			for (List<L2DropData> questDrop : questState.getDrops().values())
			{
				...
				for (L2DropData dropInfo : questDrop)
				{
					...
					// If player doesn't have this quest item(doesn't kill need
					// npc? or other) then skip it
					if (pcInstance.getInventory()
							.getItemByItemId(dropID) == null)
						continue;

					dropObjID = pcInstance.getInventory()
							.getItemByItemId(dropID).getObjectId();
