...
public class L2Attackable extends L2NpcInstance
{
	...
	// Logger.getLogger(L2Attackable.class.getName());
	 ...
	 * <li>hate : Hate level of this L2Attackable against the attaker
	 * L2Character (hate = damage)</li>
	 * <li>damage : Number of damages that the attaker L2Character gave to this
	 * L2Attackable</li><BR>
	 * <BR>
	 *
	 */
	 ...
	 * this L2Attackable</li>
	 * <li>dmg : Total amount of damage done by the attacker to this
	 * L2Attackable (summon + own)</li>
	 *
	 */
	 ...
	 */
	private FastMap<L2PcInstance, AbsorberInfo> _absorbersList = new FastMap<L2PcInstance, AbsorberInfo>()
			.setShared(true);
	...
	public AttackableKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof AttackableKnownList))
			...
			boolean awake)
	{
		 ...
		 *
		 * if ((this instanceof L2MonsterInstance)&&(attacker instanceof
		 * L2MonsterInstance))
		 * if((this.getEffect(L2Effect.EffectType.CONFUSION)!=null) &&
		 * (attacker.getEffect(L2Effect.EffectType.CONFUSION)!=null)) return;
		 */
	 ...
	 * owner) that hit the L2Attackable and to their Party members</li>
	 * <li>Notify the Quest Engine of the L2Attackable death if necessary</li>
	 * <li>Kill the L2NpcInstance (the corpse disappeared after 7 seconds)
	 * </li><BR>
	 * <BR>
	 *
	...
	public void doDie(L2Character killer)
	{
		...
		// Distribute Exp and SP rewards to L2PcInstance (including Summon
		// owner) that hit the L2Attackable and to their Party members
		...
		try
		{
			...
			{
				L2PcInstance player = killer instanceof L2PcInstance
						? (L2PcInstance) killer
						: ((L2Summon) killer).getOwner();

				if (getTemplate()
						.getEventQuests(Quest.QuestEventType.MOBKILLED) != null)
	 ...
	 * L2Party in progress</li>
	 * <li>Calculate the Experience and SP rewards in function of the level
	 * difference</li>
	 * <li>Add Exp and SP rewards to L2PcInstance (including Summon penalty) and
	 * to Party members in the known area of the last attacker</li><BR>
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T GIVE rewards
	 * to L2PetInstance</B></FONT><BR>
	...
	private void calculateRewards(L2Character lastAttacker)
	{
		...
		if (getMustRewardExpSP())
		{
			...
			FastMap<L2Character, RewardInfo> rewards = new FastMap<L2Character, RewardInfo>()
					.setShared(true);
			int rewardCount = 0;
			...
			if (!rewards.isEmpty())
			{
				...
				for (FastMap.Entry<L2Character, RewardInfo> entry = rewards
						.head(),
						end = rewards.tail(); (entry = entry.getNext()) != end;)
				{
					if (entry == null)
						continue;
					...
					if (attacker instanceof L2PcInstance
							&& ((L2PcInstance) attacker)
									.getPet() instanceof L2SummonInstance)
					{
						penalty = ((L2SummonInstance) ((L2PcInstance) attacker)
								.getPet()).getExpPenalty();
					}
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (attacker instanceof L2PcInstance)
							{
								...
								if (isOverhit()
										&& attacker == getOverhitAttacker())
								{
									player.sendPacket(new SystemMessage(
											SystemMessage.OVER_HIT));
							...
							attacker.addExpAndSp(
									Math.round(attacker.calcStat(
											Stats.EXPSP_RATE, exp, null, null)),
									(int) attacker.calcStat(Stats.EXPSP_RATE,
											sp, null, null));
						}
					} else
					{
						...
						// Get all L2Character (including L2Summon) that can be
						// rewarded in the party
						List<L2Character> rewardedMembers = new FastList<>();
						...
						for (L2PcInstance pl : attackerParty.getPartyMembers())
						{
							...
							if (reward2 != null)
							{
								...
								// Remove the L2PcInstance from the L2Attackable
								// rewards
								rewards.remove(pl);
							}

							// Add L2PcInstance of the party (that have attacked
							// or not) to members that can be rewarded if it's
							// not dead
							// and in range of the monster.
							if (!pl.isDead() && Util.checkIfInRange(1150, this,
									pl, true))
						...
						if (attacker instanceof L2PcInstance)
						{
							...
							if (isOverhit() && attacker == getOverhitAttacker())
							{
								...
								SystemMessage sms = new SystemMessage(
										SystemMessage.ACQUIRED_BONUS_EXPERIENCE_THROUGH_OVER_HIT);
	...
	public void addBufferHate()
	{
		...
		for (L2PcInstance actor : getKnownList().getKnownPlayers().values())
		{
			if (actor.isCastingNow() && target.getLastBuffer() == actor)
			{
				...
				if (skill != null && (skill.getSkillType() == SkillType.HEAL
						...
						|| skill.getSkillType() == SkillType.BALANCE_LIFE))
				{
					...
					if (targetList != null && targetList.length != 0
							&& (targetList[0] instanceof L2PcInstance
									|| targetList[0] instanceof L2SummonInstance
									|| targetList[0] instanceof L2PetInstance))
					{
						if ((getMaxHp() / 5) < target.getLastHealAmount())
							target.setLastHealAmount((getMaxHp() / 5));

						addDamageHate(actor, 0,
								(int) (target.getLastHealAmount() / divisor));
				...
				if (skill != null && (skill.getSkillType() == SkillType.BUFF
				...
				if (((L2PcInstance) target).isInParty() && skill != null
						&& skill.getTargetType() == L2Skill.SkillTargetType.TARGET_PARTY)
				{
					List<L2PcInstance> members = ((L2PcInstance) target)
					...
					for (L2PcInstance member : members)
					{
						if (member == actor)
						{
							if ((getMaxHp() / 3) < (int) (((getHating(target)
									- getHating(actor)) + 800) / divisor))
								addDamageHate(actor, 0, (getMaxHp() / 3));
							else
								addDamageHate(actor, 0,
										(int) (((getHating(target)
												- getHating(actor)) + 800)
												/ divisor));
	...
	public void addDamageHate(L2Character attacker, int damage, int aggro)
	{
		...
		} else
		{
			...
			// Add the attaker L2Character and the AggroInfo object in the
			// _aggroList of the L2Attackable
		...
		{
			if (attacker instanceof L2PcInstance
					|| attacker instanceof L2Summon)
			{
				L2PcInstance player = attacker instanceof L2PcInstance
						? (L2PcInstance) attacker
						: ((L2Summon) attacker).getOwner();

				if (getTemplate().getEventQuests(
						Quest.QuestEventType.MOBGOTATTACKED) != null)
					for (Quest quest : getTemplate().getEventQuests(
							Quest.QuestEventType.MOBGOTATTACKED))
		...
		synchronized (getAggroList())
		{
			...
			for (AggroInfo ai : getAggroListRP().values())
			{
				...
				if (ai.attacker.isAlikeDead()
						|| !getKnownList().knowsObject(ai.attacker)
						|| !ai.attacker.isVisible())
					ai.hate = 0;
				if (ai.hate > maxHate)
				{
	...
	public int getHating(L2Character target)
	{
		...
		if (ai == null)
			return 0;
		if ((ai.attacker instanceof L2PcInstance
				&& (((L2PcInstance) ai.attacker).getAppearance().getInvisible()
						|| ai.attacker.isInvul()))
				|| !ai.attacker.isVisible())
		{
	 ...
	 */
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker,
			L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		if (Config.DEEPBLUE_DROP_RULES)
			dropChance = ((drop.getChance()
					- ((drop.getChance() * levelModifier) / 100))
					/ deepBlueDrop);
...
//         if (drop.getItemId() == 57 && dropChance > L2DropData.MAX_CHANCE) dropChance = L2DropData.MAX_CHANCE; // If item is adena, dont drop multiple time
		...
		if (dropChance > L2DropData.MAX_CHANCE
				&& !Config.PRECISE_DROP_CALCULATION)
		{
			int multiplier = (int) dropChance / L2DropData.MAX_CHANCE;
			if (minCount < maxCount)
				itemCount += Rnd.get(minCount * multiplier,
						maxCount * multiplier);
	 ...
	 *            level modifier in %'s (will be subtracted from drop chance)
	 */
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker,
			L2DropCategory categoryDrops, int levelModifier)
	{
		...
		// that easy
		if (Config.DEEPBLUE_DROP_RULES)
			categoryDropChance = ((categoryDropChance
					- ((categoryDropChance * levelModifier) / 100))
					/ deepBlueDrop);
		 ...
		 * itemCount); else if (itemCount == 0 && Config.DEBUG)
		 * _log.fine("Roll produced 0 items to drop...");
		 *
		 * return null;
		 */
	 ...
	 * L2PcInstance that has killed the L2Attackable</li>
	 * <li>If the autoLoot mode isn't actif or if the L2Character that has
	 * killed the L2Attackable is not a L2PcInstance, add this or these Item(s)
	 * in the world as a visible object at the position where mob was
	 * last</li><BR>
	 * <BR>
	 *
	 * @param lastAttacker
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker)
	{
		...
		int levelModifier = calculateLevelModifierForDrop(player); // level
																	// modifier
																	// in %'s
																	// (will be
																	// subtracted
		...
		// Add Quest drops to the table containing all possible drops of this
		// L2Attackable
		FastList<L2DropData> questDrops = new FastList<>();
		...
		for (L2DropData drop : questDrops)
		{
			...
			if (Config.AUTO_LOOT)
				player.doAutoLoot(this, item); // Give this or these Item(s) to
												// the L2PcInstance that has
		...
		for (L2DropCategory cat : npcTemplate.getDropData())
		{
			...
			if (cat.isSweep())
			{
				...
				if (isSpoil()/* && !isSeeded() */)
				{
					...
					{
						item = calculateRewardItem(player, drop, levelModifier,
								true);
						if (item == null)
							continue;

						if (Config.DEBUG)
							_log.fine("Item id to spoil: " + item.getItemId()
					...
					if (!sweepList.isEmpty())
						_sweepItems = sweepList
								.toArray(new RewardItem[sweepList.size()]);
				}
			} else
			{
				if (isSeeded())
				...
				{
					item = calculateCategorizedRewardItem(player, cat,
							levelModifier);
				}
				...
				{
					if (Config.DEBUG)
						_log.fine("Item id to drop: " + item.getItemId()
								+ " amount: " + item.getCount());
					...
					if (this instanceof L2RaidBossInstance)
					{
						...
						sm = new SystemMessage(
								SystemMessage.S1_DIED_DROPPED_S3_S2);
		...
		if (rateHp <= 1
				&& String.valueOf(npcTemplate.type).contentEquals("L2Monster")) // only
																				// L2Monster
																				// with
																				// <=
																				// 1x
																				...
																				// herbs
		{
																		...
																		// yet
			{
				...
				if (Config.AUTO_LOOT && Config.AUTO_LOOT_HERBS)
					player.addItem("Loot", item.getItemId(), item.getCount(),
							this, true);
				else
				...
				for (int i = 0; i < 3; i++)
				{
					...
					if (random < Config.RATE_DROP_COMMON_HERBS)
					{
						...
						if (Config.AUTO_LOOT && Config.AUTO_LOOT_HERBS)
							player.addItem("Loot", item.getItemId(),
	 ...
	 * <li>Create this or these L2ItemInstance corresponding to this Item
	 * Identifier</li>
	 * <li>If the autoLoot mode is actif and if the L2Character that has killed
	 * the L2Attackable is a L2PcInstance, give this or these Item(s) to the
	 * L2PcInstance that has killed the L2Attackable</li>
		...
		for (DateDrop drop : EventDroplist.getInstance().getAllDrops())
		{
			if (Rnd.get(L2DropData.MAX_CHANCE) < drop.chance)
			{
				RewardItem item = new RewardItem(
						drop.items[Rnd.get(drop.items.length)],
						Rnd.get(drop.min, drop.max));
				if (Config.AUTO_LOOT)
					player.doAutoLoot(this, item); // Give this or these Item(s)
													// to the L2PcInstance that
													// has killed the
													// L2Attackable
				else
					DropItem(player, item); // drop the item on the ground
	...
	public L2ItemInstance DropItem(L2PcInstance lastAttacker, RewardItem item)
	{
		...
		for (int i = 0; i < item.getCount(); i++)
		{
			...
			int newZ = Math.max(this.getZ(), lastAttacker.getZ()) + 20; // TODO:
																		// temp
																		// hack,
																		// do
																		// somethign
			...
			ditem = ItemTable.getInstance().createItem("Loot", item.getItemId(),
					item.getCount(), lastAttacker, this);
	...
	public L2ItemInstance DropItem(L2PcInstance lastAttacker, int itemId,
			int itemCount)
	{
	...
	public void addAbsorber(L2PcInstance attacker, int crystalId)
	{
		...
		if (!(this instanceof L2MonsterInstance) || (attacker == null)
				|| (getAbsorbLevel() == 0))
			return;
	...
	private void levelSoulCrystals(L2Character attacker)
	{
		...
		for (L2PcInstance player : players)
		{
			...
			for (L2ItemInstance item : inv)
			{
				...
				for (int id : SoulCrystal.SoulCrystalTable)
				{
					...
					if (id == itemId)
					{
								...
								&& id != SoulCrystal.BLU_NEW_CRYSTAL)
						{
							try
							{
								if (item.getItem().getName().contains("Grade"))
								{
									...
									// & 'level'
									crystalNFO = item.getItem().getName().trim()
											.replace(" Grade ", "-").split("-");
									// Set Level to 13
									crystalLVL = 13;
									// Get Name
							...
							} catch (NumberFormatException nfe)
							{
								_log.log(Level.WARNING,
										"An attempt to identify a soul crystal failed, "
												+ "verify the names have not changed in etcitem "
												+ "table.",
										nfe);

								player.sendMessage(
										"There has been an error handling your soul crystal."
						...
						{
							crystalNME = item.getItem().getName().toLowerCase()
									.trim();
							crystalNEW = id + 1;
						}
			...
			if (crystalQTY < 1 || crystalQTY > 1 || !isSuccess || !doLevelup)
			{
				...
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.SOUL_CRYSTAL_ABSORBING_FAILED_RESONATION));
				}
				// The soul crystal stage of the player is way too high
				else if (!doLevelup)
					player.sendPacket(new SystemMessage(
							SystemMessage.SOUL_CRYSTAL_ABSORBING_REFUSED));

				crystalQTY = 0;
				continue;
			}

			// Ember and Anakazel(78) are not 100% success rate and each
			// individual
			...
			else if (!isBossMob && dice >= (100.0 - SoulCrystal.BREAK_CHANCE))
			{
				...
				if (crystalNME.startsWith("red"))
					exchangeCrystal(player, crystalOLD,
							SoulCrystal.RED_BROKEN_CRYSTAL, true);
				else if (crystalNME.startsWith("gre"))
					exchangeCrystal(player, crystalOLD,
							SoulCrystal.GRN_BROKEN_CYRSTAL, true);
				else if (crystalNME.startsWith("blu"))
					exchangeCrystal(player, crystalOLD,
							SoulCrystal.BLU_BROKEN_CRYSTAL, true);
				resetAbsorbList();
			} else
				player.sendPacket(new SystemMessage(
						SystemMessage.SOUL_CRYSTAL_ABSORBING_FAILED));
		}
	}
	...
	private void exchangeCrystal(L2PcInstance player, int takeid, int giveid,
			boolean broke)
	{
		L2ItemInstance Item = player.getInventory()
				.destroyItemByItemId("SoulCrystal", takeid, 1, player, this);
		if (Item != null)
		{
			...
			Item = player.getInventory().addItem("SoulCrystal", giveid, 1,
					player, this);
			...
			{
				player.sendPacket(
						new SystemMessage(SystemMessage.SOUL_CRYSTAL_BROKE));
			} else
				player.sendPacket(new SystemMessage(
						SystemMessage.SOUL_CRYSTAL_ABSORBING_SUCCEEDED));
	...
	public void setSeeded(int id, int seederLvl)
	{
		...
		List<RewardItem> harvested = new FastList<>();

		for (int i = 0; i < count; i++)
			harvested.add(new RewardItem(
					L2Manor.getInstance().getCropType(_seedType), 1));
