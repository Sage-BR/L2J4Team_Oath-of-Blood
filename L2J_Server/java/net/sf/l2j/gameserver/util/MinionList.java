...
public class MinionList
{
	private static Logger _log = Logger
			.getLogger(L2MonsterInstance.class.getName());
	...
	public void spawnMinions()
	{
		...
		synchronized (minionReferences)
		{
			...
			for (L2MinionData minion : minions)
			{
				...
				minionsToSpawn = minionCount
						- countSpawnedMinionsById(minionId);
	...
	public void spawnSingleMinion(int minionid)
	{
		...
		L2NpcTemplate minionTemplate = NpcTable.getInstance()
				.getTemplate(minionid);

		// Create and Init the Minion and generate its Identifier
		L2MinionInstance monster = new L2MinionInstance(
				IdFactory.getInstance().getNextId(), minionTemplate);
		...
		if (Config.DEBUG)
			_log.fine("Spawned minion template " + minionTemplate.npcId
					+ " with objid: " + monster.getObjectId() + " to boss "
					+ master.getObjectId() + " ,at: " + monster.getX() + " x, "
					+ monster.getY() + " y, " + monster.getZ() + " z");
	}
}
