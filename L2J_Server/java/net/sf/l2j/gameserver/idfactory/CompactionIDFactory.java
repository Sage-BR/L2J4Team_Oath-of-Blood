...
public class CompactionIDFactory extends IdFactory
{
	private static Logger _log = Logger
			.getLogger(CompactionIDFactory.class.getName());
			...
			java.sql.Connection con) throws SQLException
	{
		...
		if (Config.BAD_ID_CHECKING)
		{
			for (String check : id_checks)
			{
				...
				while (rs.next())
				{
					...
					_log.severe(
							"Bad ID " + badId + " in DB found by: " + check);
		...
		for (int i = 1; i <= hole; i++)
		{
			id = tmp_obj_ids[N - i];
			System.out.println(
					"Compacting DB object ID=" + id + " into " + (_curOID));
	...
	int id)
	{
		...
		/*
		 * if (_freeSize >= _freeOIDs.length) { int[] tmp = new int[_freeSize +
		 * STACK_SIZE_INCREMENT]; System.arraycopy(_freeOIDs, 0, tmp, 0,
		 * _freeSize); _freeOIDs = tmp; } _freeOIDs[_freeSize++] = id;
		 */
	}
