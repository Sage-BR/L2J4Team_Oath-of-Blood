...
public abstract class L2Item
{
	 ...
	 * @param set
	 *            : StatsSet corresponding to a set of couples (key,value) for
	...
	protected L2Item(Enum type, StatsSet set)
	{
		...
		_crystalType = set.getInteger("crystal_type", CRYSTAL_NONE); // default
																		// to
																		// none-grade
	...
	public final int getCrystalCount(int enchantLevel)
	{
		if (enchantLevel > 3)
			...
			case TYPE2_ACCESSORY:
				return _crystalCount
						+ crystalEnchantBonusArmor[getCrystalType()]
								* (3 * enchantLevel - 6);
			case TYPE2_WEAPON:
				return _crystalCount
						+ crystalEnchantBonusWeapon[getCrystalType()]
								* (2 * enchantLevel - 3);
			default:
				return _crystalCount;
			}
	...
	{
		return (isConsumable()
				? (int) (_referencePrice * Config.RATE_CONSUMABLE_COST)
				: _referencePrice);
	}
