...
public class RequestExEnchantSkillInfo extends ClientBasePacket
{
	...
	void runImpl()
	{
		...
		if ((trainer == null || !activeChar.isInsideRadius(trainer,
		...
		if (skill == null || skill.getId() != _skillID)
		{
			...
			activeChar.sendMessage(
					"This skill doesn't yet have enchant info in Datapack");
			return;
		}
		...
		L2EnchantSkillLearn[] skills = SkillTreeTable.getInstance()
				.getAvailableEnchantSkills(activeChar);
		...
		int requiredSp = SkillTreeTable.getInstance().getSkillSpCost(activeChar,
				skill);
		int requiredExp = SkillTreeTable.getInstance()
				.getSkillExpCost(activeChar, skill);
		byte rate = SkillTreeTable.getInstance().getSkillRate(activeChar,
				skill);
		ExEnchantSkillInfo asi = new ExEnchantSkillInfo(skill.getId(),
				skill.getLevel(), requiredSp, requiredExp, rate);

		if (Config.SP_BOOK_NEEDED
				&& (skill.getLevel() == 101 || skill.getLevel() == 141)) // only
																			// first
																			// lvl
																			// requires
																			// book
