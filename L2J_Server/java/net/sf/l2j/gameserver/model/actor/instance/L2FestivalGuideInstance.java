...
public final class L2FestivalGuideInstance extends L2FolkInstance
{
	...
	// Logger.getLogger(L2FestivalGuideInstance.class.getName());
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		} else if (command.startsWith("Festival"))
		{
			...
			case 1: // Become a Participant
				// Check if the festival period is active, if not then don't
				// allow registration.
				...
				// Check if a festival is in progress, then don't allow
				// registration yet.
				if (SevenSignsFestival.getInstance().isFestivalInitialized())
				{
					player.sendMessage(
							"You cannot sign up while a festival is in progress.");
					return;
				}
				...
				if (playerParty
						.getMemberCount() < Config.ALT_FESTIVAL_MIN_PLAYER)
				{
					showChatWindow(player, 2, "b", false);
					return;
				}

				// Check if all the party members are in the required level
				// range.
				if (playerParty.getLevel() > SevenSignsFestival
						.getMaxLevelForFestival(_festivalType))
				{
					showChatWindow(player, 2, "d", false);
					return;
				}

				// TODO: Check if the player has delevelled by comparing their
				// skill levels.

				/*
				 * Check to see if the player has already signed up, if they are
				 * then update the participant list providing all the required
				 * criteria has been met.
				 */
				if (player.isFestivalParticipant())
				{
					SevenSignsFestival.getInstance().setParticipants(
							_festivalOracle, _festivalType, playerParty);
			...
			case 2: // Festival 2 xxxx
				...
				if (!player.destroyItemByItemId("SevenSigns", stoneType,
						stonesNeeded, this, true))
					return;

				SevenSignsFestival.getInstance().setParticipants(
						_festivalOracle, _festivalType, playerParty);
				SevenSignsFestival.getInstance().addAccumulatedBonus(
						_festivalType, stoneType, stonesNeeded);
			...
			case 3: // Score Registration
				...
				// Check if a festival is in progress, if it is don't register
				// the score.
				if (SevenSignsFestival.getInstance().isFestivalInProgress())
				{
					player.sendMessage(
							"You cannot register a score while a festival is in progress.");
					return;
				}
				...
				List<L2PcInstance> prevParticipants = SevenSignsFestival
						.getInstance().getPreviousParticipants(_festivalOracle,
								_festivalType);
				...
				// Check if this player was among the past set of participants
				// for this festival.
				// Check if this player was the party leader in the festival.
				if (!prevParticipants.contains(player)
						|| (player.getObjectId() != prevParticipants.get(0)
								.getObjectId()))
				{
					showChatWindow(player, 3, "b", false);
					return;
				...
				L2ItemInstance bloodOfferings = player.getInventory()
						.getItemByItemId(
								SevenSignsFestival.FESTIVAL_OFFERING_ID);
				int offeringCount = 0;

				// Check if the player collected any blood offerings during the
				// festival.
				if (bloodOfferings == null)
				{
					player.sendMessage(
							"You do not have any blood offerings to contribute.");
					return;
				}

				offeringCount = bloodOfferings.getCount();

				int offeringScore = offeringCount
						* SevenSignsFestival.FESTIVAL_OFFERING_VALUE;
				boolean isHighestScore = SevenSignsFestival.getInstance()
						.setFinalScore(player, _festivalOracle, _festivalType,
								offeringScore);
				...
				SystemMessage sm = new SystemMessage(
						SystemMessage.CONTRIB_SCORE_INCREASED);
			...
			case 4: // Current High Scores
				...
				final StatsSet dawnData = SevenSignsFestival.getInstance()
						.getHighestScoreData(SevenSigns.CABAL_DAWN,
								_festivalType);
				final StatsSet duskData = SevenSignsFestival.getInstance()
						.getHighestScoreData(SevenSigns.CABAL_DUSK,
								_festivalType);
				final StatsSet overallData = SevenSignsFestival.getInstance()
						.getOverallHighestScoreData(_festivalType);

				final int dawnScore = dawnData.getInteger("score");
				final int duskScore = duskData.getInteger("score");
				int overallScore = 0;

				// If no data is returned, assume there is no record, or all
				// scores are 0.
				if (overallData != null)
				...
				if (dawnScore > 0)
					strBuffer.append(
							"Dawn: " + calculateDate(dawnData.getString("date"))
									+ ". Score " + dawnScore + "<br>"
									+ dawnData.getString("members") + "<br>");
				else
					strBuffer.append("Dawn: No record exists. Score 0<br>");

				if (duskScore > 0)
					strBuffer.append(
							"Dusk: " + calculateDate(duskData.getString("date"))
									+ ". Score " + duskScore + "<br>"
									+ duskData.getString("members") + "<br>");
				else
				...
				if (overallScore > 0)
				{
					...
					strBuffer.append("Consecutive top scores: "
							+ calculateDate(overallData.getString("date"))
							+ ". Score " + overallScore + "<br>Affilated side: "
							+ cabalStr + "<br>"
							+ overallData.getString("members") + "<br>");
				} else
					strBuffer.append(
							"Consecutive top scores: No record exists. Score 0<br>");

				strBuffer.append("<a action=\"bypass -h npc_" + getObjectId()
			...
			case 8: // Increase the Festival Challenge
				if ((playerParty == null) || !SevenSignsFestival.getInstance()
						.isFestivalInProgress())
					return;
				...
				if (SevenSignsFestival.getInstance()
						.increaseChallenge(_festivalOracle, _festivalType))
			...
			case 9: // Leave the Festival
				...
				/**
				 * If the player is the party leader, remove all participants
				 * from the festival (i.e. set the party to null, when updating
				 * the participant list) otherwise just remove this player from
				 * the "arena", and also remove them from the party.
				 */
				boolean isLeader = playerParty.isLeader(player);

				if (isLeader)
				{
					SevenSignsFestival.getInstance().updateParticipants(player,
							null);
				} else
				{
					SevenSignsFestival.getInstance().updateParticipants(player,
			...
			case 0: // Distribute Accumulated Bonus
				...
				{
					player.sendMessage(
							"Bonuses cannot be paid during the competition period.");
					return;
				}

				if (SevenSignsFestival.getInstance()
						.distribAccumulatedBonus(player) > 0)
			...
			boolean isDescription)
	{
		...
		html.replace("%festivalType%",
				SevenSignsFestival.getFestivalName(_festivalType));
		html.replace("%cycleMins%", String.valueOf(
				SevenSignsFestival.getInstance().getMinsToNextCycle()));
		if (!isDescription && "2b".equals(val + suffix))
			html.replace("%minFestivalPartyMembers%",
					String.valueOf(Config.ALT_FESTIVAL_MIN_PLAYER));
		...
		if (val == 1)
		{
			html.replace("%blueStoneNeeded%",
					String.valueOf(_blueStonesNeeded));
			html.replace("%greenStoneNeeded%",
					String.valueOf(_greenStonesNeeded));
	...
	private final String getStatsTable()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			int dawnScore = SevenSignsFestival.getInstance()
					.getHighestScore(SevenSigns.CABAL_DAWN, i);
			int duskScore = SevenSignsFestival.getInstance()
					.getHighestScore(SevenSigns.CABAL_DUSK, i);
			...
			tableHtml.append("<tr><td width=\"100\" align=\"center\">"
					+ festivalName + "</td><td align=\"center\" width=\"35\">"
					+ duskScore + "</td><td align=\"center\" width=\"35\">"
					+ dawnScore + "</td><td align=\"center\" width=\"130\">"
					+ winningCabal + "</td></tr>");
		}
	...
	private final String getBonusTable()
	{
		...
		// Get the accumulated scores for each of the festival level ranges
		// (types).
		for (int i = 0; i < 5; i++)
		{
			int accumScore = SevenSignsFestival.getInstance()
					.getAccumulatedBonus(i);
			String festivalName = SevenSignsFestival.getFestivalName(i);

			tableHtml.append("<tr><td align=\"center\" width=\"150\">"
					+ festivalName + "</td><td align=\"center\" width=\"150\">"
					+ accumScore + "</td></tr>");
		}
	...
	private final String calculateDate(String milliFromEpoch)
	{
		...
		return calCalc.get(Calendar.YEAR) + "/" + calCalc.get(Calendar.MONTH)
				+ "/" + calCalc.get(Calendar.DAY_OF_MONTH);
	}
}
