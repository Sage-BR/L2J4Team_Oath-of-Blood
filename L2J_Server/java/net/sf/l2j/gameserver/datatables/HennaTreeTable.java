...
public class HennaTreeTable
{
	private static Logger _log = Logger
			.getLogger(HennaTreeTable.class.getName());
	...
	private HennaTreeTable()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT class_name, id, parent_id FROM class_list ORDER BY id");
			ResultSet classlist = statement.executeQuery();
			List<L2HennaInstance> list = new FastList<>();
			// int parentClassId;
			// L2Henna henna;
			while (classlist.next())
			{
				...
				PreparedStatement statement2 = con.prepareStatement(
						"SELECT class_id, symbol_id FROM henna_trees where class_id=? ORDER BY symbol_id");
		...
		_log.config(
				"HennaTreeTable: Loaded " + count + " Henna Tree Templates.");
	...
	public L2HennaInstance[] getAvailableHenna(ClassId classId)
	{
		...
		if (henna == null)
		{
			...
			// list
			_log.warning(
					"Hennatree for class " + classId + " is not defined !");
