...
public class L2RaceManagerInstance extends L2NpcInstance
{
	...
	public L2RaceManagerInstance(int objectId, L2NpcTemplate template)
	{
		...
		if (notInitialized)
		{
			...
			s.scheduleGeneralAtFixedRate(new Announcement(
					SystemMessage.MONSRACE_TICKETS_AVAILABLE_FOR_S1_RACE), 0,
					10 * MINUTE);
			s.scheduleGeneralAtFixedRate(new Announcement(
			...
			s.scheduleGeneralAtFixedRate(
					new Announcement(
							SystemMessage.MONSRACE_TICKET_SALES_CLOSED),
					7 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(
					new Announcement(
							SystemMessage.MONSRACE_BEGINS_IN_S1_MINUTES),
					7 * MINUTE, 10 * MINUTE);
			s.scheduleGeneralAtFixedRate(
					new Announcement(
							SystemMessage.MONSRACE_BEGINS_IN_S1_MINUTES),
			...
			s.scheduleGeneralAtFixedRate(
					new Announcement(
							SystemMessage.MONSRACE_BEGINS_IN_30_SECONDS),
			...
			s.scheduleGeneralAtFixedRate(
					new Announcement(
							SystemMessage.MONSRACE_BEGINS_IN_S1_SECONDS),
			...
			s.scheduleGeneralAtFixedRate(
					new Announcement(SystemMessage.MONSRACE_RACE_START),
					9 * MINUTE, 10 * MINUTE);
			// */
		}
		managers.add(this);
	...
	public final RaceManagerKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof RaceManagerKnownList))
	...
	private void startRace()
	{
		...
		if (state == STARTING_RACE)
		{
			...
			broadcast(SRace);
			PlaySound SRace2 = new PlaySound(0, "ItemSound2.race_start", 1,
					121209259, 12125, 182487, -3559);
			broadcast(SRace2);
			packet = new MonRaceInfo(codes[1][0], codes[1][1],
					race.getMonsters(), race.getSpeeds());
			sendMonsterInfo();

			ThreadPoolManager.getInstance().scheduleGeneral(new RunRace(),
					5000);
		} else
		{
			// state++;
			race.newRace();
			race.newSpeeds();
			packet = new MonRaceInfo(codes[0][0], codes[0][1],
					race.getMonsters(), race.getSpeeds());
			sendMonsterInfo();
		}
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		if (command.startsWith("BuyTicket") && state != ACCEPTING_BETS)
		{
			player.sendPacket(new SystemMessage(
					SystemMessage.MONSRACE_TICKETS_NOT_AVAILABLE));
			command = "Chat 0";
		}
		if (command.startsWith("ShowOdds") && state == ACCEPTING_BETS)
		{
			player.sendPacket(
					new SystemMessage(SystemMessage.MONSRACE_NO_PAYOUT_INFO));
			command = "Chat 0";
		}
		...
		if (command.startsWith("BuyTicket"))
		{
			...
			if ((val == 10 && player.getRace(0) == 0) || (val == 20
					&& player.getRace(0) == 0 && player.getRace(1) == 0))
				val = 0;
	...
	public void showOdds(L2PcInstance player)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			int n = i + 1;
			search = "Mob" + n;
			html.replace(search, MonsterRace.getInstance().getMonsters()[i]
					.getTemplate().name);
		}
	...
	public void showBuyTicket(L2PcInstance player, int val)
	{
		...
		} else if (val < 20)
		{
			...
			search = "Mob1";
			replace = MonsterRace.getInstance().getMonsters()[player.getRace(0)
					- 1].getTemplate().name;
		...
		} else
		{
			...
			L2ItemInstance item = new L2ItemInstance(
					IdFactory.getInstance().getNextId(), 4443);
			item.setCount(1);
	...
	class RunRace implements Runnable
	{
		...
		public void run()
		{
			packet = new MonRaceInfo(codes[2][0], codes[2][1],
					MonsterRace.getInstance().getMonsters(),
					MonsterRace.getInstance().getSpeeds());
			sendMonsterInfo();
			ThreadPoolManager.getInstance().scheduleGeneral(new RunEnd(),
					30000);
		}
	}
		...
		public void run()
		{
			...
			for (int i = 0; i < 8; i++)
			{
				obj = new DeleteObject(
						MonsterRace.getInstance().getMonsters()[i]);
				broadcast(obj);
			}
		}
	}
