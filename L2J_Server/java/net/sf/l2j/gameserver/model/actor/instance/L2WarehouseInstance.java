...
public final class L2WarehouseInstance extends L2FolkInstance
{
	...
	// Logger.getLogger(L2WarehouseInstance.class.getName());
	...
	private void showRetrieveWindow(L2PcInstance player)
	{
		...
		player.sendPacket(new WareHouseWithdrawalList(player,
				WareHouseWithdrawalList.Private));
	}
	...
	private void showDepositWindow(L2PcInstance player)
	{
		...
		player.sendPacket(
				new WareHouseDepositList(player, WareHouseDepositList.Private));
	}
	...
	private void showWithdrawWindowClan(L2PcInstance player)
	{
		...
		if (player.getClan() != null)
		{
			...
			else
			{
				...
				{
					player.sendPacket(new SystemMessage(
							SystemMessage.YOU_DO_NOT_HAVE_THE_RIGHT_TO_USE_CLAN_WAREHOUSE));
					return;
				}
				player.setActiveWarehouse(player.getClan().getWarehouse());
				if (Config.DEBUG)
	...
	private void showWithdrawWindowFreight(L2PcInstance player)
	{
		...
		if (freight != null && getZone() != null)
		{
			...
			player.sendPacket(new WareHouseWithdrawalList(player,
					WareHouseWithdrawalList.Freight));
		} else
		{
			if (Config.DEBUG)
	...
	private void showDepositWindowFreight(L2PcInstance player)
	{
		...
		if (player.getAccountChars().size() == 0)
		{
			...
			replyMSG.append(
					"You have no other characters to make a freight for.");
		...
		else
		{
			...
			for (Integer objId : chars.keySet())
			{
				String charName = chars.get(objId);

				/*
				 * replyMSG.append("<option action=\"bypass -h npc_" +
				 * String.valueOf(getObjectId()) + "_FreightChar_" +
				 * String.valueOf(objId) + "\">"); replyMSG.append(charName);
				 * replyMSG.append("</option>");
				 */
				replyMSG.append("<a action=\"bypass -h npc_"
						+ String.valueOf(getObjectId()) + "_FreightChar_"
			...
			if (Config.DEBUG)
				_log.fine("Showing destination chars to freight - char src: "
						+ player.getName());
		}
	}
	...
	private void showDepositWindowFreight(L2PcInstance player, int obj_Id)
	{
		...
		if (destChar == null)
		{
			...
			if (Config.DEBUG)
				_log.warning("Error retrieving a target object for char "
						+ player.getName() + " - using freight.");
			return;
		}
		if (getZone() == null)
		{
			...
			if (Config.DEBUG)
				_log.warning("Error retrieving the zone for char "
						+ player.getName() + " - using freight.");
			return;
		}
		...
		player.sendPacket(
				new WareHouseDepositList(player, WareHouseDepositList.Freight));
	}
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		{
			_log.info("Player " + player.getName()
					+ " trying to use enchant exploit, ban this player!");
