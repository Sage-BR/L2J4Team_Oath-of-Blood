...
public class RequestSellItem extends ClientBasePacket
{
	 ...
	 * sample
	 *
	 * 1e 00 00 00 00 // list id 02 00 00 00 // number of items
	 *
	 * 71 72 00 10 // object id ea 05 00 00 // item id 01 00 00 00 // item count
	 *
	 * 76 4b 00 10 // object id 2e 0a 00 00 // item id 01 00 00 00 // item count
	 *
	 * format: cdd (ddd)
	 *
	 * @param decrypt
	...
	void runImpl()
	{
		...
		for (int i = 0; i < _count; i++)
		{
			...
			item = player.getInventory().destroyItem("Sell", objectId, count,
					player, null);

			/*
			 * TODO: Disabled until Leaseholders are rewritten ;-) int price =
			 * item.getReferencePrice()*(int)count/2; L2ItemInstance li = null;
			 * L2ItemInstance la = null; if (_listId > 1000000) { li =
			 * merchant.findLeaseItem(item.getItemId(),item.getEnchantLevel());
			 * la = merchant.getLeaseAdena(); if (li == null || la == null)
			 * continue; price = li.getPriceToBuy()*(int)count; // player sells,
			 * thus merchant buys. if (price > la.getCount()) continue; }
			 */
			/*
			 * TODO: Disabled until Leaseholders are rewritten ;-) if (item !=
			 * null && _listId > 1000000) {
			 * li.setCount(li.getCount()+(int)count); li.updateDatabase();
			 * la.setCount(la.getCount()-price); la.updateDatabase(); }
			 */
		}
		player.addAdena("Sell", (int) totalPrice, merchant, false);

		String html = HtmCache.getInstance().getHtm("data/html/" + htmlFolder
				+ "/" + merchant.getNpcId() + "-sold.htm");

		if (html != null)
		{
			NpcHtmlMessage soldMsg = new NpcHtmlMessage(merchant.getObjectId());
			soldMsg.setHtml(html.replaceAll("%objectId%",
