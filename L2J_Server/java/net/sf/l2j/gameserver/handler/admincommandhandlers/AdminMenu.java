...
public class AdminMenu implements IAdminCommandHandler
{
	...
	private static String[] _adminCommands =
		{ "admin_char_manage", "admin_teleport_character_to_menu",
				"admin_recall_char_menu", "admin_goto_char_menu",
				"admin_kick_menu", "admin_kill_menu", "admin_ban_menu",
				"admin_unban_menu" };
		...
		} else if (command.startsWith("admin_kick_menu"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				if (plyr != null)
				{
					...
					plyr.logout();
					sm.addString(
							"You kicked " + plyr.getName() + " from the game.");
				} else
				{
					sm.addString(
							"Player " + player + " was not found in the game.");
				}
	...
	private void setAccountAccessLevel(String user, L2PcInstance player,
			int banLevel)
	{
		...
		try
		{
			...
			{
				LoginServerThread.getInstance().sendAccessLevel(acc_name,
						banLevel);
				sm.addString("Account Access Level for " + user + " set to "
						+ banLevel + ".");
			} else
