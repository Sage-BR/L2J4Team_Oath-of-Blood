 ...
 *
 * Format: ch (hdSdh) h: Total Friend Count
 *
 * h: Unknown d: Player Object ID S: Friend Name d: Online/Offline h: Unknown
 *
 * @author Tempy
 *
 */
public class FriendList extends ServerBasePacket
{
	...
	final void writeImpl()
	{
		...
		try
		{
			...
			if (rset.getRow() > 0)
			{
				...
				while (rset.next())
				{
					...
					L2PcInstance friend = L2World.getInstance()
							.getPlayer(friendName);
		...
		{
			_log.warning(
					"Error found in " + _cha.getName() + "'s FriendList: " + e);
		} finally
		{
			try
			{
