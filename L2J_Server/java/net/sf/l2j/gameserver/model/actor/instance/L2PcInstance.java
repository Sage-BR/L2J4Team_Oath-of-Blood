...
public final class L2PcInstance extends L2PlayableInstance
{
	 ...
	 */
	protected Map<Integer, Integer> _activeSoulShots = new FastMap<Integer, Integer>()
			.setShared(true);
	...
	public class HerbTask implements Runnable
	{
		...
		HerbTask(String process, int itemId, int count, L2Object reference,
				boolean sendMessage)
		{
			_process = process;
			_itemId = itemId;
	...
	public class SkillDat
	{
		...
		protected SkillDat(L2Skill skill, boolean ctrlPressed,
	 ...
	 *
	 */
	public static L2PcInstance create(int objectId, L2PcTemplate template,
			String accountName, String name, byte hairStyle, byte hairColor,
			byte face, boolean sex)
	{
	...
	public final PcKnownList getKnownList()
	{
		if (super.getKnownList() == null
				|| !(super.getKnownList() instanceof PcKnownList))
	...
	public final PcStatus getStatus()
	{
		if (super.getStatus() == null
				|| !(super.getStatus() instanceof PcStatus))
	...
	{
		super.setTemplate(
				CharTemplateTable.getInstance().getTemplate(newclass));
	}
	 ...
	 * AI_INTENTION_MOVE_TO.<BR>
	 * <BR>
	 */
	public void explore()
	{
		if (!_exploring || (getMountType() == 2))
			return;
...
//	public boolean isInCraftMode() { return (getPrivateStoreType() == STORE_PRIVATE_MANUFACTURE); }
	...
	public L2RecipeList[] getCommonRecipeBook()
	{
		return _commonRecipeBook.values()
				.toArray(new L2RecipeList[_commonRecipeBook.values().size()]);
	}
	...
	public L2RecipeList[] getDwarvenRecipeBook()
	{
		return _dwarvenRecipeBook.values()
				.toArray(new L2RecipeList[_dwarvenRecipeBook.values().size()]);
	}
	...
	/**
	 * Add Quest drops to the table containing all possible drops of a
	 * L2NpcInstance.<BR>
	 * <BR>
	 *
	 * @param npc
	 *            The L2NpcInstance killed by the L2PcInstance
	 * @param drops
	 *            The table containing all possible drops of the L2NpcInstance
	 *
	 */
	...
	private QuestState[] addToQuestStateArray(QuestState[] questStateArray,
			QuestState state)
	{
	...
	public QuestState[] getQuestsForAttacks(L2NpcInstance npc)
	{
		...
		for (Quest quest : npc.getTemplate()
				.getEventQuests(Quest.QuestEventType.MOBGOTATTACKED))
		{
			// Check if the Identifier of the L2Attackable attck is needed for
			// the current quest
			if (getQuestState(quest.getName()) != null)
			{
				// Copy the current L2PcInstance QuestState in the QuestState
				// table
				if (states == null)
					states = new QuestState[]
						{ getQuestState(quest.getName()) };
				else
					states = addToQuestStateArray(states,
							getQuestState(quest.getName()));
			}
		}
	...
	public QuestState[] getQuestsForKills(L2NpcInstance npc)
	{
		...
		// Go through the QuestState of the L2PcInstance quests
		for (Quest quest : npc.getTemplate()
				.getEventQuests(Quest.QuestEventType.MOBKILLED))
		{
			// Check if the Identifier of the L2Attackable killed is needed for
			// the current quest
			if (getQuestState(quest.getName()) != null)
			{
				// Copy the current L2PcInstance QuestState in the QuestState
	 ...
	 * the L2PcInstance must talk to the NPC.<BR>
	 * <BR>
	 *
	 * @param npcId
	...
	public QuestState[] getQuestsForTalk(int npcId)
	{
		...
		// Go through the QuestState of the L2PcInstance quests
		Quest[] quests = NpcTable.getInstance().getTemplate(npcId)
				.getEventQuests(Quest.QuestEventType.QUEST_TALK);
		if (quests != null)
		{
	...
	public QuestState processQuestEvent(String quest, String event)
	{
		...
		if (qs != null)
		{
			...
			{
				L2Object object = L2World.getInstance()
						.findObject(getLastQuestNpcObject());
				if (object instanceof L2NpcInstance && isInsideRadius(object,
						L2NpcInstance.INTERACTION_DISTANCE, false, false))
				{
					...
					if (states != null)
					{
						...
						{
							if ((state.getQuest().getQuestIntId() == qs
									.getQuest().getQuestIntId())
									&& !qs.isCompleted())
							{
	...
	private void showQuestWindow(String questId, String stateId)
	{
		...
		{
			if (Config.DEBUG)
				_log.fine("Showing quest window for quest " + questId
						+ " state " + stateId + " html path: " + path);
	...
	/**
	 * Update the _inPvpZone flag and send a message to player.<br>
	 * <br>
	 *
	 * @param inPvpZone
	 *            : true if player is in a pvp zone
	 * @see net.sf.l2j.gameserver.model.actor.instance.L2PcInstance.setInPvpZone(boolean,
	 *      boolean)
	 */
	...
	/**
	 * Update the _inPvpZone flag.<br>
	 * <br>
	 *
	 * @param inPvpZone
	 *            : true if player is in a pvp zone
	 * @param warnPlayer
	 *            : true if a message should be send to player
	 */
	...
	/**
	 * Update the _inMotherTreeZone flag and send a message to player.<br>
	 * <br>
	 *
	 * @param inMotherTreeZone
	 *            : true if player is in a mother tree zone
	 * @see net.sf.l2j.gameserver.model.actor.instance.L2PcInstance.setInMotherTreeZone(boolean,
	 *      boolean)
	 */
	...
	/**
	 * Update the _inMotherTreeZone flag.<br>
	 * <br>
	 *
	 * @param inMotherTreeZone
	 *            : true if player is in a mother tree zone
			...
			boolean warnPlayer)
	{
		if (getRace() != Race.elf)
		{
			// if player is not a member of elven race Mother Tree have no
			// effect on him
		...
		if (isInParty())
			for (L2PcInstance member : getParty().getPartyMembers())
				...
				{
					// if player is in party with a non-elven race Mother Tree
					// effect is cancelled
					_inMotherTreeZone = false;
					return;
				}
			...
			if (warnPlayer)
				if (_inMotherTreeZone)
					sendPacket(new SystemMessage(
							SystemMessage.ENTER_SHADOW_MOTHER_TREE));
				else
					sendPacket(new SystemMessage(
							SystemMessage.EXIT_SHADOW_MOTHER_TREE));
		}
	}
	...
	public void revalidateInClanHall()
	{
		...
		if (clansHall != null)
			setIsInClanHall(clansHall.checkIfInZone(this.getX(), this.getY()));
	}
		...
		if (Config.ALT_RECOMMEND)
		{
			...
			try
			{
				...
				PreparedStatement statement = con
						.prepareStatement(ADD_CHAR_RECOM);
	...
	public void setKarma(int karma)
	{
		...
		if (_karma == 0 && karma > 0)
		{
			for (L2Object object : getKnownList().getKnownObjects().values())
			{
				...
				if (((L2GuardInstance) object).getAI()
						.getIntention() == CtrlIntention.AI_INTENTION_IDLE)
		...
		if (getMaxLoad() > 0 && !_dietMode)
		{
			...
			if (_curWeightPenalty != newWeightPenalty)
			{
				...
				if (newWeightPenalty > 0)
					super.addSkill(SkillTable.getInstance().getInfo(4270,
							newWeightPenalty));
				else
	...
	public void refreshExpertisePenalty()
	{
		...
		if (getexpertisePenalty() != newPenalty)
		{
			...
			if (newPenalty > 0)
				super.addSkill(
						SkillTable.getInstance().getInfo(4267, newPenalty));
			else
	...
	public void CheckIfWeaponIsAllowed()
	{
		...
		for (L2Effect currenteffect : this.getAllEffects())
		{
			...
			if (!(effectSkill.getTargetType() == SkillTargetType.TARGET_PARTY
					&& effectSkill.getSkillType() == SkillType.BUFF))
			{
				// Check to rest to assure current effect meets weapon
				// requirements.
				if (!effectSkill.getWeaponDependancy(this))
				{
					sendMessage(effectSkill.getName()
							+ " cannot be used with this weapon.");

					if (Config.DEBUG)
						_log.info("   | Skill " + effectSkill.getName()
								+ " has been disabled for (" + this.getName()
	...
	public void checkSSMatch(L2ItemInstance equipped, L2ItemInstance unequipped)
	{
		...
		if (unequipped.getItem().getType2() == L2Item.TYPE2_WEAPON
				&& (equipped == null ? true
						: equipped.getItem().getCrystalType() != unequipped
								.getItem().getCrystalType()))
		{
			for (L2ItemInstance ss : getInventory().getItems())
			{
				...
				if (((_itemId >= 2509 && _itemId <= 2514)
						...
						|| _itemId == 5789 || _itemId == 5790
						|| _itemId == 1835)
						&& ss.getItem().getCrystalType() == unequipped.getItem()
								.getCrystalType())
				{
					sendPacket(new ExAutoSoulShot(_itemId, 0));

					SystemMessage sm = new SystemMessage(
							SystemMessage.AUTO_USE_OF_S1_CANCELLED);
	...
	public void setClassId(int Id)
	{
				...
				&& PlayerClass.values()[Id].getLevel() == ClassLevel.Third)
		{
			if (getLvlJoinedAcademy() <= 16)
				...
				_clan.setReputationScore(
						_clan.getReputationScore()
								+ (400 - (getLvlJoinedAcademy() - 16) * 10),
						true);
			setLvlJoinedAcademy(0);

			// oust pledge member from the academy, cuz he has finished his 2nd
			// class transfer
			...
			_clan.broadcastToOnlineMembers(
					new PledgeShowMemberListDelete(this.getName()));
			_clan.removeClanMember(this.getName(), 0);
			sendPacket(new SystemMessage(
					SystemMessage.ACADEMY_MEMBERSHIP_TERMINATED));

			// receive graduation gift
			getInventory().addItem("Gift", 8181, 1, this, null); // give academy
																	// circlet
	...
	public void rewardSkills()
	{
		...
		{
			L2Skill skill = SkillTable.getInstance().getInfo(239,
					getExpertiseIndex());
			addSkill(skill, true);
	...
	private void giveAvailableSkills()
	{
		...
		L2SkillLearn[] skills = SkillTreeTable.getInstance()
				.getAvailableSkills(this, getClassId());
	...
	public Race getRace()
	{
		...
		L2PcTemplate charTemp = CharTemplateTable.getInstance()
				.getTemplate(_baseClass);
	...
	public boolean isCastleLord(int castleId)
	{
		...
		if ((clan != null) && (clan.getLeader().getPlayerInstance() == this))
		{
			...
			if ((castle != null) && (castle == CastleManager.getInstance()
					.getCastle(castleId)))
				return true;
		}
	 ...
	 * send a Server->Client ChangeWaitType packet (broadcast)<BR>
	 * <BR>
	 */
	public void sitDown()
	{
				...
				&& !isImobilised())
		{
			...
			broadcastPacket(
					new ChangeWaitType(this, ChangeWaitType.WT_SITTING));
		}
	}
	...
	public void standUp()
	{
		...
		{
			sendMessage(
					"A dark force beyond your mortal understanding makes your knees to shake when you try to stand up ...");
		} else if (_waitTypeSitting && !isInStoreMode() && !isAlikeDead())
		{
			...
			broadcastPacket(
					new ChangeWaitType(this, ChangeWaitType.WT_STANDING));
		}
	}
	 ...
	 *            this action
	 */
	public void addAdena(String process, int count, L2Object reference,
			boolean sendMessage)
	{
		if (sendMessage)
		{
	 ...
	 */
	public boolean reduceAdena(String process, int count, L2Object reference,
			boolean sendMessage)
	{
		if (count > getAdena())
		{
			if (sendMessage)
				sendPacket(
						new SystemMessage(SystemMessage.YOU_NOT_ENOUGH_ADENA));
			return false;
		}
		...
		if (count > 0)
		{
			...
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.DISSAPEARED_ADENA);
				sm.addNumber(count);
				sendPacket(sm);
			}
		}
	 ...
	 *            this action
	 */
	public void addAncientAdena(String process, int count, L2Object reference,
			boolean sendMessage)
	{
		if (sendMessage)
		{
	 ...
	 * @return boolean informing if the action was successfull
	 */
	public boolean reduceAncientAdena(String process, int count,
	 ...
	 *            this action
	 */
	public void addItem(String process, L2ItemInstance item, L2Object reference,
			boolean sendMessage)
	{
		if (item.getCount() > 0)
		{
			...
			// Cursed Weapon
			if (CursedWeaponsManager.getInstance()
					.isCursed(newitem.getItemId()))
			{
	 ...
	 *            this action
	 */
	public void addItem(String process, int itemId, int count,
			L2Object reference, boolean sendMessage)
	{
		if (count > 0)
		{
							...
							.getItemType() != L2EtcItemType.HERB))
			{
				if (count > 1)
				{
					if (process.equalsIgnoreCase("sweep")
			...
			if (ItemTable.getInstance().createDummyItem(itemId)
					.getItemType() == L2EtcItemType.HERB) // If item is herb
															// dont add it to iv
															// :]
			{
				if (!isCastingNow())
				{
					L2ItemInstance herb = new L2ItemInstance(this._charId,
							itemId);
					IItemHandler handler = ItemHandler.getInstance()
							.getItemHandler(herb.getItemId());
					if (handler == null)
						_log.warning(
								"No item handler registered for Herb - item ID "
										+ herb.getItemId() + ".");
					else
					{
				...
				} else
				{
					...
					ThreadPoolManager.getInstance()
							.scheduleAi(
									new HerbTask(process, itemId, count,
			...
			{
				// Add the item to inventory
				L2ItemInstance item = _inventory.addItem(process, itemId, count,
						this, reference);
	 ...
	 */
	@Override
	public boolean destroyItem(String process, int objectId, int count,
			L2Object reference, boolean sendMessage)
	{
		L2ItemInstance item = _inventory.getItemByObjectId(objectId);

		if (item == null || item.getCount() < count || _inventory
				.destroyItem(process, objectId, count, this, reference) == null)
		{
			if (sendMessage)
				sendPacket(new SystemMessage(SystemMessage.NOT_ENOUGH_ITEMS));
	 ...
	 */
	@Override
	public boolean destroyItemByItemId(String process, int itemId, int count,
			L2Object reference, boolean sendMessage)
	{
	 ...
	 * @return boolean informing if the action was successfull
	 */
	public void destroyWearedItems(String process, L2Object reference,
			boolean sendMessage)
	{
		...
		for (L2ItemInstance item : getInventory().getItems())
		{
			...
			if (item.isWear())
			{
				...
				if (_inventory.destroyItem(process, item, this,
						reference) == null)
				{
					_log.warning("Player " + getName()
							+ " can't destroy weared item: " + item.getName()
							+ "[ " + item.getObjectId() + " ]");
					continue;
				}
	 ...
	 *         in inventory
	 */
	public L2ItemInstance transferItem(String process, int objectId, int count,
			Inventory target, L2Object reference)
	{
			...
			L2Object reference, boolean sendMessage)
	{
		...
		item.dropMe(this, getClientX() + Rnd.get(50) - 25,
				getClientY() + Rnd.get(50) - 25, getClientZ() + 20);
		...
		{
			if (!item.isEquipable() || (item.isEquipable()
	 ...
	 *         in inventory
	 */
	public L2ItemInstance dropItem(String process, int objectId, int count,
			...
			String action)
	{
		...
		{
			_log.finest(getObjectId() + ": player tried to " + action
					+ " item not available in L2World");
			return null;
		}
		...
		if (item.isWear())
		{
			Util.handleIllegalPlayerAction(this,
					"Warning!! Character " + getName() + " tried to  " + action
							+ "  weared item: " + item.getObjectId(),
	...
	public void setProtection(boolean protect)
	{
		if (Config.DEVELOPER && (protect || _protectEndTime > 0))
			System.out.println(this.getName() + ": Protection " + (protect
					? "ON " + (GameTimeController.getGameTicks()
							+ Config.PLAYER_SPAWN_PROTECTION
	...
	public void setRecentFakeDeath(boolean protect)
	{
		_recentFakeDeathEndTime = protect
				? GameTimeController.getGameTicks()
						+ Config.PLAYER_FAKEDEATH_UP_PROTECTION
								* GameTimeController.TICKS_PER_SECOND
				: 0;
	}
	...
	/**
	 * Return the active connection with the client.<BR>
	 * <BR>
	 */
	...
	/**
	 * Set the active connection with the client.<BR>
	 * <BR>
	 */
	 ...
	 * <li>Send a Server->Client packet MyTargetSelected to the player (display
	 ...
	 * <BR>
	 * <li>Send a Server->Client packet MyTargetSelected to the player (display
	 * the select window)</li>
	 * <li>If this L2PcInstance has a Private Store, notify the player AI with
	 * AI_INTENTION_INTERACT</li>
	 * <li>If this L2PcInstance is autoAttackable, notify the player AI with
	 * AI_INTENTION_ATTACK</li><BR>
	 * <BR>
	 * <li>If this L2PcInstance is NOT autoAttackable, notify the player AI with
	 * AI_INTENTION_FOLLOW</li><BR>
	 * <BR>
	 *
		...
		} else
		{
			...
			} else
			{
				...
				if (isAutoAttackable(player))
				{
					...
					// And a cursed weapon holder can't attack players with lvl
					// < 21
					...
					} else
					{
						player.getAI().setIntention(
								CtrlIntention.AI_INTENTION_ATTACK, this);
					}
				} else
				{
					player.getAI().setIntention(
							CtrlIntention.AI_INTENTION_FOLLOW, this);
				}
			}
		}
	}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Send the Server->Client packet StatusUpdate with current HP, MP and
	 * CP to this L2PcInstance</li><BR>
	 * <li>Send the Server->Client packet PartySmallWindowUpdate with current
	 * HP, MP and Level to all other L2PcInstance of the Party</li><BR>
	 * <BR>
	 *
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T SEND current
	 * HP and MP to all L2PcInstance of the _statusListener</B></FONT><BR>
	 * <BR>
	 *
	 */
	@Override
	public void broadcastStatusUpdate()
	{
		...
		if (isInParty() && (needCpUpdate(352) || super.needHpUpdate(352)
				|| needMpUpdate(352)))
		{
			if (Config.DEBUG)
				_log.fine("Send status for party window of " + getObjectId()
						+ "(" + getName() + ") to his party. CP: "
						+ getCurrentCp() + " HP: " + getCurrentHp() + " MP: "
						+ getCurrentMp());
		...
		if (isInOlympiadMode())
		{
			...
			{
				for (L2PcInstance spectator : Olympiad.getInstance()
						.getSpectators(_olympiadGameId))
				{
					if (spectator == null)
						continue;
					spectator.sendPacket(
							new ExOlympiadUserInfo(this, getOlympiadSide()));
				}
			}
		}
	}
	 ...
	 * identified in <B>_knownPlayers</B>. In order to inform other players of
	 * this L2PcInstance state modifications, server just need to go through
	 ...
	 * _KnownPlayers of the L2PcInstance (Public data only)</li><BR>
	 * <BR>
	 *
	 ...
	 * EVT_CANCEL and send a Server->Client packet ActionFailed (if attacker is
	 * a L2PcInstance)</li>
	 * <li>If attack isn't aborted, send a message system (critical hit,
	 * missed...) to attacker/target if they are L2PcInstance</li>
	 * <li>If attack isn't aborted and hit isn't missed, reduce HP of the target
	 * and calculate reflection damage to reduce HP of attacker if necessary
	 * </li>
	 * <li>if attack isn't aborted and hit isn't missed, manage attack or cast
	 * break of the target (calculating rate, sending message...)</li><BR>
	 * <BR>
	 *
	 * @param target
	 ...
	 */
	@Override
	protected void onHitTimer(L2Character target, int damage, boolean crit,
			boolean miss, boolean soulshot, boolean shld)
	{
	 ...
	 * PrivateBuyListSell packet to the L2PcInstance</li>
	 * <li>If the private store is a STORE_PRIVATE_BUY, send a Server->Client
	 * PrivateBuyListBuy packet to the L2PcInstance</li>
	 * <li>If the private store is a STORE_PRIVATE_MANUFACTURE, send a
	 * Server->Client RecipeShopSellList packet to the L2PcInstance</li><BR>
	 * <BR>
	 *
	 * @param target
	 *            The L2Character targeted
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Send a System Message to the L2PcInstance : YOU_PICKED_UP_S1_ADENA or
	 ...
	 * with NewItem (use a new slot) or ModifiedItem (increase amount)</li>
	 * <li>Send a Server->Client packet StatusUpdate to this L2PcInstance with
	...
	protected void doPickupItem(L2Object object)
	{
		...
		StopMove sm = new StopMove(getObjectId(), getX(), getY(), getZ(),
				getHeading());
		if (Config.DEBUG)
			_log.fine("pickup pos: " + target.getX() + " " + target.getY() + " "
					+ target.getZ());
		sendPacket(sm);
		...
		synchronized (target)
		{
			...
			if (((isInParty()
					&& getParty().getLootDistribution() == L2Party.ITEM_LOOTER)
					|| !isInParty()) && !_inventory.validateCapacity(target))
			{
				sendPacket(new ActionFailed());
					...
					&& !isInLooterParty(target.getOwnerId()))
			{
				...
				if (target.getItemId() == 57)
				{
					SystemMessage smsg = new SystemMessage(
							SystemMessage.FAILED_TO_PICKUP_S1_ADENA);
					smsg.addNumber(target.getCount());
					sendPacket(smsg);
				} else if (target.getCount() > 1)
				{
					SystemMessage smsg = new SystemMessage(
							SystemMessage.FAILED_TO_PICKUP_S2_S1_s);
				...
				{
					SystemMessage smsg = new SystemMessage(
							SystemMessage.FAILED_TO_PICKUP_S1);
					smsg.addItemName(target.getItemId());
					sendPacket(smsg);
				}
		...
		{
			IItemHandler handler = ItemHandler.getInstance()
					.getItemHandler(target.getItemId());
			if (handler == null)
				_log.fine("No item handler registered for item ID "
						+ target.getItemId() + ".");
			else
		...
		else if (CursedWeaponsManager.getInstance()
				.isCursed(target.getItemId()))
		{
	 ...
	 * _knownObject and L2PcInstance to _KnownObject of the L2Object)</li><BR>
	 * <BR>
	 *
	 * @param newTarget
	...
	public void setTarget(L2Object newTarget)
	{
		...
		if (newTarget instanceof L2FestivalMonsterInstance
		...
		if (newTarget != null && newTarget instanceof L2Character)
		{
			...
			TargetSelected my = new TargetSelected(getObjectId(),
	...
	/*
	 * public boolean isWearingFormalWear() { return _IsWearingFormalWear; }
	 *
	 * public void setIsWearingFormalWear(boolean value) { _IsWearingFormalWear
	 * = value; }
	 */
	...
	public void doDie(L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			{
				CursedWeaponsManager.getInstance().drop(_cursedWeaponEquipedId,
						killer);
			} else
			{
					...
					if (!ArenaManager.getInstance().checkIfInZone(this))
					{
										...
										.isAtWarWith(_clan.getClanId()))
						{
							((L2PcInstance) killer).getClan()
									.setReputationScore(
											((L2PcInstance) killer).getClan()
													.getReputationScore() + 2,
											true);
							_clan.setReputationScore(
									_clan.getReputationScore() - 2, true);
						}
						if (Config.ALT_GAME_DELEVEL)
						{
							...
							if (getSkillLevel(L2Skill.SKILL_LUCKY) < 0
									|| getStat().getLevel() > 4)
								deathPenalty(
										(pk != null && this.getClan() != null
												&& pk.getClan() != null
												&& pk.getClan().isAtWarWith(
														this.getClanId())));
						} else
						{
	...
	private void onDieDropItem(L2Character killer)
	{
		...
		if (getKarma() <= 0 && killer instanceof L2PcInstance
				&& ((L2PcInstance) killer).getClan() != null
				&& this.getClan() != null && (((L2PcInstance) killer).getClan()
						.isAtWarWith(this.getClanId())
//                      || this.getClan().isAtWarWith(((L2PcInstance)killer).getClanId())
				...
				&& (!isGM() || Config.KARMA_DROP_GM))
		{
			...
			int dropCount = 0;
			while (dropPercent > 0 && Rnd.get(100) < dropPercent
					&& dropCount < dropLimit)
			{
				...
				for (L2ItemInstance itemDrop : getInventory().getItems())
				{
					...
					if (itemDrop.getItemId() == 57 || // Adena
							...
							|| // Quest Items
							nonDroppableList.contains(itemDrop.getItemId()) || // Item
																				// listed
																				// in
																				// the
																				// non
							...
							getPet() != null && getPet()
									.getControlItemId() == itemDrop.getItemId() // Control
																				// Item
																				// of
																				// active
																				// pet
					...
					if (itemDrop.isEquipped())
					{
						...
						// Item
						itemDropPercent = itemDrop.getItem()
								.getType2() == L2Item.TYPE2_WEAPON
										? dropEquipWeapon
										: dropEquip;
						getInventory().unEquipItemInSlotAndRecord(
								itemDrop.getEquipSlot());
					} else
					...
					if (Rnd.get(100) < itemDropPercent)
					{
						...
						if (isKarmaDrop)
							_log.warning(getName()
									+ " has karma and dropped id = "
									+ itemDrop.getItemId() + ", count = "
									+ itemDrop.getCount());
						else
							_log.warning(getName() + " dropped id = "
									+ itemDrop.getItemId() + ", count = "
									+ itemDrop.getCount());
	...
	public void onKillUpdatePvPKarma(L2Character target)
	{
		...
		if (isCursedWeaponEquiped())
		{
			CursedWeaponsManager.getInstance()
					.increaseKills(_cursedWeaponEquipedId);
			return;
		}

		// If in Arena, do nothing
		if (ArenaManager.getInstance().getArenaIndex(this.getX(),
				this.getY()) != -1
				|| ArenaManager.getInstance().getArenaIndex(target.getX(),
						target.getY()) != -1)
			return;
		...
		if ((checkIfPvP(target) && // Can pvp and
				...
				(ZoneManager.getInstance().checkIfInZonePvP(this) && // Player
																		// is
																		// inside
																		// pvp
																		// zone
																		// and
						ZoneManager.getInstance().checkIfInZonePvP(target) // Target
																			// player
																			// is
																			// inside
																			// pvp
	...
	public void updatePvPStatus()
	{
		if (getPvpFlag() == 0)
			startPvPFlag();
		if (getPvpFlag() != 0)
			setlastPvpAttack(System.currentTimeMillis()); // update last pvp
															// ATTACK controller
	}
	...
	public void updatePvPStatus(L2Character target)
	{
		if (checkIfPvP(target)
				&& (!ZoneManager.getInstance().checkIfInZonePvP(this) || // Player
					 ...
					 * target instanceof L2PcInstance && // Target is a player
					 * and !(((L2PcInstance)target).getKarma() > 0) // target
					 * doesn't have Karma (Attacking a PK doesn't flag you.) )
					 */
		)
			updatePvPStatus();
	}
	...
	public void restoreExp(double restorePercent)
	{
		if (_expBeforeDeath > 0)
		{
			...
			getStat().addExp((int) Math.round(
					(_expBeforeDeath - getExp()) * restorePercent / 100));
			_expBeforeDeath = 0;
		}
	}
	 ...
	 * <li>Set the new Experience value of the L2PcInstance and Decrease its
	 * level if necessary</li>
	...
	public void deathPenalty(boolean atwar)
	{
		...
		if (!atEvent)
			if (lvl < Experience.MAX_LEVEL)
				lostExp = Math.round((getStat().getExpForLevel(lvl + 1)
						- getStat().getExpForLevel(lvl)) * percentLost / 100);
			else
				lostExp = Math.round((getStat()
						.getExpForLevel(Experience.MAX_LEVEL)
						- getStat().getExpForLevel(Experience.MAX_LEVEL - 1))
						* percentLost / 100);
	 ...
	 * <li>Send a Server->Client packet StatusUpdate to the L2PcInstance with
	 * new LEVEL, MAX_HP and MAX_MP</li>
	 * <li>Set the current HP and MP of the L2PcInstance, Launch/Stop a HP/MP/CP
	 * Regeneration Task and send StatusUpdate packet to all other L2PcInstance
	 * to inform (exclusive broadcast)</li>
	 * <li>Recalculate the party level</li>
	 * <li>Recalculate the number of Recommandation that the L2PcInstance can
	 * give</li>
	...
	protected void reduceArrowCount()
	{
		L2ItemInstance arrows = getInventory().destroyItem("Consume",
				getInventory().getPaperdollObjectId(Inventory.PAPERDOLL_LHAND),
				1, this, null);

		if (Config.DEBUG)
			_log.fine(
					"arrow count:" + (arrows == null ? 0 : arrows.getCount()));
	...
	public boolean isInvul()
	{
		return _isInvul || _IsTeleporting
				|| _protectEndTime > GameTimeController.getGameTicks();
	}
	 ...
	 * <li>Send a Server->Client Packet MagicSkillCanceld to the L2PcInstance
	...
	{
		if (_connection != null)
			LoginServerThread.getInstance().sendAccessLevel(getAccountName(),
					level);
		else
	...
	public void updateStats()
	{
		...
		// L2PcInstance and to all L2PcInstance in its _KnownPlayers (broadcast)
		updateKarma();
	}
	...
	public void setKarmaFlag(int flag)
	{
		...
		// the L2PcInstance and all L2PcInstance to inform (broadcast)
		StatusUpdate su = new StatusUpdate(getObjectId());
	 ...
	 * and all L2PcInstance to inform (broadcast).<BR>
	 * <BR>
	 */
	...
	public void updateOnlineStatus()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"UPDATE characters SET online=?, lastAccess=? WHERE obj_id=?");
	...
	public void updateIsIn7sDungeonStatus()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"UPDATE characters SET isIn7sDungeon=?, lastAccess=? WHERE obj_id=?");
	...
	private boolean createDb()
	{
		...
		try
		{
			...
			statement.setDouble(34,
					getTemplate().collisionRadius/* getCollisionRadius() */);
			statement.setDouble(35,
					getTemplate().collisionHeight/* getCollisionHeight() */);
	...
	/**
	 * Retrieve a L2PcInstance from the characters table of the database and add
	 * it in _allObjects of the L2world.<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	...
	private static L2PcInstance restore(int objectId)
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement(RESTORE_CHARACTER);
			...
			while (rset.next())
			{
				...
				final L2PcTemplate template = CharTemplateTable.getInstance()
						.getTemplate(activeClassId);
				PcAppearance app = new PcAppearance(rset.getByte("face"),
						rset.getByte("hairColor"), rset.getByte("hairStyle"),
						female);
				...
				player.setClanJoinExpiryTime(
						rset.getLong("clan_join_expiry_time"));
				if (player.getClanJoinExpiryTime() < System.currentTimeMillis())
				{
					player.setClanJoinExpiryTime(0);
				}
				player.setClanCreateExpiryTime(
				...
				player.setFistsWeaponItem(
						player.findFistsWeaponItem(activeClassId));
				...
				// Check recs
				player.checkRecom(rset.getInt("rec_have"),
						rset.getInt("rec_left"));
						...
						&& activeClassId != player.getBaseClass())
				{
					...
					// a possible restart-while-modifysubclass cheat has been
					// attempted.
				...
				player.setIsIn7sDungeon(
						(rset.getInt("isin7sdungeon") == 1) ? true : false);
				...
				player.setAllianceWithVarkaKetra(
	...
	private static boolean restoreSubClassData(L2PcInstance player)
	{
		...
		{
			_log.warning("Could not restore classes for " + player.getName()
					+ ": " + e);
			e.printStackTrace();
		} finally
		{
	...
	private void storeRecipeBook()
	{
		...
		// book.
		if (isSubClassActive() || (getCommonRecipeBook().length == 0
				&& getDwarvenRecipeBook().length == 0))
			return;
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"DELETE FROM character_recipebook WHERE char_id=?");
			statement.setInt(1, getObjectId());
			statement.execute();
			statement.close();
			...
			for (L2RecipeList recipe : recipes)
			{
				statement = con.prepareStatement(
						"INSERT INTO character_recipebook (char_id, id, type) values(?,?,0)");
				statement.setInt(1, getObjectId());
			...
			{
				statement = con.prepareStatement(
						"INSERT INTO character_recipebook (char_id, id, type) values(?,?,1)");
				statement.setInt(1, getObjectId());
				statement.setInt(2, recipe.getId());
				statement.execute();
				statement.close();
	...
	private void restoreRecipeBook()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(
					"SELECT id, type FROM character_recipebook WHERE char_id=?");
			statement.setInt(1, getObjectId());
			ResultSet rset = statement.executeQuery();
			...
			while (rset.next())
			{
				recipe = RecipeController.getInstance()
						.getRecipeList(rset.getInt("id") - 1);
...
//			statement.setInt(30, getBaseTemplate().race.ordinal());
			...
			if (_onlineBeginTime > 0)
				totalOnlineTime += (System.currentTimeMillis()
						- _onlineBeginTime) / 1000;
	...
	private void storeCharSub()
	{
		...
		{
			_log.warning("Could not store sub class data for " + getName()
					+ ": " + e);
		} finally
		{
			try
	 ...
	 * <li>Remove all its Func objects from the L2Character calculator
	 * set</li><BR>
	 * <BR>
	 *
	...
	public void restoreEffects()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (skillId == -1 || effectCount == -1 || effectCurTime == -1
						|| reuseDelay < 0)
					continue;

				L2Skill skill = SkillTable.getInstance().getInfo(skillId,
						skillLvl);
				ISkillHandler IHand = SkillHandler.getInstance()
						.getSkillHandler(skill.getSkillType());
				if (IHand != null)
	 ...
	 * _henna and calculate stats of the L2PcInstance.<BR>
	 * <BR>
	 */
	private void restoreHenna()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (symbol_id != 0)
				{
					L2Henna tpl = HennaTable.getInstance()
							.getTemplate(symbol_id);
	 ...
	 * add to _recomChars and calculate stats of the L2PcInstance.<BR>
	 * <BR>
	 */
	private void restoreRecom()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement(RESTORE_CHAR_RECOMS);
			statement.setInt(1, getObjectId());
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{
	...
	public boolean removeHenna(int slot)
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con
					.prepareStatement(DELETE_CHAR_HENNA);
			statement.setInt(1, getObjectId());
		...
		getInventory().addItem("Henna", henna.getItemIdDye(),
				henna.getAmountDyeRequire() / 2, this, null);
	...
	public boolean addHenna(L2HennaInstance henna)
	{
		...
		{
			this.sendMessage(
					"You may not have more than three equipped symbols at a time.");
			return false;
		}
	 ...
	 * <li>If the attacker is a L2PcInstance, check if it is not in the same
	 * siege clan (Attacker, Defender)</li><BR>
	 * <BR>
	 *
	 */
	@Override
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		if (getClan() != null && attacker != null
				&& getClan().isMember(attacker.getName()))
			return false;

		if (attacker instanceof L2PlayableInstance
				&& ZoneManager.getInstance().checkIfInZonePeace(this))
			return false;
		...
		if (attacker instanceof L2PcInstance)
		{
			...
			if (ZoneManager.getInstance().checkIfInZonePvP(this)
					&& ZoneManager.getInstance().checkIfInZonePvP(attacker))
				return true;
			...
			if (this.getClan() != null)
			{
				...
				if (siege != null)
				{
					...
					if ((siege.checkIsDefender(
							((L2PcInstance) attacker).getClan())
							&& siege.checkIsDefender(this.getClan()))
							|| (siege.checkIsAttacker(
									((L2PcInstance) attacker).getClan())
				...
				if (this.getClan() != null
						...
						&& (this.getClan().isAtWarWith(
								((L2PcInstance) attacker).getClanId())
	...
	public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		 ...
		 * SystemMessage(SystemMessage.CANNOT_USE_ITEMS_SKILLS_WITH_FORMALWEAR))
		 * ;
		 *
		...
		// If summon siege golem (13), Summon Wild Hog Cannon (299), check its
		// ok to place the flag
		if ((skill.getId() == 13 || skill.getId() == 299)
		...
		// same
		// Note that this check is currently imperfect: getCurrentSkill() isn't
		// always null when a skill has
		// failed to cast, or the casting is not yet in progress when this is
		// rechecked
		if (getCurrentSkill() != null && isCastingNow())
		{
			...
			if (Config.DEBUG && getQueuedSkill() != null)
				_log.info(getQueuedSkill().getSkill().getName()
						+ " is already queued for " + getName() + ".");
		...
		if (getQueuedSkill() != null) // wiping out previous values, after
		...
		if (isSkillDisabled(skill.getId())
				&& (getAccessLevel() < Config.GM_PEACEATTACK))
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.SKILL_NOT_AVAILABLE);
		...
		if (getCurrentMp() < getStat().getMpConsume(skill)
				+ getStat().getMpInitialConsume(skill))
		{
		...
		if (skill.getItemConsume() > 0)
		{
			...
			L2ItemInstance requiredItems = getInventory()
					.getItemByItemId(skill.getItemConsumeId());

			// Check if the caster owns enought consummed Item to cast
			if (requiredItems == null
					|| requiredItems.getCount() < skill.getItemConsume())
			{
				...
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.SUMMONING_SERVITOR_COSTS_S2_S1);
				...
				} else
				{
					// Send a System Message to the caster
					sendPacket(
							new SystemMessage(SystemMessage.NOT_ENOUGH_ITEMS));
					return;
				}
			}
		}
		...
		if (!skill.getWeaponDependancy(this)
				|| !skill.checkCondition(this, false) || isAlikeDead())
		{
			// Send a Server->Client packet ActionFailed to the L2PcInstance
			sendPacket(new ActionFailed());
			return;
		...
		if (isFishing()
				&& (sklType != SkillType.PUMPING && sklType != SkillType.REELING
						&& sklType != SkillType.FISHING))
		{
			// Only fishing skills are available
			this.sendPacket(
					new SystemMessage(SystemMessage.ONLY_FISHING_SKILLS_NOW));
			return;
		}
		...
		if (skill.isOffensive())
		{
					...
					&& (getAccessLevel() < Config.GM_PEACEATTACK))
			{
				...
				// ActionFailed
				sendPacket(
						new SystemMessage(SystemMessage.TARGET_IN_PEACEZONE));
				sendPacket(new ActionFailed());
				return;
			}
			...
			if (dontMove)
			{
				...
				// target
				if (skill.getCastRange() > 0 && !isInsideRadius(target,
		...
		if (!skill.isOffensive())
		{
			// check if the target is a monster and if force attack is set.. if
		...
		if (sklType == SkillType.SPOIL)
		{
			if (!(target instanceof L2MonsterInstance))
			{
				...
				sendPacket(
						new SystemMessage(SystemMessage.TARGET_IS_INCORRECT));
		...
		if (sklType == SkillType.SWEEP && target instanceof L2Attackable)
		{
			...
			if (((L2Attackable) target).isDead())
			{
				if (!((L2Attackable) target).isSpoil())
				{
					// Send a System Message to the L2PcInstance
					sendPacket(new SystemMessage(
							SystemMessage.SWEEPER_FAILED_TARGET_NOT_SPOILED));
				...
				if (getObjectId() != spoilerId && !isInLooterParty(spoilerId))
				{
					// Send a System Message to the L2PcInstance
					sendPacket(
							new SystemMessage(SystemMessage.SWEEP_NOT_ALLOWED));
				...
				&& !SiegeFlag.checkIfOkToPlaceFlag(this, false))
		{
			...
			abortCast();
			return;
		} else if (sklType == SkillType.STRSIEGEASSAULT && !StrSiegeAssault
				.checkIfOkToUseStriderSiegeAssault(this, false))
		{
			sendPacket(new ActionFailed());
			abortCast();
			return;
	...
	{
		L2PcInstance looter = (L2PcInstance) L2World.getInstance()
				.findObject(LooterId);
	 ...
	 * @return False if the skill is a pvpSkill and target is not a valid pvp
	 *         target
	 */
	public boolean checkPvpSkill(L2Object target, L2Skill skill)
	{
		...
		if (target != null && // target not null and
				...
				!ZoneManager.getInstance().checkIfInZonePvP(this) && // Pc is
																		// not
																		// in
																		// PvP
																		// zone
				!ZoneManager.getInstance().checkIfInZonePvP(target) // target is
																	// not in
																	// PvP zone
		)
		{
			...
			{
				if (this.getClan() != null
						&& ((L2PcInstance) target).getClan() != null)
				{
					if (this.getClan().isAtWarWith(
							((L2PcInstance) target).getClan().getClanId()))
	...
	public void consumeItem(int itemConsumeId, int itemCount)
	{
		if (itemConsumeId != 0 && itemCount != 0)
			destroyItemByItemId("Consume", itemConsumeId, itemCount, null,
					false);
	}
	 ...
	 * a Server->Client packet InventoryUpdate to the L2PcInstance.<BR>
	 * <BR>
	 */
		...
		if (SiegeManager.getInstance().checkIfInZone(this)
				&& !(getClan() != null
						&& CastleManager.getInstance()
								.getCastle(this) == CastleManager.getInstance()
										.getCastleByOwner(getClan())
	...
	public void setMountType(int mountType)
	{
		...
		case 1:
			...
			if (isNoble())
			{
				L2Skill striderAssaultSkill = SkillTable.getInstance()
						.getInfo(325, 1);
	...
	public void tempInvetoryDisable()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new InventoryEnable(),
				1500);
	}
	...
	public void rechargeAutoSoulShot(boolean physical, boolean magic,
			boolean summon)
	{
		L2ItemInstance item;
				...
				if (magic)
				{
					if (!summon)
					{
						if (itemId == 2509 || itemId == 2510 || itemId == 2511
								|| itemId == 2512 || itemId == 2513
								|| itemId == 2514 || itemId == 3947
								|| itemId == 3948 || itemId == 3949
								|| itemId == 3950 || itemId == 3951
								|| itemId == 3952 || itemId == 5790)
						{
							handler = ItemHandler.getInstance()
	...
	class LookingForFishTask implements Runnable
	{
		...
		protected LookingForFishTask(int fishWaitTime, int fishGutsCheck,
				int fishType, boolean isNoob, boolean isUpperGrade)
		{
	...
	public boolean addSubClass(int classId, int classIndex)
	{
		if (getTotalSubClasses() == 3 || classIndex == 0
				|| getSubClasses().containsKey(classIndex))
			return false;
		...
		{
			_log.warning("WARNING: Could not add character sub class for "
					+ getName() + ": " + e);
			return false;
		} finally
		{
		...
		if (Config.DEBUG)
			_log.info(getName() + " added class ID " + classId
					+ " as a sub class at index " + classIndex + ".");

		ClassId subTemplate = ClassId.values()[classId];
		Collection<L2SkillLearn> skillTree = SkillTreeTable.getInstance()
				.getAllowedSkills(subTemplate);
		...
		for (L2SkillLearn skillInfo : skillTree)
		{
			if (skillInfo.getMinLevel() <= 40)
			{
				...
				L2Skill newSkill = SkillTable.getInstance()
						.getInfo(skillInfo.getId(), skillInfo.getLevel());
	...
	public boolean modifySubClass(int classIndex, int newClassId)
	{
		...
		if (Config.DEBUG)
			_log.info(getName() + " has requested to modify sub class index "
					+ classIndex + " from class ID " + oldClassId + " to "
					+ newClassId + ".");
		...
		{
			_log.warning("Could not modify sub class for " + getName()
					+ " to class index " + classIndex + ": " + e);
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		} else
		{
			...
			{
				_log.info("Could not switch " + getName()
						+ "'s sub class to class index " + classIndex + ": "
						+ e);
				return false;
			}
		}
		...
		if (isCursedWeaponEquiped())
			CursedWeaponsManager.getInstance()
					.givePassive(_cursedWeaponEquipedId);
	...
	public void startWarnUserTakeBreak()
	{
		if (_taskWarnUserTakeBreak == null)
			_taskWarnUserTakeBreak = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new WarnUserTakeBreak(),
							7200000, 7200000);
	}
	...
	public void startRentPet(int seconds)
	{
		if (_taskRentPet == null)
			_taskRentPet = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new RentPetTask(),
	...
	public void startWaterTask()
	{
		if (!isDead() && _taskWater == null)
		{
			...
			_taskWater = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new WaterTask(), timeinwater,
							1000);
		}
	}
	...
	public void checkWaterState()
	{
		...
		Zone zone = ZoneManager.getInstance().getZone(
				ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Water), getX(),
				getY()); // checking if char is in water zone

		if (zone == null)
			zone = ZoneManager.getInstance()
					.getZoneType(ZoneType
							.getZoneTypeName(ZoneType.ZoneTypeEnum.Underground))
					.getZone(getX(), getY()); // checking if char is in
												// underground and in water
	...
	public void onPlayerEnter()
	{
				...
				|| SevenSigns.getInstance().isCompResultsPeriod())
		{
			if (!isGM() && isIn7sDungeon()
					&& SevenSigns.getInstance()
							.getPlayerCabal(this) != SevenSigns.getInstance()
		...
		{
			if (!isGM() && isIn7sDungeon() && SevenSigns.getInstance()
					.getPlayerCabal(this) == SevenSigns.CABAL_NULL)
			{
				teleToLocation(MapRegionTable.TeleportWhereType.Town);
				setIsIn7sDungeon(false);
				sendMessage(
	...
	public void restartRecom()
	{
		if (Config.ALT_RECOMMEND)
		{
			...
			try
			{
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con
						.prepareStatement(DELETE_CHAR_RECOMS);
				statement.setInt(1, getObjectId());
				statement.execute();
				statement.close();
	...
	public void ReviveRequest(L2PcInstance Reviver, L2Skill skill, boolean Pet)
	{
		if (_ReviveRequested == 1)
		{
			if (_RevivePet == Pet)
			{
				Reviver.sendPacket(new SystemMessage(1513)); // Resurrection is
																// already been
																// proposed.
			} else
			{
				if (Pet)
					Reviver.sendPacket(new SystemMessage(1515)); // A pet cannot
																	// be
																	// resurrected
																	// while
																	// it's
					...
					Reviver.sendPacket(new SystemMessage(1511)); // While a pet
																	// is
																	// attempting
																	// to
																	// resurrect,
		...
		if ((Pet && getPet() != null && getPet().isDead())
				|| (!Pet && isDead()))
		{
			_ReviveRequested = 1;
			_RevivePower = Formulas.getInstance()
					.calculateSkillResurrectRestorePercent(skill.getPower(),
							Reviver.getWIT());
			_RevivePet = Pet;
			sendPacket(new ConfirmDlg(SystemMessage.RESSURECTION_REQUEST,
					Reviver.getName()));
		}
	}
	...
	public void ReviveAnswer(int answer)
	{
		if (_ReviveRequested != 1 || (!isDead() && !_RevivePet)
				|| (_RevivePet && getPet() != null && !getPet().isDead()))
			return;
		if (answer == 1)
		{
			if (!_RevivePet)
	...
	public final boolean updatePosition(int gameTicks)
	{
		...
		int distPassed = (int) getStat().getMoveSpeed()
				* (gameTicks - m._moveTimestamp)
				/ GameTimeController.TICKS_PER_SECOND;
		double distFraction = (distPassed)
...
//		if (Config.DEVELOPER) System.out.println("Move Ticks:" + (gameTicks - m._moveTimestamp) + ", distPassed:" + distPassed + ", distFraction:" + distFraction);
		...
		} else
		{
			...
			// move
			super.setXYZ(getX() + (int) (dx * distFraction + 0.5),
					getY() + (int) (dy * distFraction + 0.5),
					getZ() + (int) (dz * distFraction));
		}
	...
	public synchronized boolean validateBypass(String cmd)
	{
		...
		_log.warning("[L2PcInstance] player [" + getName()
				+ "] sent invalid bypass '" + cmd + "', ban this player!");
		return false;
	}
	...
	public void StartFishing()
	{
		...
		List<FishData> fishs = FishTable.getInstance().getfish(lvl, type,
				group);
		...
		fishs = null;
		sendPacket(
				new SystemMessage(SystemMessage.CAST_LINE_AND_START_FISHING));
		...
		if (!isDead() && _taskforfish == null)
		{
			...
			if (_lure != null)
			{
				...
				if (lureid == 6519 || lureid == 6522 || lureid == 6525
						|| lureid == 8505 || lureid == 8508 || lureid == 8511) // low
																				// grade
					checkDelay = Math
							.round((float) (_fish.getGutsCheckTime() * (1.33)));
				else if (lureid == 6520 || lureid == 6523 || lureid == 6526
						|| (lureid >= 8505 && lureid <= 8513)
						|| (lureid >= 7610 && lureid <= 7613)
						|| (lureid >= 7807 && lureid <= 7809)
						|| (lureid >= 8484 && lureid <= 8486)) // medium grade,
																// beginner,
																// prize-winning
																// & quest
																// special bait
					checkDelay = Math
							.round((float) (_fish.getGutsCheckTime() * (1.00)));
				else if (lureid == 6521 || lureid == 6524 || lureid == 6527
						|| lureid == 8507 || lureid == 8510 || lureid == 8513) // high
																				// grade
					checkDelay = Math
							.round((float) (_fish.getGutsCheckTime() * (0.66)));
			}
			_taskforfish = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(
							new LookingForFishTask(_fish.getWaitTime(),
									_fish.getFishGuts(), _fish.getType(),
									isNoob, isUpperGrade),
							10000, checkDelay);
		}
	}
	...
	public void EndFishing(boolean win)
	{
		...
		if (_fishCombat == null)
			sendPacket(
					new SystemMessage(SystemMessage.BAIT_LOST_FISH_GOT_AWAY));
		_fishCombat = null;
		_lure = null;
		// Ends fishing
	...
	{
		return Config.FREIGHT_SLOTS
				+ (int) getStat().calcStat(Stats.FREIGHT_LIM, 0, null, null);
	}
			...
			boolean shiftPressed)
	{
		...
		if (Config.DEBUG)
			_log.info("Setting current skill: " + currentSkill.getName()
					+ " (ID: " + currentSkill.getId() + ") for " + getName()
					+ ".");
	 ...
	 *
	 */
	public void setQueuedSkill(L2Skill queuedSkill, boolean ctrlPressed,
			boolean shiftPressed)
	{
		if (queuedSkill == null)
		{
		...
		if (Config.DEBUG)
			_log.info(
					"Setting queued skill: " + queuedSkill.getName() + " (ID: "
							+ queuedSkill.getId() + ") for " + getName() + ".");
	...
	public void setInJail(boolean state, int delayInMinutes)
	{
		...
		if (_inJail)
		{
			if (delayInMinutes > 0)
			{
				...
				// start the countdown
				_jailTask = ThreadPoolManager.getInstance()
						.scheduleGeneral(new JailTask(this), _jailTimer);
				sendMessage(
						"You are in jail for " + delayInMinutes + " minutes.");
			}

			// Open a Html message to inform the player
			NpcHtmlMessage htmlMsg = new NpcHtmlMessage(0);
			String jailInfos = HtmCache.getInstance()
					.getHtm("data/html/jail_in.htm");
			if (jailInfos != null)
				htmlMsg.setHtml(jailInfos);
			else
				htmlMsg.setHtml(
						"<html><body>You have been put in jail by an admin.</body></html>");
			sendPacket(htmlMsg);
		...
		} else
		{
			...
			NpcHtmlMessage htmlMsg = new NpcHtmlMessage(0);
			String jailInfos = HtmCache.getInstance()
					.getHtm("data/html/jail_out.htm");
			if (jailInfos != null)
				htmlMsg.setHtml(jailInfos);
			else
				htmlMsg.setHtml(
						"<html><body>You are free for now, respect server rules!</body></html>");
			sendPacket(htmlMsg);
	...
	private void updateJailState()
	{
		if (isInJail())
		{
			...
			if (_jailTimer > 0)
			{
				...
				_jailTask = ThreadPoolManager.getInstance()
						.scheduleGeneral(new JailTask(this), _jailTimer);
				sendMessage("You are still in jail for "
						+ Math.round(_jailTimer / 60000) + " minutes.");
			}

			// If player escaped, put him back in jail
			if (!ZoneManager.getInstance().checkIfInZone(
					ZoneType.getZoneTypeName(ZoneType.ZoneTypeEnum.Jail), this))
	...
	private FastMap<Integer, TimeStamp> ReuseTimeStamps = new FastMap<>();
