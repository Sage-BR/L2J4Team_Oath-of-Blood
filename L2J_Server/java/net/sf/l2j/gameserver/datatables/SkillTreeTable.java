...
public class SkillTreeTable
{
	private static Logger _log = Logger
			.getLogger(SkillTreeTable.class.getName());

	private static SkillTreeTable _instance;

	private Map<ClassId, Map<Integer, L2SkillLearn>> _skillTrees;

	private List<L2SkillLearn> _fishingSkillTrees; // all common skills (teached
													// by Fisherman)

	private List<L2SkillLearn> _expandDwarfCraftSkillTrees; // list of special
															// skill for dwarf
															// (expand dwarf
															// craft) learned by
															// class teacher
	...
	public int getExpertiseLevel(int grade)
	{
		...
		// for now
		Map<Integer, L2SkillLearn> learnMap = getSkillTrees()
				.get(ClassId.paladin);
	...
	private SkillTreeTable()
	{
		...
		try
		{
			...
			PreparedStatement statement = con
					.prepareStatement("SELECT * FROM class_list ORDER BY id");
			...
			while (classlist.next())
			{
				...
				if (parentClassId != -1)
				{
					Map<Integer, L2SkillLearn> parentMap = getSkillTrees()
							.get(ClassId.values()[parentClassId]);
				...
				while (skilltree.next())
				{
					...
					skillLearn = new L2SkillLearn(id, lvl, minLvl, name, cost,
							0, 0);
				...
				count += map.size();
				_log.fine("SkillTreeTable: skill tree for class " + classId
						+ " has " + map.size() + " skills");
			}
		...
		{
			_log.severe("Error while creating skill tree (Class ID " + classId
					+ "):" + e);
		}
		...
		try
		{
			...
			while (skilltree2.next())
			{
				...
				L2SkillLearn skill = new L2SkillLearn(id, lvl, minLvl, name,
						cost, costId, costCount);
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT skill_id, level, name, base_lvl, sp, min_skill_lvl, exp, success_rate76, success_rate77, success_rate78 FROM enchant_skill_trees ORDER BY skill_id, level");
			...
			while (skilltree3.next())
			{
				...
				L2EnchantSkillLearn skill = new L2EnchantSkillLearn(id, lvl,
						minSkillLvl, baseLvl, name, sp, exp, rate76, rate77,
						rate78);
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT skill_id, level, name, clan_lvl, repCost, itemId FROM pledge_skill_trees ORDER BY skill_id, level");
			...
			while (skilltree4.next())
			{
				...
				L2PledgeSkillLearn skill = new L2PledgeSkillLearn(id, lvl,
						baseLvl, name, sp, itemId);
		...
		_log.config(
				"FishingSkillTreeTable: Loaded " + count2 + " general skills.");
		_log.config(
				"FishingSkillTreeTable: Loaded " + count3 + " dwarven skills.");
		_log.config(
				"EnchantSkillTreeTable: Loaded " + count4 + " enchant skills.");
		_log.config(
				"PledgeSkillTreeTable: Loaded " + count5 + " pledge skills");
	}
	...
	public L2SkillLearn[] getAvailableSkills(L2PcInstance cha, ClassId classId)
	{
		...
		if (skills == null)
		{
			// the skilltree for this class is undefined, so we give an empty
			// list
			_log.warning(
					"Skilltree for class " + classId + " is not defined !");
	...
	public L2EnchantSkillLearn[] getAvailableEnchantSkills(L2PcInstance cha)
	{
		...
		List<L2EnchantSkillLearn> skills = new FastList<>();
	...
	{
		List<L2PledgeSkillLearn> result = new FastList<>();
	...
	public int getSkillCost(L2PcInstance player, L2Skill skill)
	{
		...
		{
			L2SkillLearn skillLearn = getSkillTrees().get(classId)
					.get(skillHashCode);
	...
	{
		int skillCost = 100000000;
		L2EnchantSkillLearn[] enchantSkillLearnList = getAvailableEnchantSkills(
				player);

		for (L2EnchantSkillLearn enchantSkillLearn : enchantSkillLearnList)
		{
			if ((enchantSkillLearn.getId() != skill.getId())
					|| (enchantSkillLearn.getLevel() != skill.getLevel())
					|| (76 > player.getLevel()))
				continue;
