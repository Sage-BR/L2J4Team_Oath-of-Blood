...
public class ScrollOfResurrection implements IItemHandler
{
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		boolean humanScroll = (itemId == 3936 || itemId == 3959
				|| itemId == 737);
			...
			if (targetPlayer != null || targetPet != null)
			{
				...
				if (targetPlayer != null)
					...
					castle = CastleManager.getInstance()
							.getCastle(targetPet.getX(), targetPet.getY());
				...
				if (targetPet != null)
				{
					if (targetPet.getOwner() != activeChar)
					{
						if (targetPet.getOwner().isReviveRequested())
						{
							if (targetPet.getOwner().isRevivingPet())
								activeChar.sendPacket(new SystemMessage(1513)); // Resurrection
																				// is
																				// already
																				// been
																				// proposed.
							else
								activeChar.sendPacket(new SystemMessage(1515)); // A
																				// pet
																				// cannot
																				// be
																				// resurrected
					...
					} else if (!petScroll)
					{
						...
						activeChar.sendMessage(
								"You do not have the correct scroll");
					}
				} else
				{
																...
																// festival.
					{
						...
						activeChar.sendPacket(SystemMessage.sendString(
								"You may not resurrect participants in a festival."));
					}
					if (targetPlayer.isReviveRequested())
					{
						if (targetPlayer.isRevivingPet())
							activeChar.sendPacket(new SystemMessage(1511)); // While
																			// a
																			// pet
																			// is
																			// attempting
				...
				if (condGood)
				{
					if (!activeChar.destroyItem("Consume", item.getObjectId(),
							1, null, false))
						return;
					...
					if (skillId != 0)
					{
						 ...
						 * activeChar.broadcastPacket(msu); SetupGauge sg = new
						 * SetupGauge(0, skill.getHitTime());
						 * activeChar.sendPacket(sg);
						 */

						SystemMessage sm = new SystemMessage(
								SystemMessage.S1_DISAPPEARED);
