...
/**
 * @author yellowperil & Fulminus This class is similar to the
 *         SiegeGuardManager, except it handles the loading of the mercenary
 *         tickets that are dropped on castle floors by the castle lords. These
 *         tickets (aka badges) need to be readded after each server reboot
 *         except when the server crashed in the middle of an ongoig siege. In
 *         addition, this class keeps track of the added tickets, in order to
 *         properly limit the number of mercenaries in each castle and the
 *         number of mercenaries from each mercenary type. Finally, we provide
 *         auxilary functions to identify the castle in which each item (and its
 *         corresponding NPC) belong to, in order to help avoid mixing them up.
 *
 */
public class MercTicketManager
{
	protected static Logger _log = Logger
			.getLogger(CastleManager.class.getName());
	...
	// =========================================================
	// Data Field
	private List<L2ItemInstance> _DroppedTickets; // to keep track of items on
													// the ground
	...
	private final int[] _ItemIds =
		{ 3960, 3961, 3962, 3963, 3964, 3965, 3966, 3967, 3968, 3969, // Gludio
				...
				5205, 5206, 5207, 5208, 5209, 5210, 5211, 5212, 5213, 5214, // Innadril
				6779, 6780, 6781, 6782, 6783, 6784, 6785, 6786, 6787, 6788, // Goddard
				7973, 7974, 7975, 7976, 7977, 7978, 7979, 7980, 7981, 7982, // Rune
	...
	private final void load()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"SELECT * FROM castle_siege_guards Where isHired = 1");
			rs = statement.executeQuery();

			int npcId;
			int itemId;
			int x, y, z;
			// start index to begin the search for the itemId corresponding to
			// this NPC
			// this will help with:
			// a) skip unnecessary iterations in the search loop
			// b) avoid finding the wrong itemId whenever tickets of different
			// spawn the same npc!
			int startindex = 0;
			...
			while (rs.next())
			{
				...
				for (int i = startindex; i < _NpcIds.length; i++)
					...
					{
						// only handle tickets if a siege is not ongoing in this
						// npc's castle
								...
								&& !(castle.getSiege().getIsInProgress()))
						{
							...
							L2ItemInstance dropticket = new L2ItemInstance(
									IdFactory.getInstance().getNextId(),
									itemId);
							dropticket.setLocation(
									L2ItemInstance.ItemLocation.INVENTORY);
							dropticket.dropMe(null, x, y, z);
							dropticket.setDropTime(0); // avoids it from beeing
														// removed by the auto
														// item destroyer
			...
			System.out.println("Loaded: " + getDroppedTickets().size()
					+ " Mercenary Tickets");
		} catch (Exception e)
		{
			System.out.println(
					"Exception: loadMercenaryData(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	public boolean isAtCasleLimit(int itemId)
	{
		...
		for (L2ItemInstance element : getDroppedTickets())
		{
			ticket = element;
			if ((ticket != null)
					&& (getTicketCastleId(ticket.getItemId()) == castleId))
				count++;
		}
		if (count >= limit)
			return true;
	...
	/**
	 * addTicket actions 1) find the npc that needs to be saved in the mercenary
	 * spawns, given this item 2) Use the passed character's location info to
	 * add the spawn 3) create a copy of the item to drop in the world returns
	 * the id of the mercenary npc that was added to the spawn returns -1 if
	 * this fails.
	 */
	public int addTicket(int itemId, L2PcInstance activeChar, String[] messages)
	{
		...
		for (int i = 0; i < _ItemIds.length; i++)
		{
			if (_ItemIds[i] == itemId) // Find the index of the item used
			{
				// Temporary tap into QuestPcSpawn to spawn, auto chat, and
				// despawn merc
				QuestPcSpawn qps = new QuestPcSpawn(activeChar);
				int spawnObjectId = qps.addSpawn(_NpcIds[i], x, y, z, 3000,
						messages, 0);
				// Spawn, auto chat, and despawn merc.
				/* L2Spawn spawn = */qps.getSpawn(spawnObjectId);

				// Hire merc for this caslte. NpcId is at the same index as the
				// item used.
				castle.getSiege().getSiegeGuardManager().hireMerc(x, y, z,
						heading, _NpcIds[i]);

				// create the ticket in the gameworld
				L2ItemInstance dropticket = new L2ItemInstance(
						IdFactory.getInstance().getNextId(), itemId);
				dropticket.setLocation(L2ItemInstance.ItemLocation.INVENTORY);
				dropticket.dropMe(null, x, y, z);
				dropticket.setDropTime(0); // avoids it from beeing removed by
											// the auto item destroyer
				L2World.getInstance().storeObject(dropticket); // add to the
																// world
	...
	/**
	 * Delete all tickets from a castle; remove the items from the world and
	 * remove references to them from this class
	 */
	public void deleteTickets(int castleId)
	{
		...
		while (i < getDroppedTickets().size())
		{
			...
			if ((item != null)
					&& (getTicketCastleId(item.getItemId()) == castleId))
			{
				item.decayMe();
	...
	/**
	 * remove a single ticket and its associated spawn from the world (used when
	 * the castle lord picks up a ticket, for example)
	 */
	public void removeTicket(L2ItemInstance item)
	{
		...
		Castle castle = CastleManager.getInstance()
				.getCastle(getTicketCastleId(itemId));

		if (npcId > 0 && castle != null)
		{
			(new SiegeGuardManager(castle)).removeMerc(npcId, item.getX(),
					item.getY(), item.getZ());
		}
