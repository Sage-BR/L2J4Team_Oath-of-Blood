...
public class ObjectKnownList
{
	...
	public boolean addKnownObject(L2Object object, L2Character dropper)
	{
		...
		if (!Util.checkIfInRange(getDistanceToWatchObject(object),
				getActiveObject(), object, true))
			return false;
	 ...
	 * L2Object alreday known by the L2Character</li><BR>
	 * <BR>
	 *
	 */
	...
	private final void findCloseObjects()
	{
		...
		{
			Collection<L2Object> objects = L2World.getInstance()
					.getVisibleObjects(getActiveObject());
			if (objects == null)
				return;
		...
		{
			Collection<L2PlayableInstance> playables = L2World.getInstance()
					.getVisiblePlayable(getActiveObject());
			if (playables == null)
				return;
	...
	private final void forgetObjects()
	{
		...
		for (L2Object object : knownObjects)
		{
			...
			if (!object.isVisible()
					|| !Util.checkIfInRange(getDistanceToForgetObject(object),
							getActiveObject(), object, true))
				if (object instanceof L2BoatInstance
						&& getActiveObject() instanceof L2PcInstance)
				{
					if (((L2BoatInstance) (object))
							.GetVehicleDeparture() == null)
					{
						//
					} else if (((L2PcInstance) getActiveObject()).isInBoat())
					{
						if (((L2PcInstance) getActiveObject())
								.getBoat() == object)
						{
							//
						} else
