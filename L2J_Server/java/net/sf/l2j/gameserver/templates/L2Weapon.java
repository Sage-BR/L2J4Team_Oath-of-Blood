...
public final class L2Weapon extends L2Item
{
	 ...
	 * @param set
	 *            : StatsSet designating the set of couples (key,value)
	...
	public L2Weapon(L2WeaponType type, StatsSet set)
	{
		...
		_atkReuse = set.getInteger("atk_reuse",
				type == L2WeaponType.BOW ? 1500 : 0);
			...
			boolean crit)
	{
		...
		{
			if (target.isRaid() && (skill.getSkillType() == SkillType.CONFUSION
			...
			L2Skill trigger)
	{
		...
		for (L2Skill skill : _skillsOnCast)
		{
			...
			// No buffing with toggle skills

			if ((trigger.isToggle() && skill.getSkillType() == SkillType.BUFF)
					|| !skill.checkCondition(caster, true))
				continue; // Skill condition not met
