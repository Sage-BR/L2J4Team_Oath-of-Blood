...
public class Recipes implements IItemHandler
{
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		L2RecipeList rp = RecipeController.getInstance()
				.getRecipeByItemId(item.getItemId());
		if (activeChar.hasRecipeList(rp.getId()))
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.RECIPE_ALREADY_REGISTERED);
				...
				if (activeChar.hasDwarvenCraft())
				{
					if (rp.getLevel() > activeChar.getDwarvenCraft())
					{
						...
						activeChar.sendPacket(sm);
							...
							.getDwarvenRecipeBook().length >= activeChar
									.GetDwarfRecipeLimit())
					{
					...
					} else
					{
						...
						activeChar.destroyItem("Consume", item.getObjectId(), 1,
								null, false);
						SystemMessage sm = new SystemMessage(614);
						sm.addString("Added recipe \"" + rp.getRecipeName()
								+ "\" to Dwarven RecipeBook");
						activeChar.sendPacket(sm);
					}
				} else
				{
					SystemMessage sm = new SystemMessage(
							SystemMessage.CANT_REGISTER_NO_ABILITY_TO_CRAFT);
					activeChar.sendPacket(sm);
				}
			} else
			{
				if (activeChar.hasCommonCraft())
				{
					if (rp.getLevel() > activeChar.getCommonCraft())
					{
						...
						activeChar.sendPacket(sm);
							...
							.getCommonRecipeBook().length >= activeChar
