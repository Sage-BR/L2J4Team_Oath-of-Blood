...
public class TownManager
{
	protected static Logger _log = Logger
			.getLogger(TownManager.class.getName());
	...
	{
		switch (MapRegionTable.getInstance().getMapRegion(
				activeObject.getPosition().getX(),
	...
	public final boolean townHasCastleInSeige(int townId)
	{
		...
		if (castleIndex > 0)
		{
			Castle castle = CastleManager.getInstance().getCastles().get(
					CastleManager.getInstance().getCastleIndex(castleIndex));
			if (castle != null)
	...
	public final boolean townHasCastleInSeige(int x, int y)
	{
		...
		int[] castleidarray =
			{ 0, 0, 0, 0, 0, 1, 0, 2, 3, 4, 5, 0, 0, 6, 8, 7, 9, 0 };
	...
	{
		return getTown(activeObject.getPosition().getX(),
				activeObject.getPosition().getY());
	}
	...
	public final int getTownIndex(int townId)
	{
		...
		{
			town = getTowns().get(i);
			if (town != null && town.getTownId() == townId)
				return i;
		}
		return -1;
	}

	public final int getTownIndex(L2Object activeObject)
	{
		return getTownIndex(activeObject.getPosition().getX(),
				activeObject.getPosition().getY());
	}
	...
	public final int getTownIndex(String name)
	{
		...
		{
			town = getTowns().get(i);
			if (town != null && town.getName().equalsIgnoreCase(name.trim()))
				return i;
		}
		return -1;
	}
