...
public class RequestRecipeBookOpen extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestRecipeBookOpen.class.getName());

	private final boolean isDwarvenCraft;

	/**
	 * packet type id 0xac packet format rev656 cd
	 *
	 * @param decrypt
	 */
	public RequestRecipeBookOpen(ByteBuffer buf, ClientThread client)
	{
		...
		if (Config.DEBUG)
			_log.info("RequestRecipeBookOpen : "
					+ (isDwarvenCraft ? "dwarvenCraft" : "commonCraft"));
	}
	...
	void runImpl()
	{
		...
		if (getClient().getActiveChar().getPrivateStoreType() != 0)
		{
			getClient().getActiveChar()
					.sendMessage("Cannot use recipe book while trading");
			return;
		}

		RecipeController.getInstance()
				.requestBookOpen(getClient().getActiveChar(), isDwarvenCraft);
	}
