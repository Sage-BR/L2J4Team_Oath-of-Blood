...
public class L2SiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2PcInstance)
		{
			...
			if (((L2PcInstance) target).isSilentMoving()
					&& !_actor.isInsideRadius(target, 100, false, false))
				return false;
		}
		// Los Check Here
			...
			Object arg1)
	{
		...
		if (aiTask == null)
		{
			aiTask = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(this,
					1000, 1000);
		}
	}
	...
	protected void onIntentionAttack(L2Character target)
	{
		...
		_attack_timeout = MAX_ATTACK_TIMEOUT
	...
	private void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			for (L2Character target : npc.getKnownList()
					.getKnownCharactersInRadius(attackRange))
			{
				if (target == null)
					continue;
			...
			if (_actor.isConfused())
				hated = _attack_target; // Force mobs to attak anybody if
										// confused
			else
	...
	private void AttackPrepare()
	{
		...
		try
		{
			...
			dist_2 = _actor.getPlanDistanceSq(_attack_target.getX(),
					_attack_target.getY());
		...
		if (_attack_target instanceof L2PcInstance
				&& sGuard.getCastle().getSiege().checkIsDefender(
						((L2PcInstance) _attack_target).getClan()))
		{
			// Cancel the target
		...
		if (!_actor.isMuted() && dist_2 > (range + 20) * (range + 20))
		{
			...
			} else
			{
				...
				if ((dx * dx + dy * dy > 10000)
						&& (homeX * homeX + homeY * homeY > 3240000) // 1800 *
																		// 1800
						&& (_actor.getKnownList().knowsObject(_attack_target)))
				{
					// Cancel the target
					...
					_actor.setTarget(null);
					setIntention(AI_INTENTION_IDLE, null, null);
				} else // Move the actor to Pawn server side AND client side by
						// sending Server->Client packet MoveToPawn (broadcast)
				{
					// Temporary hack for preventing guards jumping off towers,
					// before replacing this with geodata check
					if (dz * dz < 170 * 170) // normally 130 if guard z
												// coordinates correct
		...
		else if (dist_2 <= (range + 20) * (range + 20))
		{
			...
			if (!_actor.isMuted() && Rnd.nextInt(100) <= 5)
			{
				for (L2Skill sk : skills)
				{
					...
					if (castRange * castRange >= dist_2 && castRange <= 70
							&& !sk.isPassive()
							&& _actor.getCurrentMp() >= _actor.getStat()
									.getMpConsume(sk)
							&& !_actor.isSkillDisabled(sk.getId()))
	...
	private final void FactionNotify()
	{
		...
		if (((L2NpcInstance) _actor).getFactionId() == null
				|| _attack_target == null || _actor == null
				|| _attack_target.isInvul())
			return;
				...
				.getKnownCharactersInRadius(1000))
		{
			...
			if ((npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE
					...
					&& _actor.isInsideRadius(
							npc, npc.getFactionRange(), false, true)
					&& npc.getTarget() == null
					&& _attack_target.isInsideRadius(npc, npc.getFactionRange(),
							false, true)
					&& Math.abs(_attack_target.getZ() - npc.getZ()) < 600
			// && GeoData.getInstance().canSeeTarget(npc, _attack_target) // LOS
			// from balconies not working yet
			)
			{
	 ...
	 * <li>Set the L2Character movement type to run and send Server->Client
	 * packet ChangeMoveType to all others L2PcInstance</li>
	 ...
	 * present</li>
	 * <li>Set the actor Intention to AI_INTENTION_ATTACK (if actor is
	 * L2GuardInstance check if it isn't too far from its home
	 * location)</li><BR>
	 * <BR>
	 *
	 * @param attacker
