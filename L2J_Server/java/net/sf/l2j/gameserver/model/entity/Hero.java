...
public class Hero
{
	...
	private static final int[] _heroItems =
		{ 6842, 6611, 6612, 6613, 6614, 6615, 6616, 6617, 6618, 6619, 6620,
				6621 };
	...
	private void init()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (rset2.next())
				{
					...
					if (clanId > 0)
					{
						clanName = ClanTable.getInstance().getClan(clanId)
								.getName();
						clanCrest = ClanTable.getInstance().getClan(clanId)
								.getCrestId();

						if (allyId > 0)
						{
							allyName = ClanTable.getInstance().getClan(clanId)
									.getAllyName();
							allyCrest = ClanTable.getInstance().getClan(clanId)
									.getAllyCrestId();
						}
					}
		...
		_log.info("Hero System: Loaded " + _completeHeroes.size()
				+ " all time Heroes.");
	}
	...
	public synchronized void computeNewHeroes(List<StatsSet> newHeroes)
	{
		...
		if (_heroes.size() != 0)
		{
			for (StatsSet hero : _heroes.values())
			{
				...
				if (player != null)
				{
					...
					items = player.getInventory()
							.unEquipItemInBodySlotAndRecord(L2Item.SLOT_R_HAND);
					iu = new InventoryUpdate();
					for (L2ItemInstance item : items)
					{
						iu.addModifiedItem(item);
					...
					items = player.getInventory()
							.unEquipItemInBodySlotAndRecord(L2Item.SLOT_HAIR);
					iu = new InventoryUpdate();
					for (L2ItemInstance item : items)
					{
						iu.addModifiedItem(item);
					...
					items = player.getInventory()
							.unEquipItemInBodySlotAndRecord(L2Item.SLOT_FACE);
					iu = new InventoryUpdate();
					for (L2ItemInstance item : items)
					{
						iu.addModifiedItem(item);
					...
					items = player.getInventory()
							.unEquipItemInBodySlotAndRecord(L2Item.SLOT_DHAIR);
					iu = new InventoryUpdate();
					for (L2ItemInstance item : items)
					{
						iu.addModifiedItem(item);
					...
					for (L2ItemInstance item : player.getInventory()
							.getAvailableItems(false))
					{
						if ((item == null)
								|| !heroItems.contains(item.getItemId()))
							continue;
		...
		for (StatsSet hero : newHeroes)
		{
			...
			} else
			{
				...
				newHero.set(Olympiad.CHAR_NAME,
						hero.getString(Olympiad.CHAR_NAME));
				newHero.set(Olympiad.CLASS_ID,
						hero.getInteger(Olympiad.CLASS_ID));
				newHero.set(COUNT, 1);
			...
			} else
			{
				...
				for (Integer heroId : _heroes.keySet())
				{
					...
					if (_completeHeroes == null
							|| !_completeHeroes.containsKey(heroId))
					{
						...
						Connection con2 = L2DatabaseFactory.getInstance()
								.getConnection();
						...
						if (rset2.next())
						{
							...
							if (clanId > 0)
							{
								clanName = ClanTable.getInstance()
										.getClan(clanId).getName();
								clanCrest = ClanTable.getInstance()
										.getClan(clanId).getCrestId();

								if (allyId > 0)
								{
									allyName = ClanTable.getInstance()
											.getClan(clanId).getAllyName();
									allyCrest = ClanTable.getInstance()
											.getClan(clanId).getAllyCrestId();
								}
							}
