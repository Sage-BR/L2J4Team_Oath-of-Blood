...
public class L2ControllableMobAI extends L2AttackableAI
{
	...
	protected void thinkFollow()
	{
		...
		if (!me.isInsideRadius(getForcedTarget(), MobGroupTable.FOLLOW_RANGE,
				false, false))
		{
			...
			moveTo(getForcedTarget().getX() + signX * randX,
					getForcedTarget().getY() + signY * randY,
					getForcedTarget().getZ());
		}
	}
	...
	protected void thinkCast()
	{
		...
		if (!_actor.isMuted())
		{
			...
			{
				if (_actor.isInsideRadius(getAttackTarget(), sk.getCastRange(),
						false, true) // sk.getCastRange() * sk.getCastRange() >=
										// dist2
	...
	protected void thinkForceAttack()
	{
		...
		try
		{
			...
			skills = _actor.getAllSkills();
			dist2 = _actor.getPlanDistanceSq(getForcedTarget().getX(),
					getForcedTarget().getY());
			range = _actor.getPhysicalAttackRange();
			max_range = range;
		} catch (NullPointerException e)
		{
		...
		if (!_actor.isMuted() && dist2 > (range + 20) * (range + 20))
		{
			...
			if (!isNotMoving())
				moveToPawn(getForcedTarget(),
						_actor.getPhysicalAttackRange()/* range */);
		...
		} else
		{
			...
			if (((L2NpcInstance) _actor).getFactionId() != null)
			{
						...
						.values())
				{
					...
					if (_actor.isInsideRadius(npc, npc.getFactionRange(), false,
							true)
