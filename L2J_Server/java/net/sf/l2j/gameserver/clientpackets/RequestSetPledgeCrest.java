...
public class RequestSetPledgeCrest extends ClientBasePacket
{
	...
	static Logger _log = Logger
			.getLogger(RequestSetPledgeCrest.class.getName());
	...
	void runImpl()
	{
		...
		if (clan.getDissolvingExpiryTime() > System.currentTimeMillis())
		{
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.CANNOT_SET_CREST_WHILE_DISSOLUTION_IN_PROGRESS));
			return;
		}
		...
		if (_length == 0 || _data.length == 0)
		{
			...
			activeChar.sendPacket(new SystemMessage(
					SystemMessage.CLAN_CREST_HAS_BEEN_DELETED));
		...
		} else if (_data.length > 256)
		{
			activeChar.sendMessage(
					"The clan crest file size is greater than 256 bytes.");
			return;
		}
				...
				& L2Clan.CP_CL_REGISTER_CREST) == L2Clan.CP_CL_REGISTER_CREST)
		{
			...
			{
				activeChar.sendPacket(new SystemMessage(
						SystemMessage.CLAN_LVL_3_NEEDED_TO_SET_CREST));
				return;
			}
			...
			{
				_log.log(Level.INFO,
						"Error loading crest of clan:" + clan.getName());
				return;
			}
			...
			try
			{
				...
				PreparedStatement statement = con.prepareStatement(
						"UPDATE clan_data SET crest_id = ? WHERE clan_id = ?");
