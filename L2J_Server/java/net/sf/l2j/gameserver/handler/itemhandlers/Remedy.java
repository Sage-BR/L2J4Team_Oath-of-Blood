...
public class Remedy implements IItemHandler
{
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		if (itemId == 1831) // antidote
		{
			...
			MagicSkillUser MSU = new MagicSkillUser(playable, playable, 2042, 1,
					0, 0);
		...
		} else if (itemId == 1832) // advanced antidote
		{
			...
			MagicSkillUser MSU = new MagicSkillUser(playable, playable, 2043, 1,
					0, 0);
			activeChar.sendPacket(MSU);
			activeChar.broadcastPacket(MSU);
			playable.destroyItem("Consume", item.getObjectId(), 1, null, false);
		} else if (itemId == 1833) // bandage
		{
			...
			MagicSkillUser MSU = new MagicSkillUser(playable, playable, 34, 1,
					0, 0);
			activeChar.sendPacket(MSU);
			activeChar.broadcastPacket(MSU);
			playable.destroyItem("Consume", item.getObjectId(), 1, null, false);
		} else if (itemId == 1834) // emergency dressing
		{
			...
			MagicSkillUser MSU = new MagicSkillUser(playable, playable, 2045, 1,
					0, 0);
			activeChar.sendPacket(MSU);
			activeChar.broadcastPacket(MSU);
			playable.destroyItem("Consume", item.getObjectId(), 1, null, false);
		} else if (itemId == 3889) // potion of recovery
		{
			...
			if (activeChar.getEffect(L2Effect.EffectType.ROOT) == null)
