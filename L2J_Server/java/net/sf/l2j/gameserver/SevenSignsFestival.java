 ...
 *
 * TODO: - Archer mobs should target healer characters over other party members.
 * - ADDED 29 Sep: Players that leave a party during the Seven Signs Festival
 * will now take damage and cannot be healed.
 *
 * @author Tempy
 */
public class SevenSignsFestival implements SpawnListener
{
	protected static Logger _log = Logger
			.getLogger(SevenSignsFestival.class.getName());
	 ...
	 * ELAPSED time (in ms) since a festival begins.
	 *
	 * Festival manager start is the time after the server starts to begin the
	 * first festival cycle.
	 *
	 * The cycle length should ideally be at least 2x longer than the festival
	 * length. This allows ample time for players to sign-up to participate in
	 * the festival.
	 *
	 * The intermission is the time between the festival participants being
	 * moved to the "arenas" and the spawning of the first set of mobs.
	 *
	 * The monster swarm time is the time before the monsters swarm to the
	 * center of the arena, after they are spawned.
	 *
	...
	public static final long FESTIVAL_SIGNUP_TIME = Config.ALT_FESTIVAL_CYCLE_LENGTH
			- Config.ALT_FESTIVAL_LENGTH - 60000;
	...
	private static final int FESTIVAL_DEFAULT_RESPAWN = 60; // Specify in
															// seconds!
	...
	public static final int[] FESTIVAL_LEVEL_SCORES =
		{ 60, 70, 100, 120, 150 }; // 500 maximum possible score
	...
	//////////////////////// \\\\\\\\\\\\\\\\\\\\\\\\\\
	/*
	 * The following contains all the necessary spawn data for: - Player Start
	 * Locations - Witches - Monsters - Chests
	 *
	...
	protected List<Integer> _accumulatedBonuses; // The total bonus available
													// (in Ancient Adena)
	...
	/**
	 * _festivalData is essentially an instance of the seven_signs_festival
	 * table and should be treated as such.
	 *
	 * Data is initially accessed by the related Seven Signs cycle, with
	 * _signsCycle representing data for the current round of Festivals.
	 *
	 * The actual table data is stored as a series of StatsSet constructs. These
	 * are accessed by the use of an offset based on the number of festivals,
	 * thus:
	 *
	 * offset = FESTIVAL_COUNT + festivalId (Data for Dawn is always accessed by
	 * offset > FESTIVAL_COUNT)
	 */
	...
	protected static final boolean isFestivalArcher(int npcId)
	{
		...
		return (npcIdStr.substring(4).equals("4")
				|| npcIdStr.substring(4).equals("9"));
	}
	...
	{
		// Start the Festival Manager for the first time after the server has
		// started
		...
		_log.info(
				"SevenSignsFestival: The first Festival of Darkness cycle begins in "
						+ (Config.ALT_FESTIVAL_MANAGER_START / 60000)
						+ " minute(s).");
	}
	...
	protected void restoreFestivalData()
	{
		...
		if (Config.DEBUG)
			_log.info(
					"SevenSignsFestival: Restoring festival data. Current SS Cycle: "
							+ _signsCycle);
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (Config.DEBUG)
					_log.info(
							"SevenSignsFestival: Loaded data from DB for (Cycle = "
		...
		{
			_log.severe(
					"SevenSignsFestival: Failed to load configuration: " + e);
		} finally
		{
			try
			{
	 ...
	 * past high score data to the database.
	 *
	...
	public void saveFestivalData(boolean updateSettings)
	{
		...
		if (Config.DEBUG)
			System.out.println(
					"SevenSignsFestival: Saving festival data to disk.");
			...
			for (Map<Integer, StatsSet> currCycleData : _festivalData.values())
			{
				for (StatsSet festivalDat : currCycleData.values())
				{
					...
					if (statement.executeUpdate() > 0)
					{
						if (Config.DEBUG)
							_log.info(
									"SevenSignsFestival: Updated data in DB (Cycle = "
											+ festivalCycle + ", Cabal = "
											+ cabal + ", FestID = " + festivalId
											+ ")");
					...
					if (Config.DEBUG)
						_log.info(
								"SevenSignsFestival: Inserted data in DB (Cycle = "
		...
		{
			_log.severe(
					"SevenSignsFestival: Failed to save configuration: " + e);
		} finally
		{
			try
			{
	...
	protected void resetFestivalData(boolean updateSettings)
	{
		...
		for (int i = 0; i < FESTIVAL_COUNT * 2; i++)
		{
			...
			if (i >= FESTIVAL_COUNT)
				tempStats.set("cabal",
						SevenSigns.getCabalShortName(SevenSigns.CABAL_DAWN));
			else
				tempStats.set("cabal",
						SevenSigns.getCabalShortName(SevenSigns.CABAL_DUSK));
		...
		{
			L2ItemInstance bloodOfferings = onlinePlayer.getInventory()
					.getItemByItemId(FESTIVAL_OFFERING_ID);

			if (bloodOfferings != null)
				onlinePlayer.destroyItem("SevenSigns", bloodOfferings, null,
						false);
		}
	...
	{
		_nextFestivalCycleStart = System.currentTimeMillis()
				+ Config.ALT_FESTIVAL_CYCLE_LENGTH;
	}
	...
	public final int getMinsToNextCycle()
	{
		...
		return Math.round(
				(_nextFestivalCycleStart - System.currentTimeMillis()) / 60000);
	}
	...
	public final int getMinsToNextFestival()
	{
		...
		return Math.round(
				(_nextFestivalStart - System.currentTimeMillis()) / 60000) + 1;
	}
	...
	public final String getTimeToNextFestivalStr()
	{
		...
		return "<font color=\"FF0000\">The next festival will begin in "
				+ getMinsToNextFestival() + " minute(s).</font>";
	}
	...
	public final int[] getFestivalForPlayer(L2PcInstance player)
	{
		...
		{
			List<L2PcInstance> participants = _dawnFestivalParticipants
					.get(festivalId);

			// If there are no participants in this festival, move on to the
			// next.
	...
	public final boolean isParticipant(L2PcInstance player)
	{
		...
		for (List<L2PcInstance> participants : _dawnFestivalParticipants
				.values())
			if (participants.contains(player))
				return true;

		for (List<L2PcInstance> participants : _duskFestivalParticipants
				.values())
			if (participants.contains(player))
				return true;
	...
	public final List<L2PcInstance> getPreviousParticipants(int oracle,
			int festivalId)
	{
		if (oracle == SevenSigns.CABAL_DAWN)
	...
	public void setParticipants(int oracle, int festivalId,
			L2Party festivalParty)
	{
		...
		if (festivalParty != null)
		{
			...
			if (Config.DEBUG)
				_log.info("SevenSignsFestival: "
						+ festivalParty.getPartyMembers().toString()
						+ " have signed up to the "
						+ SevenSigns.getCabalShortName(oracle) + " "
						+ getFestivalName(festivalId) + " festival.");
		}
	...
	public void updateParticipants(L2PcInstance player, L2Party festivalParty)
	{
		...
		if (festivalId > -1)
		{
			...
			{
				L2DarknessFestival festivalInst = _managerInstance
						.getFestivalInstance(oracle, festivalId);

				if (festivalParty == null)
					for (L2PcInstance partyMember : getParticipants(oracle,
							festivalId))
	...
	public final StatsSet getHighestScoreData(int oracle, int festivalId)
	{
		...
		// Attempt to retrieve existing score data (if found), otherwise create
		// a
		...
		} catch (Exception e)
		{
			...
			if (Config.DEBUG)
				_log.info("SevenSignsFestival: Data missing for "
						+ SevenSigns.getCabalName(oracle) + ", FestivalID = "
						+ festivalId + " (Current Cycle " + _signsCycle + ")");
		}
	 ...
	 */
	public boolean setFinalScore(L2PcInstance player, int oracle,
			int festivalId, int offeringScore)
	{
		List<String> partyMembers;

		int currDawnHighScore = getHighestScore(SevenSigns.CABAL_DAWN,
				festivalId);
		int currDuskHighScore = getHighestScore(SevenSigns.CABAL_DUSK,
				festivalId);
		...
		// Check if this is the highest score for this level range so far for
		// the player's cabal.
		if (offeringScore > thisCabalHighScore)
		{
			...
			List<L2PcInstance> prevParticipants = getPreviousParticipants(
					oracle, festivalId);
			...
			currFestData.set("date",
					String.valueOf(System.currentTimeMillis()));
			currFestData.set("score", offeringScore);
			currFestData.set("members", Util.implodeString(partyMembers, ","));

			if (Config.DEBUG)
				_log.info("SevenSignsFestival: " + player.getName()
						+ "'s party has the highest score (" + offeringScore
						+ ") so far for " + SevenSigns.getCabalName(oracle)
						+ " in " + getFestivalName(festivalId));

			// Only add the score to the cabal's overall if it's higher than the
			// other cabal's score.
			if (offeringScore > otherCabalHighScore)
			{
				int contribPoints = FESTIVAL_LEVEL_SCORES[festivalId];

				// Give this cabal the festival points, while deducting them
				// from the other.
				SevenSigns.getInstance().addFestivalScore(oracle,
						contribPoints);

				// if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: This is the highest score overall so far for the "
								+ getFestivalName(festivalId) + " festival!");
			}
	...
	public void addAccumulatedBonus(int festivalId, int stoneType,
			int stoneAmount)
	{
		...
		int newTotalBonus = _accumulatedBonuses.get(festivalId)
				+ (stoneAmount * eachStoneBonus);
	...
	public void sendMessageToAll(String senderName, String message)
	{
		...
		CreatureSay cs = new CreatureSay(_dawnChatGuide.getObjectId(), 1,
				senderName, message);
		_dawnChatGuide.broadcastPacket(cs);

		cs = new CreatureSay(_duskChatGuide.getObjectId(), 1, senderName,
				message);
	...
	/**
	 * Used with the SpawnListener, to update the required "chat guide"
	 * instances, for use with announcements in the oracles.
	 *
	 * @param npc
	 */
	@Override
	public void npcSpawned(L2NpcInstance npc)
	{
		...
		// If the spawned NPC ID matches the ones we need, assign their
		// instances.
		if (npcId == 31127)
		{
			if (Config.DEBUG)
				_log.config(
						"SevenSignsFestival: Instance found for NPC ID 31127 ("
								+ npc.getObjectId() + ").");
		...
		if (npcId == 31137)
		{
			if (Config.DEBUG)
				_log.config(
						"SevenSignsFestival: Instance found for NPC ID 31137 ("
								+ npc.getObjectId() + ").");
	...
	private class FestivalManager implements Runnable
	{
		...
		public FestivalManager()
		{
			...
			setNextCycleStart();
			setNextFestivalStart(
					Config.ALT_FESTIVAL_CYCLE_LENGTH - FESTIVAL_SIGNUP_TIME);
		}
		...
		public synchronized void run()
		{
			...
			if (SevenSigns.getInstance().isSealValidationPeriod() || (SevenSigns
					.getInstance()
					.getMilliToPeriodChange() < Config.ALT_FESTIVAL_CYCLE_LENGTH))
				return;

			if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: Festival manager initialized. Those wishing to participate have "
								+ getMinsToNextFestival()
								+ " minute(s) to sign up.");

			sendMessageToAll("Festival Guide",
					"The main event will start in " + getMinsToNextFestival()
							+ " minutes. Please register now.");
			...
			// Clear past participants, they can no longer register their score
			// if not done so already.
			_dawnPreviousParticipants.clear();
			_duskPreviousParticipants.clear();

			// Get rid of random monsters that avoided deletion after last
			// festival
			...
			for (int i = 0; i < FESTIVAL_COUNT; i++)
			{
				if (_duskFestivalParticipants.get(i) != null)
					_festivalInstances.put(10 + i,
							new L2DarknessFestival(SevenSigns.CABAL_DUSK, i));

				if (_dawnFestivalParticipants.get(i) != null)
					_festivalInstances.put(20 + i,
							new L2DarknessFestival(SevenSigns.CABAL_DAWN, i));
			}
			...
			sendMessageToAll("Festival Guide",
					"The main event is now starting.");

			if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: The current set of festivals will begin in "
								+ (Config.ALT_FESTIVAL_FIRST_SPAWN / 60000)
								+ " minute(s).");
			...
			/* PROPOGATION */
			// Sequentially set all festivals to begin, spawn the Festival Witch
			// and notify participants.
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
			{
				festivalInst.festivalStart();
				festivalInst.sendMessageToParticipants(
						"The festival is about to begin!");
			}

			if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: Each of the festivals will end in "
								+ (Config.ALT_FESTIVAL_LENGTH / 60000)
								+ " minutes. New participants can signup then.");

			// After a short time period, move all idle spawns to the center of
			// the arena.
			try
			{
				wait(Config.ALT_FESTIVAL_FIRST_SWARM
						- Config.ALT_FESTIVAL_FIRST_SPAWN);
			} catch (InterruptedException e)
			{
			}

			elapsedTime += Config.ALT_FESTIVAL_FIRST_SWARM
					- Config.ALT_FESTIVAL_FIRST_SPAWN;
			...
			try
			{
				wait(Config.ALT_FESTIVAL_SECOND_SPAWN
						- Config.ALT_FESTIVAL_FIRST_SWARM);
			} catch (InterruptedException e)
			{
			}

			// Spawn an extra set of monsters (archers) on the free platforms
			// with
			// a faster respawn when killed.
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
			{
				festivalInst.spawnFestivalMonsters(FESTIVAL_DEFAULT_RESPAWN / 2,
						2);
				festivalInst
						.sendMessageToParticipants("The festival will end in "
								+ ((Config.ALT_FESTIVAL_LENGTH
										- Config.ALT_FESTIVAL_SECOND_SPAWN)
										/ 60000)
								+ " minute(s).");
			}

			elapsedTime += Config.ALT_FESTIVAL_SECOND_SPAWN
					- Config.ALT_FESTIVAL_FIRST_SWARM;

			// After another short time period, again move all idle spawns to
			// the center of the arena.
			try
			{
				wait(Config.ALT_FESTIVAL_SECOND_SWARM
						- Config.ALT_FESTIVAL_SECOND_SPAWN);
			} catch (InterruptedException e)
			{
			}
			...
			elapsedTime += Config.ALT_FESTIVAL_SECOND_SWARM
					- Config.ALT_FESTIVAL_SECOND_SPAWN;

			// Stand by until the time comes for the chests to be spawned.
			try
			{
				wait(Config.ALT_FESTIVAL_CHEST_SPAWN
						- Config.ALT_FESTIVAL_SECOND_SWARM);
			} catch (InterruptedException e)
			{
			}

			// Spawn the festival chests, which enable the team to gain greater
			// rewards
			...
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants(
						"The chests have spawned! Be quick, the festival will end soon.");
			}

			elapsedTime += Config.ALT_FESTIVAL_CHEST_SPAWN
					- Config.ALT_FESTIVAL_SECOND_SWARM;
			...
			// Participants can no longer opt to increase the challenge, as the
			// festival will soon close.
			...
			sendMessageToAll("Festival Witch",
					"That will do! I'll move you to the outside soon.");

			if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: The next set of festivals begin in "
								+ getMinsToNextFestival() + " minute(s).");
		}
		 ...
		 */
		public final L2DarknessFestival getFestivalInstance(int oracle,
				int festivalId)
		{
			 ...
			 *
			 * ID: 0 1 2 3 4 Dusk 1: 10 11 12 13 14 Dawn 2: 20 21 22 23 24
			 */
		...
		/**
		 * Returns the number of currently running festivals <b>WITH</b>
		 * participants.
		 *
		 * @return int Count
		 */
	 ...
	 * contains all the spawn information and data for the running festival.
	 *
	 * All festivals are managed by the FestivalManager class, which must be
	 * initialized first.
	 *
	 * @author Tempy
	 */
	private class L2DarknessFestival
	{
		...
		protected L2DarknessFestival(int cabal, int levelRange)
		{
			...
			if (cabal == SevenSigns.CABAL_DAWN)
			{
				...
				_witchSpawn = new FestivalSpawn(
						festivalDawnWitchSpawns[levelRange]);
				_startLocation = new FestivalSpawn(
						festivalDawnPlayerSpawns[levelRange]);
			} else
			{
				_participants = _duskFestivalParticipants.get(levelRange);
				_witchSpawn = new FestivalSpawn(
						festivalDuskWitchSpawns[levelRange]);
				_startLocation = new FestivalSpawn(
						festivalDuskPlayerSpawns[levelRange]);
			}
		...
		protected void festivalInit()
		{
			...
			if (Config.DEBUG)
				_log.info("SevenSignsFestival: Initializing festival for "
						+ SevenSigns.getCabalShortName(_cabal) + " ("
			...
			if (_participants.size() > 0)
			{
				...
				{
					_originalLocations.put(participant,
							new FestivalSpawn(participant.getX(),
									participant.getY(), participant.getZ(),
									participant.getHeading()));

					// Randomize the spawn point around the specific centerpoint
					// for each player.
					...
					// Remove all buffs from all participants on entry. Works
					// like the skill Cancel.
					for (L2Effect e : participant.getAllEffects())
						e.exit();

					// Remove any stray blood offerings in inventory
					L2ItemInstance bloodOfferings = participant.getInventory()
							.getItemByItemId(FESTIVAL_OFFERING_ID);
					if (bloodOfferings != null)
						participant.destroyItem("SevenSigns", bloodOfferings,
								null, true);
				}
			}

			L2NpcTemplate witchTemplate = NpcTable.getInstance()
					.getTemplate(_witchSpawn._npcId);
			...
			try
			{
				...
				// Needed as doSpawn() is required to be called also for the
				...
				if (Config.DEBUG)
					_log.fine("SevenSignsFestival: Spawned the Festival Witch "
							+ npcSpawn.getNpcid() + " at " + _witchSpawn._x
							+ " " + _witchSpawn._y + " " + _witchSpawn._z);
			} catch (Exception e)
			{
				_log.warning(
						"SevenSignsFestival: Error while spawning Festival Witch ID "
								+ _witchSpawn._npcId + ": " + e);
			}

			// Make it appear as though the Witch has apparated there.
			MagicSkillUser msu = new MagicSkillUser(_witchInst, _witchInst,
					2003, 1, 1, 0);
		...
		{
			if (Config.DEBUG)
				_log.info("SevenSignsFestival: Starting festival for "
						+ SevenSigns.getCabalShortName(_cabal) + " ("
						+ getFestivalName(_levelRange) + ")");
		...
		protected void moveMonstersToCenter()
		{
			...
			if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: Moving spawns to arena center for festival "
								+ SevenSigns.getCabalShortName(_cabal) + " ("
								+ getFestivalName(_levelRange) + ")");
			...
			for (L2FestivalMonsterInstance festivalMob : _npcInsts)
			{
				...
				// Only move monsters that are idle or doing their usual
				// functions.
				CtrlIntention currIntention = festivalMob.getAI()
						.getIntention();

				if (currIntention != CtrlIntention.AI_INTENTION_IDLE
						&& currIntention != CtrlIntention.AI_INTENTION_ACTIVE)
					continue;
				...
				/*
				 * Random X and Y coords around the player start location, up to
				 * half of the maximum allowed offset are generated to prevent
				 * the mobs from all moving to the exact same place.
				 */
				isPositive = (rnd.nextInt(2) == 1);
				...
				L2CharPosition moveTo = new L2CharPosition(x, y,
						_startLocation._z, rnd.nextInt(65536));

				festivalMob.setRunning();
				festivalMob.getAI().setIntention(
						CtrlIntention.AI_INTENTION_MOVE_TO, moveTo);
			}
		}
		...
		public void setSpawnRate(int respawnDelay)
		{
			if (Config.DEBUG)
				_log.info(
						"SevenSignsFestival: Modifying spawn rate of festival mobs to "
								+ respawnDelay + " ms for festival "
								+ SevenSigns.getCabalShortName(_cabal) + " ("
								+ getFestivalName(_levelRange) + ")");
		...
		protected void spawnFestivalMonsters(int respawnDelay, int spawnType)
		{
			...
			case 0:
			case 1:
				_npcSpawns = (_cabal == SevenSigns.CABAL_DAWN)
						? festivalDawnPrimarySpawns[_levelRange]
						: festivalDuskPrimarySpawns[_levelRange];
				break;
			case 2:
				_npcSpawns = (_cabal == SevenSigns.CABAL_DAWN)
						? festivalDawnSecondarySpawns[_levelRange]
						: festivalDuskSecondarySpawns[_levelRange];
				break;
			case 3:
				_npcSpawns = (_cabal == SevenSigns.CABAL_DAWN)
						? festivalDawnChestSpawns[_levelRange]
						: festivalDuskChestSpawns[_levelRange];
				break;
			}
			...
			for (int[] _npcSpawn : _npcSpawns)
			{
				...
				L2NpcTemplate npcTemplate = NpcTable.getInstance()
						.getTemplate(currSpawn._npcId);
				...
				try
				{
					...
					// if this spawn is part of an increased challenge or is a
					// festival chest.
					if (spawnType == 1)
					...
					if (Config.DEBUG)
						_log.fine("SevenSignsFestival: Spawned NPC ID "
								+ currSpawn._npcId + " at " + currSpawn._x + " "
								+ currSpawn._y + " " + currSpawn._z);
				} catch (Exception e)
				{
					_log.warning(
							"SevenSignsFestival: Error while spawning NPC ID "
									+ currSpawn._npcId + ": " + e);
				}
			}
		}
		...
		protected boolean increaseChallenge()
		{
			...
			// Set this flag to true to make sure that this can only be done
			// once.
			_challengeIncreased = true;

			if (Config.DEBUG)
				_log.info("SevenSignsFestival: "
						+ _participants.get(0).getName()
		...
		public void sendMessageToParticipants(String message)
		{
			if (_participants.size() > 0)
			{
				CreatureSay cs = new CreatureSay(_witchInst.getObjectId(), 0,
						"Festival Witch", message);
		...
		{
			if (Config.DEBUG)
				_log.info("SevenSignsFestival: Ending festival for "
						+ SevenSigns.getCabalShortName(_cabal) + " ("
						+ getFestivalName(_levelRange) + ")");
			...
			if (_participants.size() > 0)
			{
				for (L2PcInstance participant : _participants)
				{
					...
					participant.sendMessage(
							"The festival has ended. Your party leader must now register your score before the next festival takes place.");
				}
		...
		public void relocatePlayer(L2PcInstance participant, boolean isRemoving)
		{
			try
			{
				FestivalSpawn origPosition = _originalLocations
						.get(participant);
				...
				participant.getAI()
						.setIntention(CtrlIntention.AI_INTENTION_IDLE);
				participant.teleToLocation(origPosition._x, origPosition._y,
						origPosition._z, true);
			} catch (Exception e)
			{
				// If an exception occurs, just move the player to the nearest
				// town.
				participant
			...
			participant.sendMessage(
					"You have been removed from the festival arena.");
		}
	}
