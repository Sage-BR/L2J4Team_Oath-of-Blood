...
public class Seed implements IItemHandler
{
	private static int[] _itemIds =
		{ // generic seeds (lo-grade)
				...
				5670, 5671, 5672, 5673, 5674, 5675, 5676, 5678, 5678, 5679,
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		if (_target != null && !_target.isDead() && !_target.isSeeded())
		{
			...
			if (areaValid(
					MapRegionTable.getInstance().getClosestTownNumber(_char)))
			{
				// TODO: valid action animId for seed planting
				MagicSkillUser MSU = new MagicSkillUser(_char, _target, 2023, 1,
						2, 2);
				...
				if (calcSuccess())
				{
					...
					_char.sendPacket(new SystemMessage(
							SystemMessage.SEED_SUCCESSFULLY_SOWN));
				} else
				{
					_char.sendPacket(
							new SystemMessage(SystemMessage.SEED_NOT_SOWN));
				}

				// remove seed from inv & attack target
				_char.destroyItem("Consume", item.getObjectId(), 1, null,
						false);
				_target.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED, _char);
				_char.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK,
						_target);
			} else
			{
				_char.sendPacket(new SystemMessage(
						SystemMessage.SEED_CANNOT_BE_SOWN_HERE));
			}
		}
	}
	...
	private boolean areaValid(int area)
	{
		...
		{
		case 5: // gludio
			return ((_seedId >= 5016 && _seedId <= 5023)
					|| (_seedId >= 5650 && _seedId <= 5657));
		case 7: // dion
			return ((_seedId >= 5024 && _seedId <= 5032)
					|| (_seedId >= 5658 && _seedId <= 5666));
		case 8: // giran
			return ((_seedId >= 5033 && _seedId <= 5041)
					|| (_seedId >= 5667 && _seedId <= 5675));
		case 9: // oren
			return ((_seedId >= 5042 && _seedId <= 5052)
					|| (_seedId >= 5676 && _seedId <= 5686));
		case 10: // aden
			return ((_seedId >= 5053 && _seedId <= 5061)
					|| (_seedId >= 5687 && _seedId <= 5695));
		case 13: // heine
			return ((_seedId >= 5221 && _seedId <= 5227)
					|| (_seedId >= 5696 && _seedId <= 5702));
		default:
			return false;
		}
