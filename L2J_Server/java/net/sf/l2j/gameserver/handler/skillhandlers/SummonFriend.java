...
public class SummonFriend implements ISkillHandler
{
	...
	// Logger.getLogger(SummonFriend.class.getName());
	...
	L2Skill skill, L2Object[] targets)
	{
		...
		L2Object[] objects = L2World.getInstance().getVisibleObjects(activeChar,
				5000);
		...
		if (objects != null)
		{
			for (L2Object object : objects)
			{
				...
				{
					((L2PcInstance) activeChar).sendPacket(new SystemMessage(
							SystemMessage.YOU_MAY_NOT_SUMMON_FROM_YOUR_CURRENT_LOCATION));
					return;
				}
			}
		}
		...
		if (ZoneManager.getInstance().checkIfInZonePvP(activeChar))
		{
			if (activeChar instanceof L2PcInstance)
				((L2PcInstance) activeChar).sendPacket(new SystemMessage(
						SystemMessage.YOU_CANNOT_SUMMON_IN_COMBAT));
			return;
		}
				...
				if (target instanceof L2PcInstance)
				{
					...
					if (targetChar.isAlikeDead())
					{
						SystemMessage sm = new SystemMessage(
								SystemMessage.S1_IS_DEAD_AT_THE_MOMENT_AND_CANNOT_BE_SUMMONED);
					...
					{
						SystemMessage sm = new SystemMessage(
								SystemMessage.S1_CURRENTLY_TRADING_OR_OPERATING_PRIVATE_STORE_AND_CANNOT_BE_SUMMONED);
						sm.addString(targetChar.getName());
						activeChar.sendPacket(sm);
						continue;
					}
					...
					{
						SystemMessage sm = new SystemMessage(
								SystemMessage.S1_IS_ENGAGED_IN_COMBAT_AND_CANNOT_BE_SUMMONED);
						sm.addString(targetChar.getName());
						activeChar.sendPacket(sm);
						continue;
					}
					...
					if (targetChar.isInOlympiadMode())
					{
						activeChar.sendPacket(new SystemMessage(
								SystemMessage.YOU_CANNOT_SUMMON_PLAYERS_WHO_ARE_IN_OLYMPIAD));
						continue;
					}
					...
					// zones
					if (targetChar.isFestivalParticipant() || ZoneManager
							.getInstance().checkIfInZonePvP(targetChar))
					{
						activeChar.sendPacket(new SystemMessage(
								SystemMessage.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING));
						continue;
					}
					...
					{
						((L2PcInstance) activeChar).sendMessage(
								"Your target cannot be summoned while he hasn't got a Summoning Crystal");
						targetChar.sendMessage(
								"You cannot summoned while you haven't got a Summoning Crystal");
						continue;
					}
					...
					if (!Util.checkIfInRange(0, activeChar, target, false))
					{
						targetChar.getInventory().destroyItemByItemId("Consume",
								8615, 1, targetChar, activeChar);
						targetChar.sendPacket(SystemMessage.sendString(
								"You are summoned to a party member."));

						targetChar.teleToLocation(activeChar.getX(),
								activeChar.getY(), activeChar.getZ(), true);
					} else
					{
