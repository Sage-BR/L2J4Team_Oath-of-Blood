...
public abstract class Quest
{
	...
	{
		QUEST_START, QUEST_TALK, MOBGOTATTACKED, MOBKILLED
	}
	...
	public final boolean notifyDeath(L2NpcInstance npc, L2Character character,
			QuestState qs)
	{
		String res = null;
		try
	...
	@SuppressWarnings("unused")
	public String onDeath(L2NpcInstance npc, L2Character character,
			QuestState qs)
	{
		return onEvent("", qs);
	}
	 ...
	 * <LI><U>otherwise :</U> the message hold in "res" is shown in chat
	 * box</LI>
	 *
	 * @param qs
	 *            : QuestState
	...
	public static void playerEnter(L2PcInstance player)
	{
		...
		try
		{
			...
			PreparedStatement invalidQuestData = con.prepareStatement(
					"DELETE FROM character_quests WHERE char_id=? and name=?");
			PreparedStatement invalidQuestDataVar = con.prepareStatement(
					"delete FROM character_quests WHERE char_id=? and name=? and var=?");

			statement = con.prepareStatement(
					"SELECT name,value FROM character_quests WHERE char_id=? AND var=?");
			...
			while (rs.next())
			{
				...
				if (q == null)
				{
					_log.finer("Unknown quest " + questId + " for player "
							+ player.getName());
				...
				if (state == null)
				{
					_log.finer("Unknown state " + state + " in quest " + questId
			...
			statement = con.prepareStatement(
					"SELECT name,var,value FROM character_quests WHERE char_id=?");
			statement.setInt(1, player.getObjectId());
			...
			while (rs.next())
			{
				...
				if (qs == null)
				{
					_log.finer("Lost variable " + var + " in quest " + questId
							+ " for player " + player.getName());
					if (Config.AUTODELETE_INVALID_QUEST_DATA)
					{
	 ...
	 */
	public static void createQuestVarInDb(QuestState qs, String var,
			String value)
	{
		java.sql.Connection con = null;
		try
	 ...
	 */
	public static void updateQuestVarInDb(QuestState qs, String var,
			String value)
	{
		java.sql.Connection con = null;
		try
	 ...
	 * </UL>
	 * </LI>
	 * <LI>var : string "&lt;state&gt;" as the name of the variable for the
	 * quest</LI>
	 * <LI>val : string corresponding at the ID of the state (in fact, initial
	 * state)</LI>
	 *
	 * @param qs
	 *            : QuestState
	 ...
	 * state</LI>
	 * <LI>Save in database the ID state (with or without the star) for the
	 * variable called "&lt;state&gt;" of the quest</LI>
	 *
	 * @param qs
	 *            : QuestState
	 */
	...
	{
		NpcTable.getInstance().getTemplate(attackId)
				.addQuestEvent(Quest.QuestEventType.MOBGOTATTACKED, this);
		return attackId;
	}
	...
	{
		NpcTable.getInstance().getTemplate(killId)
				.addQuestEvent(Quest.QuestEventType.MOBKILLED, this);
		return killId;
	}
	...
	{
		NpcTable.getInstance().getTemplate(talkId)
				.addQuestEvent(Quest.QuestEventType.QUEST_TALK, this);
		return talkId;
	}
	...
	public L2PcInstance getRandomPartyMember(L2PcInstance player)
	{
		...
		if ((player.getParty() == null)
				|| (player.getParty().getPartyMembers().size() == 0))
			return player;
		L2Party party = player.getParty();
		return party.getPartyMembers()
				.get(Rnd.get(party.getPartyMembers().size()));
	}
	 ...
	 *            the instance of a player whose party is to be searched
	 * @param var/value:
	 *            a tuple specifying a quest condition that must be satisfied
	 ...
	 *         applied).
	 */
	public L2PcInstance getRandomPartyMember(L2PcInstance player, String var,
			String value)
	{
	 ...
	 *         applied).
	 */
	public L2PcInstance getRandomPartyMemberState(L2PcInstance player,
			State state)
	{
		// if no valid player instance is passed, there is nothing to check...
		if (player == null)
	...
	public String showHtmlFile(L2PcInstance player, String fileName)
	{
		...
		String content = HtmCache.getInstance().getHtm(
				"data/jscript/" + directory + "/" + questId + "/" + fileName);

		if (content == null)
			content = HtmCache.getInstance().getHtmForce(
					"data/jscript/quests/" + questId + "/" + fileName);

		if (player != null && player.getTarget() != null)
			content = content.replaceAll("%objectId%",
					String.valueOf(player.getTarget().getObjectId()));
