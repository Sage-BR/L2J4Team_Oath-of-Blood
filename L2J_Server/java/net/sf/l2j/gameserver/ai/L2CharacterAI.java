...
public class L2CharacterAI extends AbstractAI
{
	 ...
	 * Server->Client packet ChangeWaitType (broadcast)</li><BR>
	 * <BR>
	 *
	 */
	@Override
	protected void onIntentionIdle()
	{
		...
		// Stand up the actor server side AND client side by sending
		// Server->Client packet ChangeWaitType (broadcast)
		clientStandUp();
	 ...
	 * new Attack and Launch Think Event.<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	...
	protected void onIntentionAttack(L2Character target)
	{
		if ((target == null) || (getIntention() == AI_INTENTION_REST)
				|| _actor.isAllSkillsDisabled() || _actor.isAffraid())
		{
	 ...
	 * cast and Launch Think Event.<BR>
	 * <BR>
	 *
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	 ...
	 * ActionFailed to the L2PcInstance actor</li>
	...
	protected void onIntentionCast(L2Skill skill, L2Object target)
	{
		...
		/*
		 * if (Config.DEBUG) _log.warning("L2CharacterAI: onIntentionCast -> " +
		 * skill + " " + target);
		 */

		if ((getIntention() == AI_INTENTION_REST && skill.isMagic())
				|| _actor.isAllSkillsDisabled())
		{
			// Cancel action client side by sending Server->Client packet
			// ActionFailed to the L2PcInstance actor
			clientActionFailed();
		...
		{
			/*
			 * if (Config.DEBUG) _log.
			 * warning("L2CharacterAI: onIntentionCast -> stopping current client actions..."
			 * );
			 */
			...
			// Cancel action client side by sending Server->Client packet
			// ActionFailed to the L2PcInstance actor
			// no need for second ActionFailed packet, abortAttack() already
			// sent it
			// clientActionFailed();
		}
	 ...
	 * sending Server->Client packet CharMoveToLocation (broadcast)</li><BR>
	 * <BR>
	 *
	 */
	@Override
	 ...
	 *
	 * @see
	 * net.sf.l2j.gameserver.ai.AbstractAI#onIntentionMoveToInABoat(net.sf.l2j.
	 * gameserver.model.L2CharPosition,
	 * net.sf.l2j.gameserver.model.L2CharPosition)
	 */
	@Override
	protected void onIntentionMoveToInABoat(L2CharPosition destination,
			L2CharPosition origin)
	{
		if ((getIntention() == AI_INTENTION_REST)
				|| _actor.isAllSkillsDisabled())
		{
	...
	protected void onIntentionFollow(L2Character target)
	{
		...
		if ((getIntention() == AI_INTENTION_REST)
				|| _actor.isAllSkillsDisabled() || (_actor == target))
		{
			clientActionFailed();
			return;
		}
	...
	protected void onEvtStunned(L2Character attacker)
	{
		...
		// Launch actions corresponding to the Event onAttacked (only for
		// L2AttackableAI after the stunning periode)
	 ...
	 * System Message to the L2Character</li>
	 * <li>Break a cast and send Server->Client ActionFailed packet and a System
	 * Message to the L2Character</li><BR>
	 * <BR>
	 *
	 */
	@Override
	...
	protected void onEvtRooted(L2Character attacker)
	{
		...
		// _actor.broadcastPacket(new AutoAttackStop(_actor.getObjectId()));
		// if
		// (AttackStanceTaskManager.getInstance().getAttackStanceTask(_actor))
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>If the Intention was AI_INTENTION_MOVE_TO, set the Intention to
	 * AI_INTENTION_ACTIVE</li>
	 * <li>Launch actions corresponding to the Event Think</li><BR>
	 * <BR>
	 *
	 */
	...
	protected void onEvtArrived()
	{
		...
		if (_accessor.getActor() instanceof L2PcInstance
				&& Config.ACTIVATE_POSITION_RECORDER)
			((L2PcInstance) _accessor.getActor()).explore();

		// Launch actions corresponding to the Event Think
		onEvtThink();
		/*
		 * if (_actor instanceof L2PcInstance) if (!_actor.isAttackingNow() &&
		 * !_actor.isCastingNow() && !((L2PcInstance) _actor).isSitting())
	...
	protected void onEvtArrivedBlocked(L2CharPosition blocked_at_pos)
	{
		...
		if (Config.ACTIVATE_POSITION_RECORDER && Universe.getInstance()
				.shouldLog(_accessor.getActor().getObjectId()))
		{
			if (!_accessor.getActor().isFlying())
				Universe.getInstance().registerObstacle(blocked_at_pos.x,
						blocked_at_pos.y, blocked_at_pos.z);
			if (_accessor.getActor() instanceof L2PcInstance)
				((L2PcInstance) _accessor.getActor()).explore();
		}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>If the object was targeted and the Intention was
	 * AI_INTENTION_INTERACT or AI_INTENTION_PICK_UP, set the Intention to
	 * AI_INTENTION_ACTIVE</li>
	 * <li>If the object was targeted to attack, stop the auto-attack, cancel
	 * target and set the Intention to AI_INTENTION_ACTIVE</li>
	 * <li>If the object was targeted to cast, cancel target and set the
	 * Intention to AI_INTENTION_ACTIVE</li>
	 * <li>If the object was targeted to follow, stop the movement, cancel AI
	 * Follow Task and set the Intention to AI_INTENTION_ACTIVE</li>
	 * <li>If the targeted object was the actor , cancel AI target, stop AI
	 * Follow Task, stop the movement and set the Intention to AI_INTENTION_IDLE
	 * </li><BR>
	 * <BR>
	 *
	 */
	...
	{
		// If the object was targeted and the Intention was
		// AI_INTENTION_INTERACT or AI_INTENTION_PICK_UP, set the Intention to
		// AI_INTENTION_ACTIVE
		if (getTarget() == object)
		{
	 ...
	 * AutoAttackStop, StopMove/StopRotation, Die (broadcast)</li><BR>
	 * <BR>
	 *
	 */
	@Override
	protected void onEvtDead()
	{
		...
		// AutoAttackStop, StopMove/StopRotation, Die (broadcast)
		clientNotifyDead();
	...
	protected void onEvtFakeDeath()
	{
		...
		// Stop the actor movement and send Server->Client packet
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the distance between the current position of the L2Character and
	 * the target (x,y)</li>
	 ...
	 * (broadcast)</li>
	 * <li>If the distance <= offset+20, Stop the actor movement server side AND
	 * client side by sending Server->Client packet StopMove/StopRotation
	 * (broadcast)</li><BR>
	 * <BR>
	 *
	...
	protected boolean maybeMoveToPawn(L2Object target, int offset)
	{
		...
		if (!_actor.isInsideRadius(target, offset, false, false))
		{
			/*
			 * if (Config.DEBUG) _log.
			 * warning("L2CharacterAI: maybeMoveToPawn -> moving to catch target. Casting stopped"
			 * );
			 */
			...
			if (getFollowTarget() != null)
			{
						...
						&& target instanceof L2PlayableInstance)
				{
					...
					{
						if (((L2PlayableInstance) _actor)
								.isInsidePeaceZone(_actor, target))
						{
							stopFollow();
							setIntention(AI_INTENTION_IDLE);
							return true;
				...
				// allow larger hit range when the target is moving (check is
			...
			// If not running, set the L2Character movement type to run and send
			// Server->Client packet ChangeMoveType to all others L2PcInstance
	...
	protected boolean checkTargetLost(L2Object target)
	{
		...
		{
			L2PcInstance target2 = (L2PcInstance) target; // convert object to
															// chara
