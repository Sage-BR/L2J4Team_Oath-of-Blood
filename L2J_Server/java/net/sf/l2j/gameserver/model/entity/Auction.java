...
public class Auction
{
	...
	/*
	 * TODO: Announce to losing bidder that they have been out bidded Take adena
	 * when bidding Return adena when out bid Give item when auction end
	 * UpdateBidInDb Schedule Auction end Remove auction from auction and
	 * auction_bid table when auction end
	 */
	...
	public class AutoEndTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				long timeRemaining = getEndDate().getTimeInMillis()
						- Calendar.getInstance().getTimeInMillis();
				if (timeRemaining > 0)
				{
					ThreadPoolManager.getInstance()
							.scheduleGeneral(new AutoEndTask(), timeRemaining);
				} else
				{
					endAuction();
				}
	...
	private void StartAutoTask(boolean forced)
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new AutoEndTask(),
				1000);
	}
	...
	private void correctAuctionTime(boolean forced)
	{
		...
		if (_EndDate.getTimeInMillis() < Calendar.getInstance()
				.getTimeInMillis() || forced)
		{
			...
			if (forced)
				...
				endAuction(); // end auction normally in case it had bidders and
								// server was down when it ended
		}
	...
	{
		while (_EndDate.getTimeInMillis() < Calendar.getInstance()
				.getTimeInMillis())
		{
			// Set next auction date if auction has passed
			_EndDate.add(Calendar.DAY_OF_MONTH, 7); // Schedule to happen in 7
													// days
		}
	}
	...
	private void saveAuctionDate()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(
					"Update auction set endDate = ? where id = ?");
		...
		} catch (Exception e)
		{
			_log.log(Level.SEVERE,
					"Exception: saveAuctionDate(): " + e.getMessage(), e);
		} finally
		{
			try
			{
	...
	private void loadBid()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				_bidders.put(rs.getInt("bidderId"),
						new Bidder(rs.getString("bidderName"),
								rs.getString("clan_name"), rs.getInt("maxBid"),
								rs.getLong("time_bid")));
			}
		...
		{
			System.out
					.println("Exception: Auction.loadBid(): " + e.getMessage());
			e.printStackTrace();
		} finally
		{
			try
	...
	private void returnItem(String Clan, int itemId, int quantity,
			boolean penalty)
	{
		if (penalty)
			quantity *= 0.9; // take 10% tax fee if needed
		ClanTable.getInstance().getClanByName(Clan).getWarehouse()
				.addItem("Outbidded", _AdenaId, quantity, null, null);
	}
	...
	private boolean takeItem(L2PcInstance bidder, int itemId, int quantity)
	{
		...
		if (bidder.getClan() != null
				&& bidder.getClan().getWarehouse().getAdena() >= quantity)
		{
			bidder.getClan().getWarehouse().destroyItemByItemId("Buy",
					this._AdenaId, quantity, bidder, bidder);
			return true;
		}
		/*
		 * } else { // Take item from inventory if (bidder.getAdena() >=
		 * quantity) { bidder.reduceAdena("Buy", quantity, bidder, false);
		 * return true; } }
		 */
		...
		try
		{
			...
			} else
			{
				...
				if (L2World.getInstance().getPlayer(_HighestBidderName) != null)
					L2World.getInstance().getPlayer(_HighestBidderName)
							.sendMessage("You have been out bidded");
			}
			...
			if (_bidders.get(_HighestBidderId) == null)
				_bidders.put(_HighestBidderId,
						new Bidder(_HighestBidderName,
								bidder.getClan().getName(), bid,
								Calendar.getInstance().getTimeInMillis()));
			else
			{
				_bidders.get(_HighestBidderId).setBid(bid);
				_bidders.get(_HighestBidderId)
						.setTimeBid(Calendar.getInstance().getTimeInMillis());
			}
			bidder.sendMessage("You have bidded successfully");
		} catch (Exception e)
		{
			_log.log(Level.SEVERE,
					"Exception: Auction.updateInDB(L2PcInstance bidder, int bid): "
							+ e.getMessage());
			e.printStackTrace();
		} finally
		{
	...
	private void removeBids()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"DELETE FROM auction_bid WHERE auctionId=?");
			statement.setInt(1, getId());
			statement.execute();
		} catch (Exception e)
		{
			_log.log(Level.SEVERE,
					"Exception: Auction.deleteFromDB(): " + e.getMessage(), e);
		} finally
		{
			try
			{
		...
		{
			if (ClanTable.getInstance().getClanByName(b.getClanName())
					.getHasHideout() == 0)
			{
				returnItem(b.getClanName(), 57, 9 * b.getBid() / 10, false); // 10
																				// %
																				// tax
				ClanTable.getInstance().getClanByName(b.getClanName())
					...
					.getHasHideout() != 0)
			{
				if (L2World.getInstance().getPlayer(b.getName()) != null)
					L2World.getInstance().getPlayer(b.getName()).sendMessage(
	...
	private void deleteAuctionFromDB()
	{
		...
		try
		{
			...
			PreparedStatement statement;
			statement = con
					.prepareStatement("DELETE FROM auction WHERE itemId=?");
	...
	public void endAuction()
	{
		...
		if (_SellerId > 0)
		{
			...
			returnItem(_SellerClanName, 57, ClanHallManager.getInstance()
					.getClanHall(_ItemId).getLease(), false);
		}
		ClanHallManager.getInstance().getClanHall(_ItemId)
				.setOwner(ClanTable.getInstance().getClanByName(
						_bidders.get(_HighestBidderId).getClanName()));
		deleteAuctionFromDB();
		removeBids();
	}
	...
	public void cancelBid(int bidder)
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"DELETE FROM auction_bid WHERE auctionId=? AND bidderId=?");
			statement.setInt(1, getId());
		...
		{
			_log.log(Level.SEVERE,
					"Exception: Auction.cancelBid(String bidder): "
							+ e.getMessage(),
					e);
		} finally
		{
		...
		returnItem(_bidders.get(bidder).getClanName(), 57,
				_bidders.get(bidder).getBid(), true);
	...
	public void confirmAuction()
	{
		...
		try
		{
			...
			statement = con.prepareStatement(
					"INSERT INTO auction (id, sellerId, sellerName, sellerClanName, itemType, itemId, itemObjectId, itemName, itemQuantity, startingBid, currentBid, endDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
			statement.setInt(1, getId());
		...
		{
			_log.log(Level.SEVERE,
					"Exception: Auction.load(): " + e.getMessage(), e);
		} finally
		{
			try
			{
