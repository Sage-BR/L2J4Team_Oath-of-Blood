...
public class AttackStanceTaskManager
{
	protected static Logger _log = Logger
			.getLogger(AttackStanceTaskManager.class.getName());
	...
	public AttackStanceTaskManager()
	{
		ThreadPoolManager.getInstance()
				.scheduleAiAtFixedRate(new FightModeScheduler(), 0, 1000);
	}
			...
			try
			{
				if (_attackStanceTasks != null)
					synchronized (this)
					{
						...
						{
							if ((current
									- _attackStanceTasks.get(actor)) > 15000)
							{
								actor.broadcastPacket(new AutoAttackStop(
										actor.getObjectId()));
