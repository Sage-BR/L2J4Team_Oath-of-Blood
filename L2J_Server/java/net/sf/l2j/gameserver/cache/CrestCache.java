...
public class CrestCache
{
		...
		for (File file : files)
		{
			...
			synchronized (this)
			{
				try
				{
					...
					{
						_mapAlly.put(Integer.valueOf(file.getName()
								.substring(10, file.getName().length() - 4)),
								content);
					}
					_loadedFiles++;
	...
	public void convertOldPedgeFiles()
	{
		...
		for (File file : files)
		{
			...
			if (clan != null)
			{
				...
				{
					_log.warning(
							"could not update the crest id:" + e.getMessage());
				} finally
				{
					try
					{
	...
	public boolean savePledgeCrestLarge(int newId, byte[] data)
	{
		...
		{
			_log.log(Level.INFO,
					"Error saving Large pledge crest" + crestFile + ":", e);
			return false;
		}
	}
