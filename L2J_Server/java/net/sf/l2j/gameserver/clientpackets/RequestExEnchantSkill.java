...
public class RequestExEnchantSkill extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestAquireSkill.class.getName());
	...
	void runImpl()
	{
		...
		if (((trainer == null || !player.isInsideRadius(trainer,
				L2NpcInstance.INTERACTION_DISTANCE, false, false))
				&& !player.isGM())
				|| (player.getSkillLevel(_skillID) >= _skillLvl))// already
																	// knows the
																	// skill
																	// with this
																	// level
		...
		L2EnchantSkillLearn[] skills = SkillTreeTable.getInstance()
				.getAvailableEnchantSkills(player);
		...
		if (player.getSp() >= _requiredSp)
		{
			if (player.getExp() >= _requiredExp)
			{
				if (Config.SP_BOOK_NEEDED
						&& (_skillLvl == 101 || _skillLvl == 141)) // only first
																	// lvl
																	// requires
																	// book
				{
					...
					if (spb == null)// Haven't spellbook
					{
						player.sendPacket(new SystemMessage(
								SystemMessage.YOU_DONT_HAVE_ALL_OF_THE_ITEMS_NEEDED_TO_ENCHANT_THAT_SKILL));
						return;
					}
					// ok
			...
			} else
			{
				SystemMessage sm = new SystemMessage(
						SystemMessage.YOU_DONT_HAVE_ENOUGH_EXP_TO_ENCHANT_THAT_SKILL);
		...
		{
			SystemMessage sm = new SystemMessage(
					SystemMessage.YOU_DONT_HAVE_ENOUGH_SP_TO_ENCHANT_THAT_SKILL);
			player.sendPacket(sm);
			return;
		}
		if (Rnd.get(100) <= _rate)
		{
			...
			if (Config.DEBUG)
				_log.fine("Learned skill " + _skillID + " for " + _requiredSp
						+ " SP.");
			...
			SystemMessage sm = new SystemMessage(
					SystemMessage.YOU_HAVE_SUCCEEDED_IN_ENCHANTING_THE_SKILL_S1);
		...
		} else
		{
			if (skill.getLevel() > 100)
			{
				_skillLvl = _baseLvl;
				player.addSkill(
						SkillTable.getInstance().getInfo(_skillID, _skillLvl),
						true);
			}
			SystemMessage sm = new SystemMessage(
					SystemMessage.YOU_HAVE_FAILED_TO_ENCHANT_THE_SKILL_S1);
			sm.addSkillName(_skillID);
			player.sendPacket(sm);
		}
		...
		for (L2ShortCut sc : allShortCuts)
		{
			...
			{
				L2ShortCut newsc = new L2ShortCut(sc.getSlot(), sc.getPage(),
						sc.getType(), sc.getId(), _skillLvl, 1);
