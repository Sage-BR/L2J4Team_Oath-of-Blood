...
public abstract class PathFinding
{
	...
	public abstract List<AbstractNodeLoc> FindPath(int gx, int gy, short z,
			int gtx, int gtz, short tz);
	...
	public List<AbstractNodeLoc> searchAStar(Node start, Node end)
	{
		...
		// List of Visited Nodes
		FastNodeList visited = new FastNodeList(800 / 2);// TODO! Add limit to
															// cfg
				...
				for (Node n : node.getNighbors())
				{
					if (!visited.contains(n) && !to_visit.contains(n))
					{
						...
						n.setCost(Math.abs(start_x - n.getLoc().getNodeX())
								+ Math.abs(start_y - n.getLoc().getNodeY())
								+ Math.abs(end_x - n.getLoc().getNodeX())
