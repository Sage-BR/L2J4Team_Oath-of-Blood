...
public class AdminPForge implements IAdminCommandHandler
{
		...
		} else if (command.startsWith("admin_forge3"))
		{
			try
			{
				...
				for (int i = 0; i < format.length(); i++)
				{
					...
					{
						val = String
								.valueOf(activeChar.getTarget().getObjectId());
					} else if (val.toLowerCase().equals("$bobjid"))
					{
						if (activeChar.getBoat() != null)
						{
							val = String.valueOf(
									activeChar.getBoat().getObjectId());
						}
					...
					{
						val = String
								.valueOf(((L2PcInstance) activeChar.getTarget())
										.getCharId());
	...
	public void showPage3(L2PcInstance activeChar, String format,
			String command)
	{
		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
	...
	public void showPage2(L2PcInstance activeChar, String format)
	{
		...
		TextBuilder replyMSG = new TextBuilder("<html><body>");
		replyMSG.append(
				"<center>L2J Forge Panel 2</center><br>Format:" + format);
		replyMSG.append(
				"<br>No spaces in values please ;)<br>Decimal values for c h d, a float (with point) for f, a string for s and for x/b the hexadecimal value");
		replyMSG.append("<br>Values<br>");
		for (int i = 0; i < format.length(); i++)
		{
			replyMSG.append(format.charAt(i) + " : <edit var=\"v" + i
					+ "\" width=100> <br>");
		}
		replyMSG.append(
				"<br><button value=\"Send\" action=\"bypass -h admin_forge3 "
						+ format);
		for (int i = 0; i < format.length(); i++)
		{
			replyMSG.append(" $v" + i);
		...
		replyMSG.append(
				"<br><button value=\"Broadcast\" action=\"bypass -h admin_forge3 broadcast "
						+ format);
		for (int i = 0; i < format.length(); i++)
		{
			replyMSG.append(" $v" + i);
