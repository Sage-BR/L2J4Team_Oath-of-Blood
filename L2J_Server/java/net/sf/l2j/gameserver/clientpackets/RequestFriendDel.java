...
public class RequestFriendDel extends ClientBasePacket
{
	...
	private static Logger _log = Logger
			.getLogger(RequestFriendDel.class.getName());
		...
		try
		{
			...
			if (friend != null)
			{
				statement = con.prepareStatement(
						"SELECT friend_id FROM character_friends WHERE char_id=? and friend_id=?");
				...
				if (!rset.next())
				{
					...
					sm = new SystemMessage(
							SystemMessage.S1_NOT_ON_YOUR_FRIENDS_LIST);
					sm.addString(_name);
			...
			{
				statement = con.prepareStatement(
						"SELECT friend_id FROM character_friends, characters WHERE char_id=? AND friend_id=obj_id AND char_name=?");
				statement.setInt(1, activeChar.getObjectId());
			...
			statement = con.prepareStatement(
					"DELETE FROM character_friends WHERE char_id=? AND friend_id=?");
			statement.setInt(1, activeChar.getObjectId());
			statement.setInt(2, objectId);
			statement.execute();
			// Player deleted from your friendlist
			sm = new SystemMessage(
					SystemMessage.S1_HAS_BEEN_DELETED_FROM_YOUR_FRIENDS_LIST);
			sm.addString(_name);
			activeChar.sendPacket(sm);
			sm = null;
