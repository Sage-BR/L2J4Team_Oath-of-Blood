...
public final class L2ClassMasterInstance extends L2FolkInstance
{
	...
	private static int[] _secondClassIds =
		{ 2, 3, 5, 6, 9, 8, 12, 13, 14, 16, 17, 20, 21, 23, 24, 27, 28, 30, 33,
				34, 36, 37, 40, 41, 43, 46, 48, 51, 52, 55, 57 };
	...
	public void onAction(L2PcInstance player)
	{
		...
		} else
		{
			...
			if (((level >= 20 && jobLevel == 1)
					|| (level >= 40 && jobLevel == 2))
					&& Config.ALLOW_CLASS_MASTERS)
			{
				showChatWindow(player, classId.getId());
			} else if (level >= 76 && Config.ALLOW_CLASS_MASTERS
					&& classId.getId() < 88)
			{
				for (int i = 0; i < _secondClassIds.length; i++)
				{
					if (classId.getId() == _secondClassIds[i])
					{
						...
						sb.append("<tr><td><center>"
								+ CharTemplateTable
										.getClassNameById(classId.getId())
								+ " Class Master:</center></td></tr>");
						sb.append("<tr><td><br></td></tr>");
						sb.append("<tr><td><a action=\"bypass -h npc_"
								+ getObjectId() + "_change_class " + (88 + i)
								+ "\">Advance to "
								+ CharTemplateTable.getClassNameById(88 + i)
								+ "</a></td></tr>");
						sb.append("<tr><td><br></td></tr>");
						sb.append("<tr><td><a action=\"bypass -h npc_"
								+ getObjectId()
			...
			} else
			{
				...
				case 1:
					sb.append(
							"Come back here when you reached level 20 to change your class.<br>");
					break;
				case 2:
					sb.append(
							"Come back here when you reached level 40 to change your class.<br>");
					break;
				case 3:
					sb.append(
							"There is no class changes for you any more.<br>");
					break;
				}

				for (Quest q : Quest.findAllEvents())
					sb.append("Event: <a action=\"bypass -h Quest "
							+ q.getName() + "\">" + q.getDescr() + "</a><br>");
	...
	@Override
	public String getHtmlPath(@SuppressWarnings("unused")
	int npcId, int val)
	{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		if (command.startsWith("change_class"))
		{
			...
			ClassLevel lvlnow = PlayerClass.values()[classId.getId()]
					.getLevel();
			switch (lvlnow)
			{
			case First:
			...
			if (val >= 88)
				player.sendPacket(new SystemMessage(1606)); // system sound 3rd
															// occupation
			else
				player.sendPacket(new SystemMessage(1308)); // system sound for
															// 1st and 2nd
															// occupation
	...
	private void changeClass(L2PcInstance player, int val)
	{
		...
		if (player.isSubClassActive())
			player.getSubClasses().get(player.getClassIndex())
					.setClassId(player.getActiveClass());
		else
