...
public class GameServerRegister
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		while (!_choiseOk)
		{
			...
			{
				for (Map.Entry<Integer, String> entry : gsTable.serverNames
						.entrySet())
				{
					System.out.println("Server: id:" + entry.getKey() + " - "
							+ entry.getValue());
				}
			...
			} else
			{
				try
				{
					...
					{
						System.out.println("ID is too high (max is "
								+ (gsTable.serverNames.size() - 1) + ")");
						continue;
					}
					if (id < 0)
					{
					...
					} else
					{
						if (gsTable.isIDfree(id))
						{
							...
							gsTable.createServer(
									gsTable.new GameServer(hex, id));
							Config.saveHexid(new BigInteger(hex).toString(16),
									"hexid(server " + id + ").txt");
							System.out.println(
									"Server Registered hexid saved to 'hexid(server "
											+ id + ").txt'");
							System.out.println(
